...
export const usePolling = (
  ...
  // Only this explicit path may publish while polling is already disabled;
  // a request started before disabling remains fenced like every timer read.
  const refresh = useCallback((options) => read.refresh({ ...options, publishWhenDisabled: true }).catch(() => null), [read.refresh]);
