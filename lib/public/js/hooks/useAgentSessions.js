 ...
 * @returns {{ sessions, selectedSessionKey, setSelectedSessionKey, selectedSession, loading, error, refreshSessions }}
 */
export const useAgentSessions = ({ enabled = false, filter } = {}) => {
  ...
  // Silent list refresh (never touches selection/loading): keeps new
  // cron/Telegram sessions appearing without a reload. Visibility-paused and
  // conservative (30s) — the server micro-caches this endpoint because every
  // uncached hit spawns an OpenClaw CLI process.
  const refreshSessions = useCallback(async () => {
    try {
      const data = await fetchAgentSessions();
      const nextSessions = Array.isArray(data?.sessions) ? data.sessions : [];
      // Mirror the server's never-cache-empty rule: a transient empty payload
      // must not wipe the visible sidebar (and clear the selection) mid-use.
      if (nextSessions.length === 0) return;
      setAllSessions(nextSessions);
      writeCachedSessions(nextSessions);
    } catch {
      // Background refresh is best-effort; the visible list stays.
    }
  }, []);

  useVisibleInterval(() => refreshSessions(), 30_000, { enabled: Boolean(enabled) });
