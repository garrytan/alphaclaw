...
export const useCachedFetch = (
  ...
  observerRef.current = { key: normalizedKey, enabled: subscribeEnabled };
  ...
  useEffect(() => {
    const unsubscribe = subscribeCache(normalizedKey, (snapshot) => {
      if (observerRef.current.key !== normalizedKey) return;
      ...
      if (!observerRef.current.enabled && ![401, 403].includes(snapshot.error?.status)) return;
  ...
  }, [normalizedKey, subscribeEnabled]);
