...
export const useClaudeCodeLocal = ({ enabled = true } = {}) => {
  ...
  // Status poll through the shared primitive (fix wave PR 11): paused while
  // hidden, immediate refresh on return.
  useVisibleInterval(() => refresh(), pollMs, { enabled: Boolean(enabled), immediate: true });
