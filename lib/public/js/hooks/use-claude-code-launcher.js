...
// mode was consented to, so switching CLAUDE_CODE_LOCAL_PERMISSION_MODE (for
// example to bypassPermissions) always re-confirms with mode-naming copy.
export const kLocalConfirmedModeUiSettingKey = "claudeCodeLocalConfirmedMode";
...
//    │                      sessionUrl; error/timeout → win.close() + toast
...
// Consent memory for the LOCAL path: valid only while the stored mode still
// matches the configured permission mode (mode change = consent invalidated).
export const hasLocalConsentForMode = (permissionMode) => {
  const stored = readUiSettings()[kLocalConfirmedModeUiSettingKey];
  return typeof stored === "string" && stored !== "" && stored === permissionMode;
};

export const storeLocalConsentMode = (permissionMode) => {
  updateUiSettings((settings) => ({
    ...settings,
    [kLocalConfirmedModeUiSettingKey]: String(permissionMode || ""),
  }));
};
...
export const useClaudeCodeLauncher = ({ enabled = true, onBeforeOpen = null } = {}) => {
  ...
  const confirmContextRef = useRef({ mode: "routine", permissionMode: null });
  ...
  const openConfirmModal = useCallback((win, mode, permissionMode) => {
    ...
    confirmContextRef.current = { mode, permissionMode };
    ...
    async (win, confirmed) => {
      try {
        ...
        if (error?.code === "confirm_required") {
          openConfirmModal(win, "routine", null);
          return;
        }
  ...
  const fireLocal = useCallback(
    async (win, confirmed) => {
      try {
        const result = await createClaudeCodeLocalSession({ confirmed });
        ...
        if (code === "confirm_required") {
          openConfirmModal(
            win,
            "local",
            statusRef.current?.local?.permissionMode || null,
          );
          return;
        }
        if (
    ...
    [settle, openConfirmModal, pollLocalSession, fireRoutine, release],
  );
  ...
  const openClaudeCode = useCallback(
    (event) => {
      ...
      if (localPlausible) {
        fireLocal(win, hasLocalConsentForMode(localHint.permissionMode));
        return;
      }
  ...
  const confirmStart = useCallback(() => {
    ...
    pendingWinRef.current = null;
    setConfirmOpen(false);
    const { mode, permissionMode } = confirmContextRef.current;
    if (mode === "local") {
      storeLocalConsentMode(permissionMode);
      if (win && !win.closed) {
