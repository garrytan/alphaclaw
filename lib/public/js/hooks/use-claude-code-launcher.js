    ...
    async (win, { confirmed = false, permissionMode = null } = {}) => {
      try {
        ...
        if (code === "confirm_required") {
          // Prefer the server's AUTHORITATIVE live config carried on the 409
          // body (permissionMode + cwd): a stale cached snapshot could name the
          // wrong mode (e.g. "default" while the server is bypassPermissions).
          // Fall back to the cached status only when an older server omits the
          // fields. The modal renders THIS mode and the re-fire posts it.
          openConfirmModal(win, "local", {
            permissionMode:
              error.permissionMode ??
              statusRef.current?.local?.permissionMode ??
              null,
            cwd: error.cwd ?? statusRef.current?.local?.cwd ?? null,
          });
          return;
        }
        if (
