...
export const useAppShellController = ({ location = "" } = {}) => {
  ...
  const beginAlphaclawReconnect = useCallback(({ graceMs = 0 } = {}) => {
  ...
  const handleGatewayRollback = useCallback(async () => {
    try {
  ...
  const handleAcUpdate = useCallback(async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        // succeeds and reload only then.
        beginAlphaclawReconnect({ graceMs: data.managedUpdate ? 8000 : 5000 });
      } else {
