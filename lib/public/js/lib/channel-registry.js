...
export const kChannelRegistry = [
  ...
  {
    // Externally configured via a linked signal-cli — no token, no accordion
    // (#113). No `onboarding`/`envKey` on purpose: the wizard shows it as a
    // detected "Configured" row from /api/status, never as a token form, and
    // it can never be preselected as the onboarding pairing target.
    id: "signal",
    label: "Signal",
    icon: null,
    external: true,
...
// Channels a user can CREATE from AlphaClaw's Add-channel menu. External
// channels (e.g. signal, linked out-of-band via signal-cli) have no create
// flow — they surface as detected/Configured in status, never as a menu item
// whose modal would error server-side.
export const kCreatableChannelIds = kChannelRegistry
