...
export const formatLocaleDateTime = (
  ...
  try {
    ...
    return (withSeconds ? kFormatters.dateTimeWithSeconds : kFormatters.dateTime)
      .format(dateValue);
  } catch {
    return fallback;
  }
...
export const formatLocaleDate = (
  ...
  try {
    const dateValue = toDateValue(value, { valueIsUnixSeconds, valueIsEpochMs });
    if (!dateValue || Number.isNaN(dateValue.getTime())) return fallback;
    return kFormatters.date.format(dateValue);
  } catch {
    return fallback;
  }
};
...
export const formatLocaleTime = (
  ...
  try {
    const dateValue = toDateValue(value, { valueIsUnixSeconds, valueIsEpochMs });
    if (!dateValue || Number.isNaN(dateValue.getTime())) return fallback;
    return (withSeconds ? kFormatters.timeWithSeconds : kFormatters.time)
      .format(dateValue);
  } catch {
    return fallback;
  }
...
export const formatLocaleDateTimeWithTodayTime = (
  ...
  try {
    const dateValue = toDateValue(value, { valueIsUnixSeconds, valueIsEpochMs });
    if (!dateValue || Number.isNaN(dateValue.getTime())) return fallback;
    if (isSameLocalDay(dateValue, new Date())) {
      return (withSeconds ? kFormatters.timeWithSeconds : kFormatters.time)
        .format(dateValue);
    }
    return (withSeconds ? kFormatters.dateTimeWithSeconds : kFormatters.dateTime)
...
export const formatLocaleDateTimeWithZone = (
  value,
  { fallback = "—", valueIsUnixSeconds = false, valueIsEpochMs = false } = {},
) => {
  try {
    const dateValue = toDateValue(value, { valueIsUnixSeconds, valueIsEpochMs });
    if (!dateValue || Number.isNaN(dateValue.getTime())) return fallback;
    return kFormatters.dateTimeWithZone.format(dateValue);
  } catch {
    return fallback;
  }
};
