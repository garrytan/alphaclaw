...
export const createFormatters = (timeZone = undefined) => {
  ...
  return {
    ...
    // zone-bearing variant is built from component options. shortOffset is
    // the one option old engines lack — guard it separately so a 2021-era
    // browser degrades ONLY the zone-suffixed tooltip register, not every
...
// Guarded: a construction throw here (ancient browser missing shortOffset,
// ICU-restricted embedder) must degrade every formatter to its fallback
// string — never kill this module's importers (api.js imports us, so an
// unguarded throw would take down the whole network layer at import time).
const kFormatters = (() => {
  try {
...
// Wall-clock fields of an instant in an IANA zone (fix wave F167) — the cron
// calendar evaluates a job's `schedule.tz`, not the browser's zone. Formatters
// are cached per zone (constructing one is the slow part); an empty or
// unknown zone answers with the browser-local getters so callers never have to
// special-case it.
const kZonedPartsFormatters = new Map();
const kWeekdayIndex = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
const zonedPartsFormatter = (timeZone) => {
  const key = String(timeZone || "").trim();
  if (!key) return null;
  if (kZonedPartsFormatters.has(key)) return kZonedPartsFormatters.get(key);
  let formatter = null;
  try {
  ...
  kZonedPartsFormatters.set(key, formatter);
  return formatter;
};
export const isKnownTimeZone = (timeZone) => zonedPartsFormatter(timeZone) !== null;
export const readZonedDateParts = (value, timeZone = "") => {
  ...
  const parts = {};
  for (const part of formatter.formatToParts(dateValue)) parts[part.type] = part.value;
  return {
    year: Number(parts.year),
    month: Number(parts.month),
    dayOfMonth: Number(parts.day),
    dayOfWeek: kWeekdayIndex[parts.weekday] ?? dateValue.getDay(),
    // Some engines render midnight as "24" under h23 with certain locales.
...
export const formatLocaleDateTimeRange = (
  ...
  // Coercion guarded like every sibling: a throwing valueOf must degrade to
  // the fallback path, never escape the one formatter promising fallbacks.
  let startDate = null;
  let endDate = null;
  let startValid = false;
  let endValid = false;
  try {
    startDate = toDateValue(startValue, opts);
    startValid = Boolean(startDate) && !Number.isNaN(startDate.getTime());
  } catch {}
  try {
    endDate = toDateValue(endValue, opts);
    endValid = Boolean(endDate) && !Number.isNaN(endDate.getTime());
  } catch {}
