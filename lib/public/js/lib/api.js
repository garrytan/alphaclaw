...
import { clearApiCache, denyCachedKey } from "./api-cache.js";
import { fetchReadResponse } from "./api-read-request.js";
...
export const authFetch = async (url, opts = {}) => {
  const { rawResponse = false, ...nextOptions } = opts;
  ...
  const isRead = String(nextOptions.method || "GET").toUpperCase() === "GET";
  const checkAuthorization = (res) => {
    if (res.status === 401) {
      const error = Object.assign(new Error("Unauthorized"), { status: 401, code: "unauthorized" });
      clearApiCache(error);
      try {
    ...
    if (isRead && res.status === 403) {
      const error = Object.assign(new Error("You no longer have access to this data."), {
        status: 403, code: "forbidden",
      });
      denyCachedKey(String(url), error);
    }
  };
  if (isRead && !rawResponse) return fetchReadResponse(url, nextOptions, checkAuthorization);
...
export async function fetchStatus({ signal } = {}) {
  const res = await authFetch("/api/status", { signal });
...
export async function fetchPairings({ signal } = {}) {
  const res = await authFetch("/api/pairings", { signal });
  return parseJsonOrThrow(res, "Could not load data");
}
...
export async function fetchGoogleAccounts({ signal } = {}) {
  const res = await authFetch("/api/google/accounts", { signal });
  return parseJsonOrThrow(res, "Could not load data");
}
...
// Dedicated 15s-poll read of the persisted situation report — polling the
// settings endpoint instead would respawn `claude --version` per tab.
export async function fetchWatchdogOverseerSituation() {
  const res = await authFetch("/api/watchdog/overseer/situation");
  return parseJsonOrThrow(res, "Could not load the overseer situation report");
}

// Body-preserving parse (WI-3.5): a 502 {ok:false, error, result} carries the
// per-channel failures (result.failures[] channel/target/reason/errorCode) —
// attach `result` to the rejection so the settings card can list them instead
// of a bare toast. 200 {ok:true, result} resolves unchanged.
export async function triggerWatchdogTestNotification() {
  ...
  if (!res.ok || data?.ok === false) {
    ...
    if (data?.result && typeof data.result === "object") error.result = data.result;
    error.status = res.status;
    throw error;
  }
  return data || {};
...
export async function fetchWatchdogMemorySettings() {
  const res = await authFetch("/api/watchdog/memory");
  return parseJsonOrThrow(res, "Could not load memory monitor settings");
}

export async function updateWatchdogMemorySettings(patch) {
  const res = await authFetch("/api/watchdog/memory", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(patch || {}),
  });
  return parseJsonOrThrow(res, "Could not update memory monitor settings");
}
...
const parseClaudeCodeEnvelopeOrThrow = async (res, fallbackError) => {
  ...
  try {
    ...
    throw Object.assign(new Error(text || fallbackError), { status: res.status });
  }
  if (!res.ok || data?.ok === false) {
    const error = new Error(
...
export async function fetchAlphaclawReleaseNotes(tag = "") {
  ...
  try {
    ...
    const res = await fetchReadResponse(endpoint, {
...
export async function updateAlphaclaw() {
  ...
  return parseEnvelopeOrThrow(res, "Could not update AlphaClaw");
}

export async function resolveManagedUpdateAttempt(attemptId, outcome) {
  const res = await authFetch(`/api/alphaclaw/update/${encodeURIComponent(String(attemptId || ""))}/resolve`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ confirmProviderChecked: true, outcome }),
  });
  return parseEnvelopeOrThrow(res, "Could not resolve the deployment status");
}
...
export async function fetchOnboardStatus() {
  ...
  // A JSON-bodied 5xx (proxy, or the server's own JSON error middleware) must
  // reject (fix wave F141): resolving it as data read `onboarded: false` and
  // dropped an onboarded operator into the Welcome wizard. The shell retries
  ...
  if (!res.ok) {
    ...
    throw new Error(detail || `Could not load onboarding status (HTTP ${res.status})`);
  }
  return res.json();
}
...
const parseJsonOrThrow = async (res, fallbackError) => {
  ...
  if (!res.ok || data?.ok === false) {
    ...
    // the machine code — operators should never be toasted "rate_limited".
    throw Object.assign(new Error(data.message || data.error || text || `HTTP ${res.status}`), {
      status: res.status, code: data.code, hint: data.hint, docsUrl: data.docsUrl,
    });
  }
  return data;
};
...
// Explicit partial-success facts only. Never attach an arbitrary failure
// body to Error: consent tokens and other response credentials stay private.
const kPartialOutcomeBooleanFields = [
  "configSaved", "restartDeferred", "restartRequired", "enabled", "disabled", "changed",
  "restored", "configApplied", "gatewayApplied", "configRestored", "gatewayRestored",
  "ownerCreated", "memberSaved", "gatewayConfigSaved", "gatewayConfigDeferred",
];
const parseEnvelopeOrThrow = async (res, fallbackError) => {
  ...
  if (!res.ok || data?.ok === false) {
    ...
    // 409 backup_failed (WI-4.5): the ONE verified archive the reuse gate
    // would accept with consent ({file, at, ageMs, sha256, producer}) — the
    // Upgrade page offers "Retry using that backup" off it. Object only.
    if (data?.reusableBackup && typeof data.reusableBackup === "object") {
      error.reusableBackup = data.reusableBackup;
    }
    if (data?.backupRiskEligible === true) error.backupRiskEligible = true;
    if (typeof data?.gatewayHeld === "boolean") error.gatewayHeld = data.gatewayHeld;
    if (typeof data?.operationId === "string") error.operationId = data.operationId;
    for (const field of ["migration", "schema", "preflight", "databaseBytes", "databaseCount", "recovery", "target"]) {
      if (data?.[field] != null) error[field] = data[field];
    }
    // 409 catalog_stale (v0.9.81): the channel latest the server knows about,
    // so the Upgrade page can re-open the confirm on the right version.
    if (typeof data?.latest === "string") error.latest = data.latest;
    if (typeof data?.installedVersion === "string") error.installedVersion = data.installedVersion;
    for (const field of kPartialOutcomeBooleanFields) {
      if (typeof data?.[field] === "boolean") error[field] = data[field];
    }
    throw error;
  }
  return data;
...
// Backup inventory (WI-4.3): every archive under <root>/backups/openclaw with
// its provenance, plus recorded-but-missing ones. Read-only, cheap on the
// server (5 s SWR), never part of the channel status payload. `force` asks
// the server to rescan before answering — after an apply settles, its SWR
// copy can still describe the pre-update directory, and the client would
// otherwise cache that as fresh. A server without the knob ignores the query.
export async function fetchOpenclawBackups({ force = false } = {}) {
  const res = await authFetch(
    force ? "/api/openclaw/backups?force=1" : "/api/openclaw/backups",
  );
  return parseEnvelopeOrThrow(res, "Could not read the backup inventory");
}

export async function fetchOpenclawBackupPolicy() {
  const res = await authFetch("/api/openclaw/backup-policy");
  return parseEnvelopeOrThrow(res, "Could not read backup exclusions");
}

export async function updateOpenclawBackupPolicy(policy) {
  const res = await authFetch("/api/openclaw/backup-policy", {
    method: "PUT", headers: { "Content-Type": "application/json" },
    body: JSON.stringify(policy),
  });
  // Preserve only the public rule diagnostics, never an arbitrary error body.
  const data = await res.json();
  if (!res.ok || data?.ok === false) {
    throw Object.assign(new Error(data?.message || data?.error || "Could not save backup exclusions"), {
      status: res.status, code: data?.code, hint: data?.hint,
      refusedExcludes: Array.isArray(data?.refusedExcludes) ? data.refusedExcludes : [],
    });
  }
  return data;
}

export async function fetchOpenclawChannel({ signal } = {}) {
  const res = await authFetch("/api/openclaw/channel", { signal });
...
export async function fetchOpenclawCatalog({ refresh = false, signal } = {}) {
  const query = refresh ? "?refresh=1" : "";
  const res = await authFetch(`/api/openclaw/catalog${query}`, { signal });
...
// "Back up now" (v0.9.81): starts the standalone backup run. Same envelope as
// applyOpenclawVersion — a fast outcome answers inline, a long one 202s with
// an operationId + SSE events URL.
export async function createOpenclawBackup({ recoveryMode = "config_only" } = {}) {
  const res = await authFetch("/api/openclaw/backup", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ recoveryMode }),
  });
  return parseEnvelopeOrThrow(res, "Could not start the OpenClaw backup");
}

export async function requestOpenclawBackupRiskConsent(operationId) {
  const res = await authFetch(`/api/openclaw/runs/${encodeURIComponent(operationId)}/backup-risk-consent`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: "{}",
  });
  return parseEnvelopeOrThrow(res, "Could not confirm backup risk");
}

export async function cancelOpenclawRecoveryReview(operationId) {
  const res = await authFetch("/api/openclaw/recovery/cancel", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ operationId }),
  });
  const result = await parseEnvelopeOrThrow(res, "Could not cancel the stopped recovery review");
  if (result?.ok !== true || result.operationId !== operationId) {
    throw new Error("The server did not confirm cancellation of this recovery review. The gateway may remain held; refresh its status before continuing.");
  }
  return result;
}
...
// Installed-tree reconcile (#76 B1.2): re-activate the recorded OpenClaw
// build over a diverged live tree and relaunch. Custom parse
// (retryOpenclawReconcile precedent): refusals carry code/hint (+ the sync's
// `action`, `runId`) so the Upgrade page can name the exact blocker.
export async function reconcileInstalledOpenclaw() {
  const res = await authFetch("/api/openclaw/reconcile-installed", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({}),
  });
  ...
  if (!res.ok || data?.ok === false) {
    const error = new Error(
      data?.message || data?.error || "Could not re-activate the recorded build",
    );
    if (data?.code) error.code = data.code;
    if (data?.hint) error.hint = data.hint;
    if (data?.runId) error.runId = data.runId;
    error.status = res.status;
    throw error;
  }
  return data || {};
...
export async function rollbackOpenclaw({ confirmDataRisk = false } = {}) {
  ...
  if (!res.ok || data?.ok === false) {
    ...
    if (data?.recovery && typeof data.recovery === "object" && !Array.isArray(data.recovery)) error.recovery = data.recovery;
    // WI-4.1 fence re-stat: pruned / partial / reused caveats plus the newest
    // surviving archive — the second-stage dialog renders each of them.
    if (typeof data?.backupFileExists === "boolean") {
      error.backupFileExists = data.backupFileExists;
    }
    if (typeof data?.backupPartial === "boolean") error.backupPartial = data.backupPartial;
    if (typeof data?.backupProfile === "string") error.backupProfile = data.backupProfile;
    if (data?.backupCoverage && typeof data.backupCoverage === "object") error.backupCoverage = data.backupCoverage;
    if (Array.isArray(data?.backupPartialReasons)) error.backupPartialReasons = data.backupPartialReasons.filter((reason) => typeof reason === "string");
    // Why a present archive must NOT be restored (symlink / outside dir /
    // content_changed / unverifiable_content …) — without it a present-but-
    // unverifiable archive would read as "pruned".
    if (typeof data?.backupFileCaveat === "string" && data.backupFileCaveat) {
      error.backupFileCaveat = data.backupFileCaveat;
    }
    if (typeof data?.backupReused === "boolean") error.backupReused = data.backupReused;
    if (Number.isFinite(data?.reusedAgeMs)) error.reusedAgeMs = data.reusedAgeMs;
    if (data?.newestSurvivingBackup && typeof data.newestSurvivingBackup === "object") {
      error.newestSurvivingBackup = data.newestSurvivingBackup;
    }
    error.status = res.status;
    throw error;
  }
...
export async function fetchOpenclawRun(operationId, { signal } = {}) {
  const res = await authFetch(`/api/openclaw/runs/${encodeURIComponent(String(operationId || ""))}`, { signal });
  return parseEnvelopeOrThrow(res, "Could not load the operation status");
}
...
export const downloadBrowseFile = async (filePath) => {
  const params = new URLSearchParams({ path: String(filePath || "") });
  const res = await authFetch(`/api/browse/download?${params.toString()}`, { rawResponse: true });
  if (!res.ok) {
