...
export const fetchTeamStatus = async () => {
  const res = await authFetch("/api/team");
  return parseJsonOrThrow(res, "Could not load team status");
};

export const setTeamEnabled = async (enabled) => {
  const res = await authFetch("/api/team", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ enabled: enabled === true }),
  });
  return parseJsonOrThrow(res, "Could not update team mode");
};

export const fetchTeamOperators = async () => {
  const res = await authFetch("/api/team/operators");
  return parseJsonOrThrow(res, "Could not load operators");
};

export const saveTeamOperators = async (operators) => {
  const res = await authFetch("/api/team/operators", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ operators: Array.isArray(operators) ? operators : [] }),
  });
  return parseJsonOrThrow(res, "Could not save operators");
};
...
export async function rollbackOpenclaw() {
  const res = await authFetch("/api/openclaw/rollback", { method: "POST" });
  return parseEnvelopeOrThrow(res, "Could not roll back OpenClaw");
}
