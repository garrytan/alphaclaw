...
export const updateDoctorSettings = async ({ autoRunEnabled = false } = {}) => {
  const res = await authFetch("/api/doctor/settings", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ autoRunEnabled: Boolean(autoRunEnabled) }),
  });
...
export async function createClaudeCodeSession({ confirmed = false } = {}) {
  ...
  try {
    ...
    throw new Error(text || "Could not start a Claude Code session");
  }
  if (!res.ok || data?.ok === false) {
    const error = new Error(
      data?.message || data?.error || `HTTP ${res.status}`,
    );
    // The launcher hook branches on the machine code (confirm_required shows
    // the one-time modal, not_configured is the silent fallback) — keep it
    // on the thrown error like parseEnvelopeOrThrow does. Middleware-level
    // refusals put prose in `error` and the machine code in `code`, so a
    // dedicated code field wins when present.
...
// Watchdog rescue card, the setup modal). Same endpoint as
// fetchClaudeCodeStatus but consumed OUTSIDE useCachedFetch on purpose: a
// poller reading through the shared cache would see up-to-60s-stale local
// state mid-transition. Mutating callers still invalidate
// kClaudeCodeStatusCacheKey so cached consumers refresh too.
export async function fetchClaudeCodeStatusDirect() {
  const res = await authFetch("/api/claude-code/status");
  return parseJsonOrThrow(res, "Could not load Claude Code launcher status");
}

// Local rescue-session envelope: same thrown-error shape as
// createClaudeCodeSession (machine code on error.code) — the launcher hook
// and the rescue card branch on codes (confirm_required, needs_login, ...).
const parseClaudeCodeLocalOrThrow = async (res, fallbackError) => {
  const text = await res.text();
  let data;
  try {
    data = text ? JSON.parse(text) : {};
...
export async function createClaudeCodeLocalSession({ confirmed = false } = {}) {
  const res = await authFetch("/api/claude-code/local/session", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ confirmed }),
  });
  return parseClaudeCodeLocalOrThrow(
    res,
...
export async function startClaudeCodeLocalLogin() {
  ...
  return parseClaudeCodeLocalOrThrow(res, "Could not start the Claude login");
}
...
export async function submitClaudeCodeLocalLoginCode({ code = "" } = {}) {
  ...
  return parseClaudeCodeLocalOrThrow(res, "Could not submit the login code");
}
...
export async function cancelClaudeCodeLocalLogin() {
  ...
  return parseClaudeCodeLocalOrThrow(res, "Could not cancel the Claude login");
}
...
export async function logoutClaudeCodeLocal() {
  ...
  return parseClaudeCodeLocalOrThrow(res, "Could not log out of Claude");
}
...
export async function fetchClaudeCodeLocalTail({ source = "session" } = {}) {
  ...
  return parseClaudeCodeLocalOrThrow(res, "Could not load the rescue CLI output");
}
