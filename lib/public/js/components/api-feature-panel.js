...
export const ApiFeaturePanel = ({
  ...
  // { attempted, error } from the last failed save — cleared on next attempt.
    ...
    <div class="bg-surface border border-border rounded-xl p-4">
      ...
      ${openAiCompatApiError
        ? html`
            <div class="mt-3">
              <${InlineErrorChip}
                error=${openAiCompatApiError.error}
                headline=${`Couldn't ${openAiCompatApiError.attempted ? "enable" : "disable"} the API — still ${apiEnabled ? "enabled" : "disabled"}.`}
              />
            </div>
          `
        : null}
