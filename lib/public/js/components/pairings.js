...
export const PairingRow = ({ p, onApprove, onReject }) => {
  ...
  // The row owns its failure UX: a failed action must clear busy and leave a
  // persistent inline error here, not a silent revert behind a toast.
        ...
        ? html`<div class="mt-2">
            <${InlineErrorChip}
              ...
              headline=${actionError.action === "approve"
                ? "Couldn't approve this pairing."
                : "Couldn't reject this pairing."}
            />
          </div>`
        : null}
    </div>`;
