...
// Shared short relative-time stamp ("as of 12s ago" freeze stamps and
// success lines use it).
export const formatSecondsAgo = (thenMs, nowMs = Date.now()) => {
  ...
  const handleToggle = () => {
    ...
    if (
      ...
      Promise.resolve(onLoadEvidence(operationId))
        ...
        .catch(() => setEvidence({ status: "expired", text: "" }));
    }
  };
