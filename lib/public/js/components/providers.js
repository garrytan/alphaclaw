...
const kEnvCacheKey = "/api/env";
const kModelsConfigCacheKey = "/api/models/config";
const kCodexStatusCacheKey = "/api/codex/status";
let kProvidersTabCache = null;

// R4 dirty-check merge: a refresh must not clobber credential drafts typed
...
export const Providers = ({ onRestartRequired = () => {} }) => {
  ...
  // R4 stale-guard: only the latest refresh may apply; drafts are merged, not
  ...
  // R5: the frame renders immediately — the model select is disabled and the
