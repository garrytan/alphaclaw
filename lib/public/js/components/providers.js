...
export const Providers = ({ onRestartRequired = () => {} }) => {
  ...
  // the backup barrier lifts) keeps its own badge until a status read shows
  // the saved connection — never a false "Not connected" in between.
  const [codexDeferredSavePending, setCodexDeferredSavePending] =
    useState(false);
  ...
  const adoptCodexStatusRead = (codex) => {
    ...
    setCodexStatusError("");
    if (read.status.connected && !isStoreUnavailable(read.status)) {
      setCodexDeferredSavePending(false);
    }
    return read;
  };
  ...
  const submitCodexAuthInput = async (input) => {
    ...
    try {
      ...
      if (isCodexDeferredSuccess(result)) setCodexDeferredSavePending(true);
      setCodexAuthStarted(false);
      setCodexAuthWaiting(false);
  ...
  useEffect(() => {
    const onMessage = async (e) => {
      if (e.data?.codex === "success") {
        ...
        if (isCodexDeferredSuccess(e.data)) setCodexDeferredSavePending(true);
        await refreshCodexConnection();
