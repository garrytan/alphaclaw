...
export const Providers = ({ onRestartRequired = () => {} }) => {
  ...
  const [codexStatusKnown, setCodexStatusKnown] = useState(
    () => !!kProvidersTabCache?.codexStatus,
  );
  ...
  // the backup barrier lifts) keeps its own badge until a status read settles
  // the write — saved, failed, or never seen (codex-status.js). Mirrored in a
  // ref because the refresh that follows an exchange runs in the closure
  // that predates the exchange's state update.
  ...
  // barrier) — otherwise the "unavailable during a backup" line outlives the
  // barrier until the operator acts (the status is not otherwise polled).
  ...
  // One adoption path for every successful status READ: an `unavailable`
  // (quiet-period) payload keeps the last-known status under the marker and
  // does not advance `known`; anything else is the checked truth. Returns
  // the read so callers can decide what the tab cache may seed next mount
  // (only a KNOWN status — an unavailable placeholder would read as checked).
  const updateCodexDeferredSave = (next) => {
    codexDeferredSaveRef.current = next;
    setCodexDeferredSave(next);
  };

  const adoptCodexStatusRead = (codex) => {
    ...
    const deferredAfter = applyCodexDeferredSaveRead(deferredBefore, codex);
    ...
    settleStoreUnavailableRecheck(codexStoreRecheckRef, {
      unavailable: isStoreUnavailable(codex),
      recheck: () => refreshCodexConnection(),
    });
    return read;
  };
  ...
  const refresh = async () => {
    ...
    try {
      ...
      // treat those like rejections: a failed status CHECK must not fabricate
      // a "checked" status (or seed the tab cache with one).
      let nextCodexCache = kProvidersTabCache?.codexStatus || null;
      if (codex && codex.ok === false) {
        setCodexStatusError(
          String(codex.error || codex.message || "unknown error"),
        );
      } else {
        const read = adoptCodexStatusRead(codex);
        if (read.known) nextCodexCache = read.status;
      }
  ...
  const refreshCodexConnection = async () => {
    try {
      ...
      // {ok:false} envelope = failed check, exactly like a rejection.
      if (codex && codex.ok === false) {
        setCodexStatusError(
          String(codex.error || codex.message || "unknown error"),
        );
  ...
  const submitCodexAuthInput = async (input) => {
    ...
    try {
      ...
      setCodexManualInput("");
      showToast(buildCodexConnectedMessage(result), result.restartRequired ? "warning" : "success");
  ...
  useEffect(() => {
    const onMessage = async (e) => {
      if (e.data?.codex === "success") {
        showToast(buildCodexConnectedMessage(e.data), e.data.restartRequired ? "warning" : "success");
  ...
  const handleCodexDisconnect = async () => {
    ...
    try {
      ...
      showToast(result.warning || "Codex disconnected", result.restartRequired ? "warning" : "success");
      setCodexAuthStarted(false);
      setCodexAuthWaiting(false);
      setCodexManualInput("");
      invalidateCache(kCodexStatusCacheKey);
  ...
  const renderCodexStatusCheckError = () => {
    if (!codexStatusError) return null;
    const model = buildCodexStatusErrorModel(
      codexStatusKnown ? codexStatus : null,
      codexStatusError,
    );
    return html`<${InlineErrorChip}
      headline=${model.headline}
      error=${model.error}
      onRetry=${refreshCodexConnection}
    />`;
  };

  const renderCodexStoreUnavailable = () => {
  ...
  const renderCodexOAuth = () => {
    const statusBadge = buildCodexStatusBadgeModel({
      codexStatus,
      codexStatusKnown,
      deferredSavePending: codexDeferredSave.pending,
    });
    const deferredSaveFailedLine = buildCodexDeferredSaveFailedLine(
      codexDeferredSave.failedReason,
    );
    return html`
    <div class="border border-border rounded-lg p-3 space-y-2">
      ...
      ${deferredSaveFailedLine
        ? html`<p class="text-xs text-status-error-muted" role="alert">${deferredSaveFailedLine}</p>`
        : null}
      ${renderCodexStoreUnavailable()}
      ${renderCodexStatusCheckError()}
      ${codexAuthStarted
        ? html`
