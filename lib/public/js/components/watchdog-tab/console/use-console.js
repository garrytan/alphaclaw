...
import { loadDiagnosticsForCopy } from "./diagnostics.js";
...
export const useWatchdogConsole = ({
  // Copy-all extras: live status snapshot + recent incident rollups travel
  // with the logs (one-paste debugging handoff).
  copyExtras = {},
} = {}) => {
  ...
  useEffect(() => {
    ...
    const pollLogsDelta = async () => {
      // Hidden tab (fix wave F159): skip the fetch, keep the chain armed; the
      // visibilitychange listener below catches up immediately on return.
      ...
      try {
        ...
        if (payload?.reset) {
          ...
          // A reset delta IS a full tail — without this, a failed initial
          // tail followed by delta recovery would refetch the tail on the
          // next tab visit and then re-append from the stale cursor,
          // duplicating log lines an operator could read as repeats.
  ...
  const handleCopyAll = async () => {
    ...
    setManualCopyText("");
    try {
      const { text, limited } = await loadDiagnosticsForCopy({ copyExtras });
      let copied = false;
      try { copied = await copyTextToClipboard(text); } catch {}
      if (!copied) {
        setManualCopyText(text);
        showToast("Clipboard unavailable — select and copy the report below.", "warning");
      } else {
        showToast(limited ? "Copied limited browser snapshot" : "Copied diagnostic report", limited ? "warning" : "success");
      }
    } finally {
