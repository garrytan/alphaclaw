...
${loadingLogs ? "Loading logs..." : localizedLogs || (logsError ? "" : "No logs yet.")}</pre
    ...
    ${manualCopyText ? html`
      <label class="block mt-3 text-xs text-fg-muted">
        Diagnostic report — select and copy
        <textarea readonly value=${manualCopyText}
          aria-label="Diagnostic report for manual copy"
          onFocus=${(event) => event.currentTarget.select()}
          class="mt-2 w-full h-48 bg-field border border-border rounded-lg p-3 font-mono text-xs" />
      </label>` : null}
    <div
