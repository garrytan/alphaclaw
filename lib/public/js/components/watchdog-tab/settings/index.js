...
export const MemoryMonitorSection = () => {
  ...
  return html`
    <div class="mt-4 pt-3 border-t border-border space-y-3">
      <div class="flex items-center justify-between gap-3">
        <div class="inline-flex items-center gap-2 text-xs text-fg-muted">
          <span>Memory growth detection</span>
          <${InfoTooltip}
            text="Samples gateway group RSS once a minute and separately tracks container pressure. Reports sustained growth, its available attribution, and the relevant policy budget through events, notifications, and Drift Doctor."
          />
        </div>
        <${SavedToggle}
      ...
      <div class="flex items-center justify-between gap-3">
        <div class="inline-flex items-center gap-2 text-xs text-fg-muted">
          ...
          <${InfoTooltip}
            text="When gateway group growth reaches critical pressure, the watchdog may restart it within the per-day budget below (default twice), never during an update. Container pressure alone cannot trigger a restart. Doctor retains the growth evidence for diagnosis."
          />
        </div>
        <${SavedToggle}
      ...
      <${MemoryNumberField}
        ...
        tooltip="Optional whole-group RSS policy budget. The tighter of this budget and configured heap plus 192 MiB applies; actual container pressure is measured separately. Must exceed current group RSS. Leave blank to use the derived group budget."
        ...
        helper=${memory.memoryHydrated && !detectionOn
          ? "Enable memory growth detection to edit."
          : null}
