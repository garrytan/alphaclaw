    ...
    <div class="mt-4 pt-3 border-t border-border space-y-3">
      ...
      <div class="flex items-center justify-between gap-3">
        <div class="inline-flex items-center gap-2 text-xs text-fg-muted">
          ...
          <${InfoTooltip}
            text="When a confirmed leak turns critical, the watchdog restarts the gateway before it runs out of memory — at most twice per day, never during an update. A restart delays the leak; the Doctor finding is the fix path."
          />
        </div>
        <${SavedToggle}
