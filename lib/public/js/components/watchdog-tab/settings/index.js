...
const kNotifyChannels = ["telegram", "slack", "discord", "whatsapp"];
...
const AdminTargetRow = ({
  ...
  <div class="flex flex-wrap items-center gap-2">
    ...
    <input
      type="text"
      class=${kInputClass}
      placeholder="target (chat/user id or number)"
      value=${target.target || ""}
      disabled=${disabled}
    ...
    <button
      type="button"
      class="text-xs text-fg-muted hover:text-status-error"
      disabled=${disabled}
  ...
  return html`
    <div class="mt-4 pt-3 border-t border-border space-y-2">
      ...
      ${!loading && !loadError
        ? html`
            <div class="flex flex-wrap items-center gap-2">
              <label class="text-xs text-fg-muted">Preferred channel</label>
              <select
                class=${kSelectClass}
