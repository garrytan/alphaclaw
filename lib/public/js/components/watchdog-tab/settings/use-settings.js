...
export const describeMemoryEnabledSaveError = (attempted) =>
  `Couldn't confirm ${attempted?.enabled ? "enabling" : "disabling"} memory growth detection — showing the server's current state.`;
...
export const useWatchdogMemorySettings = () => {
  ...
  const onToggleMemoryEnabled = async (nextValue) => {
    ...
    if (outcome.ok) {
      showToast(
        `Memory growth detection ${nextValue ? "enabled" : "disabled"}`,
        "success",
      );
    }
  };
  ...
  // budget clears the operator cap (the derived group RSS budget applies).
