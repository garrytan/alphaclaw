  ...
  if (operation?.phase === "cleanup_blocked" || operation?.phase === "cleanup") {
    ...
    return {
      ...
      detail: operation.phase === "cleanup_blocked"
        ? `Cleanup has not confirmed that every repair writer has stopped. Other gateway operations remain queued.${tracked} Use the rescue session to inspect and stop remaining writers; confirm they have exited before restarting AlphaClaw. Ownership remains held until cleanup confirms termination.`
