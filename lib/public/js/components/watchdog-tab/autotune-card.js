...
import { invalidateCache, setCached } from "../../lib/api-cache.js";
...
export const buildAutotuneSummaryModel = (ledger = null) => {
  ...
  if (counts.pending === 0 && counts.skipped === 0 && counts.clamped === 0) {
    ...
    return {
      ...
      partial: false,
      counts,
      text: `Tuned for this ${formatBytes(profile?.memory?.limitBytes)} / ${profile?.cpu?.cores ?? "—"} vCPU container — no action needed.`,
    };
  }
  if (counts.tuned === 0) {
...
export const buildAutotuneRecommendationModel = (ledger = null) => {
  ...
  return {
    command: `NODE_OPTIONS=--max-old-space-size=${row.value}`,
    text: `Optional — set NODE_OPTIONS=--max-old-space-size=${row.value} in your host's environment settings to cap AlphaClaw's own memory`,
    owner:
...
export const buildAutotuneDisabledParagraph = (profile = null) => {
  const tier = profile?.tier || "unknown";
  const capacity = profile
    ? ` (${formatBytes(profile.memory?.limitBytes)} / ${profile.cpu?.cores ?? "—"} vCPU)`
    : "";
  return `Autotune is off. AlphaClaw and the gateway run with built-in defaults sized for small containers. Detection stays on — this machine is a ${tier} tier${capacity}.`;
};
        ...
        : html`
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                      ...
                      typeof onRestartGateway === "function"
                        ? html`<button
                            type="button"
                            class="text-xs text-status-warning-muted hover:text-status-warning underline underline-offset-2"
                            disabled=${restartingGateway}
                            onclick=${onRestartGateway}
                          >
                            ${restartingGateway
            ...
            ${!enabled
              ? html`<p class="text-sm text-fg-muted">
                  ${buildAutotuneDisabledParagraph(profile)}
                </p>`
              : html`
                  ${environmentNote
                    ? html`<p
                  ...
                  <div class="border-t border-border pt-3 space-y-1">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                      ...
                      <button
                        ...
                        onclick=${() =>
                          handleCopy(recommendation.command, "Environment setting")}
                      >
                        Copy
                      </button>
                    </div>
