...
import {
  hasLocalConsentForMode,
  storeLocalConsentMode,
    ...
    for (let row = 0; row < moduleCount; row += 1) {
      for (let col = 0; col < moduleCount; col += 1) {
        if (qr.isDark(row, col)) {
          path += `M${col + kQrQuietZoneModules},${row + kQrQuietZoneModules}h1v1h-1z`;
        }
      }
    }
    return {
...
const RescueQr = ({ url }) => {
  const model = buildRescueQrModel(url);
...
export const WatchdogRescueSessionCard = () => {
  ...
  const doStart = useCallback(
    async (confirmed) => {
      ...
      setWorking(true);
      try {
        const result = await start({ confirmed });
        showToast(
  ...
  const onStartClick = useCallback(() => {
    ...
    // to the permission mode, so a mode change re-confirms here too.
    doStart(hasLocalConsentForMode(local?.permissionMode));
  }, [doStart, local]);

  const onConfirmStart = useCallback(() => {
    storeLocalConsentMode(local?.permissionMode);
      ...
      ${state === "error" && local.error
        ? html`
            <div class="ac-surface-inset border border-border rounded-lg p-3 space-y-2">
              ...
              <button
                type="button"
                class="ac-tip-link text-xs text-left"
