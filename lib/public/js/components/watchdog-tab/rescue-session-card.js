...
export const WatchdogRescueSessionCard = () => {
  ...
  // The mode+cwd the pending confirm belongs to. Seeded from the server's
  // AUTHORITATIVE live config carried on the confirm_required (409) body so the
  // modal names the mode the server is actually set to — never a possibly
  // stale polled snapshot. Null until a refusal captures it (older servers that
  // omit the fields leave it null and the modal falls back to `local`).
  ...
  const doStart = useCallback(
    async (confirmed, permissionModeOverride = undefined) => {
      if (working) return;
      setWorking(true);
      try {
        ...
        const result = await start({ confirmed, permissionMode });
        showToast(
        ...
        if (error?.code === "confirm_required") {
          // Prefer the server's live mode+cwd on the 409 body; fall back to the
          // polled snapshot only when an older server omits them.
          setConfirmScope({
            permissionMode: error.permissionMode ?? local?.permissionMode ?? null,
            cwd: error.cwd ?? local?.cwd ?? null,
          });
          setConfirmOpen(true);
          return;
        }
  ...
  const onConfirmStart = useCallback(() => {
    // Consent + the re-fire use the mode+cwd the modal actually displayed: the
    // server's live scope when the refusal carried it, otherwise the snapshot.
    ...
    doStart(true, scope.permissionMode);
  }, [doStart, confirmScope, local]);
              ...
              <div class="flex flex-wrap items-start gap-3">
                ...
                <div class="min-w-0 space-y-1">
                  ...
                  <p class="text-xs text-fg-muted">
                    ...
                    This link stops working when the session is stopped.
                  </p>
                </div>
              </div>
              ${local.hosting === "tmux"
...
${tail || local.error.tailSanitized || "Loading CLI output…"}</pre
      ...
      <${ClaudeCodeConfirmModal}
        visible=${confirmOpen}
        mode="local"
        permissionMode=${confirmScope ? confirmScope.permissionMode : local.permissionMode}
