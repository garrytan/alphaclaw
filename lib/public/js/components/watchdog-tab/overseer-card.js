...
// A `skipped` record means the deterministic admission gate declined an
// AUTOMATIC review (#87) — nothing ran, so there is no verdict and no CTA. The
// incident row's "Review this incident" action still offers a manual review.
export const kOverseerSkipReasonCopy = {
...
const kSkipFallbackLine = "Automatic review skipped.";
...
const buildIncidentReport = (list, nowMs) => {
  ...
  if (state === "skipped") {
    return {
      ...
      state,
      shortLabel: "Skipped",
      line: kOverseerSkipReasonCopy[current.reason] || kSkipFallbackLine,
    };
  }
...
export const describeOverseerToggleError = (attempted) =>
  attempted
    ? "Couldn't enable the overseer — still disabled."
    : "Couldn't disable the overseer — still enabled.";
...
export const WatchdogOverseerCard = ({
  ...
  saving = false,
  onToggle = () => {},
  // useSavedSetting slice from the tab hook (F158); the legacy props above
  // stand in when a caller renders the card without one.
  setting = null,
  situation = null,
  situationError = null,
  ephemeral = null,
    ...
    <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-3">
          ...
          <${SavedToggle}
            value=${enabled}
            hydrated=${setting ? setting.hydrated : settingsLoaded}
            saving=${setting ? setting.saving : saving}
            savingContext=${setting?.savingContext ?? null}
            saveError=${setting?.saveError ?? null}
            loadError=${setting?.loadError ?? null}
            onRetryLoad=${setting?.retryLoad ?? null}
            onChange=${onToggle}
            describe=${describeOverseerToggleError}
          />
        </div>
      </div>
