...
export const buildWatchdogOverseerModel = (incidents = [], nowMs = Date.now()) => {
  ...
  const badge = kOverseerVerdictBadge[current.verdict] || kOverseerVerdictBadge.unparseable;
...
export const WatchdogOverseerCard = ({
  ...
  useEffect(() => {
    if (!loaded || !availability || availability.available !== null) return;
    if (probeRetriesRef.current >= 5) return;
    probeRetriesRef.current += 1;
    const timer = setTimeout(load, 3000);
    return () => clearTimeout(timer);
  }, [loaded, availability, load]);
  ...
  return html`
    <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
        ...
        <div class="flex items-center gap-3">
          ${enabled
            ? html`<${ActionButton}
                ...
                loading=${reviewing}
                disabled=${!!availability && availability.available !== true}
              />`
            : null}
          <${ToggleSwitch}
            checked=${enabled}
      ...
      ${availabilityLine
        ? html`<p
            class=${`text-xs ${availability?.available === false ? "text-status-warning-muted" : "text-fg-muted"}`}
          >
