...
const kOverseerChip = {
  resolved: { tone: "success", label: "Overseer: resolved" },
  monitoring: { tone: "warning", label: "Overseer: monitoring" },
  action_needed: { tone: "danger", label: "Overseer: action needed" },
};
...
const IncidentCard = ({
  ...
  // A stale review's verdict describes a version of the incident that gained
  // events afterwards — keep it visible but never with a live-verdict tone.
  const overseerChip =
    overseerCurrent?.state === "stale" && kOverseerChip[overseerCurrent.verdict]
      ? {
          tone: "neutral",
          label: `Overseer: ${kOverseerChip[overseerCurrent.verdict].label.replace("Overseer: ", "")} (stale)`,
        }
      : kOverseerChip[overseerCurrent?.verdict] || null;
  return html`
    <details
      ref=${cardRef}
