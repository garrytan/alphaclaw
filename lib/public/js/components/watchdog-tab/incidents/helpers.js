...
export const kWatchdogEventLabels = {
  ...
  // #87: the detached Doctor's structured evidence for an open readiness
  // incident, and the event-loop diagnostic that is telemetry, not readiness.
  readiness_advisory: "Doctor advisory",
  event_loop_pressure: "Event loop pressure",
...
export const describeEventOutcome = (event = {}) => {
  ...
  if (eventType === "health_check" && status === "ok") {
    ...
    // #87 E5: a transitional readiness (the gateway said starting|draining)
    // and a held recovery (the readiness probe could not be read while
    // readiness was not_ready) are "up" but not recovery — name which.
    if (details.readinessStatus === "starting") {
      return { phrase: "up, still starting", tone: "warning" };
    }
    if (details.readinessStatus === "draining") {
      return { phrase: "up, draining", tone: "warning" };
    }
    if (typeof details.readinessProbe === "string" && details.readinessProbe) {
      return { phrase: `up, readiness probe ${details.readinessProbe}`, tone: "warning" };
    }
  ...
  // #87: the detached Doctor's structured finding — checkId + severity are
  // the fact; the sanitized message rides as the salient detail.
  if (eventType === "readiness_advisory") {
    const finding =
      details.finding && typeof details.finding === "object" ? details.finding : {};
    const checkId = typeof finding.checkId === "string" && finding.checkId
      ? finding.checkId
      : "unknown";
    const severity = typeof finding.severity === "string" && finding.severity
      ? finding.severity
      : "unknown";
    return { phrase: `doctor: ${checkId} (${severity})`, tone: "warning" };
  }
  // #87: event-loop pressure is telemetry — never a readiness degradation.
  if (eventType === "event_loop_pressure") {
    if (status === "ok") {
      return { phrase: "event loop recovered", tone: "success" };
    }
    const reasons = Array.isArray(details.reasons)
      ...
      : [];
    return {
      phrase: reasons.length
        ? `event loop under pressure: ${reasons.join(", ")}`
        : "event loop under pressure",
      tone: "warning",
    };
  }
  // #87 RT7: a fail-open close carries {recovered, assumed, kind} — readiness
  // could not be read (X1 bound expiry, a 404 `unsupported`, a thrown
  // evaluation), so this is NOT a real recovery and must not render as one.
  if (eventType === "readiness_degraded" && status === "ok" && details.assumed === true) {
    const kind = typeof details.kind === "string" && details.kind
      ? details.kind.replaceAll("_", " ")
      : "unknown";
    return { phrase: `recovery assumed — readiness unreadable (${kind})`, tone: "warning" };
  }
...
const salientDetail = (event = {}) => {
  ...
  // An assumed recovery's outcome phrase (describeEventOutcome) is the whole
  // story — a bare "recovered" beside it would contradict it.
  if (details.recovered) return details.assumed === true ? null : "recovered";
