...
// "no GPU"). On genuine bare metal host values ARE the correct values, so no
// source badge renders at all — the warning is reserved for container/unknown
// environments where host fallback means degraded detection. Returns null
// when the server doesn't send a profile (older server) — the card renders
// exactly as before.
export const buildCapacityHeaderModel = (profile = null) => {
  ...
  return {
    ...
    hostValues:
      profile.memory?.source === "host" &&
      profile.environment !== "bare-metal",
    gpuLabel,
  };
};

// Memory-leak trend view-model (pure — exported for tests). Every detector
// state renders HONESTLY: "off" and "collecting" are stated, never dressed
// up as healthy; no_gateway (or a missing/legacy payload) hides the row.
// Badge = word + tone (never color-only); copy stays calm-narrative — a
// projection reads "projected to reach its limit in ~3h", never scare-styled.
export const buildMemoryTrendModel = (trend = null, { nowMs = Date.now() } = {}) => {
  ...
  const statsParts = () => {
    ...
    if (trend.rssMb != null) {
      const cap =
        trend.effectiveCapMb != null ? ` of ${trend.effectiveCapMb} MB cap` : "";
      parts.push(`${trend.rssMb} MB${cap}`);
    }
    if (trend.slopeMbPerHour != null && trend.slopeMbPerHour > 0) {
      parts.push(`+${trend.slopeMbPerHour} MB/h`);
    }
    return parts;
  };
  const projectionPart = () => {
    // Future-only: a stale payload (tab left open) can carry a projection
    // that has already passed — "reach its limit in ~-3m" is nonsense.
    const projectedMs = Date.parse(trend.projectedExhaustionAt ?? "");
    if (!Number.isFinite(projectedMs) || projectedMs <= nowMs) return null;
    const eta = formatRelativeTime(trend.projectedExhaustionAt, {
      nowMs,
      style: "unit",
      allowFuture: true,
      fallback: "",
    });
    return eta ? `projected to reach its limit in ~${eta}` : null;
  };
  const joinParts = (...parts) => parts.flat().filter(Boolean).join(" · ");
  switch (trend.state) {
    case "disabled":
      return {
        tone: "neutral",
        label: "Detection off",
        detail: "Enable memory leak detection in Settings.",
        alwaysVisible: false,
      };
    case "warming_up":
    case "insufficient_samples":
      return {
        tone: "neutral",
        label: "Collecting",
        detail: `Collecting samples… (${trend.sampleCount ?? 0}/${trend.requiredSamples ?? 24})`,
        alwaysVisible: false,
      };
    case "normal":
      return {
    ...
    case "watch":
      return {
        tone: "warning",
        label: "Rising",
        detail: joinParts(statsParts(), "unconfirmed — watching"),
        alwaysVisible: false,
      };
    case "leak_suspected":
      return {
        ...
        detail: joinParts("Memory rising steadily", statsParts(), projectionPart()),
        alwaysVisible: true,
      };
    case "critical":
      return {
        tone: "danger",
        label: "Critical",
        detail: joinParts("Memory nearly exhausted", statsParts(), projectionPart()),
        alwaysVisible: true,
      };
    default:
      return null; // no_gateway or an unknown future state: hidden
  }
};

const MemoryTrendRow = ({ trend }) => {
  const model = buildMemoryTrendModel(trend);
  if (!model) return null;
  return html`
    <div class="mt-2 flex flex-wrap items-center gap-2 text-xs">
      <span class="inline-flex items-center gap-1 text-fg-muted">
        Gateway memory trend
        <${InfoTooltip}
          text="RSS sampled once a minute. Rising per-window floors suggest a leak (or allocator fragmentation — recurrence after a restart tells them apart). Suspected leaks also open a Drift Doctor finding with a guided fix."
        />
      </span>
      <${Badge} tone=${model.tone}>${model.label}</${Badge}>
      ${model.detail
        ? html`<span class="text-fg-muted">${model.detail}</span>`
        : null}
    </div>
  `;
};
...
export const WatchdogResourcesCard = ({
  ...
  const memoryTrend = resources?.gatewayMemoryTrend || null;
  // The trend row lives in the expandable memory section, but an active
  // leak/critical verdict stays visible without expanding — hiding a live
  // warning behind a click would be dishonest.
  const memoryTrendVisible =
    memoryExpanded || buildMemoryTrendModel(memoryTrend)?.alwaysVisible === true;
    ...
    ? html`
        ${memoryExpanded
          ...
          : html`
            ...
            `}
        ${memoryTrendVisible ? html`<${MemoryTrendRow} trend=${memoryTrend} />` : null}
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              ${capacity.hostValues
                ? html`<${TooltipBadge}
                    tone="warning"
                    label="Host values"
                    text="Container limits couldn't be detected — these numbers are the host's full capacity, not this container's share."
                  />`
                : null}
              ${capacity.gpuLabel
