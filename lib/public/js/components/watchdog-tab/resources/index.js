...
export const buildMemoryTrendModel = (trend = null, { nowMs = Date.now() } = {}) => {
  ...
  const statsText = () => {
    const rss = trend.rssMb != null ? `${trend.rssMb} MB` : null;
    const cap =
      trend.effectiveCapMb != null ? ` of ${trend.effectiveCapMb} MB cap` : "";
    const slope =
      trend.slopeMbPerHour != null && trend.slopeMbPerHour > 0
        ? ` · +${trend.slopeMbPerHour} MB/h`
        : "";
    return rss ? `${rss}${cap}${slope}` : "";
  };
  const projectionText = () => {
    if (!trend.projectedExhaustionAt) return "";
    ...
    return eta ? ` · projected to reach its limit in ~${eta}` : "";
  };
  switch (trend.state) {
    case "disabled":
      return {
    ...
    case "watch":
      return {
        tone: "warning",
        label: "Rising",
        detail: `${statsText()} · unconfirmed — watching`,
        alwaysVisible: false,
      };
    case "leak_suspected":
      return {
        ...
        detail: `Memory rising steadily · ${statsText()}${projectionText()}`,
        alwaysVisible: true,
      };
    case "critical":
      return {
        tone: "danger",
        label: "Critical",
        detail: `Memory nearly exhausted · ${statsText()}${projectionText()}`,
        alwaysVisible: true,
      };
    default:
