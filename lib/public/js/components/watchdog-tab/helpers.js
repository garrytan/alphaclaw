...
import { describeEventOutcome } from "./incidents/helpers.js";
...
const kDegradedReasonCopy = {
  readiness_failing: (status) =>
    `Readiness checks are failing${
      status.readinessReason ? ` (${status.readinessReason})` : ""
    }; the port answers and /health is green.`,
  gateway_conflict_unhealthy: () =>
    "Another gateway holds the state directory but is not healthy; a repair will replace it.",
  ...
  incumbent_unhealthy: () =>
    "The gateway answering the port is not healthy; the degraded ladder owns recovery.",
  replacement_not_ready: () =>
    "The relaunched gateway never became ready within its budget.",
};
...
const kIncidentDotClassByTone = {
  success: "bg-green-500/90",
  danger: "bg-red-500/90",
  warning: "bg-yellow-400/90",
  info: "bg-blue-400/80",
  neutral: "bg-gray-500/70",
};
...
export const getIncidentStatusTone = (event) => {
  ...
  // status word, and the dot follows the same tone table the incidents
  // timeline uses. A `failed` row stays "Failed" above (a probe-detected
  // death IS a failure; its phrase rides in the row detail).
  const outcome = describeEventOutcome(event);
  if (outcome) {
    return {
      dotClass: kIncidentDotClassByTone[outcome.tone] || kIncidentDotClassByTone.neutral,
