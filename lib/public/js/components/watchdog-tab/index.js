...
// resources (live usage — checked daily) → autotune (capacity/tuning —
// checked on incidents and resizes) → settings. The Gateway card owns
// safe-mode presentation when the server sends the unified state; the
// standalone banner survives only for old servers (version skew).
  ...
  return html`
    <div class="space-y-4">
      ...
      <${WatchdogIncidentsCard}
        events=${state.events}
        hasLoaded=${state.eventsLoaded}
        error=${state.eventsError}
        isRefreshing=${state.refreshingEvents}
