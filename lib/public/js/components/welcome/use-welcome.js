...
export const useWelcome = ({ onComplete }) => {
  ...
  const {
    ...
    codexStatusUnknown,
    codexStatusKnown,
    codexDeferredSavePending,
    codexDeferredSaveFailedReason,
    codexStatusError,
    codexManualInput,
    setCodexManualInput,
    codexExchanging,
  ...
  // Channels the server already knows about (e.g. a Signal channel linked via
  // signal-cli, #113): the wizard shows them as "Configured" and stops telling
  // the operator they'll continue with web chat. Detection is additive — a
  // failed/slow status fetch just renders the wizard exactly as before.
  const statusFetchState = useCachedFetch("/api/status", fetchStatus, {
    maxAgeMs: 15000,
  });
  const detectedChannelIds = Object.entries(
    statusFetchState.data?.channels || {},
  )
    .filter(([, channel]) =>
      ["configured", "paired"].includes(String(channel?.status || "")),
    )
  ...
  const applyModelCatalog = useCallback((payload) => {
    ...
    // 2026.8 defaults: a fresh Codex OAuth setup prefers gpt-5.6-sol, a fresh
    // OpenAI API-key setup the gpt-5.6 alias — only when the catalog lists
    // them (C8), and only while the user hasn't picked a model themselves.
    const defaultModelKey = resolveOnboardingModelDefault({
      catalog: list,
      authProvider: codexStatus.connected
        ? "openai-codex"
        : String(vals.OPENAI_API_KEY || "").trim()
          ? "openai"
          : "",
  ...
  }, [setVals, vals.MODEL_KEY, vals.OPENAI_API_KEY, codexStatus.connected]);
  ...
  useEffect(() => {
    ...
    // D12: the bootstrap catalog keeps the picker usable — say so instead of
    // presenting a dead end.
    const cause = modelsFetchState.error?.message;
    setModelsError(
      cause
        ? `Live model list unavailable (${cause}) — using compatible defaults.`
        : "Live model list unavailable — using compatible defaults.",
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!pairingChannel) {
        // Zero channels is a valid setup (2.4/D12): the built-in web chat works
        // without one, so there is nothing to pair — finish straight to the
        // dashboard, where General → Channels adds one later.
        setLoading(false);
        setSetupError(null);
        finishOnboarding();
        return;
