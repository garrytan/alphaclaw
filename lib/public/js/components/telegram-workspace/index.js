...
import { TelegramIcon } from "../icons.js";
...
export const TelegramWorkspace = ({ accountId = "default", onBack }) => {
  ...
  // Mutation generation: bumped by resetOnboarding so bootstrap GET responses
  // dispatched before the latest mutation are discarded (stale-clobber guard).
  const workspaceGenerationRef = useRef(0);

  const goNext = () => setStep((s) => Math.min(kSteps.length - 1, s + 1));
  const goBack = () => setStep((s) => Math.max(0, s - 1));
  const resetOnboarding = async (mode = "keep") => {
    try {
      const data = await api.resetWorkspace({ accountId, mode });
  ...
  useEffect(() => {
    ...
    // Discard responses once unmounted, on account switch, or after a newer
    // mutation (reset) has already dispatched fresher state.
    const isCurrent = () =>
      active && generation === workspaceGenerationRef.current;
      ...
      try {
        ...
        setBootstrapError(
          String(e?.message || "").trim() ||
            "Failed to load Telegram workspace",
        );
      }
    };
    bootstrapWorkspace();
                ...
                <div class="min-h-[220px] flex items-center justify-center">
                  <div class="w-full max-w-md">
                    <${InlineErrorChip}
                      ...
                      headline="Couldn't load your Telegram workspace."
