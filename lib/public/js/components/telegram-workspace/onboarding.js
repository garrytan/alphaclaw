...
export const VerifyBotStep = ({ accountId, botInfo, setBotInfo, onNext }) => {
  ...
  return html`
    <div class="space-y-4">
      ...
      ${error &&
      html`
        <div class="bg-red-500/10 border border-red-500/20 rounded-lg p-3 space-y-2">
          <p class="text-sm text-status-error-muted">${error}</p>
          <button
            onclick=${verify}
            disabled=${loading}
            class="text-xs px-2 py-1 rounded border border-border text-fg-muted hover:text-body hover:border-fg-muted ${loading
    ...
    <div class="space-y-4">
      <div class="max-w-xl mx-auto text-center space-y-10 mt-10">
        ...
        <p class="text-xs text-fg-muted">
          ...
          <code class="bg-field px-1 rounded">hooks/bootstrap/AGENTS.md</code>
          so your agent knows which thread ID maps to which topic name.
        </p>
