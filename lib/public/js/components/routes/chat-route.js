...
// itself, so escapeHtmlForMarkdown is no longer applied here.
...
export const ChatRoute = ({ sessions = [], selectedSessionKey = "" }) => {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onopen = () => {
        if (!mounted) return;
        // The socket is live again — clear any HTTP-fallback latch so a
        // transient early fallback doesn't disable realtime for the page
        // lifetime (MW5).
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        ...
        // Only latch realtime OFF when the socket is genuinely unavailable —
        // not while it is still CONNECTING (readyState 0). Latching on a
        // connecting socket (HTTP history resolved before ws.onopen) and closing
        // it disabled realtime for the whole page lifetime (MW5).
        const ws = wsRef.current;
        const wsConnectingOrOpen =
          ws && (ws.readyState === 0 || ws.readyState === 1);
        if (!isConnected && !wsConnectingOrOpen) {
          // HTTP history works while WS is truly down: stop noisy reconnect loops.
