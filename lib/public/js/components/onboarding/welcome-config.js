...
// Channels are OPTIONAL since OpenClaw's web chat works without one (2.4/D12):
// zero channels is a valid setup — the wizard's secondary action reads "Continue
// with web chat". Only half-configured Slack still blocks (it can never work).
...
// Whether any chat channel is configured — drives the pairing-step skip and the
// "Continue with web chat" framing. Besides locally typed tokens, channels the
// server already reports as configured/paired (ctx.detectedChannelIds — e.g. a
// Signal channel linked via signal-cli, #113) count too: the CTA must not tell
// a Signal-only operator they'll "continue with web chat".
export const hasAnyChannelConfigured = (vals = {}, ctx = {}) =>
  (Array.isArray(ctx.detectedChannelIds) &&
    ctx.detectedChannelIds.length > 0) ||
  hasValue(vals.CLICKCLACK_BOT_TOKEN) ||
  hasValue(vals.TELEGRAM_BOT_TOKEN) ||
  hasValue(vals.DISCORD_BOT_TOKEN) ||
  (hasValue(vals.SLACK_BOT_TOKEN) && hasValue(vals.SLACK_APP_TOKEN));
...
export const kWelcomeGroups = [
  ...
  },
  {
    ...
    description:
      "Add a chat channel now, or continue with the built-in web chat and add channels later from General → Channels",
    fields: [
      {
      ...
      {
        key: "CLICKCLACK_BOT_TOKEN",
        label: "ClickClack Bot Token",
        hint: "From ClickClack under Workspace settings → Integrations (fresh installs use the manual token; the paste-code flow arrives with OpenClaw 2026.8)",
        placeholder: "ccb_...",
      },
    ],
