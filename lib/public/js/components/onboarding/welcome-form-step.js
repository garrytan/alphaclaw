...
import { SearchableModelPicker } from "../models-tab/model-picker.js";
import { getFeaturedModels } from "../../lib/model-config.js";
import { getChannelMeta } from "../channels.js";
import { kOnboardingChannelDefs } from "../../lib/channel-registry.js";
import {
...
// Derived from the shared channel registry (5.0) so new providers land once.
const kChannelAccordionDefs = kOnboardingChannelDefs;
...
export const WelcomeFormStep = ({
  ...
  // Older callers only pass codexStatusUnknown (= error && !known); when the
  // hook doesn't say, a non-unknown status counts as checked.
  codexStatusKnown = !codexStatusUnknown,
  codexDeferredSavePending = false,
  // Set once a deferred profile write is known lost (server verdict or the
  // saved connection never appeared): the step must say so, not stay silent.
  codexDeferredSaveFailedReason = null,
  codexStatusError = "",
  startCodexAuth,
  ...
  // Externally-configured channels the server detected (e.g. Signal linked
  // via signal-cli, #113). Only ids WITHOUT a token accordion render as rows —
  // a detected telegram/discord/slack/clickclack already has its accordion
  // entry and must not appear twice.
  const kAccordionChannelIds = new Set(
    kChannelAccordionDefs.map((section) => section.id),
  );
  const externallyConfiguredIds = (detectedChannelIds || []).filter(
    (channelId) => !kAccordionChannelIds.has(channelId),
  );
  const renderChannelAccordion = () =>
    html`<div class="space-y-2">
      ${externallyConfiguredIds.map((channelId) => {
        const channelMeta = getChannelMeta(channelId);
        return html`
          <div
            class="bg-field border border-border rounded-lg px-3 py-2 flex items-center gap-2"
          >
            ${channelMeta.icon
              ? html`<${channelMeta.icon} className="w-4 h-4 rounded-sm" />`
              : null}
            <span class="text-sm text-body">${channelMeta.label}</span>
            <${Badge} tone="success">Configured</${Badge}>
            <span class="text-xs text-fg-muted"
              >Detected in your OpenClaw config</span
            >
          </div>
        `;
      })}
  ...
  return html`
    ...
    (() => {
      const selectedModel =
        modelOptions.find((model) => model.key === vals.MODEL_KEY) ||
        (vals.MODEL_KEY ? { key: vals.MODEL_KEY } : null);
      const showSkeleton = modelsLoading && modelOptions.length === 0;
      return html`
        <div class="space-y-1.5">
          ...
          ${showSkeleton
            ? html`
                <div
                  class="h-[38px] bg-field border border-border rounded-lg animate-pulse"
                  aria-hidden="true"
                ></div>
              `
            : html`
                ${selectedModel
                  ? html`
                      <div
                        class="flex items-center justify-between gap-2 bg-field border border-border rounded-lg px-3 py-2"
                      >
                        <div class="min-w-0">
                          <div class="text-sm text-body truncate">
                            ${selectedModel.featuredLabel ||
                            selectedModel.label ||
                            selectedModel.key}
                          </div>
                        ...
                        <span
                          class="text-xs font-medium px-2 py-0.5 rounded-full bg-status-success-bg text-status-success shrink-0"
                          >Selected</span
                        >
                      </div>
                    `
                ...
                <${SearchableModelPicker}
                  ...
                  popularModels=${showAllModels
                    ? getFeaturedModels(modelOptions)
                    : []}
                  placeholder=${selectedModel
                    ? "Search to change model..."
                    : "Search models..."}
                  onSelect=${(modelKey) => setValue("MODEL_KEY", modelKey)}
                />
              `}
          <p class="text-xs text-fg-dim">
            ${modelsLoading
      ...
      <div class="bg-field border border-border rounded-lg p-3 space-y-2">
        <div class="flex items-center justify-between">
          ...
          ${codexLoading
            ...
            : (() => {
                const badge = codexStatusUnknown
                  ? kCodexStatusBadges.unknown
                  : buildCodexStatusBadgeModel({
                      codexStatus,
                      codexStatusKnown,
                      deferredSavePending: codexDeferredSavePending,
                    });
                return html`<${Badge} tone=${badge.tone}>${badge.label}</${Badge}>`;
              })()}
        </div>
        ${(() => {
          const line = buildCodexDeferredSaveFailedLine(codexDeferredSaveFailedReason);
          return line
            ? html`<p class="text-xs text-status-error-muted" role="alert">${line}</p>`
            : null;
        })()}
        ${(() => {
          const line = buildCodexStoreUnavailableLine({ codexStatus, codexStatusKnown });
          return line
            ? html`<p class="text-xs text-status-warning-muted" role="status">${line}</p>`
            : null;
        })()}
        ${codexStatusError && !codexStatusUnknown
          ? html`<p class="text-xs text-fg-muted">
              Status check failed — ${codexStatusError} — showing the last
              known status.
            </p>`
          : null}
        ...
        ? html`
            ...
            <${ActionButton}
              ...
              githubFlow === kGithubFlowImport
                ? "Check compatibility"
                : activeGroup.id === "channels" &&
                    !hasAnyChannelConfigured(vals, { detectedChannelIds })
                  ? "Continue with web chat"
                  : "Next"}
