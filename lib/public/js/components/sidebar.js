...
const renderNavItem = ({ item, selectedNavId, onSelectNavItem, claudeCode }) => {
  ...
  // in a new tab instead of routing inside the SPA.
  if (item.href) {
    return html`
        ...
        ? html`
            ...
            <nav class="sidebar-nav">
              ${setupSection.items.map((item) =>
                renderNavItem({ item, selectedNavId, onSelectNavItem, claudeCode }),
              )}
            </nav>
          `
        : null}
