...
export const AgentSendModal = ({
  ...
  // Optional per-selection hint rendered under the session select (e.g. the
  // Doctor fix modal's "delivers to chat" vs "runs in main thread").
  renderSessionHint = null,
}) => {
  const {
    sessions,
    selectedSessionKey,
  ...
  return html`
    ...
    >
      ...
      <div class="space-y-1">
        ...
        />
        ${typeof renderSessionHint === "function"
          ? renderSessionHint(selectedSession)
          : null}
      </div>
      <div class="space-y-2">
