...
export const createSendOutbox = ({
  ...
  const retry = (clientMsgId) => {
    ...
    if (item.status === "unknown") {
      // The bridge REPLAYS an unknown terminal for the same clientMsgId for
      // its whole dedupe window — an explicit Retry here is the user
      // consciously choosing "send again" after checking the transcript
      // (the chip says so), so it must be a genuinely fresh attempt.
      // Failed items keep their id: their `error` terminals fall through to
      // a fresh send server-side, and the stable id stays dedupe-safe if the
      // old attempt is somehow still live.
      removedIds.add(item.clientMsgId);
      item.clientMsgId = uuid();
    }
    item.status = "queued";
    item.attempts = 0;
