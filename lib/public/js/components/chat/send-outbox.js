...
export const createSendOutbox = ({
  storage = null,
  storageKey = "alphaclaw.chat.sendOutbox",
  now = () => Date.now(),
  ...
  const markFailed = (clientMsgId, { code = "", message = "", retryable = false } = {}) => {
    ...
    if (code === "session_busy") {
      ...
      item.status = "queued";
      const backoff = Math.min(kRetryBaseMs * 2 ** (item.attempts - 1), kRetryMaxMs);
      item.nextAttemptAt = now() + Math.round(backoff * (0.85 + 0.3 * random()));
    } else {
      item.status = "failed";
    }
    persist();
    ...
    for (const item of items) {
      ...
      if (item.attempts >= kMaxAutoAttempts) {
        ...
        item.nextAttemptAt =
          nowMs +
          Math.round(
            Math.min(kRetryBaseMs * 2 ** (item.attempts - 1), kRetryMaxMs) *
              (0.85 + 0.3 * random()),
          );
      }
    }
  ...
  // Socket died before acks arrived: inflight items go straight back to
  // queued (same clientMsgId — the bridge dedupes, so the retry is safe).
