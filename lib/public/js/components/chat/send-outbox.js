...
// confirm delivery. A history outage parks uncertainty; it never authorizes a send.
...
export const createSendOutbox = ({
  ...
  const persist = ({ requiredSubmittedId = "" } = {}) => {
    if (memoryOnly) {
      onChange(items);
      return false;
    }
    ...
    try {
      ...
      // A successful write alone is insufficient: pruning may have removed
      // the exact item whose submission this write was meant to authorize.
      persisted = !requiredSubmittedId || list.some(
        (item) => item.clientMsgId === requiredSubmittedId && item.possiblySubmitted === true,
      );
    } catch (err) {
      // Quota exceeded / private mode: retain the text in memory, but never
      // authorize transmission without its durable submission marker.
      if (!memoryOnly) {
        memoryOnly = true;
        onPersistError(err);
      }
  ...
  const markInflight = (clientMsgId) => {
    ...
    const previous = {
      possiblySubmitted: item.possiblySubmitted,
      sentAt: item.sentAt,
      attempts: item.attempts,
    };
    item.status = "inflight";
    // Persist before socket transmission; attempts are refundable and cannot
    // serve as submission evidence.
    item.possiblySubmitted = true;
    item.sentAt = now();
    item.attempts += 1;
    if (!persist({ requiredSubmittedId: item.clientMsgId })) {
      Object.assign(item, previous);
      item.status = previous.possiblySubmitted ? "unknown" : "failed";
      item.lastError = previous.possiblySubmitted ? {
        code: "unknown_outcome", retryable: false,
        message: "This message may have been sent — check the transcript before sending again.",
      } : {
        code: "browser_storage_unavailable", retryable: true, notSubmitted: true,
        message: "Not sent — this browser could not save its delivery marker. Restore browser storage and try again.",
      };
      onChange(items);
      return null;
    }
  ...
  const markFailed = (clientMsgId, { code = "", message = "", retryable = false, notSubmitted = false } = {}) => {
    const item = findItem(clientMsgId);
    if (!item) return;
    if (notSubmitted === true) item.possiblySubmitted = false;
    item.lastError = { code, message, retryable, notSubmitted };
    if (code === "unknown_outcome" || (item.possiblySubmitted && notSubmitted !== true)) {
      item.status = "unknown";
      item.lastError = { code: "unknown_outcome", message: "This message may have been sent — check the transcript before sending again.", retryable: false };
    ...
    for (const item of items) {
      ...
      if (item.attempts >= kMaxAutoAttempts) {
        ...
        item.lastError = {
          ...
          message: "The server did not acknowledge this message — check the transcript before sending again.",
          retryable: false,
        };
      } else {
        item.status = "queued";
  ...
  const retry = (clientMsgId) => {
    ...
    // A human retry may probe recovered storage. Automatic flushes stay
    // parked after a write failure; markInflight still requires a new proof.
    if (storage !== null) memoryOnly = false;
    if (item.possiblySubmitted || item.status === "unknown") {
      // An explicit send-again is a NEW logical message. Keep the uncertain
      // original available for review/discard (and future history merging).
      const replacement = enqueue({ sessionKey: item.sessionKey, content: item.content });
      item.resentAs = replacement.clientMsgId;
      persist();
      return replacement;
    }
    item.status = "queued";
  ...
  const nextEligible = (sessionKey) => {
    const nowMs = now();
    if (items.some((item) => item.sessionKey === String(sessionKey || "") &&
        item.status === "unknown" && !item.resentAs && !item.followupsConfirmed)) return null;
    return (
      items.find(
        (item) =>
  ...
  const confirmQueued = (sessionKey) => {
    for (const item of items) {
      if (item.sessionKey === sessionKey && item.status === "unknown") item.followupsConfirmed = true;
    }
    persist();
  };
  ...
  const requeue = (clientMsgId, { possiblySubmitted = false } = {}) => {
    const item = findItem(clientMsgId);
    if (!item || item.status !== "inflight") return;
    item.status = "queued";
    item.possiblySubmitted = possiblySubmitted;
    item.attempts = Math.max(0, item.attempts - 1);
  ...
  const releaseAwaitingHistory = (sessionKey = "", { force = false, olderThanMs = 0, failed = false } = {}) => {
    const nowMs = now();
    let changed = false;
    for (const item of items) {
      ...
      item.status = failed || force ? "unknown" : "queued";
      if (item.status === "unknown") item.lastError = {
        code: "unknown_outcome", retryable: false,
        message: "History could not confirm this message — check the transcript before sending again.",
      };
      item.nextAttemptAt = 0;
      changed = true;
    }
  ...
  const restoreOnLoad = () => {
    ...
    items = persisted
      ...
      .map((item) => {
        // Old snapshots have no explicit marker. Treat any attempted send
        // conservatively; a queued, never-attempted draft is still unsent.
        const possiblySubmitted = item.possiblySubmitted === true ||
          (item.possiblySubmitted === undefined && (item.attempts > 0 || ["inflight", "acked", "unknown"].includes(item.status)));
        if (item.status === "unknown" || item.status === "failed") return { ...item, possiblySubmitted };
        return {
          ...item, possiblySubmitted,
          status: possiblySubmitted ? "unknown" : "failed",
          lastError: {
            code: possiblySubmitted ? "unknown_outcome" : "restored",
            message: possiblySubmitted ? "This message may have been sent — check the transcript before sending again." :
              "Not sent — this message was queued when the page closed.",
            retryable: !possiblySubmitted,
          },
        };
      });
    persist();
