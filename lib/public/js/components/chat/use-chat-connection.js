...
export const useChatConnection = ({
    ...
    const connect = () => {
      ...
      ws.onmessage = (event) => {
        ...
        if (payload.type === "hello") {
          capabilitiesRef.current = payload;
          monitor.recordHello();
          setHello(payload);
          syncStatus();
          if (!readyFiredRef.current) {
  ...
  // Keepalive ping (shared primitive, fix wave PR 11). pauseWhenHidden:false
  // on purpose — a backgrounded tab must keep its socket alive.
  useVisibleInterval(
    () => {
      const ws = wsRef.current;
    ...
    kClientPingIntervalMs,
    { enabled: Boolean(enabled), pauseWhenHidden: false },
  );
  ...
  const noteHttpHistoryWorked = useCallback(() => {
    ...
    // One successful HTTP fetch during a momentary socket gap does not prove
    // upgrades are unavailable — a transient blip must not latch the sticky
    // fallback (which cancels reconnects). Require repeated failed attempts.
    if (monitor.getAttempts() < 3) return;
  ...
  const supportsSafeRetry = useCallback(() => capabilitiesRef.current?.safeRetry === true, []);
