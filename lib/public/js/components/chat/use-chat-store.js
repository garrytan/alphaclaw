...
export const useChatStore = ({ enabled = true, selectedSessionKey = "", onRunStarted }) => {
  ...
  const applyRunEvent = useCallback((sessionKey, event) => {
    ...
    setRunStateBySession((currentMap) => ({
      ...currentMap,
      [sessionKey]: reduceRunState(
        currentMap[sessionKey] || createSessionRunState(),
        event,
      ),
    }));
  }, []);
  ...
  const handleFrame = useCallback(
    (payload) => {
      ...
      if (type === "error") {
        // Errors are never fake assistant bubbles — toast + debug log only.
