...
export const ChatRoute = ({
  ...
  // useNowMs re-renders on its cadence (paused while hidden) — the value is
  // the clock the stalled check reads.
  const heartbeatNowMs = useNowMs(15_000, { enabled: runActive });
  const stalledHint =
    runActive &&
    store.runState.assistantStreamStarted &&
    store.lastActivityAt > 0 &&
    heartbeatNowMs - store.lastActivityAt > 120_000;
  ...
  const handleCancelQueued = useCallback(
    ...
    // Depend on the STABLE callback, not the composer container (whose
    // identity tracks the draft) — row memoization rests on this.
    [composer.appendToDraft, refocusComposer, store.actions],
  );
