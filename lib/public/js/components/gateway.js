...
const SupervisionDetails = ({
  ...
  const lastCheck = formatTimestamp(watchdogStatus?.lastHealthCheckAt);
            ...
            <dl class="mt-2 space-y-1 text-xs text-fg-muted">
              ...
              ${notifDeliveredAt !== undefined
                ? html`<div class="flex gap-2">
                    ...
                    <dd>${formatTimestamp(notifDeliveredAt) || "never"}</dd>
                  </div>`
                : null}
              ${reasons.length > 0
                ? html`<div>
...
const GatewayStateCard = ({
  ...
  const sinceLabel =
    sinceMs > 0 ? formatDuration(Math.max(0, nowMs - sinceMs)) : null;
  const staleStamp = frozen
    ? formatSecondsAgo(shell.lastFrameAtMs, nowMs)
    : null;
