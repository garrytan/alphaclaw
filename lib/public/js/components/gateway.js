...
import { RollbackDataRiskConfirmDialog } from "./upgrade-tab/dialogs.js";
...
// monitored through the discovered pid, exit events never arrive.
...
// "pending (pid 4242, since 11:59 AM) — …": a relaunched child the watchdog
// has not yet proven to be the process answering the port. One string, so
// the copy cannot be split across text nodes.
const formatReplacementPending = (pending) => {
  const since = formatLocaleDateTime(pending?.since, { fallback: null });
  const facts = [
    pending?.pid != null ? `pid ${pending.pid}` : null,
    since ? `since ${since}` : null,
  ].filter(Boolean);
  const scope = facts.length ? ` (${facts.join(", ")})` : "";
  return `pending${scope} — the new gateway is not yet confirmed to be the one answering the port`;
};
  ...
  return html`
    <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
      ...
      <${RollbackDataRiskConfirmDialog}
        model=${shell.rollbackDataRisk || null}
        onConfirm=${shellActions.confirmRollbackDataRisk}
        onCancel=${shellActions.cancelRollbackDataRisk}
      />
    </div>
  `;
};
