...
export const UpgradeOverseerCard = ({
  ...
  rollingBack = false,
  onRequestRollback = () => {},
  // Runs already fetched by the page hook; undefined = fetch our own copy.
  runs = undefined,
}) => {
  ...
  const load = useCallback(async () => {
    ...
    // The page hook already fetched /api/openclaw/runs — reuse it and only
    // fall back to a second request when the prop isn't wired.
    if (runs === undefined) {
      try {
        const data = await fetchOpenclawRuns();
        const fetched = Array.isArray(data?.runs) ? data.runs : [];
        setLatestRun(fetched[0] || null);
      } catch {}
    }
    setLoaded(true);
  }, [runs]);
  ...
  // Render the shell while loading so the card stack doesn't shift when the
  // fetches settle (matches the catalog card's loading pattern).
  if (!loaded) {
    return html`
  ...
  const report = buildOverseerReportModel(
    runs === undefined ? latestRun : (Array.isArray(runs) && runs[0]) || null,
  );
