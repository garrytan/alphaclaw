...
export const prepareBackupRiskApply = async (offer, requestConsent) => {
  ...
  const expectLatest = offer.expectLatest === true;
  ...
  // v0.9.81: the token-bound re-apply carries the failed run's declared
  // direction — a stable/beta body without `intent` is refused invalid_body.
