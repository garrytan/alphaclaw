...
export const fetchBackupPreflight = async ({ signal } = {}) => {
  ...
  const validCheckpoint = data.profile === "config_only" && checkpoint &&
    Number.isFinite(checkpoint.bytes) && checkpoint.bytes >= 0 &&
    Number.isInteger(checkpoint.fileCount) && checkpoint.fileCount >= 0 &&
    checkpoint.maxBytes === 16 * 1024 * 1024 &&
    (data.blocked || checkpoint.bytes <= checkpoint.maxBytes);
  if ((data.profile === "config_only" ? !validCheckpoint : !data.diagnosis) || typeof data.blocked !== "boolean") {
...
export const useBackupPreflight = () => {
  ...
  const [recoveryMode, setRecoveryMode] = useState("config_only");
  useEffect(() => {
    active.current = true;
  ...
  const request = useCallback((action, { recoveryMode = "config_only" } = {}) => {
  ...
  const cancel = useCallback(() => {
    ...
    setDialogOpen(false);
    setReady(false);
    setRecoveryMode("config_only");
  }, []);
  const confirm = useCallback(async () => {
    if (!ready || checking || read.error || read.isFetching || read.stale || !read.data || read.data.blocked) return;
    if (recoveryMode === "database_set" && (!Number.isInteger(read.data.databaseCount) || read.data.databaseCount < 0 || !Number.isFinite(read.data.databaseBytes) || read.data.databaseBytes < 0)) return;
    ...
    setRecoveryMode("config_only");
    await action();
  }, [checking, ready, read.data, read.error, read.isFetching, read.stale, recoveryMode]);
  return {
    data: read.data, error: read.error, checking: checking || read.isFetching,
    ready: ready && !read.stale && !read.error, dialogOpen, recoveryMode, refresh, request, cancel, confirm,
  };
};
