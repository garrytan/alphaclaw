...
// Explicit human backup-risk confirmation. The hook requests a single-use
// session-bound token only after this default-OFF checkbox is confirmed.
export const NoBackupConsentConfirmDialog = ({
  ...
  return html`
    <${ConfirmDialog}
      visible=${true}
      title="Upgrade without database snapshot?"
      message=${model.message ||
      "The configuration checkpoint cannot restore database data. Continuing is forward-only and may leave no safe rollback path."}
      details=${html`
        <div class="space-y-2">
