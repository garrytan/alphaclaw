...
// Second-stage no-backup consent (#79 (b)): the apply stopped at 409
// backup_required_for_migration — the target migrates the databases and no
// backup exists, so the running build could not read them back. This is the
// ONLY backup refusal that shows a consent checkbox (Codex 20: backup_failed
// never does). The checkbox is default OFF, dialog-local state owned by the
// hook, and gates the confirm; confirming resends the same target with
// `confirmNoBackup: true`. The body renders the server's message verbatim.
export const NoBackupConsentConfirmDialog = ({
  model = null,
  checked = false,
  starting = false,
  onToggle = () => {},
  ...
  return html`
    <${ConfirmDialog}
      ...
      "This update migrates your database and no backup exists — the previous build could not read the migrated database."}
      details=${html`
        <div class="space-y-2">
          <p class="text-xs text-status-warning-muted">
            ${buildNoBackupConsentRiskLine(model.label)}
          </p>
          <${ToggleSwitch}
            checked=${checked === true}
            onChange=${onToggle}
            label=${kNoBackupConsentLabel}
          />
        </div>
      `}
      confirmLabel=${kNoBackupConsentConfirmLabel}
      confirmLoadingLabel="Starting..."
      confirmTone="danger"
      confirmDisabled=${checked !== true}
      confirmLoading=${starting}
      onConfirm=${onConfirm}
      onCancel=${onCancel}
    />
