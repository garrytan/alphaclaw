import { h } from "preact";
import htm from "htm";
import { ActionButton } from "../action-button.js";
import { InlineErrorChip } from "../inline-error-chip.js";
import { PageHeader } from "../page-header.js";
import { useUpgradeTab } from "./use-upgrade-tab.js";
import {
  ChannelMismatchBanner,
  GatewayHoldCard,
  UpgradeStatusCard,
} from "./status-card.js";
import { WhatsNewCard } from "./whats-new-card.js";
import { UpgradeCatalogCard } from "./catalog-card.js";
import { UpgradeProgressCard } from "./progress-card.js";
import { UpgradeIncidentCard } from "./incident-card.js";
import { ReconcileInstalledCard } from "./reconcile-installed-card.js";
import { RunLogViewer, UpgradeTimelineCard } from "./timeline-card.js";
import { UpgradeMedicCard } from "./medic-card.js";
import { UpgradeOverseerCard } from "./overseer-card.js";
import { UpgradeBackupsCard } from "./backups-card.js";
import { RecoveryChoiceCard } from "./recovery-choice.js";
import { BackupPreflightCard, BackupPreflightDialog } from "./backup-preflight-card.js";
import {
  ApplyConfirmDialog,
  BackupReuseRetryConfirmDialog,
  NoBackupConsentConfirmDialog,
  RollbackConfirmDialog,
  RollbackDataRiskConfirmDialog,
  StripBlamedKeysConfirmDialog,
} from "./dialogs.js";
import {
  buildAvailabilityLine,
  buildBackupReuseOfferLabels,
  buildCatalogGatingModel,
  buildChannelMismatchModel,
  buildGatewayHoldModel,
  buildReconcileInstalledModel,
  buildIncidentModel,
  buildStatusCardModel,
  getLatestApplicableTarget,
  kAlphaclawCrossLink,
  kNoBackupConsentCaption,
  kNoBackupConsentCtaLabel,
  kBackupFailureCodes,
} from "./helpers.js";

const html = htm.bind(h);

const VerdictBanner = ({ verdict = null, onDismiss = () => {} }) => {
  if (!verdict) return null;
  // Three tones: success (green), warning (amber — activated but the gateway
  // is held), danger (red). Tone leads; ok keeps tone-less { ok, message }
  // models (the reconnect-timeout verdict) rendering as before.
  const tone = verdict.tone || (verdict.ok ? "success" : "danger");
  return html`
    <div
      class=${`bg-surface border rounded-xl p-4 flex flex-wrap items-start justify-between gap-3 ${
        tone === "success"
          ? "border-green-500/40"
          : tone === "warning"
            ? "border-yellow-500/40"
            : "border-red-500/40"
      }`}
    >
      <div class="flex items-center gap-2 min-w-0">
        <span
          class=${`w-2 h-2 rounded-full ${
            tone === "success"
              ? "bg-green-500"
              : tone === "warning"
                ? "bg-yellow-500"
                : "bg-red-500"
          }`}
        ></span>
        <p class="text-sm text-body">${verdict.message}</p>
      </div>
      <button
        type="button"
        class="text-xs text-fg-muted hover:text-body"
        onclick=${onDismiss}
      >
        Dismiss
      </button>
    </div>
  `;
};

// Post-restart continuity: the latest run finished as activation_failed or
// interrupted — the persisted result envelope plus a way into the full log.
const RunFailureCard = ({
  runFailure = null,
  onDismiss = () => {},
  onViewRunLog = () => {},
  runLog = null,
  onCloseRunLog = () => {},
}) => {
  if (!runFailure) return null;
  return html`
    <div class="bg-surface border border-red-500/40 rounded-xl p-4 space-y-2">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="flex items-center gap-2 min-w-0">
          <span class="w-2 h-2 rounded-full bg-red-500 shrink-0"></span>
          <p class="text-sm font-medium text-body min-w-0 break-words">
            ${runFailure.title}
          </p>
        </div>
        <button
          type="button"
          class="text-xs text-fg-muted hover:text-body shrink-0"
          onclick=${onDismiss}
        >
          Dismiss
        </button>
      </div>
      ${runFailure.error
        ? html`
            <p class="text-sm text-status-error">${runFailure.error.message}</p>
            ${runFailure.error.hint
              ? html`<p class="text-xs text-fg-muted">${runFailure.error.hint}</p>`
              : null}
          `
        : null}
      ${runFailure.hasLog && runFailure.operationId
        ? html`
            <button
              type="button"
              class="text-xs text-fg-muted hover:text-body"
              onclick=${() => onViewRunLog(runFailure.operationId)}
            >
              View full log
            </button>
          `
        : null}
      <${RunLogViewer} runLog=${runLog} onCloseRunLog=${onCloseRunLog} />
    </div>
  `;
};

// Quick-result apply failure (sub-400ms rejection). A 409 backup_failed that
// names a reusable archive also offers the consented retry here (WI-4.5); a
// 409 backup_required_for_migration offers the no-backup consent (#79 (b)).
const ApplyErrorCard = ({
  applyError = null,
  onDismiss = () => {},
  backupReuseOffer = null,
  onRequestBackupReuseRetry = () => {},
  noBackupConsentOffer = null,
  onRequestNoBackupConsent = () => {},
  // v0.9.81 (C4): a quick 409 backup_failed / backup_required_for_migration
  // offers "Retry backup" beside the consent CTAs — re-staging the version
  // would not touch the backup.
  onRetryBackup = () => {},
  nowMs = Date.now(),
}) => {
  if (!applyError) return null;
  const backupClass = kBackupFailureCodes.includes(applyError.code);
  // Age strings derive from the offer's timestamp against the live clock.
  const offerLabels = backupReuseOffer
    ? buildBackupReuseOfferLabels(backupReuseOffer, nowMs)
    : null;
  return html`
    <div class="bg-surface border border-red-500/40 rounded-xl p-4 space-y-1">
      <div class="flex items-start justify-between gap-3">
        <p class="text-sm text-status-error">${applyError.message}</p>
        <button
          type="button"
          class="text-xs text-fg-muted hover:text-body shrink-0"
          onclick=${onDismiss}
        >
          Dismiss
        </button>
      </div>
      ${applyError.hint
        ? html`<p class="text-xs text-fg-muted">${applyError.hint}</p>`
        : null}
      ${backupClass
        ? html`
            <div class="pt-1 space-y-1">
              <${ActionButton}
                onClick=${onRetryBackup}
                tone="warning"
                idleLabel="Retry backup"
                loadingLabel="Starting backup..."
              />
              <p class="text-xs text-fg-muted">
                Runs the backup on its own; when it succeeds you can retry the update.
              </p>
            </div>
          `
        : null}
      ${backupReuseOffer
        ? html`
            <div class="pt-1 space-y-1">
              <${ActionButton}
                onClick=${onRequestBackupReuseRetry}
                tone="warning"
                idleLabel=${offerLabels.ctaLabel}
                loadingLabel="Starting..."
              />
              <p class="text-xs text-status-warning-muted">
                A fresh backup could not be made. ${offerLabels.lossWindowLine}
              </p>
            </div>
          `
        : null}
      ${noBackupConsentOffer
        ? html`
            <div class="pt-1 space-y-1">
              <${ActionButton}
                onClick=${onRequestNoBackupConsent}
                tone="warning"
                idleLabel=${kNoBackupConsentCtaLabel}
                loadingLabel="Starting..."
              />
              <p class="text-xs text-status-warning-muted">${kNoBackupConsentCaption}</p>
            </div>
          `
        : null}
      ${typeof applyError.docsUrl === "string" && applyError.docsUrl
        ? html`<a
            class="ac-tip-link text-xs"
            href=${applyError.docsUrl}
            target="_blank"
            rel="noreferrer"
            >Learn more</a
          >`
        : null}
    </div>
  `;
};

// Pure presentational component: all state comes in via the `state` prop so
// the page can be rendered (and tested) without any hooks.
export const UpgradeTabView = ({ state = {} }) => {
  const {
    channelInfo = null,
    channelError = null,
    onRetryChannel = () => {},
    runsError = null,
    onRetryRuns = () => {},
    loadingChannel = false,
    catalog = null,
    whatsNew = null,
    catalogError = null,
    loadingCatalog = false,
    refreshingCatalog = false,
    activeChannel = "stable",
    nowMs = Date.now(),
    onSelectChannel = () => {},
    savingChannel = false,
    channelSaveError = null,
    onDismissChannelSaveError = () => {},
    runs = [],
    runFailure = null,
    onDismissRunFailure = () => {},
    runLog = null,
    onViewRunLog = () => {},
    onCloseRunLog = () => {},
    pendingApply = null,
    onRequestApply = () => {},
    onConfirmApply = () => {},
    onCancelApply = () => {},
    onToggleBackupReuseConsent = () => {},
    backupReuseOffer = null,
    backupReuseRetryPrompt = false,
    onRequestBackupReuseRetry = () => {},
    onCancelBackupReuseRetry = () => {},
    onConfirmBackupReuseRetry = () => {},
    noBackupConsentOffer = null,
    noBackupConsentPrompt = false,
    noBackupConsentChecked = false,
    noBackupConsentStarting = false,
    onRequestNoBackupConsent = () => {},
    onCancelNoBackupConsent = () => {},
    onToggleNoBackupConsent = () => {},
    onConfirmNoBackupConsent = () => {},
    backupsInventory = null,
    backupPreflight = null,
    backupsError = null,
    backupsLoading = false,
    onRetryBackups = () => {},
    operation = null,
    onDismissOperation = () => {},
    onRetryOperationStatus = () => {},
    applyError = null,
    onDismissApplyError = () => {},
    actionError = null,
    onDismissActionError = () => {},
    logOpen = false,
    onToggleLog = () => {},
    verdict = null,
    onDismissVerdict = () => {},
    repairAvailable = false,
    onRunRepair = () => {},
    onRetryApply = () => {},
    onBackupNow = () => {},
    backupNowStarting = false,
    onRetryBackup = () => {},
    onRetryUpdate = () => {},
    lastManualBackup = null,
    markingGood = false,
    onMarkGood = () => {},
    rollingBack = false,
    onRollback = () => {},
    rollbackPrompt = false,
    onRequestRollback = () => {},
    onCancelRollback = () => {},
    rollbackDataRisk = null,
    onConfirmRollbackDataRisk = () => {},
    onCancelRollbackDataRisk = () => {},
    retryingReconcile = null,
    onRetryReconcile = () => {},
    stripReconcilePrompt = false,
    onRequestStripReconcile = () => {},
    onCancelStripReconcile = () => {},
    onConfirmStripReconcile = () => {},
    reconcileError = null,
    onDismissReconcileError = () => {},
    reconcilingInstalled = false,
    onReconcileInstalled = () => {},
    reconcileInstalledError = null,
    onDismissReconcileInstalledError = () => {},
    clearingBlocklistId = null,
    lastClearedId = null,
    onClearBlocklist = () => {},
    onCheckNow = () => {},
    onUpdateToLatest = () => {},
    expandedNotesId = null,
    onToggleNotes = () => {},
    devAdvancedOpen = false,
    onToggleDevAdvanced = () => {},
    actionsDisabled = false,
  } = state;

  const statusModel = buildStatusCardModel(channelInfo, nowMs);
  const gatewayHold = buildGatewayHoldModel(channelInfo, runs);
  const reconcileInstalled = buildReconcileInstalledModel(channelInfo);
  const incident = buildIncidentModel({
    lastUpdateRun: channelInfo?.lastUpdateRun || null,
    blocklist: channelInfo?.blocklist || [],
  });
  const availabilityLine = buildAvailabilityLine({
    catalog,
    releaseChannel: activeChannel,
    installedVersion: channelInfo?.installedVersion || null,
    nodeVersion: channelInfo?.nodeVersion || null,
  });
  const latestTarget = getLatestApplicableTarget({
    catalog,
    releaseChannel: activeChannel,
    nodeVersion: channelInfo?.nodeVersion || null,
    installedVersion: channelInfo?.installedVersion || null,
  });
  const applyGate = buildCatalogGatingModel(catalog);
  const mismatch = operation
    ? null
    : buildChannelMismatchModel({
        catalog,
        channelInfo,
        releaseChannel: activeChannel,
      });
  // One log pane at a time: the failure card owns the viewer for its own run,
  // the timeline card shows every other run's log.
  const failureLog =
    runFailure && runLog && runLog.operationId === runFailure.operationId
      ? runLog
      : null;
  const timelineLog = failureLog ? null : runLog;
  const isInitialLoading = loadingChannel && !channelInfo && !channelError;

  return html`
    <div class="space-y-4">
      <${PageHeader} title="OpenClaw — Versions & Channels" />

      <${VerdictBanner} verdict=${verdict} onDismiss=${onDismissVerdict} />

      <${RunFailureCard}
        runFailure=${runFailure}
        onDismiss=${onDismissRunFailure}
        onViewRunLog=${onViewRunLog}
        runLog=${failureLog}
        onCloseRunLog=${onCloseRunLog}
      />

      ${!operation && !state.recoveryChoice
        ? html`<${GatewayHoldCard}
            model=${gatewayHold}
            retryingReconcile=${retryingReconcile}
            reconcileError=${reconcileError}
            actionsDisabled=${actionsDisabled}
            onRetryReconcile=${onRetryReconcile}
            onRequestStripReconcile=${onRequestStripReconcile}
            onDismissReconcileError=${onDismissReconcileError}
            onRequestApply=${onRequestApply}
          />`
        : null}

      ${!operation
        ? html`<${ReconcileInstalledCard}
            model=${reconcileInstalled}
            reconcilingInstalled=${reconcilingInstalled}
            reconcileInstalledError=${reconcileInstalledError}
            actionsDisabled=${actionsDisabled}
            onReconcileInstalled=${onReconcileInstalled}
            onDismissReconcileInstalledError=${onDismissReconcileInstalledError}
          />`
        : null}

      ${!operation
        ? html`<${UpgradeIncidentCard}
            incident=${incident}
            actionsDisabled=${actionsDisabled}
            clearingBlocklistId=${clearingBlocklistId}
            onClearBlocklist=${onClearBlocklist}
            rollingBack=${rollingBack}
            onRequestRollback=${onRequestRollback}
          />`
        : null}

      <${UpgradeProgressCard}
        operation=${state.recoveryChoice ? null : operation}
        nowMs=${nowMs}
        logOpen=${logOpen}
        onToggleLog=${onToggleLog}
        repairAvailable=${repairAvailable}
        onRunRepair=${onRunRepair}
        onRetryApply=${onRetryApply}
        onRetryBackup=${onRetryBackup}
        onRetryUpdate=${onRetryUpdate}
        onDismissOperation=${onDismissOperation}
        onRetryOperationStatus=${onRetryOperationStatus}
        backupReuseOffer=${backupReuseOffer}
        onRequestBackupReuseRetry=${onRequestBackupReuseRetry}
        noBackupConsentOffer=${noBackupConsentOffer}
        onRequestNoBackupConsent=${onRequestNoBackupConsent}
      />

      <${ApplyErrorCard}
        applyError=${state.recoveryChoice ? null : applyError}
        onDismiss=${onDismissApplyError}
        onRetryBackup=${onRetryBackup}
        backupReuseOffer=${backupReuseOffer}
        onRequestBackupReuseRetry=${onRequestBackupReuseRetry}
        noBackupConsentOffer=${noBackupConsentOffer}
        onRequestNoBackupConsent=${onRequestNoBackupConsent}
        nowMs=${nowMs}
      />

      <${RecoveryChoiceCard}
        model=${state.recoveryChoice}
        onSnapshot=${state.onChooseDatabaseSnapshot}
        onWithoutSnapshot=${onRequestNoBackupConsent}
        onCancel=${state.onCancelRecoveryChoice}
        cancelling=${state.cancellingRecoveryChoice}
        cancelError=${state.recoveryCancelError}
      />

      ${channelError && !channelInfo
        ? html`
            <div class="bg-surface border border-red-500/40 rounded-xl p-4 space-y-1">
              <p class="text-sm text-status-error">${channelError.message}</p>
              ${channelError.hint
                ? html`<p class="text-xs text-fg-muted">${channelError.hint}</p>`
                : null}
            </div>
          `
        : null}

      ${channelError && channelInfo
        ? html`
            <div class="space-y-1">
              <p class="text-xs text-status-warning-muted">
                Could not refresh status — showing last loaded data
                (${channelError.message}).
              </p>
              ${channelError.hint
                ? html`<p class="text-xs text-fg-muted">${channelError.hint}</p>`
                : null}
            </div>
          `
        : null}

      ${channelError ? html`<${ActionButton} onClick=${onRetryChannel} idleLabel="Retry status check" tone="secondary" />` : null}
      ${runsError ? html`<div role="status" class="space-y-2">
        <p class="text-sm text-status-warning">Could not check active operations: ${runsError.message}</p>
        <${ActionButton} onClick=${onRetryRuns} idleLabel="Retry operation history" tone="secondary" />
      </div>` : null}

      <${ChannelMismatchBanner}
        model=${mismatch}
        actionsDisabled=${actionsDisabled}
        savingChannel=${savingChannel}
        onRequestApply=${onRequestApply}
        onToggleNotes=${onToggleNotes}
        onSelectChannel=${onSelectChannel}
      />

      <${UpgradeStatusCard}
        model=${statusModel}
        availabilityLine=${availabilityLine}
        activeChannel=${activeChannel}
        onSelectChannel=${onSelectChannel}
        actionsDisabled=${actionsDisabled}
        savingChannel=${savingChannel}
        channelSaveError=${channelSaveError}
        onDismissChannelSaveError=${onDismissChannelSaveError}
        latestTarget=${latestTarget}
        applyGate=${applyGate}
        onUpdateToLatest=${onUpdateToLatest}
        markingGood=${markingGood}
        onMarkGood=${onMarkGood}
        rollingBack=${rollingBack}
        onRequestRollback=${onRequestRollback}
        loadingChannel=${isInitialLoading}
      />

      ${actionError
        ? html`
            <div class="flex items-start gap-2">
              <div class="flex-1 min-w-0">
                <${InlineErrorChip}
                  headline=${actionError.headline}
                  error=${actionError.error}
                />
              </div>
              <button
                type="button"
                class="text-xs text-fg-muted hover:text-body shrink-0"
                onclick=${onDismissActionError}
              >
                Dismiss
              </button>
            </div>
          `
        : null}

      <${WhatsNewCard} whatsNew=${whatsNew} activeChannel=${activeChannel} />

      <${UpgradeOverseerCard}
        actionsDisabled=${actionsDisabled}
        markingGood=${markingGood}
        onMarkGood=${onMarkGood}
        rollingBack=${rollingBack}
        onRequestRollback=${onRequestRollback}
        runs=${state.runs}
      />

      <${UpgradeMedicCard} />

      <${UpgradeCatalogCard}
        catalog=${catalog}
        catalogError=${catalogError}
        loadingCatalog=${loadingCatalog}
        refreshingCatalog=${refreshingCatalog}
        nowMs=${nowMs}
        installedVersion=${channelInfo?.installedVersion || null}
        nodeVersion=${channelInfo?.nodeVersion || null}
        actionsDisabled=${actionsDisabled}
        applyGate=${applyGate}
        onCheckNow=${onCheckNow}
        devAdvancedOpen=${devAdvancedOpen}
        onToggleDevAdvanced=${onToggleDevAdvanced}
        onRequestApply=${onRequestApply}
        onClearBlocklist=${onClearBlocklist}
        clearingBlocklistId=${clearingBlocklistId}
        lastClearedId=${lastClearedId}
        expandedNotesId=${expandedNotesId}
        onToggleNotes=${onToggleNotes}
      />

      <${UpgradeTimelineCard}
        runs=${runs}
        nowMs=${nowMs}
        onViewRunLog=${onViewRunLog}
        runLog=${timelineLog}
        onCloseRunLog=${onCloseRunLog}
      />

      <${BackupPreflightCard} model=${backupPreflight} />

      <${UpgradeBackupsCard}
        inventory=${backupsInventory}
        loading=${backupsLoading}
        error=${backupsError}
        onRetry=${onRetryBackups}
        nowMs=${nowMs}
        onBackupNow=${onBackupNow}
        onDatabaseSnapshot=${state.onDatabaseSnapshot}
        backupNowDisabled=${actionsDisabled || backupNowStarting}
        backupNowStarting=${backupNowStarting}
        lastManualBackup=${lastManualBackup}
      />

      <p class="text-xs text-fg-muted">${kAlphaclawCrossLink}</p>

      <${ApplyConfirmDialog}
        pendingApply=${pendingApply}
        onConfirm=${onConfirmApply}
        onCancel=${onCancelApply}
        onToggleReuseConsent=${onToggleBackupReuseConsent}
        onRetryBackups=${onRetryBackups}
      />

      <${BackupPreflightDialog} model=${backupPreflight} />

      <${BackupReuseRetryConfirmDialog}
        model=${backupReuseRetryPrompt ? backupReuseOffer : null}
        starting=${Boolean(operation) && operation.phase === "running"}
        nowMs=${nowMs}
        onConfirm=${onConfirmBackupReuseRetry}
        onCancel=${onCancelBackupReuseRetry}
      />

      <${NoBackupConsentConfirmDialog}
        model=${noBackupConsentPrompt ? noBackupConsentOffer : null}
        checked=${noBackupConsentChecked === true}
        starting=${noBackupConsentStarting || (Boolean(operation) && operation.phase === "running")}
        onToggle=${onToggleNoBackupConsent}
        onConfirm=${onConfirmNoBackupConsent}
        onCancel=${onCancelNoBackupConsent}
      />

      <${RollbackConfirmDialog}
        visible=${Boolean(rollbackPrompt)}
        rollingBack=${rollingBack}
        onConfirm=${onRollback}
        onCancel=${onCancelRollback}
      />

      <${RollbackDataRiskConfirmDialog}
        model=${rollbackDataRisk}
        rollingBack=${rollingBack}
        nowMs=${nowMs}
        onConfirm=${onConfirmRollbackDataRisk}
        onCancel=${onCancelRollbackDataRisk}
      />

      <${StripBlamedKeysConfirmDialog}
        visible=${Boolean(stripReconcilePrompt)}
        keyCount=${gatewayHold?.keyCount || 0}
        retrying=${retryingReconcile === "strip"}
        onConfirm=${onConfirmStripReconcile}
        onCancel=${onCancelStripReconcile}
      />
    </div>
  `;
};

export const UpgradeTab = ({
  statusData = null,
  onRefreshStatuses = () => {},
}) => {
  const state = useUpgradeTab({ statusData, onRefreshStatuses });
  return html`<${UpgradeTabView} state=${state} />`;
};
