...
import { cachedFetch, invalidateCache, setCached } from "../../lib/api-cache.js";
...
export const useUpgradeTab = ({
        ...
        data = await cachedFetch(kChannelCacheKey, fetchOpenclawChannel, {
          ...
          onRevalidate: (fresh) => {
            if (channelMutationStampRef.current !== stampAtDispatch) {
              ...
              // its result is fresher. Drop this frame and the cache entry
              // cachedFetch just re-stamped with our pre-mutation payload.
          ...
          data = await cachedFetch(kCatalogCacheKey, fetchCatalog, {
            ...
            onRevalidate: (fresh) => {
              if (catalogMutationStampRef.current !== stampAtDispatch) {
                // Same guard as loadChannel: a fresher direct reload landed
                // while this revalidation was in flight.
                invalidateCache(kCatalogCacheKey);
                return;
              }
  ...
  const onDismissOperation = useCallback(() => {
    setOperation((op) => (op && op.phase === "failed" ? null : op));
    loadChannel();
  }, [loadChannel]);
  ...
  const onSelectChannel = useCallback(
    ...
    [activeChannel, loadChannel, loadCatalog, operation, savingChannel],
  );
  ...
  const onRequestApply = useCallback(
    ...
    [catalog, channelInfo, operation, whatsNew],
  );
