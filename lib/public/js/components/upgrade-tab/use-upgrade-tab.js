...
import { cachedFetch, setCached } from "../../lib/api-cache.js";
  ...
  const loadChannel = useCallback(async ({ fromCache = false } = {}) => {
    try {
      const data = fromCache
        ? await cachedFetch(kChannelCacheKey, fetchOpenclawChannel, {
        ...
        : setCached(kChannelCacheKey, await fetchOpenclawChannel());
      setChannelInfo(data);
      setChannelError(null);
      return data;
    } catch (err) {
  ...
  const loadCatalog = useCallback(
    async ({ refresh = false, fromCache = false } = {}) => {
      ...
      try {
        ...
        const data =
          fromCache && !refresh
            ? await cachedFetch(kCatalogCacheKey, fetchCatalog, {
                maxAgeMs: kUpgradeCacheMaxAgeMs,
                onRevalidate: (fresh) => {
            ...
            : setCached(kCatalogCacheKey, await fetchCatalog());
