...
export const useUpgradeTab = ({
  ...
  const startApply = useCallback((options) => {
    if (options?.allowBackupReuse || options?.confirmNoBackup) return performApply(options);
  ...
  const startBackup = useCallback((options) => backupPreflight.request(() => performBackup(options)),
