...
export const useUpgradeTab = ({
  ...
  const subscribeToOperation = useCallback(
    ...
    [beginRestartHandoff, loadChannel, refreshBackups, stopStream],
  );
  ...
  const startApply = useCallback(
    async ({ payload, label, allowBackupReuse = null, confirmNoBackup = false, confirmNoBackupToken = null }) => {
      setApplyError(null);
      setVerdict(null);
      setRunFailure(null);
      setBackupReuseOffer(null);
      setBackupReuseRetryPrompt(false);
      clearNoBackupConsent();
      applyTargetRef.current = { payload, label: label || describeTarget(payload) };
      expectedRef.current = {
      ...
      try {
        ...
        setBackupReuseOffer(
          buildBackupReuseOfferModel({ error: err, target: payload, label }),
        );
        // 409 backup_required_for_migration: offer the explicit no-backup
        // consent (second-stage dialog) next to the error.
        setNoBackupConsentOffer(
          buildNoBackupConsentOfferModel({ error: err, target: payload, label }),
        );
        refreshBackups();
      }
    },
    [channelInfo, refreshBackups, subscribeToOperation],
  );
  ...
  const onRetryApply = useCallback(() => {
    ...
    startApply({ payload: target, label: operation?.label || null });
  ...
  const onRequestApply = useCallback(
    ...
    [backups.error, backups.inventory, backups.loading, catalog, channelInfo, operation, whatsNew],
  );
  ...
  }, [noBackupConsentChecked, noBackupConsentOffer, startApply]);
  ...
  const onDismissOperation = useCallback(() => {
    if (!operation || operation.phase !== "failed") return;
    stopStream();
    setOperation(null);
    setLogOpen(false);
    setBackupReuseOffer(null);
  ...
  // rows without a click. Declared LAST so the earlier effects keep their
  // order (the hook harness in tests runs effects by index).
