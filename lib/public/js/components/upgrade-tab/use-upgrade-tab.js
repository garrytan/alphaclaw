...
export const useUpgradeTab = ({
  ...
  const [catalog, setCatalog] = useState(() => getCached(kCatalogCacheKey));
  const [catalogError, setCatalogError] = useState(null);
  const [loadingCatalog, setLoadingCatalog] = useState(() => getCached(kCatalogCacheKey) === null);
  ...
  // Latest-request-wins guards: overlapping loads are reachable (mount +
  ...
  const loadCatalog = useCallback(async ({ refresh = false } = {}) => {
    ...
    try {
      ...
      setCached(kCatalogCacheKey, data?.catalog || null);
