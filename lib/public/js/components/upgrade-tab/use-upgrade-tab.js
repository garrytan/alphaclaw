...
export const useUpgradeTab = ({
  ...
  const startApply = useCallback(
    async ({ payload, label, allowBackupReuse = null, confirmNoBackup = false }) => {
      setApplyError(null);
      setVerdict(null);
      setRunFailure(null);
      setBackupReuseOffer(null);
      ...
      try {
        const result = await applyOpenclawVersion({
          ...
          ...(confirmNoBackup === true ? { confirmNoBackup: true } : {}),
        });
        if (result?.noop) {
          setOperation(null);
  ...
  const onConfirmNoBackupConsent = useCallback(async () => {
    ...
    if (!offer?.target || noBackupConsentChecked !== true) return;
    setNoBackupConsentPrompt(false);
    setNoBackupConsentChecked(false);
    await startApply({
      payload: offer.target,
