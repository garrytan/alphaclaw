...
import { buildRecoveryChoice, resolveRecoveryRequest } from "./recovery-choice.js";
import { isMigrationOnlyBackup, buildBackupResultMessage } from "./backup-presentation.js";
import { useCallback, useEffect, useRef, useState } from "preact/hooks";
import {
  applyOpenclawVersion,
  createOpenclawBackup,
  requestOpenclawBackupRiskConsent,
  cancelOpenclawRecoveryReview,
...
import { useBackupPreflight } from "./use-backup-preflight.js";
...
const withRequest = (offer, request) =>
  offer && request ? { ...offer, intent: request.intent, expectLatest: request.expectLatest === true } : offer;
...
export const useUpgradeTab = ({
  ...
  const [recoveryCancelError, setRecoveryCancelError] = useState(null);
  const [cancellingRecoveryChoice, setCancellingRecoveryChoice] = useState(false);
  const recoveryCancelBusy = useRef(false);
  const restoredRecoveryReview = useRef(null);
  ...
  useEffect(() => {
    const hold = channelInfo?.gatewayHold;
    if (hold?.reason !== "recovery_review" || !hold.operationId || recoveryChoice || operation || restoredRecoveryReview.current === hold.operationId) return;
    const run = runs.find((entry) => entry?.operationId === hold.operationId) ||
      (channelInfo.lastUpdateRun?.operationId === hold.operationId ? channelInfo.lastUpdateRun : null);
    if (!run?.target) return;
    const request = resolveRecoveryRequest(run.result, { payload: run.target, label: describeTarget(run.target), intent: run.intent || run.result?.intent, expectLatest: (run.expectLatest ?? run.result?.expectLatest) === true, gatewayHeld: true });
    const error = { ...run.result, code: "recovery_choice_required", operationId: hold.operationId, gatewayHeld: true };
    restoredRecoveryReview.current = hold.operationId;
    setRecoveryChoice(buildRecoveryChoice(error, request));
    setNoBackupConsentOffer(withRequest(buildNoBackupConsentOfferModel({ error, target: request.payload, label: request.label }), request));
  }, [channelInfo, operation, recoveryChoice, runs]);
  ...
  const backupPreflight = useBackupPreflight();
        ...
        onMessage: ({ event, data }) => {
          ...
          if (event === "done") {
            ...
            if (backup) {
              ...
              showToast(
                buildBackupResultMessage(data, { toast: true }),
                isMigrationOnlyBackup(data?.archive) ? "warning" : "success",
              );
              refreshBackups();
              loadRuns();
              return;
          ...
          if (event === "error") {
            ...
            const inFlight = resolveRecoveryRequest(data, applyTargetRef.current);
            setRecoveryChoice(buildRecoveryChoice({ ...data, message: data?.error, operationId }, inFlight));
  ...
  const performApply = useCallback(
    async ({
      ...
      applyTargetRef.current = {
        ...
        recoveryMode,
        gatewayHeld: Boolean(channelInfo?.gatewayHold),
      };
      expectedRef.current = {
      ...
      try {
        ...
        setApplyError(buildErrorEnvelopeModel(err));
        const reviewedRequest = resolveRecoveryRequest(err, applyTargetRef.current);
        setRecoveryChoice(buildRecoveryChoice(err, reviewedRequest));
        ...
        setNoBackupConsentOffer(
          withRequest(buildNoBackupConsentOfferModel({ error: err, target: reviewedRequest.payload, label: reviewedRequest.label }), reviewedRequest),
        );
        refreshBackups();
      }
    },
  ...
  const startApply = useCallback((options) => {
    if (options?.confirmNoBackup) return performApply(options);
    return backupPreflight.request(() => performApply(options));
  }, [backupPreflight.request, performApply]);

  const onChooseDatabaseSnapshot = useCallback(() => {
    if (!recoveryChoice?.request || recoveryCancelBusy.current) return;
    return performApply({ ...recoveryChoice.request, recoveryMode: "database_set" });
  }, [performApply, recoveryChoice]);

  const onCancelRecoveryChoice = useCallback(async () => {
    if (!recoveryChoice || recoveryCancelBusy.current) return;
    recoveryCancelBusy.current = true;
    setCancellingRecoveryChoice(true);
    setRecoveryCancelError(null);
    try {
      if (recoveryChoice.requiresServerCancel) {
        await cancelOpenclawRecoveryReview(recoveryChoice.operationId);
        restoredRecoveryReview.current = recoveryChoice.operationId;
        updateChannel((info) => info?.gatewayHold?.reason === "recovery_review" && info.gatewayHold.operationId === recoveryChoice.operationId ? { ...info, gatewayHold: null } : info);
        loadChannel();
        loadRuns();
      }
      stopStream();
      ...
      setRecoveryCancelError(buildErrorEnvelopeModel(error));
      loadChannel();
    } finally {
      recoveryCancelBusy.current = false;
      setCancellingRecoveryChoice(false);
    }
  }, [loadChannel, loadRuns, recoveryChoice, stopStream, updateChannel]);
  ...
  const performBackup = useCallback(
    async ({ retryUpdate = null, recoveryMode = "config_only" } = {}) => {
      ...
      try {
        const result = await createOpenclawBackup({ recoveryMode });
        const operationId = result?.operationId || null;
        if (operationId && result?.events) {
          setOperation((op) => (op ? { ...op, operationId } : op));
        ...
        showToast(
          buildBackupResultMessage(result, { toast: true }),
          isMigrationOnlyBackup(result?.archive) ? "warning" : "success",
        );
        refreshBackups();
        loadRuns();
      } catch (err) {
  ...
  const startBackup = useCallback((options) => backupPreflight.request(() => performBackup(options), { recoveryMode: options?.recoveryMode || "config_only" }),
    [backupPreflight.request, performBackup]);
  ...
  const onDatabaseSnapshot = useCallback(() => {
    if (operation) return;
    return startBackup({ recoveryMode: "database_set" });
  }, [operation, startBackup]);
  ...
  const onRetryBackup = useCallback(() => {
    if (backupNowStarting) return;
    if (operation?.phase === "failed" && operation.target?.channel && operation.recoveryMode === "database_set") {
      return performApply({ payload: operation.target, label: operation.label, intent: operation.intent, expectLatest: operation.expectLatest === true, recoveryMode: "database_set" });
    }
    if (!operation && applyError && applyTargetRef.current?.recoveryMode === "database_set") {
      return performApply({ ...applyTargetRef.current });
    }
      ...
      retryUpdate =
        operation.target?.kind === "backup"
          ...
          : operation.target?.channel
            ? { payload: operation.target, label: operation.label || describeTarget(operation.target), intent: operation.intent || null, ...(operation.expectLatest ? { expectLatest: true } : {}) }
            : null;
      stopStream();
      setLogOpen(false);
    } else if (applyError && applyTargetRef.current?.payload?.channel) {
      const inFlight = applyTargetRef.current;
      retryUpdate = { payload: inFlight.payload, label: inFlight.label, intent: inFlight.intent || null, ...(inFlight.expectLatest ? { expectLatest: true } : {}) };
    }
    setOperation(null);

    clearNoBackupConsent();
    return startBackup({ retryUpdate, recoveryMode: operation?.target?.kind === "backup" ? operation.recoveryMode || "config_only" : "config_only" });
  }, [applyError, backupNowStarting, operation, performApply, startBackup, stopStream]);
  ...
  // intent — the operator confirms, and a fresh apply runs.
  const onRetryUpdate = useCallback(() => {
    ...
    onRequestApplyRef.current?.({
      ...
      expectLatest: retry.expectLatest === true,
  ...
  const onRetryApply = useCallback(() => {
    ...
    return startApply({
      ...
      expectLatest: operation?.expectLatest === true,
      recoveryMode: operation?.recoveryMode || "config_only",
    });
  ...
  const operationMonitor = useOperationMonitor({
    ...
    onTerminal: (run, phase) => {
      ...
      const request = resolveRecoveryRequest(run.result, applyTargetRef.current || { payload: run.target, label: describeTarget(run.target), intent: run.intent, expectLatest: run.expectLatest, gatewayHeld: Boolean(channelInfo?.gatewayHold) });
      setRecoveryChoice(buildRecoveryChoice({ ...run.result, operationId: run.operationId }, request));

      setNoBackupConsentOffer(withRequest(buildNoBackupConsentOfferModel({
        error: { ...run.result, operationId: run.operationId },
        target: request.payload, label: request.label,
      }), request));
    },
  ...
  const onRequestApply = useCallback(
    ...
    [catalog, channelInfo, deriveIntentFor, operation, whatsNew],
  );
  ...
  const onRequestNoBackupConsent = useCallback(() => {
    if (!noBackupConsentOffer || recoveryCancelBusy.current) return;
    setNoBackupConsentChecked(false);
      ...
      try {
        ...
        if (err?.code === "rollback_requires_confirmation") {
          ...
          setRollbackDataRisk({
            ...
            ...(err.recovery && typeof err.recovery === "object" && !Array.isArray(err.recovery) ? { recovery: err.recovery } : {}),
            ...
            backupProfile: err.backupProfile,
            backupCoverage: err.backupCoverage,
            backupPartialReasons: err.backupPartialReasons,
  ...
  return {
    backupPreflight,
    recoveryChoice,
    onChooseDatabaseSnapshot,
    onCancelRecoveryChoice,
    recoveryCancelError,
    cancellingRecoveryChoice,
    // data
    channelInfo,
    channelError,
