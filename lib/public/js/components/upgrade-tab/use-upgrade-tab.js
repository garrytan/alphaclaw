...
// v0.9.81 (RC1c): the catalog re-reads itself while the tab is visible (the
// server's 10-min SWR window — a stale row set is otherwise never re-delivered
// after the mount load), and a response the server marked `stale` schedules
// ONE direct follow-up read so the background revalidation's result lands
// without a click.
const kCatalogPollIntervalMs = 10 * 60_000;
...
export const useUpgradeTab = ({
  ...
  const [whatsNew, setWhatsNew] = useState(
    () => getCached(kCatalogCacheKey)?.whatsNew || null,
  );
  ...
  // One pending stale follow-up at a time; a follow-up's own stale answer
  // never schedules another (the server's SWR revalidation may have failed —
  // the degraded line says so; looping would not help).
  const staleFollowUpRef = useRef(null);
  const scheduleStaleFollowUp = useCallback(
    (catalogData, { followUp = false } = {}) => {
      if (!catalogData?.stale || followUp || staleFollowUpRef.current) return;
      staleFollowUpRef.current = setTimeout(() => {
        staleFollowUpRef.current = null;
        // A DIRECT fetch (never the 60 s client cache) that seeds the
        // canonical cache key — the same path Check now takes.
        loadCatalogRef.current?.({ followUp: true });
      }, Math.max(0, Number(catalogStaleFollowUpMs) || 0));
    },
    [catalogStaleFollowUpMs],
  );
  const loadCatalogRef = useRef(null);

  const loadCatalog = useCallback(
    async ({ refresh = false, fromCache = false, followUp = false } = {}) => {
      ...
      try {
        ...
        if (fromCache && !refresh) {
          data = await cachedFetch(kCatalogCacheKey, fetchCatalog, {
            ...
            onRevalidate: (fresh) => {
              ...
              setWhatsNew(fresh?.whatsNew || null);
              setCatalogError(null);
              scheduleStaleFollowUp(fresh?.catalog);
            },
          });
        } else {
        ...
        setWhatsNew(data?.whatsNew || null);
        setCatalogError(null);
        scheduleStaleFollowUp(data?.catalog, { followUp });
  ...
  useEffect(() => {
    ...
    if (!inFlight) {
      // v0.9.81 (C3): a standalone backup never writes lastUpdateRun — an
      // unfinished `kind: "backup"` run in the ledger list rehydrates instead
      // (the runs list is loaded on mount beside the channel).
      const backupRun = (Array.isArray(runs) ? runs : []).find(
        (entry) => isBackupRun(entry) && entry.state === "running" && entry.finishedAt == null,
      );
      if (!backupRun) return;
      rehydratedRef.current = true;
      setOperation({
        operationId: backupRun.operationId || null,
        resumed: true,
        target: { kind: "backup" },
        label: "manual backup",
        startedAt: backupRun.startedAt || Date.now(),
        steps: [],
        output: "",
        lastOutputAt: null,
        phase: "running",
  ...
  // settles (the archive set only changes then). Declared AFTER the four
  // page-level effects above on purpose — the hook harness in tests runs
  // effects by declaration index.
  ...
  }, [loadChannel, subscribeToOperation]);
  ...
  const startApply = useCallback(
    async ({
      ...
      setBackupReuseRetryPrompt(false);
      clearNoBackupConsent();
      const declaredIntent = intent || deriveIntentFor(payload);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        // v0.9.81 (C3): a resumed standalone backup reads its own ledger run
        // (lastUpdateRun never describes it).
        if (operation.target?.kind === "backup") {
          const list = await loadRuns();
          if (cancelled) return;
          const run = (list || []).find((entry) => entry.operationId === operation.operationId) || null;
          if (run && run.finishedAt != null) {
            if (run.ok) {
              setOperation((op) =>
                op ? { ...op, phase: "completed", finishedAt: run.finishedAt, result: run.result || null } : op,
              );
            } else {
              setOperation((op) =>
                op
                  ? {
                      ...
                      error: buildErrorEnvelopeModel({
                        message: run.result?.message || "The backup did not complete.",
            ...
            setBackupReuseOffer(
              buildBackupReuseOfferModel({
                error: {
                  ...
                  reusableBackup: run.result?.reusableBackup,
                },
                target: run.target || null,
                label: describeTarget(run.target),
              }),
            ...
            setNoBackupConsentOffer(
              buildNoBackupConsentOfferModel({
                error: {
                  ...
                  backupRiskEligible: run.result?.backupRiskEligible,
                  operationId: run.operationId || operation?.operationId,
                },
                target: run.target || null,
                label: describeTarget(run.target),
              }),
  ...
  useEffect(() => {
    ...
    const finish = (verdictModel) => {
      ...
      loadCatalog();
      onRefreshStatuses();
      // The update settled — its pre-update archive (and any prune) is now
      // in the inventory.
      refreshBackups();
  ...
  const onRequestApply = useCallback(
    ({
      ...
      const intent = derivedIntent || intentClaim || null;
  ...
  // Catalog re-read while the tab is visible (v0.9.81, RC1c) — through the
  // 60 s client cache, so the immediate refresh usePolling fires when it
  // becomes enabled is served from the entry the mount load just seeded
  // (no second network round-trip), while the 10-min ticks (and a return to
  // a backgrounded tab) go through the SWR path and land the server's fresh
  // rows without a click. Declared BEFORE the reuse-consent rebind effect,
  // which must stay the hook's LAST effect (tests run it by last index).
  usePolling(() => loadCatalog({ fromCache: true }), kCatalogPollIntervalMs, {
    enabled: catalog !== null,
    pauseWhenHidden: true,
  });
  useEffect(
    () => () => {
      if (staleFollowUpRef.current) {
        clearTimeout(staleFollowUpRef.current);
        staleFollowUpRef.current = null;
      }
    },
    [],
  ...
  const onUpdateToLatest = useCallback(() => {
    ...
    onRequestApply({
      ...
      expectLatest: target.expectLatest !== false,
    });
