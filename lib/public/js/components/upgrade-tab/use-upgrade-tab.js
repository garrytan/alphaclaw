...
export const useUpgradeTab = ({
  ...
  // `allowBackupReuse` ({ sha256 } | null) rides the request body ONLY — the
  // operation's recorded target stays the bare payload, so a later "Re-stage
  // version" never silently inherits a consent given for one attempt.
  const startApply = useCallback(
    async ({ payload, label, allowBackupReuse = null }) => {
      setApplyError(null);
      setVerdict(null);
      setRunFailure(null);
      setBackupReuseOffer(null);
      ...
      try {
        const result = await applyOpenclawVersion(
          allowBackupReuse ? { ...payload, allowBackupReuse } : payload,
        );
        if (result?.noop) {
          setOperation(null);
  ...
  // one a pre-failure snapshot remembered. Declared last so the harness's
  // effect indices for the earlier effects stay stable.
  useEffect(() => {
    setPendingApply((pending) => {
      ...
      return { ...pending, confirm: { ...pending.confirm, backupReuse: next } };
    });
  ...
  const onToggleBackupReuseConsent = useCallback(
    (next) =>
      setPendingApply((pending) =>
        pending ? { ...pending, reuseConsent: next === true } : pending,
      ),
    [],
  );
  ...
  const runReconcileRetry = useCallback(
    async ({ stripBlamedKeys = false } = {}) => {
      ...
      try {
        ...
        // hold reason inline instead of a generic failure.
        const heldReason =
          err?.code === "reconcile_still_held"
            ? err?.outcome?.hold?.reason || null
            : null;
        setReconcileError(
          heldReason
