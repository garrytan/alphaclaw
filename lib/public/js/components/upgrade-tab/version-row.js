...
export const VersionRow = ({
  ...
  // v0.9.80: a row this AlphaClaw's Node cannot run (OpenClaw 2026.9.3 needs
  // Node 24.16+) is named and its Apply disabled — the same evaluator the
  // server preflight refuses with, so the two never disagree.
  const enginesGate = buildEnginesGateModel({ row, nodeVersion });
              ...
              <${ActionButton}
                onClick=${() =>
                  onRequestApply({
                    ...
                    // v0.9.81 (D13): the button's own word is the declared
                    // direction the server judges.
                    intent: action.intent,
                  })}
                tone=${action.isDowngrade ? "warning" : "secondary"}
                idleLabel=${actionLabel}
                disabled=${actionsDisabled || applyDisabled || enginesGate.blocked}
              />
            `
          : null}
      </div>
      ${enginesGate.blocked
        ? html`<p class="mt-1 text-xs text-status-warning-muted">
            ${enginesGate.note}
          </p>`
        : null}
      <${BlocklistNotice}
