...
import { buildChannelOptions, kChannelLabels, kGatewayHoldTitle } from "./helpers.js";
...
// Held gateway: the boot reconciler failed the settings migration and
// deliberately did NOT start OpenClaw. Names the reason and the blamed keys,
// and offers Retry / Strip-and-retry — the strip path only OPENS the confirm
// dialog (it never fires the api directly). Persistent warning, not a toast.
export const GatewayHoldCard = ({
  model = null,
  ...
  return html`
    <div class="bg-surface border border-yellow-500/40 rounded-xl p-4 space-y-2">
      <div class="flex items-center gap-2 min-w-0">
        ...
        <p class="text-sm font-medium text-body">${model.title || kGatewayHoldTitle}</p>
      </div>
      <p class="text-sm text-body min-w-0 break-words">${model.reason}</p>
      ${model.blamedKeys.length > 0
        ? html`
            <ul class="flex flex-wrap gap-1">
              ${model.blamedKeys.map(
                (key) => html`
                  <li
                    class="ac-surface-inset border border-border rounded px-1.5 py-0.5 font-mono text-xs text-body break-all"
                  >
                    ${key}
                  </li>
                `,
      ...
      <div class="flex flex-wrap items-center gap-2">
        ${model.requiresRecoveryChoice ? html`<${ActionButton}
          onClick=${() => onRequestApply({ payload: model.recoveryTarget, label: model.recoveryTarget?.version || model.recoveryTarget?.sha, isDowngrade: false })}
          tone="secondary" idleLabel="Review database recovery choices"
          disabled=${actionsDisabled || !model.recoveryTarget}
        />` : html`<${ActionButton}
          onClick=${onRetryReconcile}
          tone="primary"
          idleLabel="Retry migration"
          loadingLabel="Retrying..."
          loading=${retryingReconcile === "retry"}
          disabled=${actionsDisabled && retryingReconcile !== "retry"}
        />`}
        ${model.requiresRecoveryChoice && !model.recoveryTarget ? html`<p class="text-xs text-status-warning">The held build could not be identified. Refresh version status or choose a supported version from the catalog.</p>` : null}
        ${model.canStrip
          ? html`
              <${ActionButton}
                onClick=${onRequestStripReconcile}
                tone="warning"
                idleLabel="Strip blamed keys and retry"
                loadingLabel="Retrying..."
                loading=${retryingReconcile === "strip"}
                disabled=${actionsDisabled && retryingReconcile !== "strip"}
              />
            `
          : null}
      </div>
        ...
        ? html`
            <div class="flex items-start gap-2">
              <div class="flex-1 min-w-0">
                <${InlineErrorChip}
                  headline=${reconcileError.headline}
              ...
              <button
                ...
                onclick=${onDismissReconcileError}
              >
                Dismiss
              </button>
            </div>
