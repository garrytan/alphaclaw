...
export const UpgradeCatalogCard = ({
  ...
  nowMs = Date.now(),
  installedVersion = null,
  // The Node this AlphaClaw runs (channelInfo.nodeVersion); rows whose
  // `engines.node` it fails render a note and a disabled Apply (v0.9.80).
  nodeVersion = null,
  actionsDisabled = false,
  applyGate = null,
  onCheckNow = () => {},
  ...
  // npm degradation gates installs (Apply disabled with an explanation);
  // GitHub-only degradation only costs release notes — Apply stays enabled.
  const applyDisabled = Boolean(applyGate?.applyDisabled);
  const rowProps = {
    nodeVersion,
    actionsDisabled,
    applyDisabled,
  ...
  return html`
    <div class="bg-surface border border-border rounded-xl p-4 space-y-4">
      ...
      ${catalog && applyGate?.notesNote
        ? html`<p class="text-xs text-fg-muted">${applyGate.notesNote}.</p>`
        : null}

      ${catalog && applyDisabled
        ? html`<p class="text-xs text-status-warning-muted">
