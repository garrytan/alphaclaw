...
import { setCached, invalidateCache } from "../../lib/api-cache.js";
import { kBackupPreflightCacheKey } from "./use-backup-preflight.js";
...
export const useBackupPolicy = () => {
  ...
  const setting = useSavedSetting({
    ...
    onSaved: (_document, response) => {
      if (active.current) {
        setCached(kBackupPolicyCacheKey, response);
        invalidateCache(kBackupPreflightCacheKey);
      }
    },
