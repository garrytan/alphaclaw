...
export const kBackupsEmptyLabel =
  "No backups yet — the first cross-channel update creates one";
...
export const kBackupsInventoryCap = 50;
...
// Mirrors the server's `reuseWindowStartMs`: an archive taken before the
// newest successful apply / activation / settings migration predates state
// the current build has already rewritten, so the reuse gate refuses it. The
// channel payload carries the same three records the server reads
// (`applied.at`, `lastUpdateRun.startedAt` when that run succeeded,
// `configMigration.lastAttempt.at` when that attempt succeeded); the run
// ledger's older activations are not in the payload, but `lastUpdateRun` is
// the compatibility pointer to the newest one.
export const buildBackupReuseWindowStartMs = (channelInfo = null) => {
  let since = 0;
  ...
  const lastRunStartedAt = toEpochMs(lastRun?.startedAt);
  if (lastRun?.ok === true && lastRunStartedAt != null) {
    since = Math.max(since, lastRunStartedAt);
  }
...
// kBackupReuseMaxAgeMs old · not older than the last update); the sha256
// must come from the inventory entry itself — the UI never hashes and never
// sends a path. `available:false` carries the visible reason.
export const buildBackupReuseConsentModel = ({
  ...
  const since = buildBackupReuseWindowStartMs(channelInfo);
  const candidates = verifiedCandidates.filter(
    (entry) =>
      nowMs - Number(entry.at) <= kBackupReuseMaxAgeMs && Number(entry.at) >= since,
  );
...
export const buildApplyConfirmModel = ({
  ...
  return {
    ...
    backupReuse: hardGate
      ? buildBackupReuseConsentModel({ inventory: backupInventory, channelInfo, nowMs })
      : null,
  };
};
...
export const buildBackupReuseOfferModel = ({
  ...
  const ageMs = Number.isFinite(Number(offer.ageMs))
    ? Number(offer.ageMs)
    : at != null
      ? Math.max(0, nowMs - at)
      : null;
  const ageLabel =
    ageMs != null
      ? formatRelativeTime(nowMs - ageMs, { nowMs, style: "long" })
      : "at an unknown time";
  ...
  return entries
    ...
    .map((entry) => {
      ...
      if (entry.partial === true) {
        badges.push({
          id: "partial",
          label: "partial — workspace files excluded",
          tone: "warning",
        });
      }
