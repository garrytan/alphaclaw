...
import { isMigrationOnlyBackup, kMigrationOnlyRestoreCaption, buildBackupSnapshotCaption, buildRecoveryCaption } from "./backup-presentation.js";
import {
  formatBytes,
  formatLocaleDate,
...
export const kBreakingSafetyNote =
  "120s health check · auto-rollback stays armed for 24h when state remains compatible · a failing version gets blocklisted. A configuration checkpoint cannot undo database changes.";
export const kBackupHardGateNote = "If configuration cannot be checkpointed or compatibility is unknown, nothing is installed. A required database migration asks you to choose recovery protection first.";
...
export const kBackupsEmptyLabel =
  "No recovery artifacts yet — the next OpenClaw update creates a configuration checkpoint";
...
export const buildBackupPartialBadgeLabel = (entry = null) =>
  isMigrationOnlyBackup(entry)
    ? "migration-only — workspace and other state omitted; not reusable by a later update"
    : `${kBackupPartialBadgePrefix} — ${buildBackupPartialReasonText(entry)}`;

export const kApplyStepPreview = [
  "Prepare target",
  "Check compatibility and choose database protection if required",
  "Configuration checkpoint (database data omitted by default)",
  "Restart (~2 min)",
...
export const buildApplyConfirmModel = ({
  ...
  lines.push(channelInfo?.gatewayHold ? "The gateway is already held. It remains stopped while the target and recovery protection are reviewed." : kBackupPauseNote);
  return {
...
export const kNoBackupConsentCtaLabel = "Upgrade without database snapshot";
export const kNoBackupConsentConfirmLabel = "Confirm forward-only upgrade";
...
export const buildNoBackupConsentRiskLine = (label = "") =>
  `The update to ${label || "this version"} may change your databases. The configuration checkpoint cannot restore database data. If it goes wrong, this attempt has no database snapshot to restore; continuing is forward-only.`;
export const buildNoBackupConsentOfferModel = ({
  error = null,
  target = null,
  label = "",
} = {}) => {
  if (!["recovery_choice_required", "backup_required_for_migration", "backup_failed"].includes(error?.code) || error?.backupRiskEligible !== true || typeof error?.operationId !== "string" || !error.operationId) return null;
  return {
    code: error.code,
...
export const buildBackupReuseOfferModel = ({
  ...
  if (isMigrationOnlyBackup(offer)) return null;
...
export const buildBackupInventoryRows = (inventory = null, nowMs = Date.now()) => {
  ...
  return entries
    ...
    .map((entry) => {
      ...
      if (entry.recovery || ["config_only", "database_set"].includes(entry.profile)) {
        badges.push({ id: "recovery", label: buildRecoveryCaption(entry.recovery), tone: entry.recovery?.restore?.databaseSetAvailable === true && entry.recovery?.databases?.complete === true && entry.recovery?.databases?.verified === true ? "success" : "info" });
      ...
      if (entry.partial === true || isMigrationOnlyBackup(entry)) {
        badges.push({
          id: "partial",
      ...
      } else if (entry.eligible !== true && !entry.recovery && !["config_only", "database_set"].includes(entry.profile)) {
        const reason =
      ...
      return {
        ...
        captureLabel: buildBackupSnapshotCaption(entry),
        sizeLabel:
...
export const buildRollbackDataRiskLine = (modelOrFile = null, nowMs = Date.now()) => {
  ...
  if (recovery && recovery.kind !== "database_set") {
    const config = recovery.restore?.configAvailable === true
      ? `A configuration checkpoint is available${recovery.checkpoint?.file ? ` (${recovery.checkpoint.file})` : ""}, but it cannot restore database data or make database rollback safe.`
      : "No verified configuration checkpoint or database recovery set is available.";
    return `${recovery.kind === "forward_only" ? "This upgrade was approved as forward-only. " : ""}${config} No database snapshot is available from this attempt. AlphaClaw does not restore database data automatically; rolling back anyway may leave newer data unreadable.`;
  }
  if (!backupFile) {
    if (recovery) return "No verified pre-update database recovery set is available. A configuration checkpoint alone cannot restore database data or make database rollback safe. AlphaClaw does not restore database data automatically; rolling back anyway may leave newer data unreadable.";
  ...
  if (model.backupFileExists === false && caveat && caveat !== "missing") {
    ...
    const survivorLine =
      survivor && survivor.file
        ? ` The newest surviving verified archive is ${survivor.file} — check its date before restoring.${isMigrationOnlyBackup(survivor) ? ` It is migration-only. ${kMigrationOnlyRestoreCaption}` : ""}`
        : " No other verified archive survives.";
    return `The recorded pre-update ${recovery ? "database recovery directory" : "backup"} (${backupFile}) is on disk but failed verification — ${why} — do not restore it.${survivorLine} Or roll back anyway — data written by the newer version may be unreadable.`;
  }
  ...
  if (model.backupFileExists === false) {
    ...
    if (survivor && survivor.file) {
      ...
      return `The recorded pre-update ${recovery ? "database recovery directory" : "backup"} (${backupFile}) is no longer on disk — the original pre-migration backup was pruned. The newest surviving archive is ${survivor.file}${survivorMeta ? ` (${survivorMeta})` : ""} — it may not predate the migration, so check its date before restoring.${isMigrationOnlyBackup(survivor) ? ` It is migration-only. ${kMigrationOnlyRestoreCaption}` : ""} Or roll back anyway — data written by the newer version may be unreadable.`;
    }
    return `The recorded pre-update ${recovery ? "database recovery directory" : "backup"} (${backupFile}) is no longer on disk — the original pre-migration backup was pruned and no other archive survives. Rolling back anyway may leave data written by the newer version unreadable.`;
  }
  if (recovery) {
    if (model.backupFileExists !== true || recovery.checkpoint?.file !== backupFile || recovery.checkpoint?.verified !== true || recovery.restore?.databaseSetAvailable !== true || recovery.databases?.complete !== true || recovery.databases?.verified !== true) {
      return `The recorded database recovery directory (${backupFile}) has not passed current database-set verification; do not restore it. A configuration checkpoint alone cannot make database rollback safe. Rolling back anyway may leave newer data unreadable.`;
    }
    return `The verified pre-update database recovery directory (${backupFile}) is available; this is not a tar archive. Before rollback, follow the offline restore runbook: verify the manifest and database integrity, restore the complete captured database set with its matching source build, and preserve omitted files. AlphaClaw does not restore database data automatically. Rolling back without restoring the matching set may leave newer data unreadable.`;
  }
  const caveats = [];
  if (model.backupProfile === "migration-minimal") {
    caveats.push(`this is a migration-only backup; workspace and other state are omitted. ${kMigrationOnlyRestoreCaption}`);
  } else if (model.backupPartial === true) {
    caveats.push(Array.isArray(model.backupPartialReasons) && model.backupPartialReasons.length
      ? model.backupPartialReasons.join("; ") : "workspace files were excluded from it");
  }
...
export const buildGatewayHoldModel = (channelInfo = null, runs = []) => {
  const hold = channelInfo?.gatewayHold ?? null;
  if (hold == null || typeof hold !== "object") return null;
  if (["recovery_choice_required", "recovery_intent_stale", "recovery_review"].includes(hold.reason)) {
    const applied = channelInfo.applied;
    const stale = hold.reason === "recovery_intent_stale";
    const reviewing = hold.reason === "recovery_review";
    const run = (Array.isArray(runs) ? runs : []).find((entry) => entry?.operationId === hold.operationId) ||
      (channelInfo.lastUpdateRun?.operationId === hold.operationId ? channelInfo.lastUpdateRun : null);
    const version = (stale ? channelInfo.installedVersion : channelInfo.expectedVersion) || channelInfo.installedVersion || channelInfo.pinVersion;
    const target = reviewing ? run?.result?.target || run?.target || null : applied?.channel === "dev"
      ? applied.sha ? { channel: "dev", sha: applied.sha } : null
      : version ? { channel: applied?.channel === "beta" ? "beta" : "stable", version } : null;
    return {
      title: stale ? "Database recovery approval is stale" : reviewing ? "Database recovery review is holding the gateway" : "Database recovery choice required",
      reason: stale
        ? "The gateway is stopped because the previously approved build or recovery data changed. Review and reapply this build to renew recovery protection; the old approval cannot be reused. Choose a database snapshot or explicitly approve a forward-only upgrade after the current state is checked."
        : reviewing
          ? "The gateway is stopped for an unfinished database recovery review. Resume the recorded target's recovery choices, or cancel the review so AlphaClaw can verify and resume the prior gateway safely."
          : "The gateway is stopped because this version requires a database migration. Configuration recovery alone cannot restore database data. Review the target, then choose a database snapshot or explicitly approve a forward-only upgrade. Neither choice is selected by default.",
      recoveryTarget: target,
      requiresRecoveryChoice: true,
      blamedKeys: [], keyCount: 0, canStrip: false,
    };
  }
  ...
  (Array.isArray(runs) ? runs : [])
    ...
    .map((run) => {
      ...
      return {
        ...
        recoveryLabel: run.recovery || run.result?.recovery ? buildRecoveryCaption(run.recovery || run.result.recovery) : null,
...
export const buildLastManualBackupLine = (runs = [], nowMs = Date.now()) => {
  ...
  if (newest.state === "completed" && newest.result?.ok) {
    const recovery = newest.recovery || newest.result?.recovery;
    if (recovery) return { state: "completed", tone: "success", text: `Last manual recovery checkpoint: ${when || "just now"} — ${buildRecoveryCaption(recovery)}`, operationId: newest.operationId };
    const archive = newest.result?.archive || null;
    if (isMigrationOnlyBackup(archive)) {
      return { state: "completed", tone: "warning", text: `Last manual backup: ${when || "just now"} — migration-only; workspace and other state omitted`, operationId: newest.operationId };
    }
...
export const buildFailureCtaModel = (operation = null) => {
  ...
  if (operation.phase === "completed" && isBackupOp) {
    ...
    return {
      ...
      retryUpdate,
      hint: retryUpdate
        ? isMigrationOnlyBackup(operation.result?.archive)
          ? "The migration-only backup is verified. Retrying the update will run a fresh backup; this archive is not reusable by a later update."
