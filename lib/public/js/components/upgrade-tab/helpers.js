...
// #79 (b): the post-preflight checkpoint's consent. The server refuses a
// soft-gated apply whose database preflight says the target MIGRATES the
// databases while no backup could be taken — 409 backup_required_for_migration,
// the ONLY overridable backup code (Codex 20: backup_failed, the hard gate, is
// never overridable and never shows this checkbox). The offer model is
// non-null for that code alone; the second-stage dialog holds a default-OFF
// checkbox with this copy and resends the same target with
...
export const buildNoBackupConsentRiskLine = (label = "") =>
  `The update to ${label || "this version"} migrates your database. If it goes wrong, auto-rollback cannot restore the previous build's data — there is no backup to restore from. Fix the backup instead if you can (the failed backup step says why it failed).`;
export const buildNoBackupConsentOfferModel = ({
  error = null,
  target = null,
  label = "",
} = {}) => {
  if (error?.code !== kNoBackupConsentCode) return null;
  return {
    code: kNoBackupConsentCode,
