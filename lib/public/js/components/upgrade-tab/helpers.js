...
export const kDevApplyImpactNote =
  "compiles from source, 10-30 minutes; your agent stays up until the final restart";
...
export const compareVersions = (a, b) => {
  ...
  if (ra === 1) {
    // both hotfixes: numeric order
    return Number.parseInt(va.pre, 10) > Number.parseInt(vb.pre, 10) ? 1 : -1;
  }
    ...
    appliedChannel === "dev"
      ? `dev ${formatShortSha(info.appliedId)}${
          info.installedVersion
            ? ` (pin ${info.installedVersion} dormant)`
            : ""
        }`
...
export const buildAvailabilityLine = ({
  ...
  return latest.current
    ? `You're on the latest ${releaseChannel} version.`
    : `Latest ${releaseChannel}: ${latest.version}`;
};
...
export const buildVerdictBannerModel = ({
  ...
  const displayVersion =
    installedVersion || formatShortSha(appliedId) || channel.pinVersion || "";
  ...
  if (expected.devHead) {
    const matches =
      !channel.isPin &&
      Boolean(appliedId) &&
      (!expected.previousId || appliedId !== expected.previousId);
    return matches
      ? { ok: true, message: okMessage }
      : {
          ok: false,
