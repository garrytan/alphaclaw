...
// (eligible · on disk · verified · not partial · has a timestamp); the
// sha256 must come from the inventory entry itself — the UI never hashes and
// never sends a path. `available:false` carries the visible reason.
...
export const buildApplyConfirmModel = ({
  ...
  return {
    ...
    backupReuse: hardGate
      ? buildBackupReuseConsentModel({ inventory: backupInventory, nowMs })
      : null,
  };
};
