...
export const buildApplyConfirmModel = ({
  ...
  if (isBreaking) {
    ...
    // Cross-channel applies quiesce the gateway while the pre-update backup
    // runs — say so where the user commits.
...
// Second-stage rollback fence (issue #20): the server refused the rollback
// because this update migrated the state DBs. The guidance line names the
// verified pre-update backup when the server recorded one.
export const buildRollbackDataRiskLine = (backupFile = null) =>
  backupFile
    ? `Restore the verified pre-update backup first (${backupFile}), or roll back anyway — data written by the newer version may be unreadable.`
