...
// three records the server reads: `applied.at`, `lastUpdateRun.startedAt`
// when that run succeeded, `configMigration.lastAttempt.at` when that attempt
// succeeded) stays as the fallback for older servers, and is folded in with
...
// path. `available:false` carries the visible reason.
