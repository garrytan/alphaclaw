...
import { isMigrationOnlyBackup, buildBackupResultMessage, kMigrationOnlyBackupCaption, kMigrationOnlyRestoreCaption } from "./backup-presentation.js";
import {
  OperationStepList,
  getCurrentStepName,
...
export const UpgradeProgressCard = ({
  ...
  const isUnknown = operation.phase === "unknown";
  const isRepairOp = Boolean(operation.target?.repair);
  const isBackupOp = operation.target?.kind === "backup";
  const migrationOnly = isMigrationOnlyBackup(operation.result?.archive);
  const recoveryKind = operation.result?.recovery?.kind;
  ...
  const heading = isRepairOp
    ? isFailed
      ? "Repair failed"
      : isCompleted ? "Repair completed" : "Repairing the dev build"
    : isBackupOp
      ? isFailed
        ? "Backup failed"
        : isCompleted
          ? recoveryKind === "config_only" ? "Configuration checkpoint completed" : recoveryKind === "database_set" ? "Database snapshot completed" : migrationOnly ? "Migration-only backup completed" : "Backup completed"
          : "Backing up OpenClaw"
      : isFailed
    ...
    >
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-2 min-w-0">
          ${!isFailed && !isCompleted && !isUnknown ? html`<${LoadingSpinner} className="h-4 w-4" />` : null}
      ...
      ${operation.monitorError || isUnknown
        ? html`<div class="ac-surface-inset border border-border rounded-lg p-3 space-y-2" role="status">
            <p class="text-sm text-status-warning">${operation.monitorMessage || operation.monitorError?.message || "Operation status is unavailable."}</p>
            <p class="text-xs text-fg-muted">The last confirmed progress is shown below.</p>
            <${ActionButton} onClick=${onRetryOperationStatus} idleLabel="Retry status check" tone="secondary" />
          </div>`
        : null}

      ${isCompleted && isRepairOp ? html`<p class="text-sm text-status-success">The dev build repair finished.</p>` : null}
              ...
              <div class="flex flex-wrap items-center gap-2 pt-1">
                ...
                ${noBackupConsentOffer
                  ? html`
                    ...
                    `
                  : null}
                ${repairAvailable && cta.retryBackup
                  ? html`<${ActionButton} onClick=${onRetryBackup} tone="warning" idleLabel="Retry backup" loadingLabel="Starting backup..." />`
                  : null}
                ${repairAvailable
                  ? html`
                      <${ActionButton}
      ...
      ${isCompleted && isBackupOp
        ? html`
            <div class="space-y-2">
              <p class=${`text-sm ${migrationOnly ? "text-status-warning-muted" : "text-status-success"}`}>
                ${buildBackupResultMessage(operation.result)}
              </p>
              ${migrationOnly ? html`<p class="text-xs text-status-warning-muted">${kMigrationOnlyBackupCaption} ${kMigrationOnlyRestoreCaption}</p>` : null}
              <div class="flex flex-wrap items-center gap-2 pt-1">
                ${cta.retryUpdate
                  ? html`
                      <${ActionButton}
