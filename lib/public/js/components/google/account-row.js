...
export const GoogleAccountRow = ({
  ...
  const remoteOperation = gmailWatchStatus?.remoteOperation || account.gmailWatch?.remoteOperation;
  const disconnectFailed = remoteOperation?.kind === "disconnect" && remoteOperation.status === "failed";
  const accountDisconnecting = disconnecting ||
    (remoteOperation?.kind === "disconnect" && remoteOperation.status === "pending");
  return html`
    <div class="border border-border rounded-lg bg-field overflow-visible">
      ...
      ${disconnectFailed
        ? html`<div class="px-3 pb-2"><${InlineErrorChip}
            error=${new Error(remoteOperation.message || "Google disconnect did not finish.")}
            headline="Disconnect did not finish. Gmail delivery remains disabled."
            onRetry=${() => onDisconnect?.(account.id)}
        ...
        ? html`
            <div class="px-3 pb-3 space-y-3 border-t border-border">
                    ...
                    <div class="-mx-3 mt-4 mb-2 border-y border-border">
                      <div class="px-3 py-3 space-y-2">
                        ...
                        <${GmailWatchToggle}
                          ...
                          busy=${gmailWatchBusy || accountDisconnecting}
              ...
              <div class="pt-1 space-y-2 sm:space-y-0 sm:flex sm:justify-between sm:items-center">
                ...
                >
                  ${accountDisconnecting ? "Disconnecting..." : "Disconnect"}
                </button>
              </div>
            </div>
          `
