            ...
            <div class="space-y-2 mt-3">
              ${accountsError
                ? html`<${InlineErrorChip}
                    error=${accountsError}
                    headline="Couldn't refresh Google accounts. Showing the last known accounts."
              ...
              ${gmailWatchLoadError
                ? html`<${InlineErrorChip}
                    error=${gmailWatchLoadError}
                    headline="Couldn't refresh Gmail watch status. Showing the last known status."
                    onRetry=${() => refreshGmailWatch().catch(() => {})}
                  />`
                : null}
              ${accounts.map(
                (account) =>
