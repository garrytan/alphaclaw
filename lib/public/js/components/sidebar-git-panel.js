...
export const SidebarGitPanel = ({
  ...
  // Latest-request-wins across the interval poll, file-saved refreshes, and
  // the post-sync fetch: an older response landing late must not overwrite a
  // newer summary (component-level so handleSyncChanges shares it).
  ...
  useEffect(() => {
    ...
    const loadSummary = async () => {
      ...
      try {
        ...
        if (!active || summaryRequestIdRef.current !== requestId) return;
        setSummary(data);
        setError("");
      } catch (nextError) {
        if (!active || summaryRequestIdRef.current !== requestId) return;
        ...
        if (active && summaryRequestIdRef.current === requestId) {
          setLoading(false);
        }
      }
    };
    ...
    return () => {
      active = false;
      loadSummaryRef.current = null;
      window.removeEventListener("alphaclaw:browse-file-saved", handleFileSaved);
    };
  }, [isActive]);
  // Refresh poll through the shared primitive (fix wave F202): a hidden tab
  // no longer spawns git every 10s.
  useVisibleInterval(() => loadSummaryRef.current?.(), kRefreshMs, { enabled: isActive });
  ...
  // Full-panel error only when there is no data to show; with a summary the
  // panel stays up and the failure renders as an inline warning below.
  if (error && !summary) {
        ...
        ? html`
            <div class="sidebar-git-bar sidebar-git-bar-secondary">
              <${Tooltip}
                ...
                tooltipId="ac-tooltip-git-refresh-error"
              >
                <span
                  class="sidebar-git-panel-error"
                  tabindex="0"
                  aria-describedby="ac-tooltip-git-refresh-error"
                >
