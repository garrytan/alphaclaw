...
// plus an optional preparing_plugins step the server may skip entirely.
...
export const buildRestartBannerProgress = (operation = null) => {
  ...
  const done = models.filter((step) => step.status === "done").length;
  return { step: Math.min(total, done + 1), of: total };
};
