...
// and, only when the restart had to wait for another lifecycle operation, a
// leading waiting_for_lock step (v0.9.72). Optional steps widen the total so
// the counter never runs ahead of the plan.
const kRestartStepBaseCount = 4;
const kOptionalRestartSteps = ["preparing_plugins", "waiting_for_lock"];
...
export const buildRestartBannerProgress = (operation = null) => {
  ...
  const optionalCount = kOptionalRestartSteps.filter((name) =>
    models.some((step) => step.name === name),
  ).length;
  const total = kRestartStepBaseCount + optionalCount;
