...
export const DoctorTab = ({ isActive = false, onOpenFile = () => {} }) => {
  ...
  // Safari suppresses hover on disabled controls and touch never sees the
  // title attribute — render the no-changes reason as visible text too. The
  // gateway-not-ready case already has its own visible note.
  const runDisabledNote = gatewayNotReadyNote ? "" : runDoctorDisabledReason;
