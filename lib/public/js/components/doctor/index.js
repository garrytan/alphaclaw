...
export const DoctorTab = ({ isActive = false, onOpenFile = () => {} }) => {
  ...
  const gatewayNotReadyNote =
    doctorStatus?.gatewayReadiness?.ok === false
      ? `Gateway not ready: ${
          String(doctorStatus.gatewayReadiness?.reason || "").trim() ||
          "unavailable"
        }`
      : "";
  ...
  const handleRunDoctor = async () => {
    try {
      ...
      const message = error?.gatewayUnavailable
        ? `Gateway not ready: ${error.reason || error.message || "unavailable"}`
