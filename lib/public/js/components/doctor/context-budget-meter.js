...
export const buildContextBudgetMeterModel = (doctorStatus = null) => {
  ...
  // Absent files (rejected reads / plain missing) never appear as rows, and
  // the not_configured synthetic entry lives only in hardening.files — the
  // hint line is the meter's honest pointer at both.
  const rejectedHints =
    String(hardening.state || "") === "blocked"
      ? (Array.isArray(hardening.files) ? hardening.files : [])
          .filter(
            (file) =>
              file &&
              (!file.exists || String(file.reason || "") === "not_configured"),
          )
          .map((file) => {
            const reason = String(file.reason || "");
            const phrase =
              reason === "not_configured"
                ? "is on disk but has no bootstrap config entry"
                : kRejectedHintReasons.has(reason)
                  ? "is rejected before it can be read"
                  : "is missing from disk";
            return `${String(file.path || "")} ${phrase} — run a scan for the full finding.`;
          })
      : [];
  const summary =
    budgetChars > 0
