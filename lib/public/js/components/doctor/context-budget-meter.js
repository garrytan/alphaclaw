...
export const buildContextBudgetMeterModel = (doctorStatus = null) => {
  ...
  const rows = files.map((file) => {
    ...
    return {
      ...
      detail: `${Number(file.injectedChars || 0).toLocaleString()} / ${Number(
        file.rawChars || 0,
  ...
  const summary =
    budgetChars > 0
      ? `${Number(usedChars).toLocaleString()} / ${formatDoctorCharCount(budgetChars)}`
...
export const ContextBudgetMeter = ({
  ...
  return html`
    <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
      ...
      <div class="h-1.5 w-full rounded-full bg-field overflow-hidden">
        <div
