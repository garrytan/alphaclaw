...
export const getDoctorRunDisabledReason = (doctorStatus = null) => {
  const gatewayNotReadyMessage = getGatewayNotReadyMessage(doctorStatus);
  if (gatewayNotReadyMessage) return gatewayNotReadyMessage;
  if (doctorStatus?.runInProgress || doctorStatus?.needsInitialRun) return "";
  if (Number(doctorStatus?.changeSummary?.changedFilesCount || 0) > 0)
    return "";
  // Environment-side degradation (hardening blocked/starved, active
  // truncation) produces P0-worthy findings with zero workspace delta, and
  // the server's zero-change reuse path recomputes environment checks
  // cheaply — keep the Run button enabled in those states.
  const hardeningState = String(
    doctorStatus?.bootstrapContext?.hardening?.state || "",
  );
  if (hardeningState === "blocked" || hardeningState === "starved") return "";
  if (doctorStatus?.bootstrapContext?.hasActiveTruncation === true) return "";
