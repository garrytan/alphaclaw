...
export const getDoctorRunDisabledReason = (doctorStatus = null) => {
  if (doctorStatus?.gatewayReadiness?.ok === false) {
    const reason = String(doctorStatus.gatewayReadiness?.reason || "").trim();
    return reason ? `Gateway not ready: ${reason}` : "Gateway not ready.";
  }
