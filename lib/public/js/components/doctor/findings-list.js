...
export const DoctorFindingsList = ({
  ...
  // Deep-link focus (#/doctor?focus=context): the card carrying this id gets
  // the incident-style persistent highlight border.
  highlightCardId = "",
}) => {
    ...
    <div class="space-y-4">
      ${cards.length
        ? html`
            <div class="space-y-3">
              ${cards.map((card) => {
                // busyAction = { cardId, status }: the clicked control shows
                // its own pending label; the card's sibling actions disable.
                const busyStatus =
                  busyAction && busyAction.cardId === card.id
                    ? String(busyAction.status || "")
                    : "";
                const cardBusy = !!busyStatus;
                return html`
                  <div
                    class=${`bg-surface border ${
                      highlightCardId && String(card.id) === String(highlightCardId)
                        ? "border-cyan-500/60"
                        : "border-border"
                  ...
                  >
                        ...
                        <div class="space-y-2 min-w-0">
                          ...
                          <div class="flex flex-wrap items-center gap-2">
                            ...
                            ${card.status === "working"
                              ...
                              : null}
                            ${card.status === "working" && card.fixDelivery
                              ? html`<span class="text-[10px] text-fg-dim">
                                  ${card.fixDelivery.attached
                                    ? `delivery requested → ${card.fixDelivery.replyChannel}${
                                        card.fixDelivery.replyTo
                                          ? ` · ${card.fixDelivery.replyTo}`
                                          : ""
                                      }`
                                    : "main session"}
                              ...
                              : null}
                            ${card.status === "open" &&
                            card.fixDelivery?.gatewayOk === false
                              ? html`<span
                                  class="text-[10px] text-status-error"
                                  >last fix dispatch failed</span
                                >`
                              : null}
                            ${
                              showRunMeta
                    ...
                    <div class="flex flex-wrap gap-2">
                      <${ActionButton}
                        ...
                        disabled=${card.status !== "open" || cardBusy}
                        tone="primary"
                        ...
                        card.status !== "fixed"
                          ? html`
                              <${ActionButton}
                                ...
                                loading=${busyStatus === "fixed"}
                                disabled=${cardBusy && busyStatus !== "fixed"}
                                tone="secondary"
                                idleLabel="Mark fixed"
                                loadingLabel="Marking fixed..."
                              />
                            `
                          : html`
                              <${ActionButton}
                                onClick=${() => onUpdateStatus(card, "open")}
                                loading=${busyStatus === "open"}
                                disabled=${cardBusy && busyStatus !== "open"}
                                tone="secondary"
                                idleLabel="Reopen"
                        ...
                        card.status !== "dismissed"
                          ? html`
                              <${ActionButton}
                                ...
                                loading=${busyStatus === "dismissed"}
                                disabled=${cardBusy &&
                                busyStatus !== "dismissed"}
                                tone="ghost"
                                idleLabel="Dismiss"
