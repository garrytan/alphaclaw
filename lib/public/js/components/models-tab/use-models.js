...
export const useModels = (agentId) => {
  ...
  // First-sighting guard (same pattern as use-gmail-watch): the three
  // useCachedFetch hooks already fetch on mount, and a forced refresh here
  // would double-hit /api/models, /api/models/config, and /api/codex/status
  // on every cold mount (force deliberately never dedupes onto an in-flight
  // request). Only force-refresh on genuine agent SWITCHES.
  const refreshedAgentRef = useRef(null);
  useEffect(() => {
    if (refreshedAgentRef.current === null) {
      refreshedAgentRef.current = agentId;
      return;
    }
    if (refreshedAgentRef.current === agentId) return;
    refreshedAgentRef.current = agentId;
    refresh();
  }, [agentId]);
