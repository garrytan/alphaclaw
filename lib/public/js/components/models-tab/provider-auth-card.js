...
const CodexOAuthSection = ({
  ...
  // the backup barrier lifts. The badge says so until a status read confirms
  // the saved connection (a plain "Not connected" in between would be false).
  ...
  const connectedSaved =
    codexStatus?.connected === true && !isStoreUnavailable(codexStatus);
  useEffect(() => {
    if (connectedSaved) setDeferredSavePending(false);
  }, [connectedSaved]);

  const noteConnected = (payload) => {
    showToast(buildCodexConnectedMessage(payload), "success");
    if (isCodexDeferredSuccess(payload)) setDeferredSavePending(true);
  };
