...
import { ManagedUpdateAttemptCard } from "./managed-update-attempt-card.js";
  ...
  return html`
    <div class="ac-surface-inset border border-border rounded-lg px-3 py-2">
      <p class="text-[11px] uppercase tracking-[0.18em] text-fg-muted">
        ${label}${note
          ? html`<span class="ml-2 normal-case tracking-normal">${note}</span>`
          : null}
      </p>
...
export const UpdateModal = ({
  ...
  const publishedAtLabel = formatLocaleDateTime(notesData?.publishedAt, {
    fallback: "",
  });
  const releaseBody = String(notesData?.body || "").trim();
  const releasePreviewHtml = useMemo(
    () =>
      renderMarkdownSafe(releaseBody),
    [releaseBody],
  );
  ...
  const handlePrimaryAction = () => {
    if (canApplyUpdate) {
      if (managedUpdate?.disabled) return;
      onUpdate();
      return;
    }
    if (primaryActionUrl) {
  ...
  return html`
    ...
    >
      ...
      <div class="space-y-1 pr-10">
        <h3 class="text-sm font-semibold">${managedUpdate?.blocked ? "Deployment status" : "Update available"}</h3>
      ...
      <div class="grid gap-2 sm:grid-cols-2">
        ...
        <${VersionSummaryRow}
          ...
          note=${openclawChannel && !openclawChannel.isPin
            ? `${openclawChannel.releaseChannel} channel`
            : ""}
        />
      </div>

      <${ManagedUpdateAttemptCard} managedUpdate=${managedUpdate} />

      <a
        class=${`ac-tip-link text-xs ${
          updating ? "opacity-50 cursor-not-allowed pointer-events-none" : ""
        }`}
        aria-disabled=${updating ? "true" : "false"}
        href="#/upgrade"
        onclick=${(event) => {
          // Mid-update the modal must stay put — block the navigation too,
          // not just the close.
          if (updating) {
        ...
        >Looking for OpenClaw versions? → Upgrade page</a
      >
      ...
      <div class="flex items-center justify-end gap-2 pt-1">
        ...
        <${ActionButton}
          ...
          disabled=${canApplyUpdate ? loadingNotes || Boolean(managedUpdate?.disabled) : false}
        />
      </div>
    </${ModalShell}>
  `;
