...
export const useNodesTab = () => {
  ...
  const refreshNodes = useCallback(async () => {
    ...
    try {
      // Post-mutation refreshes must not dedupe onto a poll dispatched before
      // the mutation — force a fresh request (latest-request-wins applies).
      await connectedNodesState.refresh({ force: true });
    } finally {
