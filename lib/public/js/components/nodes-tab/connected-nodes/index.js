...
// Exported for direct vnode tests (module-private otherwise).
export const renderNodeStatusBadge = (node) => {
  if (node?.connected) {
  ...
  if (node?.paired) {
    return html`<${TooltipBadge}
      tone="warning"
      label="Disconnected"
      text="Paired but currently disconnected — run the node command again on your device to reconnect."
    />`;
  }
  return html`<${TooltipBadge}
    tone="danger"
    label="Pending approval"
    text="This node requested pairing and is waiting for approval — approve or reject it in the Add node pairing flow."
  />`;
};
      ...
      ${pending.length
        ? html`
            ...
            >
              ...
              waiting for approval — approve them in the Add node pairing flow.
            </div>
          `
        : null}
      ${loading
            ...
            : html`
                <div class="space-y-2">
                  ${nodes.map((node) => {
                    ...
                    const checkingBrowser = checkingBrowserNodeIds.has(nodeId);
                    ...
                    const showResolvingSpinner =
                      ...
                      !checkingBrowser;
                    // The Check button stays mounted while its own check runs
                    // (loading state on the clicked control) instead of
                    // vanishing behind a bare spinner.
