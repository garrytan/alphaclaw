...
export const useConnectedNodesCard = ({
  ...
  const [checkingBrowserNodeIds, setCheckingBrowserNodeIds] = useState(
    () => new Set(),
  );
  ...
  // The round-robin browser probe spawns an openclaw CLI per tick on the server;
  // the shared primitive pauses it while the tab is hidden (fix wave F202).
  const browserPollRef = useRef(null);
  useVisibleInterval(() => browserPollRef.current?.(), kBrowserPollIntervalMs, { enabled: true });
  // Per-node single-flight: a busy node ignores re-clicks, but checks on
  // OTHER nodes proceed concurrently instead of being silently dropped.
  const browserCheckInFlightNodeIdsRef = useRef(new Set());
  ...
  const handleCheckNodeBrowser = useCallback(
    async (nodeId, { silent = false } = {}) => {
      ...
      if (
        !normalizedNodeId ||
        browserCheckInFlightNodeIdsRef.current.has(normalizedNodeId)
      ) {
        return;
      }
      browserCheckInFlightNodeIdsRef.current.add(normalizedNodeId);
      if (!silent) {
        setCheckingBrowserNodeIds((prev) => {
      ...
      try {
        ...
        browserCheckInFlightNodeIdsRef.current.delete(normalizedNodeId);
        if (!silent) {
          setCheckingBrowserNodeIds((prev) => {
            const next = new Set(prev);
  ...
  useEffect(() => {
    const pendingInitialNodeId = nodes
      ...
      .find((entry) => {
        ...
        if (browserCheckInFlightNodeIdsRef.current.has(entry.nodeId)) return false;
        return true;
      })?.nodeId;
    ...
    checkingBrowserNodeIds,
    handleCheckNodeBrowser,
    nodes,
  ]);

  useEffect(() => {
    if (checkingBrowserNodeIds.size) return;
    ...
    if (!pollableNodeIds.length) {
      browserPollRef.current = null;
      return undefined;
    }
    browserPollRef.current = async () => {
      // Skip the tick when anything is in flight — polls must not pile up;
      // the next interval retries. User-initiated checks are never blocked.
      if (browserCheckInFlightNodeIdsRef.current.size) return;
