...
export const useSetupWizard = ({
  ...
  const connectCommand = useMemo(() => {
    ...
    // Team (trusted-proxy) mode: the gateway refuses token auth, so a
    // connect command without a token cannot work — surface the server's
    // reason instead of emitting a command that silently fails.
    if (!String(connectInfo.gatewayToken || "").trim() && connectInfo.tokenUnavailableReason) {
      return `# ${connectInfo.tokenUnavailableReason}`;
    }
  ...
  // Discovery poll while the wizard sits on the node-list step; pauses when
  // the tab is hidden (shared primitive, fix wave PR 11).
  useVisibleInterval(
    () => {
      refreshNodeList().catch(() => {});
    },
    kNodeDiscoveryPollIntervalMs,
    { enabled: Boolean(visible) && step === 1, immediate: true },
  );
  ...
  // Approve/Reject rethrow so callers (DeviceRow) can clear their busy state
  // and surface the failure. The post-action refresh sits outside the
  // try/catch and swallows its own errors: the mutation already succeeded, so
  // a refresh failure must not read as an approve/reject failure — the 3s
  // poll loop converges the list regardless.
