...
export const useWebhookDetail = ({
  ...
  const refreshDetail = useCallback(() => {
    // Fire-and-forget: each hook already records failures in its own error
    // state; an uncaught rejection here would only surface as noise.
    detailFetchState.refresh({ force: true }).catch(() => {});
    agentsFetchState.refresh({ force: true }).catch(() => {});
  }, [agentsFetchState.refresh, detailFetchState.refresh]);

  // Set once the user picks a destination manually; server-sync must not
  // clobber that unsaved choice until the stored destination actually changes
  // (destinationResetKey) or the save succeeds.
  const userSelectedDestinationRef = useRef(false);
  const hasLoadedWebhook = !!selectedWebhook;

  useEffect(() => {
    userSelectedDestinationRef.current = false;
  }, [destinationResetKey]);

  useEffect(() => {
    if (!selectedHookName || selectedWebhookManaged || !selectedWebhook) return;
    if (userSelectedDestinationRef.current) return;
    ...
    // Keyed to destinationResetKey (value-based), NOT the selectedWebhook
    // object: a background refetch returns a new object with the same
    // destination and must not reset the select.
  }, [
    selectableSessions,
    selectedHookName,
    destinationResetKey,
  ...
  const handleSelectDestinationSession = useCallback(
    (key) => {
      userSelectedDestinationRef.current = true;
  ...
  const handleDeleteConfirmed = useCallback(async () => {
    ...
    try {
      ...
      // Drop the deleted webhook's cached detail instead of refetching it (a
      // refetch would 404); re-opening the name later must start cold.
  ...
  return {
    ...
    actions: {
      refreshDetail,
      setAuthMode,
      setDestinationSessionKey: handleSelectDestinationSession,
