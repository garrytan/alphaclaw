...
export const useGeneralTab = ({
  ...
  // The launcher 302 resolves the token server-side (tokenless when that is
  // the success path, e.g. trusted-proxy), so opening is synchronous — no
  // pre-fetch, no popup-blocker race, no false "token missing" warning.
