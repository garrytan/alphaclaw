...
import { AgentAdminPanel } from "../agent-admin-panel.js";
import { GeneralDoctorWarning } from "../doctor/general-warning.js";
import { InlineErrorChip } from "../inline-error-chip.js";
import { useCachedFetch } from "../../hooks/use-cached-fetch.js";
import { fetchOpenclawCapabilities } from "../../lib/api.js";
import { ChevronDownIcon, WhatsAppIcon } from "../icons.js";
import { UpdateActionButton } from "../update-action-button.js";
import { GeneralHardeningBadge } from "./hardening-badge.js";
import {
  GeneralHardeningCard,
  buildHardeningCardModel,
} from "./hardening-card.js";
import { useGeneralTab } from "./use-general-tab.js";
import { WhatsNextCard } from "./whats-next-card.js";
...
export const GeneralTab = ({
  ...
  // D14: the "What's next" Team row is gated on the installed OpenClaw
  // actually supporting trusted-proxy team access.
  const { data: capsPayload } = useCachedFetch(
    "/api/openclaw/capabilities",
    fetchOpenclawCapabilities,
    { maxAgeMs: 5 * 60_000, enabled: isActive },
  );
  ...
  const showPairings =
    ...
    state.pairingStatusRefreshing;
  // Computed once per render: gates the GeneralDoctorWarning suppression AND
  // feeds the card, so the model builder never runs twice per SSE tick.
  const hardeningCardModel = buildHardeningCardModel(state.doctorStatus);
  ...
  return html`
    <div class="space-y-4">
      ...
      />
      <${GeneralHardeningBadge} doctorStatus=${state.doctorStatus} />
      <${GeneralHardeningCard}
        doctorStatus=${state.doctorStatus}
        model=${hardeningCardModel}
        onOpenDoctor=${() => onSwitchTab("doctor?focus=context")}
      />
      ${/* The hardening card subsumes the "go run the doctor" prompt —
            never two stacked alert cards with the same CTA label. */
      hardeningCardModel
        ? null
        : html`<${GeneralDoctorWarning}
            doctorStatus=${state.doctorStatus}
      ...
      <${WhatsNextCard}
        ...
        teamSupported=${capsPayload?.capabilities?.trustedProxyTeam === true}
        teamEnabled=${statusData?.alphaclaw?.team?.enabled === true}
      />
            ...
            ? html`
                <div class="bg-surface border border-border rounded-xl p-4">
                  ...
                  <div class="text-center py-4 space-y-3">
                    <${WhatsAppIcon} className="w-10 h-10 mx-auto" />
      ...
      <${ApiFeaturePanel}
        ...
        openAiCompatApiError=${state.openAiCompatApiError}
        onToggleOpenAiCompatApi=${actions.handleOpenAiCompatApiToggle}
      />
      <${AgentAdminPanel}
        agentAdmin=${statusData?.agentAdmin || null}
        isActive=${isActive}
        onToggle=${() => onRefreshStatuses()}
        onRotated=${() => onRefreshStatuses()}
      />
        ...
        <div class="bg-surface border border-border rounded-xl p-4">
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2 shrink-0">
              <span class="text-xs text-fg-muted">
                ${state.savingSyncCron ? "Saving..." : "Auto-sync"}
              </span>
              <div class="relative">
                <select
          ...
          ${state.syncCronError
            ? html`
                <div class="mt-3">
                  <${InlineErrorChip}
                    error=${state.syncCronError}
                    headline="Couldn't save the sync schedule — reverted to the previous setting."
                  />
                </div>
              `
            : null}
