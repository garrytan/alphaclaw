...
import {
  kWhatsNextHiddenForSessionKey,
  kWhatsNextUpdateChannelReviewedKey,
...
// every row is done.
const kHideKey = kWhatsNextHiddenForSessionKey;
// "Review" has no config signal — visiting the Upgrade page from this row is
// the completion event, remembered per browser.
const kUpdateChannelReviewedKey = kWhatsNextUpdateChannelReviewedKey;
...
export const WhatsNextCard = ({
  ...
  const [updateChannelReviewed, setUpdateChannelReviewed] = useState(() =>
    readFlag(kUpdateChannelReviewedKey),
  );

  const hasChannel =
    channels && typeof channels === "object" && Object.keys(channels).length > 0;
  // D14 done-state: the Google row hides once any account is authenticated.
  const { data: googleAccountsPayload } = useCachedFetch(
    "/api/google/accounts",
    fetchGoogleAccounts,
    { maxAgeMs: 60_000 },
  );
  const googleConnected = (
    Array.isArray(googleAccountsPayload?.accounts)
      ? googleAccountsPayload.accounts
      : Array.isArray(googleAccountsPayload)
        ? googleAccountsPayload
        : []
  ).some((account) => account?.authenticated);
  ...
  const rows = [
    ...
    !updateChannelReviewed && {
      ...
      action: {
        ...
        onClick: () => {
          try {
            localStorage.setItem(kUpdateChannelReviewedKey, "1");
          } catch {}
          setUpdateChannelReviewed(true);
          onSwitchTab("upgrade");
        },
      },
    },
