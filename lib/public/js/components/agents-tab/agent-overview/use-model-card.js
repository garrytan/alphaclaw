  ...
  useEffect(() => {
    ...
    fetchThinkingOptions(modelKey)
      .then((payload) => {
        if (cancelled) return;
        if (!payload?.ok) {
          // HTTP errors resolve as {ok:false} envelopes — a silent return
          // here would leave the PREVIOUS model's levels selectable with no
          // error shown. Route it into the same retryable error state.
          setThinkingOptions(kDefaultThinkingOptions);
          setThinkingOptionsError(
            new Error(String(payload?.error || "Failed to load thinking options")),
          );
          return;
        }
        setThinkingOptions({
