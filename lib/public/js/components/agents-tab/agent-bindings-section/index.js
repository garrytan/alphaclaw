        ...
        : loadError && channels.length === 0
          ? html`
              <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
                <h2 class="card-label">Channels</h2>
                <${InlineErrorChip}
                  error=${loadError}
                  headline="Couldn't load channels."
                  onRetry=${retryLoad}
                />
              </div>
            `
            ...
            <div class="space-y-3">
              ${loadError
                ? html`
                    <${InlineErrorChip}
                      error=${loadError}
                      headline="Couldn't refresh channels — showing the last loaded state."
                      onRetry=${retryLoad}
                    />
                  `
                : null}
