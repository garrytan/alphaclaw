...
import { buildBindKey } from "./use-agent-bindings.js";
...
export const ChannelItemTrailing = ({
  ...
  if (isOwned) {
    ...
    // Per-row pending: the clicked Bind shows "Binding..." while every Bind
    // control is disabled, so one in-flight bind can't race a sibling's.
    const bindKey = buildBindKey(channel, accountId);
    statusTrailing = html`
      <button
        type="button"
      ...
      >
        ${bindPendingKey === bindKey ? "Binding..." : "Bind"}
      </button>
    `;
  } else {
    statusTrailing = html`
