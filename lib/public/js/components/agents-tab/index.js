...
export const AgentsTab = ({
  ...
  const { create, loadAgents, loadError, remove, setDefault, update } =
    agentsActions;
  ...
  const handleUpdateAgent = async (
    ...
    try {
      ...
      // Callers that render their own inline error surface pass
      // toastOnError:false — errors must not be toast-only, and doubling the
      // surfaces is noise. The rethrow still reaches them either way.
      if (toastOnError) {
  ...
  if (loadError && agents.length === 0) {
    // A failed agents fetch must not masquerade as "no agent selected".
    return html`
      <div class="agents-detail-panel">
        <div class="w-full max-w-md mx-auto p-4">
          <${InlineErrorChip}
            error=${loadError}
            headline="Couldn't load agents."
            onRetry=${loadAgents}
          />
        </div>
      </div>
