...
import { kClaudeCodeStatusCacheKey, kEnvCacheKey } from "../lib/cache-keys.js";
...
import { kEnvarsSecretsBannerDismissedKey } from "../lib/storage-keys.js";
...
// OpenClaw 2026.8+ ships a shared, team-scoped secret store managed in the Control UI
// (Settings -> Secrets). Capability-gated banner (3.4): only shown when the installed
// OpenClaw supports it; dismissible (localStorage) since it is purely informational.
const kSecretsBannerDismissKey = kEnvarsSecretsBannerDismissedKey;

const SecretsStoreBanner = () => {
  const [dismissed, setDismissed] = useState(() => {
    try {
      return localStorage.getItem(kSecretsBannerDismissKey) === "1";
    } catch {
      return false;
    }
  });
  ...
  const supported = capsPayload?.capabilities?.secretsStore === true;
  if (dismissed || !supported) return null;
  // The launcher 302 primes the auth fragment server-side; a synchronous
  // window.open keeps Safari's popup blocker out of the way.
  const openSecrets = () => {
    window.open(`${kDashboardLaunchUrl}?to=secrets`, "_blank", "noopener");
  };
  return html`
    <div class="bg-surface border border-border rounded-xl p-3 mb-3 flex flex-wrap items-center justify-between gap-3">
      <p class="text-xs text-fg-muted min-w-0">
        <span class="text-body font-medium">OpenClaw now has a shared secret store.</span>
        ${" "}Keep API keys and tokens in the OpenClaw dashboard (Settings → Secrets)
        instead of plain environment variables — secrets there can be locked to exact
        destination hosts.
      </p>
      <div class="flex items-center gap-2 shrink-0">
        <button type="button" class="text-xs px-2 py-1 rounded-lg ac-btn-ghost" onclick=${openSecrets}>
          Open Secrets
        </button>
        <button
          type="button"
          ...
          onclick=${() => {
            try {
              localStorage.setItem(kSecretsBannerDismissKey, "1");
            } catch {}
            setDismissed(true);
          }}
        >
...
export const Envars = ({ onRestartRequired = () => {} }) => {
  // Version-gated (2026.8.1-beta.1+): informational only, no config UI in v1.
  ...
  // Cancel/reload failures surface here (adjacent to the Save/Cancel actions)
  // instead of vanishing into the console — cleared on the next attempt.
  const [loadActionError, setLoadActionError] = useState(null);
  ...
  } = useCachedFetch(kEnvCacheKey, fetchEnvVars, {
    maxAgeMs: 30000,
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Saved env vars may flip the Claude Code launcher's availability; drop
      // its cached status so the sidebar's next revalidation (focus/remount)
      // sees fresh state instead of a full page reload being required.
  ...
  const addVars = (pairs) => {
    ...
    setVars((prev) => {
      ...
      for (const { key: rawKey, value } of pairs) {
        ...
        const existingIndex = next.findIndex((v) => v.key === key);
        if (existingIndex >= 0) {
          // Copy, never mutate (fix wave F164): `prev` items are the cached
          // /api/env objects, so an in-place write made an UNSAVED value read
          // as saved after a remount and get silently reverted.
  ...
  // A failed page load with nothing to show must never render as a confident
  // empty page — surface the error with a Retry instead.
  if (envPayloadError && !vars.length) {
    return html`
      <${PaneShell}
        header=${html`<${PageHeader} title="Envars" />`}
      >
        <div class="bg-surface border border-border rounded-xl p-4">
          <${InlineErrorChip}
            error=${envPayloadError}
            headline="Couldn't load environment variables."
            onRetry=${load}
          />
        </div>
      </${PaneShell}>
    ...
    >
      ${loadActionError
        ? html`<${InlineErrorChip}
            error=${loadActionError}
            headline="Couldn't reload environment variables — still showing your current edits."
            onRetry=${load}
          />`
        : null}
      <${SecretsStoreBanner} />
