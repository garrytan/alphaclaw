    ...
    >
      ${loadActionError
        ? html`<${InlineErrorChip}
          ...
          />`
        : null}
      ${openclawFeatures?.secretEgressBinding === true
        ? html`<div
            class="bg-surface border border-border rounded-xl px-4 py-2.5 text-xs text-fg-muted"
          >
            This OpenClaw version binds secrets to exact HTTPS hosts — a key can
            only be sent to its provider's API endpoint.${" "}
            <a
              class="ac-tip-link"
