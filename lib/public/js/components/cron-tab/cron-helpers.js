...
// Cron schedule times are wall-times, NEVER tz-converted: the UTC-pinned
// formatter renders a synthetic UTC instant built from the raw cron fields.
// Guarded like format.js's own singletons: a module-scope construction throw
// (ICU-restricted browser) must degrade schedule labels, never abort the app
// via the cron tab's static import chain.
const kUtcTimeFormatters = (() => {
  try {
    return createFormatters("UTC");
  } catch {
    return null;
  }
})();
...
const formatHourMinute = ({ hourField, minuteField }) => {
  ...
  // Out-of-range cron fields render nothing rather than rolling over into a
  // plausible-looking wrong time (Date.UTC would turn minute 99 into +1h39m).
  if (!Number.isFinite(hour) || !Number.isFinite(minute)) return "";
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) return "";
  const normalizedHour = hour;
  if (!kUtcTimeFormatters) {
    // Degraded (no Intl): plain 24h wall time beats an app-killing throw.
    return `${String(normalizedHour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`;
  }
  return kUtcTimeFormatters.time.format(
    new Date(Date.UTC(2000, 0, 1, normalizedHour, minute)),
  );
};
...
const formatHourOnly = (hourField) => {
  ...
  return kUtcTimeFormatters.hour.format(
    new Date(Date.UTC(2000, 0, 1, normalizedHour)),
  );
};
...
export const formatRelativeMs = (targetMs, nowMs = Date.now()) => {
  ...
  return formatRelativeTime(value, { nowMs, allowFuture: true });
};

export const formatRelativeCompact = (targetMs, nowMs = Date.now()) => {
  const value = Number(targetMs || 0);
  if (!Number.isFinite(value) || value <= 0) return "—";
  return formatRelativeTime(value, { nowMs, style: "unit" });
};
...
export const formatNextRunRelativeMs = (nextRunAtMs, nowMs = Date.now()) => {
  ...
  const overdueMinutes = Math.floor(overdueDeltaMs / (60 * 1000));
