...
import { formatChartBucketLabel, formatDurationCompactMs, getBrowserTimeZone } from "../../lib/format.js";
...
export const CronJobTrendsPanel = ({
  ...
  return html`
    <section class="bg-surface border border-border rounded-xl p-4 space-y-3">
      ...
      ${trends && trends.timeZone === null && getBrowserTimeZone()
        ? html`<div class="text-xs text-fg-muted">
            Day buckets use the server's timezone (yours wasn't recognized by
            the server).
          </div>`
        : null}
    </section>
