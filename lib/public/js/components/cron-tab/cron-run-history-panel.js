...
// Seconds are shown on run rows because sub-minute jobs exist: multiple runs
// in the same minute must not render identically.
const formatRunTimestamp = (timestampMs) =>
        ...
        ${isDetail
          ? html`
              <span class="truncate text-xs text-body">
                ${formatRunTimestamp(runEntry.ts)}
              </span>
            `
          : hasRunTitle
            ? html`
...
const renderCollapsedGroupRow = ({ row, rowIndex, onSelectJob = () => {} }) => {
  ...
  const timeRangeLabel = formatLocaleDateTimeRange(row.oldestTs, row.newestTs, {
    valueIsEpochMs: true,
  });
  return html`
    <details
