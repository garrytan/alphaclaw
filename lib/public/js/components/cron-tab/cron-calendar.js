...
const formatHourLabel = (hourOfDay) => {
  ...
  return formatLocaleTime(dateValue);
};
...
const formatUpcomingTime = (timestampMs) =>
  formatLocaleTime(timestampMs, { valueIsEpochMs: true });
...
const buildJobTooltipText = ({
  ...
  if (isPastSlot) {
    ...
    if (slotRun?.ts) {
      lines.push(
        `Run time: ${formatLocaleDateTime(slotRun.ts, { valueIsEpochMs: true })}`,
      );
    }
  } else {
    lines.push(
  ...
  if (!isPastSlot && latestRun?.status) {
    lines.push(
      `Latest run: ${latestRun.status} (${formatLocaleDateTime(latestRun.ts, { valueIsEpochMs: true })})`,
    );
  } else if (!isPastSlot) {
  ...
  if (Number(job?.state?.runningAtMs || 0) > 0) {
    lines.push(
      `Current run: active (${formatLocaleDateTime(job.state.runningAtMs, { valueIsEpochMs: true })})`,
    );
  }

  if (scheduledAtMs > 0) {
    const slotLabel = formatLocaleDateTime(scheduledAtMs, {
      valueIsEpochMs: true,
    });
    const slotState =
...
export const CronCalendar = ({
  ...
  const nowMs = useNowMs(kNowRefreshMs);
  ...
  const renderCompactStrip = () => {
    return html`
      <div class="space-y-2">
        ...
        ${
          timeline.scheduleMismatchJobIds?.length
            ? html`<p class="text-xs text-status-warning mt-2">
                Schedule preview may be inaccurate for
                ${" "}${timeline.scheduleMismatchJobIds.length === 1 ? "1 job" : `${timeline.scheduleMismatchJobIds.length} jobs`}
                ${" "}— the scheduler's next run differs from the calendar's estimate. Trust "Next run" on the job.
              </p>`
            : null
        }
      </div>
