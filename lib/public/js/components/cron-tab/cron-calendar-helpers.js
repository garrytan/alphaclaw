...
// Day-of-week (SUN=0..SAT=6) and month (JAN=1..DEC=12) name aliases (MW4):
// without these, `MON-FRI` / `JAN` parsed to nothing → an empty set that
// cronFieldMatches treated as "match all", plotting every day.
const kDayOfWeekAliases = {
  SUN: 0,
  MON: 1,
  TUE: 2,
  WED: 3,
...
const applyNameAliases = (token, aliases) => {
  if (!aliases) return token;
  return token.replace(/[A-Za-z]+/g, (name) => {
    const mapped = aliases[name.toUpperCase()];
    return mapped === undefined ? name : String(mapped);
  });
};

const buildStepValues = (startValue, endValue, step, minValue, maxValue) => {
  ...
  const values = [];
  for (let value = safeStart; value <= safeEnd; value += step) values.push(value);
  return values;
};
...
const parseToken = (token = "", minValue = 0, maxValue = 0) => {
  ...
  // Range with step, e.g. 0-30/5 (MW4): without this the whole token fell
  // through to [] → empty set → match-all → a 10k-slot render blowup.
  const stepRangeMatch = token.match(/^(\d+)-(\d+)\/(\d+)$/);
  if (stepRangeMatch) {
    return buildStepValues(
      Number.parseInt(stepRangeMatch[1], 10),
      Number.parseInt(stepRangeMatch[2], 10),
      Number.parseInt(stepRangeMatch[3], 10),
      minValue,
      maxValue,
    );
  }
  // Start-with-step, e.g. 9/2 → 9,11,13,... to max.
  const startStepMatch = token.match(/^(\d+)\/(\d+)$/);
  if (startStepMatch) {
    return buildStepValues(
      Number.parseInt(startStepMatch[1], 10),
      maxValue,
      Number.parseInt(startStepMatch[2], 10),
      minValue,
      maxValue,
    );
  }
...
const parseCronFieldSet = (field = "", minValue = 0, maxValue = 0, aliases = null) => {
  const raw = String(field || "").trim();
  if (!raw) return new Set();
  const tokens = raw.split(",").map((segment) => segment.trim()).filter(Boolean);
  const values = tokens.flatMap((token) =>
    parseToken(applyNameAliases(token, aliases), minValue, maxValue),
  );
...
const buildCronMatcher = (cronFields = null) => {
  ...
  return {
    ...
    monthSet: parseCronFieldSet(cronFields.monthField, 1, 12, kMonthAliases),
    dayOfWeekSet: parseCronFieldSet(
...
// Wall-clock fields of an instant in the job's zone (fix wave F167): the
// job's `schedule.tz` decides what "09:00" means, not the browser's zone.
// The Intl work lives in lib/format.js (the one home for date formatting).
export const readZonedCronFields = (dateValue, tz = "") => {
  const parts = readZonedDateParts(dateValue, tz);
  return {
    minute: parts.minute,
    hour: parts.hour,
...
const cronFieldMatches = (dateValue, cronMatcher = null, tz = "") => {
  if (!cronMatcher) return false;
  const { minuteSet, hourSet, dayOfMonthSet, monthSet, dayOfWeekSet } = cronMatcher;

  const { minute, hour, dayOfMonth, month, dayOfWeek } = readZonedCronFields(dateValue, tz);
  const dayOfWeekAliases = dayOfWeek === 0 ? [0, 7] : [dayOfWeek];

  // An empty set means the field was unparseable (a valid field — `*`, `N`,
  // ranges, steps, name aliases — always yields ≥1 value). Treat it as
  // match-NOTHING, not match-all (MW4): the old match-all rendered a
  // 10k-slot blowup / every-day plot for `0-30/5` and `MON-FRI`.
  const minuteMatches = minuteSet.has(minute);
  const hourMatches = hourSet.has(hour);
  const dayOfMonthMatches = dayOfMonthSet.has(dayOfMonth);
  const monthMatches = monthSet.has(month);
  const dayOfWeekMatches = dayOfWeekAliases.some((candidate) =>
    dayOfWeekSet.has(candidate),
  );
...
export const expandJobsToRollingSlots = ({
  ...
  const slots = [];
  // Day headers step by CALENDAR day, not 24h (fix wave F168): across a DST
  // fall-back the 24h step landed twice on the same local date (duplicate
  // header) and the window's last day lost its column.
  const firstDay = new Date(startOfDayMs(range.rangeStartMs));
  const days = Array.from({ length: range.dayCount }, (_, offset) => {
    ...
    return {
      dayStartMs,
      dayKey: toLocalDayKey(dayStartMs),
      label: formatChartBucketLabel(dayStartMs, { range: "7d" }),
    };
  });
  // Cross-check (F167): the store's `state.nextRunAtMs` is the authoritative
  // scheduler answer; if our first computed future occurrence disagrees by
  // more than a minute the preview may be wrong (unsupported syntax, zone
  // drift) and the calendar says so instead of silently drifting.
  const scheduleMismatchJobIds = [];
  ...
  jobs.forEach((job) => {
    ...
    const tz = String(job?.schedule?.tz || job?.schedule?.timezone || "").trim();
    let firstFutureMs = 0;
    for (
      ...
      if (!cronFieldMatches(dateValue, cronMatcher, tz)) continue;
      if (!firstFutureMs && tickMs > nowMs) firstFutureMs = tickMs;
      ...
      slots.push({
        ...
        hourBucketMs,
        dayKey: toLocalDayKey(tickMs),
        // Grid rows are the BROWSER's hour of day (the axis the operator reads).
    ...
    if (
      ...
      nextRunAtMs > nowMs &&
      nextRunAtMs <= range.rangeEndMs &&
      Math.abs(nextRunAtMs - firstFutureMs) > kMinuteMs
    ) {
      scheduleMismatchJobIds.push(String(job?.id || ""));
    }
  });
