...
// The unavailable status arrives as { state, reason, hint } (server:
// lib/server/routes/system.js buildAgentAdminStatus). Prefer the server's
// hint; fall back per reason — never claim a specific cause we can't see.
const kUnavailableDetailByReason = {
  token_missing: "Flag is on but no token file exists — check server logs.",
  error: "Status probe failed — check server logs.",
};

export const getTokenUnavailableDetail = (agentAdmin) =>
  agentAdmin?.hint ||
  kUnavailableDetailByReason[agentAdmin?.reason] ||
  kUnavailableDetailByReason.error;
      ...
      ${enabled
        ? html`
            <div class="mt-3 text-xs text-fg-muted">
              ${state === "unavailable"
                ? html`<span class="text-status-error">⚠ Token unavailable — ${getTokenUnavailableDetail(agentAdmin)}</span>`
