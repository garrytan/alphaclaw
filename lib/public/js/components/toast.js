...
const kDefaultToastDurationMs = 4000;

export function showToast(text, type = "info", { durationMs = kDefaultToastDurationMs } = {}) {
  if (addToastFn) {
    addToastFn({ id: ++toastId, text, type: normalizeToastType(type), durationMs });
  }
}
  ...
  useEffect(() => {
    addToastFn = (t) => {
      ...
      setTimeout(
        ...
        Number(t.durationMs) > 0 ? Number(t.durationMs) : kDefaultToastDurationMs,
      );
    };
