...
const formatLastSeen = (lastSeenAt, nowMs = Date.now()) => {
  const deltaSec = Math.max(0, Math.round((nowMs - lastSeenAt) / 1000));
  if (deltaSec < 15) return "now";
...
const InvitesCard = ({
  ...
  return html`
    <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
      ...
      <p class="text-xs text-fg-muted">
        An invite creates AlphaClaw access. Approving a device (below in
        OpenClaw) authorizes a browser or client inside OpenClaw — two
        different things.
      </p>
      <div class="space-y-2">
        <input
