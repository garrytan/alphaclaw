...
export const PresenceCard = ({ presence = [], presenceUnavailable = null }) => html`
  <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
      ...
      ? html`
          <div class="space-y-1">
            <p class="text-sm text-status-warning-muted">
              Presence refresh failed — showing last known presence. Retrying
              automatically.
            </p>
            <p class="text-xs text-fg-muted">${presenceUnavailable}</p>
          </div>
        `
      : null}
    ${presence.length === 0
