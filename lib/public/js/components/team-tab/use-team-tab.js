...
const warnIfRosterDeferred = (result) => {
  if (result?.gatewayConfigDeferred !== true && result?.restartDeferred !== true) return;
  showToast(result.hint ||
    "Member settings saved. Resolve the gateway blocker, then save member settings again to sync gateway authentication.",
    "warning", { durationMs: 10_000 });
};
...
export const useTeamTab = ({ onRefreshStatuses = () => {} } = {}) => {
  ...
  // 10s presence poll while the tab is mounted and team is enabled; paused
  // while the tab is hidden (fix wave F170).
  useVisibleInterval(
    () =>
      fetchTeamPresence()
        .then((data) => {
  ...
  // Error and empty are different states (F170): a failed poll used to be
  // swallowed, so the queue read as "nothing pending".
  ...
  const refreshDevices = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) setDevicesError(error?.message || "Could not load device requests");
    }
  }, [isAdmin, team?.enabled]);

  // Each tick spawns an openclaw CLI on the server — never while hidden.
  useVisibleInterval(() => refreshDevices(), kPresencePollMs, {
    enabled: Boolean(isAdmin && team?.enabled),
    immediate: true,
  });
  ...
  const onEnable = useCallback(
    async (payload) => {
      ...
      try {
        ...
        // A refused restart can follow a successful auth/config write. Read
        // the persisted mode even when the overall transition failed.
        await load();
        onRefreshStatuses();
        return null;
      } finally {
  ...
  const onUpdateMember = useCallback(
    async (memberId, payload) => {
      ...
      try {
        const result = await updateTeamMember(memberId, payload);
        await load();
        onRefreshStatuses();
        warnIfRosterDeferred(result);
        return true;
      } catch (err) {
  ...
  const onDisableTeam = useCallback(async () => {
    try {
      ...
      const savedDisabled = (err?.disabled === true || err?.enabled === false) &&
        (err?.configSaved === true || err?.configRestored === true);
      if (savedDisabled) setTeam((current) => current ? { ...current, enabled: false } : current);
      await load();
      onRefreshStatuses();
      if (savedDisabled) {
        showToast(`Team access is disabled. Gateway restoration still needs verification. ${err.hint || err.message || "Check the Watchdog page."}`,
          "warning", { durationMs: 10_000 });
        return true;
      }
