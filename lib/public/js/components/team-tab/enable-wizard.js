      ...
      return html`
        <div class="space-y-3">
          ...
          <ul class="text-sm text-fg-muted space-y-1 list-disc pl-5">
            ...
            <li>Needs OpenClaw 2026.8+ and one gateway restart to activate.</li>
          </ul>
        </div>
      `;
    }
    ...
    return html`
      <div class="space-y-3">
        ...
        <ul class="text-sm text-fg-muted space-y-1 list-disc pl-5">
          ...
          <li>The gateway needs one restart to pick this up.</li>
        </ul>
        <button
          type="button"
...
gateway.auth.trustedProxy.deviceAutoApprove = { enabled: true, scopes: [read, write, approvals] }
gateway.auth.trustedProxy.allowUsers / identityScopes = built from the member roster</pre
            >`
          : null}
        ${enableError
