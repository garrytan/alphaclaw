...
const getToneClass = (tone, isInteractive, loading = false) => {
  // The inline-Tailwind tones need an explicit disabled treatment — the
  // ac-btn-* tones get it from their :disabled CSS, but without this a
  // disabled subtle/neutral/warning button differs from idle only by losing
  // hover styles, which reads as "clickable but broken". Loading buttons keep
  // their own affordance (pulse/opacity from loadingClass), so the disabled
  // add-on applies only to the plain-disabled case.
  const disabledExtra = !isInteractive && !loading ? " opacity-50 cursor-not-allowed" : "";
  if (tone === "subtle") {
    return isInteractive
      ? "border border-border text-fg-muted hover:text-body hover:border-fg-muted"
      : `border border-border text-fg-muted${disabledExtra}`;
  }
  if (tone === "neutral") {
    return isInteractive
      ? "border border-border text-fg-muted hover:text-body hover:border-fg-muted"
  ...
  if (tone === "warning") {
    return isInteractive
      ...
      : `border border-yellow-500/35 text-status-warning-muted bg-yellow-500/10${disabledExtra}`;
  }
...
export const ActionButton = ({
  ...
  const toneClass = getToneClass(tone, isInteractive, loading);
