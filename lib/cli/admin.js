...
const kContextHeader = "X-AlphaClaw-Request-Context";

const resolveTokenPath = (rootDir) =>
  path.join(rootDir, ".openclaw", ".alphaclaw", "agent-admin-token");

const readToken = (rootDir) => {
  try {
    const token = String(
      fs.readFileSync(resolveTokenPath(rootDir), "utf8"),
    ).trim();
    return token || null;
  } catch {
    return null;
...
const request = ({ method, urlPath, body, headers, timeoutMs }) =>
  new Promise((resolve) => {
    const req = http.request(
      { host: "127.0.0.1", port: headers.__port, method, path: urlPath, headers },
      (res) => {
        const chunks = [];
...
const runAdminCommand = async ({ argv, rootDir }) => {
  ...
  const port = String(process.env.PORT || flagValue(argv, "--port") || "3000").trim();
  ...
  if (sub === "manifest") {
    ...
    if (token) {
      ...
      const result = await request({
        method: "GET",
        urlPath,
        headers: { Authorization: `Bearer ${token}`, __port: port },
  ...
  if (!["GET", "POST", "PUT", "DELETE", "PATCH"].includes(method)) {
    return fail({
      ...
      code: "usage",
      message:
        "Usage: alphaclaw admin <GET|POST|PUT|DELETE> /api/<path> [--data '<json>' | --data-stdin] [--confirm CODE] [--context STR] [--summary] [--json]",
  ...
  if (context) headers[kContextHeader] = String(context).slice(0, 256);
