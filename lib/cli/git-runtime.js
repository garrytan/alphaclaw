...
// Returns the operator-chosen askpass destination, or "" when unset. There is
// deliberately no shared-tmp default anymore: a predictable name in a shared
// tmp dir is the H14 symlink-hijack class (git executes whatever lands there),
// so callers without an explicit override must use writeGitAskpassScript()
// (private mkdtemp 0700) instead.
const resolveGitAskPassPath = ({ env = process.env } = {}) =>
  String(env.ALPHACLAW_GIT_ASKPASS_PATH || "").trim();
