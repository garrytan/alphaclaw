...
const { shellCmd, execFileCmd, clawCmd, clawCmdWithRetry, gogCmd } =
  createCommands({
    gatewayEnv,
  });
...
const watchdogIncidentTracker = createWatchdogIncidentTracker({
  ...
  onIncidentActivity: (context) => {
    try {
      claudeCodeLocalService?.ensureForIncident?.(context);
    } catch {}
  },
  getResourceSample: () => {
    try {
...
const {
  ...
  gatewayMedic,
  runBackupSqlite,
  // #76: listening-path closers + boot-report server phase, run as the first
  // steps of the onboarded boot sequence (startup.js fixes the order).
  bootSteps: bootReportSteps,
  proxy,
  getGatewayUrl,
  SETUP_API_PREFIXES,
  webhookMiddleware,
