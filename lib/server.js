...
const watchdogIncidentTracker = createWatchdogIncidentTracker({
  ...
  getResourceSample: () => {
    try {
      ...
      return {
        ...getSystemResources({
          gatewayPid: watchdogStatus?.servingPid ?? watchdogStatus?.gatewayPid ?? null,
        }),
        // Memory-leak trend at close time (cached snapshot, full numerics —
        // persisted record, not the SSE path). Includes the frozen
        // lastEpisodeSummary so a post-crash close still carries the episode
        // that killed the predecessor; the tracker applies the incident-
        // window correlation before persisting it.
        gatewayMemoryTrend: watchdogRef?.getMemoryTrend?.() ?? null,
      };
    } catch {
      return null;
    }
