...
const runStreamedDoctorFix = async ({ timeoutMs = 10 * 60 * 1000 } = {}) => {
  try {
    fs.copyFileSync(
      path.join(constants.OPENCLAW_DIR, "openclaw.json"),
      path.join(constants.OPENCLAW_DIR, "openclaw.json.pre-doctor.bak"),
    );
  } catch {}
  const guarded = await serverDoctorGuard.withDoctorRestoreGuard({
    run: () =>
      repairRunStream.runStreamed({
        command: "openclaw",
        args: ["doctor", "--fix", "--yes"],
        env: gatewayEnv(),
        timeoutMs,
  ...
  if (guarded.code === "doctor_restored_stale_config") {
    upgradeNotifier.notify(
      `⚠️ OpenClaw's doctor tried to replace your settings with a stale backup (${guarded.droppedKeyPaths.length} setting path(s) would have been lost). AlphaClaw blocked it — your settings are unchanged.`,
      { eventType: "health", id: `doctor-restore-blocked-${Date.now()}` },
    );
    return {
      ok: false,
      stdout: "",
