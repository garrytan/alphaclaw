...
const doctorService = createDoctorService({
  ...
  getInstalledVersion: () =>
    openclawChannelService.getChannelInfo().installedVersion || "",
  getReleaseChannel: () =>
  ...
  isOnboarded,
  // Upstream-doctor bridge: `doctor --lint --json` is the cross-version
  // machine invocation (bare --json is NOT lint mode on stable). Spawn-capped
  // capture; a cap-sized tail is treated as truncated → the bridge skips.
  ...
  // burn its 10-minute timeout for nothing.
  getGatewayReadiness: () => {
    const status = watchdog.getStatus();
    if (status.operationInProgress) {
      return { ok: false, reason: "an OpenClaw update operation is in progress" };
    }
    if (status.safeMode) {
      return { ok: false, reason: "gateway is in safe mode" };
    }
    if (["configuration_error", "crash_loop", "crashed", "stopped"].includes(status.lifecycle)) {
      return { ok: false, reason: `gateway lifecycle is ${status.lifecycle}` };
    }
