...
const { isVerboseEnabled } = require("./server/notification-policy");
const {
  gatewayEnv,
  getGatewayPort,
  getGatewayUrl,
...
const topicDiscovery = createTopicDiscoveryService({
  ...
  notify: (message, opts) => {
    ...
    // Raw-notifier wrapper (bypasses the outbox): apply the verbose policy
    // here, mirroring the central gate. Reads the flag off opts without
    // constructing a tag literal (conventions scan, notification-policy.js).
    if (opts?.verbose === true && !isVerboseEnabled()) {
      return { ok: false, skipped: true, reason: "verbose_notifications_disabled" };
    }
