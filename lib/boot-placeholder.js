...
const { buildStepListModel, formatElapsed, toEpochMs } = require("./update-progress-model.js");
...
// trap: on short viewports a step list taller than the screen must scroll,
// not push its top edge out of reach. .elapsed shares the page's secondary
// text color #9aa4b2 — #6b7684 on #0b0e14 fails WCAG AA at this size.
const kPageStyle = [
  ...
  "ul.steps .elapsed{color:#9aa4b2;font-size:.85rem;margin-left:1.25rem}",
  "p.note{color:#c9a227}",
].join("");
...
const renderPlaceholderPage = (bodyHtml) => [
  ...
  // viewport — illegible for the admin staring at this during the update.
...
const resolvePlaceholderView = (progress, nowMs) => {
  ...
  if (terminal) {
    const finishedAtMs = toEpochMs(run.finishedAt);
...
const renderProgressPage = (progress, nowMs) => {
  ...
  // The backup line binds to THIS run's own record (carries verified/noBackup)
  // — never state.backups[0], which is the newest backup from ANY prior run
  // and would show a stale backup as if it protects this migration when the
  // current run's backup was skipped or soft-failed.
  const backup = run.backup && typeof run.backup === "object" ? run.backup : null;
  if (backup && backup.verified === true) {
    const backupAtMs = toEpochMs(backup.at);
    if (backupAtMs != null) {
    ...
    try {
      ...
      if (pointer != null) {
        ...
        for (const name of names) {
          ...
          const startedAt = toEpochMs(read.json.startedAt) || 0;
          if (!run || startedAt > (toEpochMs(run.startedAt) || 0)) {
            run = read.json;
      ...
      // No state.backups here: the page renders the run's OWN backup record
      // (run.backup), so a stale entry from a prior run can never leak in.
      const value = {
