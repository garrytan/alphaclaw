...
const { kOpenclawManagedDir, OPENCLAW_DIR } = require("./constants");
...
const deriveTunings = (profile, { overrides = {} } = {}) => {
  ...
  // Heap overrides may exceed the formula ceiling but never the container:
  // an override larger than the box is an OOM instruction, not a preference.
  const gatewayHeapMb = applyOverride(
    "gatewayHeapMb",
    clamp(Math.round(0.5 * M), 256, 8192),
    { min: 128, max: Math.max(128, Math.round(0.85 * M)) },
  );
...
const getGatewayNodeOptionsSuffix = (options = {}) => {
  const heapMb = safeDerivedValue("gatewayHeapMb", options);
...
// ownership rules. Returns the ledger row describing the outcome.
const applyConcurrency = ({ active, derivation, ledger, deps }) => {
  ...
  // Disable / kill-switch / suppression revert: restore pre-feature semantics.
  // A key we adopted from absent is deleted; and any value ABOVE the legacy 64
  // ceiling is clamped back down — pre-feature, nothing could write >64 (the
  // telegram formula capped there), so an oversized value can only exist
  // because autotune raised the ceiling. Sub-64 values are left alone: they
  // may be genuine operator intent.
  if (!active || derivation.suppressed) {
    try {
      updateOpenclawConfigFn({
        ...
        mutate: (cfg) => {
          ...
          if (
            ownership &&
            !ownership.intent &&
            ownership.ownedFromAbsent &&
            current === ownership.lastApplied
          ) {
            if (cfg.agents?.defaults) delete cfg.agents.defaults.maxConcurrent;
            return {};
          }
          if (typeof current === "number" && current > 64) {
            cfg.agents.defaults.maxConcurrent = 64;
            if (
              typeof cfg.agents.defaults.subagents?.maxConcurrent === "number" &&
              cfg.agents.defaults.subagents.maxConcurrent > 62
            ) {
              cfg.agents.defaults.subagents.maxConcurrent = 62;
            }
            console.log(
              `[autotune] disabled: agents.defaults.maxConcurrent ${current} exceeded the legacy 64 ceiling — clamped back`,
            );
          }
          return {};
        },
  ...
  try {
    const { action, current } = updateOpenclawConfigFn({
      ...
      mutate: (cfg) => {
        const current = readCurrent(cfg);
        if (current == null) {
          if (!intentPersisted) return { action: "skipped_no_intent", current };
  ...
  if (outcome.action === "adopted" || outcome.action === "reapplied") {
    ...
    ledger.ownedKeys[kConcurrencyKey] = {
      ownedFromAbsent:
        outcome.action === "adopted" ? true : ownership?.ownedFromAbsent === true,
      lastApplied: target,
...
const runApply = async ({
  ...
  if (previous && !sameCapacity(previous, capacity)) {
    ...
    if (previous.memoryLimitBytes !== capacity.memoryLimitBytes) {
      parts.push(`memory ${fmt(previous.memoryLimitBytes)}→${fmt(capacity.memoryLimitBytes)}`);
    }
    if (previous.cpuCores !== capacity.cpuCores) {
      parts.push(`cpu ${previous.cpuCores}→${capacity.cpuCores} cores`);
    }
    if ((previous.diskTotalBytes ?? null) !== (capacity.diskTotalBytes ?? null)) {
      parts.push(`disk ${fmt(previous.diskTotalBytes)}→${fmt(capacity.diskTotalBytes)}`);
    }
    if (tuningActive) {
      const message = `Container resized (${parts.join(", ")}) — settings retuned`;
      ...
      console.log(
        `[autotune] container capacity changed (${parts.join(", ")}) — autotune is off, nothing retuned`,
      );
    }
  }
  ledger.lastObservedProfile = capacity;

  const concurrencyRow = applyConcurrency({ active, derivation, ledger, deps: {
    updateOpenclawConfigFn,
    openclawDir,
    fsModule,
  } });
