...
const deriveTunings = (profile, { overrides = {} } = {}) => {
  ...
  const agentConcurrencyCap = applyOverride(
    ...
    { min: 1, max: Math.min(1024, Math.max(8, Math.round(C * 16))) },
  );
      ...
      updateOpenclawConfigFn({
        ...
        mutate: (cfg) => {
          ...
          if (typeof current === "number" && current > kAgentConcurrencyLegacyCap) {
            ...
            console.log(
              `[autotune] disabled: agents.defaults.maxConcurrent ${current} exceeded the legacy ${kAgentConcurrencyLegacyCap} ceiling — clamped back`,
            );
          }
          return {};
        },
