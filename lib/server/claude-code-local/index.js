...
const createClaudeCodeLocalService = ({
  ...
  const refreshProbes = ({ force = false } = {}) => {
    ...
    probeInFlight = (async () => {
      ...
      const rescueEnv = buildRescueEnv();
      // The memory floor must hold for the probes too: two claude CLI boots
      // per tick add exactly the pressure the floor exists to prevent during
      // a resource-starved incident. Serve the stale memo instead.
      const floor = memoryFloorOk();
      if (!floor.ok && probeState) return probeState;
  ...
  const startSession = async ({
    ...
    // Server-verified consent mode (TOCTOU guard): the client names the mode
    // its confirmation displayed; a mid-flight .env mode switch therefore
    // forces a fresh mode-named confirmation instead of riding stale consent.
    if (
      source === "click" &&
      consentedMode != null &&
      consentedMode !== configuredPermissionMode()
    ) {
      return {
        ok: false,
