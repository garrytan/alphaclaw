...
const defaultGetMachineSummary = () => {
  ...
  const suffix = getGatewayNodeOptionsSuffix();
  const match =
    typeof suffix === "string" ? suffix.match(/--max-old-space-size=(\d+)/) : null;
  const derivedHeapMb = match ? num(Number(match[1])) : null;
...
module.exports = {
  ...
  // Shared shape-based scrub (tokens that live in no store) — also used by
  // the watchdog overseer so the two AI evidence paths share one shape list.
  redactSecretShapes,
};
