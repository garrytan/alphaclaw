...
const { createMedicAdmission, medicBlockedOutcome } = require("./medic-admission");
...
// Bounded, single-line summary of WHY a `doctor --fix` run failed: exit code,
// then the last lines doctor itself printed (stdout, else stderr). The run
// stream already redacted secrets; this only trims.
const kDoctorFailureTailLines = 3;
const kDoctorFailureMaxChars = 400;
const describeDoctorFailure = (result) => {
  const parts = [];
  if (typeof result?.code === "number") parts.push(`exit ${result.code}`);
  else if (typeof result?.code === "string" && result.code) parts.push(result.code);
  else if (result?.timedOut) parts.push("timed out");
  else if (result?.error) parts.push(String(result.error?.message || result.error).slice(0, 160));
  else parts.push("nonzero exit");
  ...
  if (said) {
    const tail = said
      ...
      .map((line) => line.trim())
      .filter(Boolean)
      .slice(-kDoctorFailureTailLines)
      .join(" | ");
    parts.push(`doctor said: ${tail.slice(0, kDoctorFailureMaxChars)}`);
  }
  return parts.join("; ");
};
...
const createGatewayMedic = ({
  ...
  const admission = createMedicAdmission({ getChannelInfo });
  ...
  const removeConfigKeys = (keyPaths, assertAdmission) => {
    assertAdmission();
    const removed = [];
    let backup = null;
    updateOpenclawConfig({
  ...
  const applyRemoveKeys = (keys, tier, assertAdmission) => {
    const { removed, backup } = removeConfigKeys(keys, assertAdmission);
    if (!removed.length) {
      return {
        fixed: false,
        tier,
  ...
  const applyDoctorFix = async (tier, { remainingMs, operation, assertAdmission, extras = {} }) => {
    try {
      assertAdmission({ doctor: true });
      ...
      const route = await operation.read(routeDoctorBin);
      assertAdmission({ doctor: true });
      if (remainingMs() < kMinDoctorRunwayMs) {
        return { fixed: false, tier: "none", ...extras, error: "medic run budget exhausted before doctor --fix could start" };
      }
      if (route.refused) {
      ...
      const result = await operation.runWriter(() => {
        ...
        return runDoctorFix({
          ...
          operation: { ...operation, assertActive: () => assertAdmission({ doctor: true }) },
        });
      });
      operation.assertActive();
      const ok = !!result?.ok;
      // A failed doctor run must say WHY in the log and on the ledger: the
      // 2026-09-22 container journeys logged only "doctor_fix failed (backup …)"
      // while 2026.9.5's Doctor had refused a state migration, and the reason
      // lived in an incident bundle nobody could reach. The tail is doctor's
      // own last lines (already redacted by the run stream), bounded.
      const failureDetail = ok ? null : describeDoctorFailure(result);
      log(
        `${tier} ${ok ? "succeeded" : "failed"}${backup ? ` (backup ${backup})` : " (no backup taken)"}${failureDetail ? ` — ${failureDetail}` : ""}`,
      );
      return {
        fixed: ok,
        tier,
        ...
        ...(route.bin
          ...
          : {}),
        ...extras,
        ...(ok ? {} : { error: `doctor --fix failed: ${failureDetail}` }),
      };
    } catch (error) {
      if (error.code === "medic_doctor_prohibited") return medicBlockedOutcome(error);
      throw error;
    }
  };
  ...
  const runAiTier = async ({
    ...
    let doctorText = null;
    try {
      doctorText = collectDoctorJson ? await operation.read(collectDoctorJson) : null;
    } catch {}
    assertAdmission();
    ...
    if (remedy === "remove_keys") {
      ...
      return { ...applyRemoveKeys(allowed, "ai_remove_keys", assertAdmission), model: modelRef, diagnosis };
    }
  ...
  const run = async ({
    ...
    signal = null,
    deadlineAt = Infinity,
    assertMutationAllowed = null,
  } = {}) => {
    const operation = suppliedOperation || createRepairOperation({ now: nowFn, signal });
    if (!suppliedOperation) operation.start(budgetMs, deadlineAt);
    const remainingMs = operation.remainingMs;
    const assertAdmission = ({ doctor = false } = {}) => {
      operation.assertActive();
      assertMutationAllowed?.({ doctor });
      admission.assert({ doctor, allowDoctorFix });
    };
    const execute = async () => {
    assertAdmission();
    ...
    if (managed.length) {
      try {
        const outcome = applyRemoveKeys(managed, "managed_key", assertAdmission);
        if (outcome.fixed) return outcome;
        log(`managed_key tier removed nothing (${outcome.error}); escalating`);
      } catch (error) {
        if (medicBlockedOutcome(error) || error.name === "AbortError") throw error;
    ...
    const tryBlamedKeyStrip = (note) => {
      ...
      try {
        const outcome = applyRemoveKeys(removableKeyPaths, "blamed_key_strip", assertAdmission);
    ...
    try {
      ...
      const blocked = medicBlockedOutcome(error);
      if (blocked) {
        log(`repair skipped: ${blocked.reason}`);
        return blocked;
      }
      if (error.name !== "AbortError") throw error;
      return { fixed: false, tier: "cancelled", error: error.message, code: error.code };
    } finally {
