...
const createGatewayMedic = ({
  ...
  const removeConfigKeys = (keyPaths, operation) => {
  ...
  const applyRemoveKeys = (keys, tier, operation) => {
    const { removed, backup } = removeConfigKeys(keys, operation);
    if (!removed.length) {
      return {
        fixed: false,
        tier,
  ...
  const applyDoctorFix = async (tier, { remainingMs, operation, extras = {} }) => {
    operation.assertActive();
    ...
    const result = await operation.runWriter(() => runDoctorFix({
  ...
  const runAiTier = async ({
    ...
    if (remedy === "remove_keys") {
      ...
      return { ...applyRemoveKeys(allowed, "ai_remove_keys", operation), model: modelRef, diagnosis };
    }
  ...
  const run = async ({
    ...
    if (managed.length) {
      try {
        const outcome = applyRemoveKeys(managed, "managed_key", operation);
    ...
    const tryBlamedKeyStrip = (note) => {
      ...
      try {
        const outcome = applyRemoveKeys(removableKeyPaths, "blamed_key_strip", operation);
