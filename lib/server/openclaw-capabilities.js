...
//
// WHICH BINARY (issue #76 C6): probes normally run the `openclaw` on PATH
// through clawCmd. While the watchdog has a versionMismatch latched, the
// optional `resolveBin()` seam names the bin that can read the CURRENT DBs
// (the expected overlay's, via channel-sync's resolveExpectedBin) and probes
// run there through `clawCmdWithBin(bin, cmd, opts)` instead. The cache key is
// then (installed version, bin path): an answer from one binary is never
// served for the other, and the CLI-unavailable suppression window below is
// scoped to the bin that crashed — the diverged PATH binary's startup crash
// must not silence the expected bin the rescue path just switched to.
//
...
const kProbes = {
  ...
  gatewayStopForce: {
    ...
    run: async (clawCmd) => {
      ...
      // A determinate "unsupported" caches for the installed version, so a
      // FAILED probe may only be read as help output when it really is the
      // `gateway stop` usage text — a transient crash whose diagnostic merely
      // mentions "options" or "--help" must stay "unknown" (retried).
      if (!r.ok && !/usage:[^\n]*\bstop\b/i.test(text)) return "unknown";
...
const createOpenclawCapabilities = ({
  ...
  // Optional alternate-binary seam (#76 C6). `resolveBin()` returns the path
  // of the bin probes must run against, or null/undefined for the PATH
  // `openclaw` (the normal case). It is consulted on EVERY probe, so the
  // caller wires it to the live mismatch state (e.g. `() =>
  // watchdog.versionMismatch ? resolveExpectedBin() : null`) and the layer
  // follows the latch/clear without an invalidate. `clawCmdWithBin(bin, cmd,
  // opts)` is commands.js's argv-form twin of clawCmd and is required when
  // resolveBin is given.
  resolveBin = null,
  clawCmdWithBin = null,
} = {}) => {
  ...
  if (resolveBin !== null && typeof resolveBin !== "function") {
    throw new Error("createOpenclawCapabilities: resolveBin must be a function");
  }
  if (resolveBin && typeof clawCmdWithBin !== "function") {
    throw new Error(
      "createOpenclawCapabilities: resolveBin requires a clawCmdWithBin function",
    );
  }
  const { matchesCliStartupFailure } = require("./doctor/classify-doctor-cli");

  // Map<capabilityKey, { key, version, bin, value, at, positive, timedOut }>
  const cache = new Map();
  // CLI-unavailable suppression windows, one per binary: Map<bin | null,
  // { until, reason }> where null is the PATH `openclaw`. A window never
  // applies to probes against a different bin, and arming one never clears
  // another. Instance-level (NOT module-level: this factory is per-instance
  // DI-clocked and tests build many instances with fake clocks).
  const suppression = new Map();
  const suppressedUntilFor = (bin) => suppression.get(bin)?.until ?? 0;

  // The bin probes run against right now: null for the PATH `openclaw`, else
  // the alternate path from the seam. A throwing seam is logged and treated
  // as "no alternate" so a bug there degrades to today's behavior.
  const currentBin = () => {
    if (!resolveBin) return null;
    try {
      const bin = resolveBin();
      return typeof bin === "string" && bin.trim() !== "" ? bin : null;
    } catch (error) {
      logger.warn?.(
        `[openclaw-capabilities] resolveBin failed (${error.message}) — probing the PATH openclaw`,
      );
      return null;
    }
  };

  // A cache entry is only ever served for the (version, bin) it was probed
  // under.
  const inScope = (entry, version, bin) =>
    Boolean(entry) && entry.version === version && (entry.bin ?? null) === bin;

  const armSuppression = (ttlMs, reason, detail, bin = null) => {
    const until = nowFn() + ttlMs;
    if (until <= suppressedUntilFor(bin)) return;
    suppression.set(bin, { until, reason });
    logger.warn?.(
  ...
  const isFresh = (entry, version, bin) => {
    if (!inScope(entry, version, bin)) return false;
  ...
  const probe = async (key, { cmdOpts = null } = {}) => {
    ...
    if (isFresh(cached, version, bin)) return cached.value;
    // Suppression window: the CLI itself is down, so new spawns are pure
    // waste. Serve a cached answer whenever one exists for this version and
    // bin — even a stale one (an Infinity-TTL positive is still trustworthy;
    // only NEW spawns are unsafe). Falsy only on a true cache miss. The
    // window belongs to the bin that crashed: probes redirected to the
    // expected bin (#76 C6) are not what it observed.
    if (nowFn() < suppressedUntilFor(bin)) {
      if (inScope(cached, version, bin)) return cached.value;
      return spec.falsy;
    }
    ...
    const runClawCmd = bin
      ? (cmd, opts) => clawCmdWithBin(bin, cmd, opts)
      : clawCmd;
    const trackingClawCmd = async (cmd, opts) => {
      const result = await runClawCmd(
        cmd,
  ...
  // Per-getMany probe accounting for the hang-class arm below (`bin` is the
  // binary the pass's probes ran against, so the arm scopes to it).
  let lastProbeStats = { probed: 0, timedOut: 0, bin: null };

  // Fetch several capabilities at once (each still individually cached).
  const getMany = async (keys) => {
    lastProbeStats = { probed: 0, timedOut: 0, bin: null };
    const out = {};
  ...
  const invalidate = (key) => {
    if (key === undefined) {
      ...
      // build may have fixed the CLI, so the suppression windows reset too.
      suppression.clear();
    } else {
      cache.delete(key);
    }
