const { spawn } = require("child_process");
const kStderrTailBytes = 2048;

const kDefaultStopTimeoutMs = 5000;
const kDefaultBindHost = "127.0.0.1";
const kDefaultServerHost = "127.0.0.1";
const kPersonalClientName = "personal";

const resolveClientName = (account = {}) => {
  const rawClient = String(account?.client || "").trim();
  if (rawClient) return rawClient;
  if (account?.personal) return kPersonalClientName;
  return "default";
};

const isPidRunning = (pid) => {
  const normalizedPid = Number.parseInt(String(pid || ""), 10);
  if (!Number.isFinite(normalizedPid) || normalizedPid <= 0) return false;
  try {
    process.kill(normalizedPid, 0);
    return true;
  } catch (error) {
    return error?.code !== "ESRCH";
  }
};

const createStatus = (entry = {}) => ({
  // ChildProcess.killed means a signal was SENT, not that the process died.
  running: Boolean(entry.child && !entry.exited),
  stopping: Boolean(entry.stopping),
  pid: entry.child?.pid || null,
  port: entry.port || null,
  accountId: entry.accountId || "",
  email: entry.email || "",
  client: entry.client || "default",
  startedAt: entry.startedAt || null,
});

const createGmailServeManager = ({
  constants,
  onServeExit = () => {},
}) => {
  const entriesByAccountId = new Map();

  const getEntry = (accountId = "") =>
    entriesByAccountId.get(String(accountId || "").trim()) || null;

  const removeEntry = (accountId, entry) => {
    if (getEntry(accountId) === entry) entriesByAccountId.delete(accountId);
  };

  const getServeStatus = (accountId = "") => {
    const entry = getEntry(accountId);
    if (!entry) return createStatus({ accountId });
    return createStatus(entry);
  };

  const listServeStatuses = () =>
    Array.from(entriesByAccountId.values()).map((entry) => createStatus(entry));

  const buildArgs = ({
    account,
    port,
    webhookToken,
  }) => {
    const client = resolveClientName(account);
    const args = [];
    if (client !== "default") {
      args.push("--client", client);
    }
    args.push(
      "gmail",
      "watch",
      "serve",
      "--account",
      String(account?.email || ""),
      "--bind",
      kDefaultBindHost,
      "--port",
      String(port),
      "--path",
      "/",
      "--hook-url",
      `http://${kDefaultServerHost}:${constants.PORT}/hooks/gmail`,
      "--hook-token",
      String(webhookToken || ""),
      "--include-body",
      "--max-bytes",
      String(constants.kGmailMaxBodyBytes || 20000),
    );
    return args;
  };

  const startServe = async ({
    account,
    port,
    webhookToken,
  }) => {
    const accountId = String(account?.id || "").trim();
    if (!accountId) throw new Error("Account id is required");
    const email = String(account?.email || "").trim();
    if (!email) throw new Error("Account email is required");
    const previous = getEntry(accountId);
    if (previous && !isPidRunning(previous.child?.pid)) {
      previous.exited = true;
      removeEntry(accountId, previous);
    }
    const existingEntry = getEntry(accountId);
    if (existingEntry?.stopping) {
      throw Object.assign(new Error("The previous Gmail serve process has not exited."), {
        code: "gmail_serve_stop_unconfirmed", statusCode: 409, retryable: true,
      });
    }
    if (existingEntry?.child?.pid && isPidRunning(existingEntry.child.pid)) {
      return createStatus(existingEntry);
    }
    const normalizedPort = Number.parseInt(String(port || ""), 10);
    if (!Number.isFinite(normalizedPort) || normalizedPort <= 0) {
      throw new Error("A valid serve port is required");
    }
    const token = String(webhookToken || "").trim();
    if (!token) {
      throw new Error("WEBHOOK_TOKEN is required to start Gmail watch serve");
    }

    const args = buildArgs({ account, port: normalizedPort, webhookToken: token });
    const env = {
      ...process.env,
      XDG_CONFIG_HOME: constants.OPENCLAW_DIR,
      GOG_KEYRING_PASSWORD: constants.GOG_KEYRING_PASSWORD,
    };
    const child = spawn("gog", args, {
      env,
      stdio: ["ignore", "pipe", "pipe"],
    });

    // Keep the last few hundred bytes of stderr so an exit can be explained
    // in the log (fix wave F205: exits used to be silent).
    const stderrTail = { chunks: [], bytes: 0 };
    child.stdout.on("data", () => {});
    child.stderr.on("data", (chunk) => {
      const text = String(chunk || "");
      stderrTail.chunks.push(text);
      stderrTail.bytes += text.length;
      while (stderrTail.bytes > kStderrTailBytes && stderrTail.chunks.length > 1) {
        stderrTail.bytes -= stderrTail.chunks.shift().length;
      }
    });

    const startedAtMs = Date.now();
    const nextEntry = {
      accountId,
      email,
      client: resolveClientName(account),
      port: normalizedPort,
      startedAt: new Date(startedAtMs).toISOString(),
      child,
      exited: false,
      stopping: false,
    };
    entriesByAccountId.set(accountId, nextEntry);

    // Settle ONCE across "error" and "exit" (fix wave F093): a spawn failure
    // (gog missing after a fail-open installer, EACCES) emits "error" and, on
    // some Node versions, no "exit" at all. Without a listener that error was
    // an uncaughtException → gracefulExit(1) → a crash-looping boot whenever
    // any Gmail watch was enabled.
    let settled = false;
    const settle = ({ code = null, signal = null, error = null }) => {
      if (settled) return;
      settled = true;
      nextEntry.exited = true;
      const currentEntry = getEntry(accountId);
      if (currentEntry !== nextEntry) return;
      removeEntry(accountId, nextEntry);
      onServeExit({
        accountId,
        pid: child.pid || null,
        email,
        client: nextEntry.client,
        port: normalizedPort,
        code,
        signal,
        error: error ? String(error.message || error) : null,
        uptimeMs: Math.max(0, Date.now() - startedAtMs),
        stderrTail: stderrTail.chunks.join("").trim().slice(-kStderrTailBytes),
        expected: nextEntry.stopping,
      });
    };
    child.on("error", (error) => {
      // Failed spawn proves no child exists. A failed kill does not.
      if (!child.pid || !isPidRunning(child.pid)) settle({ error });
    });
    child.on("exit", (code, signal) => settle({ code, signal }));

    return createStatus(nextEntry);
  };

  const stopServe = ({
    accountId,
    timeoutMs = kDefaultStopTimeoutMs,
    killTimeoutMs = 1000,
  }) => {
    const normalizedAccountId = String(accountId || "").trim();
    const entry = getEntry(normalizedAccountId);
    if (!entry?.child) {
      return Promise.resolve({ stopped: true, accountId: normalizedAccountId });
    }
    const child = entry.child;
    if (entry.exited || !isPidRunning(child.pid)) {
      entry.exited = true;
      removeEntry(normalizedAccountId, entry);
      return Promise.resolve({ stopped: true, accountId: normalizedAccountId });
    }
    if (entry.stopPromise) return entry.stopPromise;
    entry.stopping = true;
    const work = new Promise((resolve) => {
      let settled = false;
      let forced = false;
      let timeoutHandle;
      let killHandle;
      const finalize = (result) => {
        if (settled) return;
        settled = true;
        clearTimeout(timeoutHandle);
        clearTimeout(killHandle);
        child.removeListener("exit", onExit);
        if (result.stopped) {
          entry.exited = true;
          removeEntry(normalizedAccountId, entry);
        }
        resolve(result);
      };
      const onExit = () => finalize({ stopped: true, forced, accountId: normalizedAccountId });
      timeoutHandle = setTimeout(() => {
        forced = true;
        try {
          child.kill("SIGKILL");
        } catch {}
        if (settled) return;
        killHandle = setTimeout(() => {
          const stopped = entry.exited || !isPidRunning(child.pid);
          finalize({ stopped, forced: true, accountId: normalizedAccountId });
        }, Math.max(100, Number(killTimeoutMs) || 1000));
      }, Math.max(100, Number(timeoutMs) || kDefaultStopTimeoutMs));
      child.once("exit", onExit);
      try {
        child.kill("SIGTERM");
      } catch {
        if (!isPidRunning(child.pid)) onExit();
      }
    });
    entry.stopPromise = work.finally(() => { entry.stopPromise = null; });
    return entry.stopPromise;
  };

  const restartServe = async ({
    account,
    port,
    webhookToken,
  }) => {
    const result = await stopServe({ accountId: account?.id || "" });
    if (!result.stopped) throw Object.assign(new Error("The previous Gmail serve process has not exited."), { code: "gmail_serve_stop_unconfirmed" });
    return await startServe({ account, port, webhookToken });
  };

  const stopAll = async () => {
    const accountIds = Array.from(entriesByAccountId.keys());
    const results = [];
    for (const accountId of accountIds) {
      // eslint-disable-next-line no-await-in-loop
      const result = await stopServe({ accountId });
      results.push(result);
    }
    return results;
  };

  return {
    getServeStatus,
    listServeStatuses,
    startServe,
    stopServe,
    restartServe,
    stopAll,
    isPidRunning,
  };
};

module.exports = {
  createGmailServeManager,
};
