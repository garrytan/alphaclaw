...
// Built from the detector's own exported list (single source of truth; a new
// state added there is forwarded here without a silent drop) while staying a
// structural allowlist: membership is still checked, never assumed.
const { kMemoryTrendStates } = require("./gateway-memory-monitor");
const kForwardableTrendStates = new Set(kMemoryTrendStates);
    ...
    try {
      ...
      if (trend && typeof trend === "object") {
        ...
        if (state) {
          ...
          // Scope and closed-enum causes prevent the legacy state name
          // "leak_suspected" from being mistaken for a V8 heap diagnosis.
          summary.gatewayMemoryScope = "gateway_group_rss";
          if (trend.evidence) {
            const { projectMemoryEvidence } = require("./gateway-memory/evidence");
            summary.gatewayMemoryAttribution = projectMemoryEvidence(trend.evidence).attribution;
          }
        }
      }
    } catch {}
