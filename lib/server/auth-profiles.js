...
const loadAuthStore = (agentId = kDefaultAgentId) => {
  if (agentId === kDefaultAgentId) {
    ...
    if (backend === "state-db") {
      ...
      return loadSharedAuthStore() || { version: 1, profiles: {} };
    }
    // "unreadable" (transient lock): reading the agent db is a safe stale
    // fallback — the matching WRITE path below fails closed instead.
  }
