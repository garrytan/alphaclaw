...
// always exposed. `ask`/`auto` have no exact legacy equivalent — best-effort
// semantics (approval-gated full access), matching how openclaw treats them.
const kModeToLegacy = {
  full: { security: "full", ask: "off" },
  allowlist: { security: "allowlist", ask: "off" },
  deny: { security: "deny", ask: "off" },
  ask: { security: "full", ask: "always" },
  auto: { security: "full", ask: "on-miss" },
};
...
const registerNodeRoutes = ({
  ...
  app.post("/api/nodes/:id/route", async (req, res) => {
    ...
    // allowlist + on-miss is one of the combos openclaw's mode migration
    // deliberately does NOT convert — the legacy pair stays schema-valid and
    // unflagged on both the pinned version and the 2026.9.x beta, so these
    // writes are era-safe as-is (see deriveExecMode).
    const commands = [
  ...
  const readApprovalsViaCli = async () => {
    ...
    return { ok: true, approvals: ensureWildcardAgent(parsed) };
  };
