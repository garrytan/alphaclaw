...
const { deletePairingRequestByCode } = require("../openclaw-state-era");
...
// ClickClack joins this set is the tracked F088 decision (TODOS.md).
...
const registerPairingRoutes = ({
  ...
  // Was 1000ms for an empty cache: with the UI polling every 3s, an idle
  // instance spawned one openclaw CLI (a full Node process) per channel per
  // second, forever. Empty and non-empty results now share one TTL.
  ...
  const refreshPendingPairings = async () => {
    const pending = [];
    const channels = [...kAllowedPairingChannels];
    ...
    pairingCache = {
      pending,
      ts: Date.now(),
      ttlMs: kPairingCacheTtlMs,
    };
    return pending;
  };

  app.get("/api/pairings", wrapAsync(async (req, res) => {
    if (Date.now() - pairingCache.ts < Number(pairingCache.ttlMs || 0)) {
      return res.json({ pending: pairingCache.pending });
    }
    // Single-flight: concurrent polls (multiple tabs) share one CLI sweep
    // instead of each spawning openclaw per channel.
    if (!pairingRefreshPromise) {
      pairingRefreshPromise = refreshPendingPairings().finally(() => {
        pairingRefreshPromise = null;
      });
    }
    try {
      const pending = await pairingRefreshPromise;
      res.json({ pending });
    } catch (error) {
      res.status(500).json({ ok: false, error: error.message || "Pairing list failed" });
    }
  }));

  app.post("/api/pairings/:id/approve", wrapAsync(async (req, res) => {
    ...
    // Approve is a CLI shell-out that opens the state DB: exactly the live
    // openclaw traffic the backup barrier exists to suppress. Refuse BEFORE
    // spawning (409 backup_in_progress + Retry-After), the same contract the
    // direct-write reject path answers — never mutate-then-refuse.
    if (isStateDbQuiet()) {
  ...
  app.post("/api/pairings/:id/reject", (req, res) => {
    ...
    const accountId = String(req.body?.accountId || "").trim();
    if (!kAllowedPairingChannels.has(channel)) {
      logRejectedInput({ req, field: "channel", reason: "unsupported_channel", value: channel });
      return res.status(400).json({
        ok: false,
        error: `Unsupported pairing channel "${channel}"`,
      });
    }
    if (accountId && !isValidChannelAccountId(accountId)) {
      logRejectedInput({ req, field: "accountId", reason: "invalid_shape", value: accountId });
      return res.status(400).json({ ok: false, error: "Invalid account id" });
    }
    try {
      // openclaw >= 2026.9.1-beta.1 keeps pending pairings in the state db
      // (channel_pairing_requests) and deletes the legacy files at gateway
      // startup; neither version ships a `pairing reject` CLI, so this is the
      // documented direct-write exception (schema-guarded, parameterized).
      // The file path stays for pre-import boxes — whichever store holds the
      // request wins.
      const sqliteResult = deletePairingRequestByCode({
        openclawDir,
        channel,
        code: req.params.id,
        accountId,
      });
      const removedFromDb = sqliteResult.ok === true && sqliteResult.deleted > 0;
      const removedFromFile = removeRequestFromPairingStore({
        fsModule,
        openclawDir,
        channel,
        code: req.params.id,
      ...
      pairingCache.ts = 0;
      if (removedFromDb || removedFromFile) {
        console.log(
          `[alphaclaw] Rejected pairing request ${req.params.id} for ${channel}${accountId ? `/${accountId}` : ""} (${removedFromDb ? "state db" : "pairing file"})`,
        );
        return res.json({ ok: true, removed: true });
      }
      if (sqliteResult.ok === false) {
        // The state db could not be read/written (e.g. transient lock) and no
        // file matched — surface as retryable rather than a misleading 404.
        return res.status(503).json({
          ok: false,
          removed: false,
          error: `Pairing store unavailable, retry shortly: ${sqliteResult.error}`,
        });
      }
  ...
  app.get("/api/devices", wrapAsync(async (req, res) => {
    if (!isOnboarded()) {
  ...
  app.post("/api/devices/:id/approve", wrapAsync(async (req, res) => {
    const requestId = String(req.params.id || "").trim();
    if (!isValidDeviceRequestId(requestId)) {
      return res.status(400).json({ ok: false, error: "Invalid device request id" });
    }
    // Same barrier contract as pairing approve: the device approve/reject
    // paths are CLI shell-outs against the state DB.
    if (isStateDbQuiet()) {
      sendIfStateDbQuietError(res, new StateDbQuietError());
      return;
    }
  ...
  app.post("/api/devices/:id/reject", wrapAsync(async (req, res) => {
    const requestId = String(req.params.id || "").trim();
    if (!isValidDeviceRequestId(requestId)) {
      return res.status(400).json({ ok: false, error: "Invalid device request id" });
    }
