...
const registerAgentRoutes = ({
  ...
  app.post("/api/channels/accounts", wrapAsync(async (req, res) => {
    try {
      const body = req.body || {};
      const result = await agentsService.createChannelAccount(body);
      return res.status(201).json({ ok: true, ...result });
    } catch (error) {
      // The stale pairing-row clear at the head of the add is a state-DB
      // write: a held barrier answers 409 backup_in_progress BEFORE any
      // config/env mutation, never a 400 dressed as a validation failure.
  ...
  app.post("/api/channels/accounts/login", wrapAsync(async (req, res) => {
    try {
      const body = req.body || {};
  ...
  app.delete("/api/channels/accounts", wrapAsync(async (req, res) => {
    try {
      const body = req.body || {};
      // The service's additive outcome flags (`gatewayRestartFailed`,
      // `pairingRowsCleanupDeferred`) must reach the client — dropping the
      // result here silently reported a clean delete when the restart failed
      // or the pairing-row cleanup was deferred past a backup barrier.
      // `ok: true` stays authoritative: the delete itself succeeded.
      const result = await agentsService.deleteChannelAccount(body);
      return res.json({
  ...
  app.put("/api/agents/:id", wrapAsync(async (req, res) => {
    try {
