...
const registerUsageRoutes = ({
  ...
  app.get("/api/usage/summary", requireAuth, (req, res) => {
    try {
      // Clamp to the same ceiling getPeriodRange enforces (3650): unclamped
      // values above it compute identical summaries under distinct cache
      // keys, letting a client cycle ?days= to bust the cache.
      const days = Math.min(parsePositiveInt(req.query.days, 30), 3650);
