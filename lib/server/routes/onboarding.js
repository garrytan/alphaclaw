...
const { partitionReservedEnvVars } = require("../onboarding/reserved-env");
...
const registerOnboardingRoutes = ({
  ...
  app.post("/api/onboard", wrapAsync(async (req, res) => {
  ...
  app.post("/api/onboard/github/verify", wrapAsync(async (req, res) => {
    if (hasExplicitOnboardingMarker()) {
      return res.json({ ok: false, error: "Already onboarded" });
    }
  ...
  app.post("/api/onboard/import/scan", wrapAsync(async (req, res) => {
    if (hasExplicitOnboardingMarker()) {
      return res.json({ ok: false, error: "Already onboarded" });
    }
  ...
  app.post("/api/onboard/import/apply", wrapAsync(async (req, res) => {
    if (hasExplicitOnboardingMarker()) {
      return res.json({ ok: false, error: "Already onboarded" });
    }
    ...
    try {
      ...
      const { accepted: importableEnvVars, skippedReservedKeys } =
        partitionReservedEnvVars(envVars, constants.kSystemVars);
      if (skippedReservedKeys.length > 0) {
        console.warn(
          `[onboard] import skipped ${skippedReservedKeys.length} reserved env key(s): ${skippedReservedKeys.join(", ")}`,
        );
      }
      envVars = importableEnvVars;
