...
// Start sampling event-loop lag as soon as the server wires its routes, so
// the sustained-lag warning works even before anyone opens the Watchdog tab.
startLoopLagMonitor();
...
const registerWatchdogRoutes = ({
  ...
  app.get("/api/watchdog/logs", requireAuth, (req, res) => {
    try {
      ...
      // (readLogTail also enforces an absolute 4MB ceiling internally).
      const kMaxTailBytes = 4 * 1024 * 1024;
      const parsedTail = Number.parseInt(String(req.query.tail || "65536"), 10) || 65536;
      const tail = Math.min(kMaxTailBytes, Math.max(1024, parsedTail));
