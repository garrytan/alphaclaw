...
const { resolveGatewayMemoryTarget, withGatewayMemoryDetails } = require("../gateway-memory/resources");
const { kGatewayHoldCopy, getGatewayHoldCopy, kAutoRepairPauseCopy } = require("../gateway-state");
...
const registerWatchdogRoutes = ({
  ...
  app.get("/api/watchdog/overseer", requireAuth, wrapAsync(async (req, res) => {
    try {
  ...
  app.post("/api/watchdog/overseer/review", requireAuth, wrapAsync(async (req, res) => {
    try {
  ...
  // Operator copy for the repair toast (v0.9.75 contract): the route returns
  // 200 for an honest failed relaunch — doctor ran, nothing replaced the
  // gateway — and the toast needs words, not a verdict enum.
  const kRepairOutcomeCopy = {
    pending:
      "Doctor finished; the relaunched gateway is not yet confirmed as the one answering the port.",
    launch_failed: "Doctor finished, but the gateway relaunch failed. Check the watchdog events.",
    launch_aborted:
      "Doctor finished, but the relaunch was aborted (prelaunch hook, shutdown or an expired lease).",
    replacement_failed:
      "Doctor finished, but the running gateway could not be replaced. Check the watchdog events.",
    incumbent_unhealthy:
      "Doctor finished, but the gateway holding the port is not healthy and was not relaunched.",
    lease_expired: "Repair skipped: another gateway operation took over the lifecycle lock.",
  };
  // ok:false shapes: `skipped` (a gate refused), a `verdict` (Doctor ran, the
  // relaunch failed), or neither (Doctor itself failed — `result.result` is
  // Doctor's output, nothing was relaunched).
  const repairFailureCode = (result) => {
    if (result?.skipped) return String(result?.reason ?? "skipped");
    if (result?.verdict != null) return String(result.verdict);
    return "doctor_failed";
  };
  const describeRepairFailure = (result) => {
    if (result?.skipped) return `Repair skipped (${String(result?.reason ?? "unknown")}).`;
    if (result?.verdict == null) {
      return "doctor --fix did not fix the configuration; the gateway was not relaunched. Check the watchdog events.";
    }
    return (
      kRepairOutcomeCopy[result.verdict] ||
      "Repair did not restore the gateway. Check the watchdog events."
    );
  };
  app.post("/api/watchdog/repair", requireAuth, wrapAsync(async (req, res) => {
    try {
      // `force: true` (Stage 3, Codex 10 c) is the explicit one-shot resume of
      // a latched auto-repair pause; anything else in the body is ignored, a
      // non-boolean `force` is rejected loudly.
      const body =
        req.body && typeof req.body === "object" && !Array.isArray(req.body) ? req.body : {};
      if (body.force !== undefined && typeof body.force !== "boolean") {
        res.status(400).json({ ok: false, error: "Expected force to be a boolean" });
        return;
      }
      const result = await watchdog.triggerRepair({ force: body.force === true });
      // The hold refusals and the auto-repair pause are 409s: they mirror the
      // restart route's fail-closed contract (issue #20) — the repair was
      // refused for a reason the operator must act on. Other skips keep their
      // legacy 200 shape — see TODOS.md "Manual repair should queue on the
      // lifecycle lock, not skip" before widening this.
      if (result?.skipped && result.reason === "auto_repair_paused") {
        res.status(409).json({
          ok: false,
      ...
      if (
        result?.skipped &&
        (result.reason === "gateway_held" ||
          result.reason === "gateway_hold_unreadable")
      ) {
        const unreadable = result.reason === "gateway_hold_unreadable";
        let holdCopy = kGatewayHoldCopy;
        try { holdCopy = getGatewayHoldCopy(watchdog.getGatewayHold?.()); } catch {}
        res.status(409).json({
          ok: false,
          code: result.reason,
          error: unreadable
            ? kGatewayHoldCopy.unreadableRefusal
            : holdCopy.repairRefusal,
          hint: unreadable ? kGatewayHoldCopy.unreadableHint : holdCopy.hint,
          result,
        });
        return;
      }
      // v0.9.75 repair contract: `ok` = doctor succeeded AND the relaunch did
      // not fail; `pending` = a replacement was requested but not yet verified
      // (the verifier books `restart ok {verified: true}` later); `verdict` is
      // the relaunch verdict read after the verify probe; replacementPending
      // mirrors getStatus(). An honest failure is still HTTP 200 (doctor ran),
      // so the body carries operator copy: api.js surfaces `message`/`error`
      // on ok:false — a bare JSON body would render as a blob in the toast.
      const ok = !!result?.ok;
      ...
      res.json({
        ...
        verdict,
        pending,
        replacementPending: watchdog.getStatus?.()?.replacementPending ?? null,
        ...(ok
          ? pending
            ? { message: kRepairOutcomeCopy.pending }
            : {}
          : {
              error: repairFailureCode(result),
              message: describeRepairFailure(result),
            }),
        result,
      });
    } catch (err) {
  ...
  app.post("/api/watchdog/resume-channels", requireAuth, wrapAsync(async (req, res) => {
    try {
  ...
  app.get("/api/watchdog/resources", requireAuth, (req, res) => {
    try {
      ...
      res.json({
        ...
        profile,
        resources: {
          // The process answering the port (an adopted incumbent has no
          // gatewayPid — AlphaClaw did not spawn it — but a serving identity).
          ...withGatewayMemoryDetails(
            getSystemResources(resolveGatewayMemoryTarget(status)),
            typeof watchdog.getMemoryTrend === "function" ? watchdog.getMemoryTrend() : null,
          ),
  ...
  app.post("/api/watchdog/test-notification", requireAuth, wrapAsync(async (req, res) => {
    try {
