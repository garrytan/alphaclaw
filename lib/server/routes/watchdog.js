...
const { kGatewayHoldCopy } = require("../gateway-state");
...
const registerWatchdogRoutes = ({
  ...
  app.post("/api/watchdog/repair", requireAuth, wrapAsync(async (req, res) => {
    try {
      ...
      // Only the hold refusal is a 409: it mirrors the restart route's
      // fail-closed contract (issue #20). Other skips keep their legacy 200
      // shape — see TODOS.md "Manual repair should queue on the lifecycle
      // lock, not skip" before widening this.
      if (
        result?.skipped &&
