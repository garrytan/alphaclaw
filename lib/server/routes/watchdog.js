...
const registerWatchdogRoutes = ({
  ...
  const kReviewRefusalMessages = {
    ...
    not_steady_state:
      "Reviews only run once the gateway is healthy with no open incident.",
    disabled: "Enable the incident overseer first.",
    incident_open: "That incident is still open — reviews cover settled incidents.",
    no_incident: "No settled incident to review yet.",
  ...
  app.post("/api/watchdog/overseer/review", requireAuth, async (req, res) => {
    try {
      ...
      if (!result.ok) {
        // Status codes match the error class: retry-later vs missing vs bug.
        const status =
          result.code === "no_incident"
            ? 404
        ...
        res.status(status).json({
          ok: false,
          error: result.code,
          message: kReviewRefusalMessages[result.code] || "Review refused.",
          result,
        });
        return;
      }
