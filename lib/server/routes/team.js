...
const registerTeamRoutes = ({
  ...
  const reconcile = async (reason) => {
    ...
    if (!(await teamService.reconcileRoster())) return false;
  ...
  const inviteUrlFor = (req, token) => {
    ...
    // only right when it matches how this request actually arrived. The
    // request-derived branch goes through the shared resolver (fix wave PR 8a):
    // Host, or X-Forwarded-Host only from a trusted proxy hop.
    const base =
      configured && !isLocalhostOrigin(configured)
        ? configured
        : resolveRequestOrigin(req) || configured || resolvePublicOrigin(req);
    return `${base}/login.html?invite=${encodeURIComponent(token)}`;
  };

  app.get("/api/team", wrapAsync(async (req, res) => {
  ...
  app.post("/api/team/enable", requireAdmin, wrapAsync(async (req, res) => {
    if (teamService.isTransitionInFlight?.()) return res.status(409).json({
      ok: false, code: "transition_in_flight", error: "A team-mode transition is already running.",
    });
    const {
      ownerEmail = "",
      ownerPassword = "",
    ...
    const prepareEnable = () => {
      owner = membersStore.getMemberByEmail(ownerEmail);
      if (owner) {
        ...
        if (!verified) {
          ...
          return { statusCode: 500,
            ...
            ownerCreated: !!membersStore.getMember(owner.id),
          };
        }
      }
      return { ok: true, ownerCreated };
    };
    // Full transition (snapshot -> write from roster -> restart -> identity
    // probe): routing follows the persisted auth under lifecycle ownership.
    // Lockdown arms only after identity verification; a failed probe attempts
    // to restore the previous auth and verifies that restoration separately.
    const result = await teamService.setEnabled(true, { prepareEnable });
    if (result.ok && !owner) return res.status(409).json({
      ok: false, code: "already_enabled", error: "Team access is already enabled.",
    });
    if (!result.ok) {
      const status = result.blocked || result.code === "transition_in_flight" ? 409 : result.statusCode || 502;
      if (result.restartDeferred) markRestartRequired("team_restart_deferred");
      return res.status(status).json({
        ok: false,
        code: result.code || "team_enable_failed",
        restored: result.restored === true,
        ownerCreated: result.ownerCreated === true,
        ...(result.ownerCreated === true && owner ? { owner: { email: owner.email, role: owner.role } } : {}),
        enabled: result.enabled === true,
        changed: result.changed === true,
        configSaved: result.configApplied === true || result.configRestored === true,
        ...(result.configApplied !== undefined ? { configApplied: result.configApplied } : {}),
        ...(result.gatewayApplied !== undefined ? { gatewayApplied: result.gatewayApplied } : {}),
        ...(result.configRestored !== undefined ? { configRestored: result.configRestored } : {}),
        ...(result.gatewayRestored !== undefined ? { gatewayRestored: result.gatewayRestored } : {}),
        ...(result.restartDeferred ? { restartDeferred: true, hint: result.hint } : {}),
        error:
          result.error ||
          (result.restored === true
            ? "Could not enable team access — the previous gateway auth was restored."
            : "Could not finish enabling team access. Check the Watchdog page for the current gateway state."),
      });
    }
  ...
  app.post("/api/team/disable", requireAdmin, wrapAsync(async (req, res) => {
    if (teamService.isTransitionInFlight?.()) return res.status(409).json({
      ok: false, code: "transition_in_flight", error: "A team-mode transition is already running.",
    });
    if (readTeamSettings().enabled !== true) {
    ...
    if (!result.ok && (result.code === "transition_in_flight" || result.enabled !== false)) {
      if (result.restartDeferred && result.configRestored) markRestartRequired("team_restart_deferred");
      return res.status(result.blocked || result.code === "transition_in_flight" ? 409 : 502).json({
        ok: false,
        code: result.code || "team_disable_failed",
        error: result.error,
        disabled: false,
        enabled: result.enabled !== false,
        changed: result.changed === true,
        configSaved: result.configRestored === true,
        ...(result.blocked ? { restartDeferred: true, hint: result.hint,
          configRestored: result.configRestored === true, gatewayRestored: false } : {}),
      });
    }
    ...
    if (!result.ok) {
      if (result.restartDeferred) markRestartRequired("team_restart_deferred");
      return res.status(result.blocked ? 409 : 502).json({
        ok: false,
        code: result.code || "gateway_probe_failed",
        disabled: true,
        enabled: false,
        changed: result.changed === true,
        configSaved: result.configRestored === true,
        configRestored: result.configRestored === true,
        gatewayRestored: result.gatewayRestored === true,
        ...(result.restartDeferred ? { restartDeferred: true, hint: result.hint } : {}),
        error:
          result.error ||
  ...
  app.patch("/api/team/members/:id", requireAdmin, wrapAsync(async (req, res) => {
    ...
    try {
      ...
      if (err instanceof GatewayMutationBlockedError) {
        markRestartRequired("team_member_changed");
        audit("team_member_updated", { memberId: member.id, email: member.email,
          role: member.role, disabled: member.disabled, gatewayConfigDeferred: true, code: err.code });
        return res.json({ ok: true, member, ...rosterReconciliationDeferredFields(err) });
      }
  ...
  app.delete("/api/team/members/:id", requireAdmin, wrapAsync(async (req, res) => {
    ...
    try {
      ...
      if (err instanceof GatewayMutationBlockedError) {
        markRestartRequired("team_member_removed");
        audit("team_member_removed", { memberId: member.id, email: member.email,
          gatewayConfigDeferred: true, code: err.code });
        return res.json({ ok: true, ...rosterReconciliationDeferredFields(err) });
      }
      return res.status(500).json({
        ok: false,
        code: "gateway_config_failed",
