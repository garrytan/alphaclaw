...
const { kStrippedInboundHeaders } = require("../utils/forwarded-headers");
...
const createGatewayProxyHeaders = ({ reqHeaders, bodyBuffer }) => {
  ...
  // Never forward client-supplied forwarding/identity headers to the gateway:
  // OpenClaw 2026.8 rejects forwarded headers from an unlisted proxy (403
  // proxy_attribution_required), and a forged identity header would let a caller
  // impersonate another user. See lib/server/utils/forwarded-headers.js.
