...
const { applyProxyIdentity, sanitizeProxyHeaders } = require("../proxy-identity");
...
// loopback request disqualifies the local-direct password fallback that the
// OpenAI-compat bridge relies on (trusted-proxy-auth.md "Runtime rules"), and
// the gateway never needs them from this machine-auth path.
const kForwardedEvidenceHeaders = [
  "forwarded",
  "x-forwarded-for",
  "x-forwarded-host",
  "x-forwarded-proto",
...
const registerProxyRoutes = ({
  ...
  // Every gateway-bound request goes through here: inbound identity headers
  // and the setup_token cookie are ALWAYS stripped (team mode on or off), and
  // the operator identity header is injected only when the session resolves
  // to a known operator (team mode on).
