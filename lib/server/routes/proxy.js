...
const registerProxyRoutes = ({
  ...
  const forwardToGateway = (req, res, { machinePeer = false } = {}) => {
    if (!enforceProxiedBodyLimit(req, res)) return;
    let operator = null;
    try {
      if (!machinePeer) operator = resolveProxyIdentity(req);
    } catch {
      operator = null;
    }
  ...
  app.all(kA2aPathPattern, (req, res) => {
    ...
    if (rejectUnsafePath(req, res)) return;
    forwardToGateway(req, res, { machinePeer: true });
  });
