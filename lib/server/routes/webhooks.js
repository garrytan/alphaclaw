...
const buildWebhookGitSyncCommand = (message) =>
  `alphaclaw git-sync -m ${quoteShellArg(message, { strategy: "single" })}`;
...
const registerWebhookRoutes = ({
  ...
  const runWebhookGitSync = async (action, name) => {
    if (typeof execFileCmd !== "function" && typeof shellCmd !== "function") return null;
    const safeName = String(name || "").trim();
    const message = `webhooks: ${action} ${safeName}`.replace(/[\r\n"]/g, "");
    try {
      if (typeof execFileCmd === "function") {
        // argv form: the message is one operand, never shell-parsed (fix wave,
        // guard b — the name is slug-validated, but the shape stays safe even
        // if that validation ever loosens).
        await execFileCmd("alphaclaw", ["git-sync", "-m", message], { timeout: 30000 });
      } else {
        await shellCmd(buildWebhookGitSyncCommand(message), { timeout: 30000 });
      }
      return null;
    } catch (err) {
  ...
  app.post("/api/webhooks", wrapAsync(async (req, res) => {
    try {
      const {
        name: rawName,
        destination = null,
  ...
  app.put("/api/webhooks/:name/destination", wrapAsync(async (req, res) => {
    try {
      const name = validateWebhookName(req.params.name);
  ...
  app.delete("/api/webhooks/:name", wrapAsync(async (req, res) => {
    try {
      const name = validateWebhookName(req.params.name);
