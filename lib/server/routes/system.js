...
const registerSystemRoutes = ({
  ...
  // must stay up and the operator must be told.
  const kIncumbentRestartReason = "incumbent_gateway_still_running";
  class GatewayIncumbentRestartError extends Error {
    constructor(detail, evidence) {
      ...
      this.reason = kIncumbentRestartReason;
  ...
  const runRestartOperation = async ({ operationId }) => {
    ...
    try {
      ...
      if (result?.ok === false && result?.incumbent) {
        // envRestartPending deliberately stays set: nothing new is running.
        throw new GatewayIncumbentRestartError(result.detail, result.evidence);
      }
      envRestartPending = false;
