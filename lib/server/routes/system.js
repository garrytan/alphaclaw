...
const registerSystemRoutes = ({
  ...
  const statusPayloadMemo = { promise: null, value: null, computedAt: 0 };
  ...
  const buildStatusPayload = () => {
    ...
    statusPayloadMemo.promise = computeStatusPayload()
      .then((payload) => {
    ...
    return statusPayloadMemo.promise;
  };
  ...
  app.get("/api/events/status", async (req, res) => {
    ...
    // Doctor status is the heaviest reader on this stream (SQLite run history
    // + workspace delta) — recompute it on a slow cadence, not every 2s tick.
    const kDoctorStatusIntervalMs = 30000;
    let cachedDoctorStatus = null;
    let doctorStatusFetchedAt = 0;
    const getDoctorStatusThrottled = () => {
      const now = Date.now();
      if (
        typeof doctorService?.buildStatus === "function" &&
        now - doctorStatusFetchedAt >= kDoctorStatusIntervalMs
      ) {
        try {
          cachedDoctorStatus = doctorService.buildStatus();
        } catch {
          // keep last known doctor status
        }
        doctorStatusFetchedAt = now;
      }
      return cachedDoctorStatus;
    };
