...
const registerSystemRoutes = ({
    ...
    compute: async () => {
      ...
      status.state = gatewayStateTracker.track(
        reduceGatewayState({
          ...
          gatewayHeld: !!status.openclawChannel?.gatewayHold,
  ...
  // One policy for BOTH gates of a manual restart — the fast pre-check (409
  // before any record exists) and the post-lock re-check. A restart that
  // QUEUED behind a repair / reconcile retry / rollback must re-validate once
  // it holds the lock. Soundness of the re-check: the reconciler sets a hold
  // only inside the boot hold or under "reconcile_retry", so a hold cannot
  // appear while THIS restart is the active operation; channel applies do not
  // take the lock at all — they soft-gate on getActiveGatewayOperation() and
  // refuse (409) while any operation is active — so an apply that started
  // while we were queued is visible to isApplyInProgress() here and wins (it
  // carries its own restart step, so the operator's intent is still served).
  // Boot is refused at the fast gate rather than queued: boot IS the launch,
  // and a restart landing right after it would only recycle a gateway that
  // just came up (the card encodes the same exemption for `booting`).
  // Hold reads FAIL CLOSED: an unreadable or corrupted release-channel state
  // is treated as held — launching on a config that may be held is the exact
  // hazard the hold exists to prevent (the watchdog's config-change retry
  // documents the same rule).
  const readRestartBlocker = ({ preLock = false } = {}) => {
    if (preLock && gatewayLifecycleLock?.getActiveOperation?.()?.kind === "boot") {
      return {
        code: "booting",
        statusCode: 409,
        error:
          "AlphaClaw is still starting the gateway — wait for boot to finish before restarting.",
    ...
    if (openclawChannelService?.isApplyInProgress?.()) {
      return {
        ...
        hint: "Wait for the channel update to finish, then restart.",
      };
    }
    let info = null;
    try {
      ...
      // The cause rides along for the operator, redacted and capped like every
      // other failure message this route persists or serves.
      const cause = redactFailureMessage(String(err?.message || err)).slice(0, 200);
      return {
        code: "gateway_hold_unreadable",
        statusCode: 409,
        error: `${kGatewayHoldCopy.unreadableRefusal} (${cause})`,
    ...
    if (info?.stateCorrupted) {
      return {
        code: "gateway_hold_unreadable",
        statusCode: 409,
        error: `${kGatewayHoldCopy.unreadableRefusal} (state file corrupted)`,
        hint: kGatewayHoldCopy.unreadableHint,
      };
    }
    ...
    if (info?.gatewayHold) {
      return {
        code: "gateway_held",
        statusCode: 409,
        error: kGatewayHoldCopy.restartRefusal,
        hint: kGatewayHoldCopy.hint,
      };
    }
    return null;
  };
  ...
  const runRestartOperation = async ({ operationId, baseUrl = "" }) => {
    ...
    try {
      ...
      const blocker = readRestartBlocker();
      if (blocker) {
