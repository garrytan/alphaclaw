...
const registerSystemRoutes = ({
  ...
  const readRestartBlocker = ({ preLock = false } = {}) => {
    ...
    try {
      ...
      return {
        ...
        statusCode: 409,
        error: `${kGatewayHoldCopy.unreadableRefusal} (${String(err?.message || err)})`,
  ...
  //          ├─ onExpectedRestart · restartGateway · complete
  //          └─ finally: release · onExpectedRestartSettled
  ...
  app.post("/api/gateway/restart", wrapAsync(async (req, res) => {
    ...
    if (restartInFlight) {
      ...
      try {
        ...
        // policy refusal carries its 409 + code, everything else stays 500.
    ...
    try {
      ...
      // A post-lock policy refusal carries its own status (409) and code so
      // the synchronous caller sees the same shape as the fast gate.
