...
const registerSystemRoutes = ({
  ...
  const runRestartOperation = async ({ operationId }) => {
    ...
    try {
      ...
      envRestartPending = false;
      restartRequiredState.completeRestart?.({ operationId, ok: true });
  ...
  app.post("/api/gateway/restart", async (req, res) => {
    ...
    // Attach semantics: a second click (or second tab) joins the active
    // restart instead of starting a competing one.
    if (restartInFlight) {
