  ...
  const applySystemCronConfig = (nextConfig) => {
    ...
    if (nextConfig.enabled) {
      ...
      if (cronContent) {
        ...
        // cron.d until the rename lands. Injected fs mocks without
        // renameSync fall back to a plain write (same contract as
        // utils/safe-file's writeFileAtomic, which lacks the mode option
        // this root-owned file requires).
        if (typeof fs.renameSync === "function") {
          const tmpPath = `${kSystemCronPath}.tmp`;
          fs.writeFileSync(tmpPath, cronContent, { mode: 0o644 });
          fs.renameSync(tmpPath, kSystemCronPath);
        } else {
