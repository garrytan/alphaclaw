...
const registerSystemRoutes = ({
  ...
  // loop never runs sync FS work inline.
  ...
  const computeStatusPayload = async () => {
  ...
  // Memoized across all SSE clients and /api/status: N open tabs cost one
  // payload computation per window, and concurrent callers share the
  // in-flight promise.
  const kStatusPayloadMemoMs = 1500;
  const statusPayloadMemo = { promise: null, value: null, computedAt: 0, generation: 0 };

  // Doctor status is the heaviest reader on the SSE stream (SQLite run
  // history + workspace delta) — recompute it on a slow cadence, SHARED
  // across all connections (a per-connection cache multiplies the recompute
  // by open-tab count, at unsynchronized offsets).
  const kDoctorStatusIntervalMs = 30000;
  const doctorStatusShared = { value: null, fetchedAt: 0 };
  const getDoctorStatusThrottled = () => {
    const now = Date.now();
    if (
      typeof doctorService?.buildStatus === "function" &&
      now - doctorStatusShared.fetchedAt >= kDoctorStatusIntervalMs
    ) {
      try {
        doctorStatusShared.value = doctorService.buildStatus();
      } catch {
        // keep last known doctor status
      }
      doctorStatusShared.fetchedAt = now;
    }
    return doctorStatusShared.value;
  };
  const invalidateStatusCaches = () => {
    ...
    statusPayloadMemo.value = null;
    statusPayloadMemo.computedAt = 0;
    // Bump the generation so a compute that started BEFORE this invalidation
    // cannot re-memoize its pre-mutation payload when it lands; drop the
    // in-flight promise so the next caller starts a fresh compute.
    statusPayloadMemo.generation += 1;
    statusPayloadMemo.promise = null;
  };
  const buildStatusPayload = () => {
    const now = Date.now();
    if (statusPayloadMemo.value && now - statusPayloadMemo.computedAt < kStatusPayloadMemoMs) {
      return Promise.resolve(statusPayloadMemo.value);
    }
    if (statusPayloadMemo.promise) return statusPayloadMemo.promise;
    const generation = statusPayloadMemo.generation;
    const promise = computeStatusPayload()
      .then((payload) => {
        if (statusPayloadMemo.generation === generation) {
          statusPayloadMemo.value = payload;
          statusPayloadMemo.computedAt = Date.now();
        }
        return payload;
      })
      .finally(() => {
        if (statusPayloadMemo.promise === promise) {
          statusPayloadMemo.promise = null;
        }
      });
    statusPayloadMemo.promise = promise;
    return promise;
  };
  ...
  app.get("/api/events/status", async (req, res) => {
    ...
    // A client whose socket stays backed up across consecutive ticks is
    // disconnected rather than allowed to buffer status events without bound.
    ...
    const writeStatusEvent = async () => {
      try {
        ...
        const doctorStatus = getDoctorStatusThrottled();
        if (res.writableEnded || res.destroyed) return;
        // One write per event: halves syscalls and makes the backpressure
        // counter tick-accurate (it counts writes).
        writeToClient(
          `event: status\ndata: ${JSON.stringify({
            status,
            watchdogStatus,
