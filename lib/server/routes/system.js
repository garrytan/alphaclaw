...
const registerSystemRoutes = ({
  ...
  // QUEUED behind boot / reconcile_retry / apply must re-validate once it
  // holds the lock: every transition it guards against happens under that
  // same lock (the reconciler sets a hold inside the boot hold; reconcile
  // retries acquire "reconcile_retry"; applies acquire at start), so a
  // re-check after acquire is sound. If an apply began while we were queued,
  // the apply wins — it carries its own restart step, so the operator's
  // intent is still served — and the manual restart is refused.
  const readRestartBlocker = () => {
    if (openclawChannelService?.isApplyInProgress?.()) {
      return {
    ...
    let gatewayHold = null;
    try {
      gatewayHold = openclawChannelService?.getChannelInfo?.()?.gatewayHold || null;
    } catch {
      gatewayHold = null;
    }
    if (gatewayHold) {
