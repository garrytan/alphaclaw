...
const registerGoogleRoutes = ({
  ...
  // OAuth callback binding (E-C11): the callback below bypasses session auth
  // by design (it's a Google redirect), so completion must be bound to a flow
  // an ADMIN started here. Single-use server-side nonces, 10-minute TTL.
  const kGoogleOauthNonces = new Map();
  const kGoogleOauthNonceTtlMs = 10 * 60 * 1000;
  const issueGoogleOauthNonce = () => {
    const now = Date.now();
    for (const [nonce, createdAt] of kGoogleOauthNonces) {
      if (now - createdAt > kGoogleOauthNonceTtlMs) {
        kGoogleOauthNonces.delete(nonce);
      }
    }
    const nonce = crypto.randomBytes(16).toString("hex");
    kGoogleOauthNonces.set(nonce, now);
    return nonce;
  };
  const consumeGoogleOauthNonce = (nonce) => {
    const createdAt = kGoogleOauthNonces.get(String(nonce || ""));
    if (createdAt === undefined) return false;
    kGoogleOauthNonces.delete(String(nonce));
    return Date.now() - createdAt <= kGoogleOauthNonceTtlMs;
  };
  ...
  app.get("/auth/google/callback", async (req, res) => {
    ...
    try {
      ...
      if (!consumeGoogleOauthNonce(decodedState.nonce)) {
        throw new Error(
