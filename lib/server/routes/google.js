const {
  ...
  isValidGoogleClientName,
} = require("../google-state");
const { sendIfConfigUnreadable } = require("../utils/config-unreadable");
const { revokeGoogleAccount, retryableDisconnectError } = require("../google-disconnect");
...
const registerGoogleRoutes = ({
  ...
  getApiEnableUrl,
  constants,
  // Best-effort watch teardown on disconnect (injected by register-server-
  // routes after the gmail-watch service is built). Optional: absent in tests
  // and non-gmail deployments, so every call site guards it.
  stopGmailWatch = null,
  runGoogleAccountDisconnect = null,
}) => {
  const {
    GOG_CONFIG_DIR,
    GOG_STATE_PATH,
  ...
  // Locked read-modify-write for mutations that race a concurrent OAuth-callback
  // completion (disconnect removal, callback upsert). The mutator receives fresh
  // state read under the lock and returns the next state.
  const updateState = (mutator) =>
    updateGoogleState({ fs, statePath: GOG_STATE_PATH, mutator });
  ...
  // Live probe. `probedClients` records which clients actually answered so a
  // gog outage is distinguishable from "no valid accounts" (fix wave F098).
  const listAuthenticatedAccounts = async (state) => {
    ...
    combined.probedClients = probedClients;
    return combined;
  };

  // The probe wins over the sticky state flag (F098): a revoked/expired grant
  // shows "Connected" forever otherwise. Only when gog itself did not answer
  // for this account's client does the last known flag stand in.
  const resolveAuthenticated = ({ account, authenticatedAccounts }) => {
    const client = String(account.client || kDefaultGoogleClient).trim() || kDefaultGoogleClient;
    const probed = authenticatedAccounts?.probedClients?.has?.(client) === true;
    if (!probed) return Boolean(account.authenticated);
    return accountIsAuthenticated({ account, authenticatedAccounts });
  };
  ...
  const ensureClientCredentials = ({ client, clientId, clientSecret, req }) => {
    ...
    // client_secret is a credential: atomic 0600 write, never a torn or
    // world-readable file.
  ...
  app.get("/api/google/accounts", wrapAsync(async (req, res) => {
    ...
    const accounts = listGoogleAccounts(state).map((account) => {
      ...
      return {
        ...
        activeScopes,
        hasCredentials,
        authenticated: hasCredentials && resolveAuthenticated({ account, authenticatedAccounts }),
      };
    });
    res.json({
      ok: true,
  ...
  app.get("/api/google/status", wrapAsync(async (req, res) => {
    ...
    res.json({
      ...
      hasCredentials,
      authenticated:
        hasCredentials && resolveAuthenticated({ account: selected, authenticatedAccounts }),
  ...
  app.get("/api/google/credentials", (req, res) => {
    ...
    if (requestedClient && !isValidGoogleClientName(requestedClient)) {
      logRejectedInput({ req, field: "client", reason: "invalid_shape", value: requestedClient });
      return res.status(400).json({ ok: false, error: "Invalid client name" });
    }
  ...
  app.post("/api/google/credentials", wrapAsync(async (req, res) => {
    const body = req.body || {};
    ...
    const client = String(body.client || (personal ? "personal" : kDefaultGoogleClient)).trim()
      || kDefaultGoogleClient;
    if (!isValidGoogleClientName(client)) {
      logRejectedInput({ req, field: "client", reason: "invalid_shape", value: client });
      return res.status(400).json({ ok: false, error: "Invalid client name" });
    }
    ...
    try {
      ...
      // Locked upsert against fresh state: clearStoredGoogleAuthForEmail and
      // the credentials-set gogCmd above are multi-second awaits during which a
      // concurrent OAuth-callback completion may have mutated state — a plain
      // saveState(upsert(staleRead)) would clobber it. Identity (id/personal)
      // is decided from the pre-await read (stable for this request).
      const resolvedId = existing?.id || accountId || createGoogleAccountId();
      const resolvedServices = body.services || existing?.services || kDefaultGoogleScopes;
      let account;
      updateState((current) => {
        const merged = upsertGoogleAccount({
          state: current,
  ...
  app.get("/api/google/check", wrapAsync(async (req, res) => {
    const state = readState();
  ...
  app.post("/api/google/disconnect", wrapAsync(async (req, res) => {
    const accountId = String(req.body?.accountId || "").trim();
    let state;
    try { state = readGoogleStateForWrite({ fs, statePath: GOG_STATE_PATH }); }
    catch (error) {
      if (sendIfConfigUnreadable(res, error, { status: 409 })) return;
      throw error;
    }
    // The first-account fallback is only safe when it cannot retarget: with
    // multiple accounts, an accountId-less disconnect whose response is lost
    // (proxy timeout, crash) would resolve a DIFFERENT account on each
    // retry, cascade-revoking live tokens. The Setup UI always sends an
    // explicit accountId; agents are told to (admin manifest).
    const accounts = listGoogleAccounts(state);
    if (!accountId && accounts.length > 1) {
      return res.json({
        ok: false,
        error: "accountId is required when multiple Google accounts are configured",
      });
    }
    const account = getSelectedAccount({ state, accountId, fallbackToFirst: true });
    if (!account) return res.json({ ok: true });
    try {
      if (!disconnects.has(account.id)) {
        // Disable local delivery before awaiting either watch cleanup or
        // revocation. The durable result remains visible after a reload.
        updateState((current) => setAccountGmailWatch({ state: current, accountId: account.id, watch: {
          enabled: false,
          remoteOperation: { kind: "disconnect", status: "pending", updatedAt: Date.now() },
        } }).state);
        const revokeAndRemove = async () => {
          await revokeGoogleAccount({ account, gogCmd, withPrivateTokenFile, fs });
          updateState((current) => removeGoogleAccount({ state: current, accountId: account.id }).state);
          syncBootstrapTools(req);
          return { ok: true };
        };
        const perform = runGoogleAccountDisconnect
          ? () => runGoogleAccountDisconnect({ accountId: account.id, run: revokeAndRemove })
          : async () => {
              if (stopGmailWatch) {
                const stopped = await stopGmailWatch({ accountId: account.id });
                if (stopped?.ok === false) throw retryableDisconnectError(stopped.error || "Gmail watch stop failed");
              }
              return revokeAndRemove();
            };
        let work;
        try { work = perform(); } catch (error) { work = Promise.reject(error); }
        const pending = Promise.resolve(work).catch((error) => {
          updateState((current) => setAccountGmailWatch({ state: current, accountId: account.id, watch: {
            enabled: false,
            remoteOperation: { kind: "disconnect", status: "failed", code: error.code || "google_disconnect_failed",
              message: "Google disconnect did not finish. Gmail delivery is disabled; retry disconnecting this account.", updatedAt: Date.now() },
          } }).state);
          throw error;
        }).finally(() => {
          if (disconnects.get(account.id) === pending) disconnects.delete(account.id);
        });
        disconnects.set(account.id, pending);
      }
      res.json(await disconnects.get(account.id));
    } catch (err) {
      if (sendIfConfigUnreadable(res, err, { status: 409 })) return;
      console.error("[alphaclaw] Google disconnect error:", err);
      res.json({
        ok: false, retryable: true, accountId: account.id,
  ...
  app.get("/auth/google/callback", wrapAsync(async (req, res) => {
    ...
    try {
      ...
      // Who actually consented? (fix wave F095) The flow was started for
      // `expectedEmail`; Google lets the user pick ANY signed-in account on the
      // consent screen. Before this check a different account's refresh token
      // was imported and labeled with the expected email. Verify via userinfo
      // whenever we have an access token; a mismatch is a hard, visible
      // rejection (nothing imported, flow consumed). If userinfo is
      // unreachable we log and proceed on the expected email — a network blip
      // must not block connecting, and gog's own auth list re-probes later.
      const expectedEmail = String(existingAccount?.email || flow.email || "").trim();
      let consentedEmail = "";
      if (tokens.access_token) {
        try {
          ...
          consentedEmail = String(info?.email || "").trim();
        } catch (error) {
          console.warn(
            `[alphaclaw] Google OAuth: could not verify the consenting account (${error?.message || error}); proceeding on the expected email`,
          );
        }
      }
      if (
        expectedEmail &&
        consentedEmail &&
        consentedEmail.toLowerCase() !== expectedEmail.toLowerCase()
      ) {
        throw new Error(
          `account_mismatch: you signed in to Google as ${consentedEmail}, but this connection was started for ${expectedEmail}. Nothing was saved — sign out of Google in the popup (or choose the right account on the consent screen) and try again.`,
        );
      }
      const email = expectedEmail || consentedEmail;
      ...
      const grantedServices = tokens.scope
        ? tokens.scope
            ...
            .filter(Boolean)
        : requestedServices;
      // Stable account identity for this flow (id/personal), decided from the
      // pre-exchange read. New id only if this flow has none — a concurrent
      // change can't invent one for us.
      const resolvedAccountId =
        existingAccount?.id || accountId || createGoogleAccountId();
      const resolvedPersonal = Boolean(existingAccount?.personal);
      // Upsert under the state lock, reading fresh inside it: token exchange +
      // import above are multi-second awaits during which a concurrent
      // disconnect may have mutated state. A plain saveState(upsert(staleRead))
      // would clobber that; the locked read-modify-write merges this account
      // into current state and preserves the other side's changes.
      let account;
      updateState((current) => {
        const merged = upsertGoogleAccount({
          state: current,
