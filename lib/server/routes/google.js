...
const registerGoogleRoutes = ({
  ...
  app.post("/api/google/disconnect", wrapAsync(async (req, res) => {
    ...
    let removed = false;
    try {
      // Stop the account's Gmail watch FIRST — before revocation, while its
      // creds still work. Deleting the account row without this leaves the
      // local push-serve process holding its port and Google delivering
      // notifications for up to 7 days. Best-effort: a watch-stop failure must
      // never block the disconnect (the account may have no watch at all).
      if (stopGmailWatch) {
        try {
          await stopGmailWatch({ accountId: account.id });
        } catch (err) {
          console.warn(
            "[alphaclaw] Gmail watch stop during disconnect failed (continuing):",
      ...
      // Remove under the state lock, reading fresh inside it: the awaits above
      // (watch stop, gog export, revoke fetch, auth remove) leave a
      // multi-second window in which a concurrent OAuth-callback completion may
      // add or re-auth accounts. A plain read-then-write would clobber that; the
      // locked read-modify-write serializes against the callback's own update.
      updateState((current) => removeGoogleAccount({
        state: current,
