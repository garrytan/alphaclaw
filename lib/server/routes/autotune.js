...
const { isNotificationsDisabled } = require("../notification-policy");
const {
...
const buildAutotuneApplyDeps = ({
  ...
  notify: (message, opts) => {
    try {
      ...
      // row still records the fact). Defense-in-depth: the upgradeNotifier
      // sink re-checks the same policy at enqueue/flush.
      ...
      // Promise rejections escape a synchronous try/catch — attach the
      // swallow explicitly so a notifier failure can never surface as an
      // unhandled rejection.
      const completion = sink?.notify?.(message, {
        eventType: "autotune",
        ...(opts || {}),
      });
...
const registerAutotuneRoutes = ({
  ...
  app.put("/api/autotune/settings", requireAuth, wrapAsync(async (req, res) => {
    try {
      const body = req.body || {};
  ...
  app.post("/api/autotune/reapply", requireAuth, wrapAsync(async (req, res) => {
    try {
