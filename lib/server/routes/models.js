...
// The fail-closed config writer refuses an existing-but-unparseable
// openclaw.json (OpenClaw accepts JSON5 / env includes AlphaClaw does not
// rewrite). Surface that as 503 + code so the UI shows the repair copy and
// never as a generic 500 (fix wave F213).
// Shared envelope (fix wave PR 7): also covers the agent auth store's
// AUTH_STORE_UNREADABLE refusals.
...
const registerModelRoutes = ({
  ...
  app.get("/api/models", wrapAsync(async (req, res) => {
    const response = await modelCatalogCache.getCatalogResponse();
    return res.json(response);
  }));

  app.get("/api/models/thinking-options", wrapAsync(async (req, res) => {
  ...
  app.get("/api/models/status", wrapAsync(async (req, res) => {
    try {
  ...
  app.post("/api/models/set", wrapAsync(async (req, res) => {
  ...
  app.get("/api/models/config", (req, res) => {
    // Carries stored credentials — never cacheable (F080).
  ...
  app.put("/api/models/config", wrapAsync(async (req, res) => {
    ...
    try {
      ...
      const runtime = await authProfiles.refreshGatewayAuth?.(agentId, restartRequiredState);
      ...
      if (sendIfConfigUnreadable(res, err)) return;
      res
        .status(500)
  ...
  app.put("/api/models/auth/:profileId", wrapAsync(async (req, res) => {
    ...
    try {
      ...
      modelCatalogCache.markStale();
      res.json({ ok: true, ...await authProfiles.refreshGatewayAuth?.(agentId, restartRequiredState) });
    } catch (err) {
      if (sendIfStateDbQuietError(res, err)) return;
      if (sendIfConfigUnreadable(res, err)) return;
      res
  ...
  app.delete("/api/models/auth/:profileId", wrapAsync(async (req, res) => {
    const { profileId } = req.params;
    if (!profileId) {
      return res
        .status(400)
    ...
    try {
      ...
      modelCatalogCache.markStale();
      res.json({ ok: true, removed, ...(removed ? await authProfiles.refreshGatewayAuth?.(agentId, restartRequiredState) : {}) });
    } catch (err) {
      if (sendIfStateDbQuietError(res, err)) return;
      if (sendIfConfigUnreadable(res, err)) return;
      res
