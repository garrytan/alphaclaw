...
const kConfigUnreadableCopy =
  "AlphaClaw will not rewrite openclaw.json because it cannot parse it (OpenClaw allows JSON5 and env includes that AlphaClaw does not). Fix or restore the file, then retry.";
const sendIfConfigUnreadable = (res, err) => {
  if (err?.code !== "OPENCLAW_CONFIG_UNREADABLE") return false;
  res.status(503).json({ ok: false, error: kConfigUnreadableCopy, code: "config_unreadable" });
  return true;
};
