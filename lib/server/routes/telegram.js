const fs = require("fs");
const { isConfigUnreadableError, noteConfigUnreadable } = require("../utils/config-unreadable");
const { OPENCLAW_DIR, kSubagentConcurrencyDelta } = require("../constants");
...
const respondFailClosed = (res, error) => {
  ...
  noteConfigUnreadable({ error, source: "telegram-route" });
...
const registerTelegramRoutes = ({
  ...
  // Boundary check for every Telegram route (fix wave F084): accountId is a
  // config KEY (channels.telegram.accounts[accountId]) and a path segment.
  // An unvalidated `__proto__` made Object.prototype the write target —
  // process-wide pollution, and later account writes silently never
  // persisted. Anything that is not a channel-account slug is a 400 + audit
  // line; an absent value still means the default account.
  app.use("/api/telegram", (req, res, next) => {
    ...
    if (typeof raw === "string" && isValidChannelAccountId(normalizeAccountId(raw))) {
      return next();
    }
    logRejectedInput({ req, field: "accountId", reason: "invalid_shape", value: raw });
  ...
  // Verify bot token
  app.get("/api/telegram/bot", wrapAsync(async (req, res) => {
    try {
  ...
  app.post("/api/telegram/groups/verify", wrapAsync(async (req, res) => {
  ...
  app.post("/api/telegram/discovery/sweep", wrapAsync(async (req, res) => {
  ...
  app.post("/api/telegram/groups/:groupId/topics", wrapAsync(async (req, res) => {
  ...
  // Bulk-create topics
  app.post("/api/telegram/groups/:groupId/topics/bulk", wrapAsync(async (req, res) => {
    const { groupId } = req.params;
    const body = req.body || {};
  ...
  app.put("/api/telegram/groups/:groupId/topics/:topicId", wrapAsync(async (req, res) => {
    const { groupId, topicId } = req.params;
    const body = req.body || {};
    const name = String(body.name ?? "").trim();
  ...
  app.post("/api/telegram/groups/:groupId/configure", wrapAsync(async (req, res) => {
    const { groupId } = req.params;
    const body = req.body || {};
  ...
  app.get("/api/telegram/workspace", wrapAsync(async (req, res) => {
    try {
      const accountId = resolveAccountId(req);
        ...
        concurrency: (() => {
          ...
          return {
            ...
            computedSubagentMaxConcurrent: Math.max(
              computedMaxConcurrent - kSubagentConcurrencyDelta,
              kSubagentConcurrencyFloor,
            ),
            resourceCap,
  ...
  app.post("/api/telegram/workspace/reset", wrapAsync(async (req, res) => {
    try {
      const accountId = resolveAccountId(req);
