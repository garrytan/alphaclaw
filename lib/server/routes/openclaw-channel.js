  ...
  app.post("/api/openclaw/rollback", (req, res) => {
    try {
      ...
      if (migrated && req.body?.confirmDataRisk !== true) {
        ...
        const backupFile =
          migratedRun?.backup?.verified === true
            ? migratedRun.backup.file || null
            : null;
        return res.status(409).json({
          ...
          hint: backupFile
            ? `Restore the verified pre-update backup first (${backupFile}), or resend with confirmDataRisk: true to roll back anyway.`
