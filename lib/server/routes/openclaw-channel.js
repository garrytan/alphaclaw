...
// the request path and therefore WITHOUT hashing: lstat (a symlink swapped
// onto the recorded path is never "the verified pre-update backup"),
// containment in the backups dir, and the inventory's own view of the file
// (its lstat-based scan plus the provenance digest when both sides carry
// one). Anything unprovable is reported as not-verified with the reason —
// the operator must never be told to restore a file this code cannot vouch
// for. `missing` keeps the WI-4.1 "no longer on disk" wording.
    ...
    try {
      ...
      if (migrated && req.body?.confirmDataRisk !== true) {
        ...
        const hint = !backupFile
          ...
          : backupFileExists
            ? `Restore the verified pre-update backup first (${backupFile}), or resend with confirmDataRisk: true to roll back anyway.${caveatText}`
