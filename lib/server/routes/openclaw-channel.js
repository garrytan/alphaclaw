...
// accepts ONLY --global/--agent/--repository (required)/--json — there is no
// --verify flag (an unknown option fails the whole run). Verification is a
// separate `backup sqlite verify <snapshotPath>` against the exact snapshot
// the create reported (create --json emits `{ ok, snapshotPath, manifest }`).
// Split of the single backup budget: create does the heavy online-backup +
// VACUUM work; verify is a bounded read-back pass.
const kSqliteCreateBudgetShare = 0.8;

// The CLI can interleave log noise with the pretty-printed JSON report — try
// line-wise candidates first, then the widest brace span (same doctrine as
// gateway-restart-handoff's consume parser).
...
// ok=true ONLY when create succeeded AND its snapshotPath parsed AND verify
// passed on that snapshot — a created-but-unverified snapshot is reported as
// a failure with the output tail, never as "verified".
const createSqliteBackupRunner = ({
  ...
  const createTimeoutMs = Math.floor(timeoutMs * kSqliteCreateBudgetShare);
  const verifyTimeoutMs = timeoutMs - createTimeoutMs;
  return async () => {
    ...
    const snapshotPath = parseSnapshotPath(create.tail);
    if (!snapshotPath) {
      return {
        ok: false,
        step: "create",
        ...
        code: create.code,
        timedOut: create.timedOut,
        tail: `Create output had no parsable snapshotPath — snapshot not verified.\n${create.tail}`,
      };
    }
    ...
    if (!verify.ok) {
      return {
        ...
        tail: `Snapshot created at ${snapshotPath} but verify FAILED — do not trust this snapshot.\n${verify.tail}`,
      };
    }
    return {
      ok: true,
...
const registerOpenclawChannelRoutes = ({
  ...
  // Verified SQLite backup (`backup sqlite create --repository <dir> --json`,
  // then `backup sqlite verify <snapshotPath>` on the reported snapshot) —
  // version-gated: the subcommand ships in the 2026.8.1 beta line, so the
  // route fails closed with a clear message on older installs. A create that
  // succeeds without a passing verify is a failure (tail says which step).
