  ...
  app.post("/api/openclaw/rollback", (req, res) => {
    try {
      ...
      if (migrated && req.body?.confirmDataRisk !== true) {
        ...
        // WI-4.1: re-stat — a recorded archive can be pruned or quarantined
        // by the time the operator reaches for it. Missing → name the newest
        // survivor with the honest caveat; partial/reused → say what is in it.
        let backupFileExists = false;
        if (backupFile) {
          try {
            backupFileExists = fs.statSync(backupFile).isFile();
          } catch {
        ...
        const reusedAgeMs = backupReused && Number.isFinite(backupRecord.reusedAgeMs)
          ...
          : null;
        const survivor = (() => {
          if (!backupFile || backupFileExists) return null;
          try {
            const inventory = openclawChannelService.listBackupInventory?.();
            return inventory?.entries?.find((entry) => entry.exists) || null;
          } catch {
            return null;
          }
        })();
        ...
        const hint = !backupFile
          ...
          : backupFileExists
            ...
            : survivor
              ? `The recorded pre-update backup (${backupFile}) is no longer on disk. The newest surviving archive is ${survivor.file} (${survivor.producer}) — it may not predate the migration, so check its date before restoring. Or resend with confirmDataRisk: true to roll back anyway.`
