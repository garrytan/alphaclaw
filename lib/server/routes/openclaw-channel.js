...
const registerOpenclawChannelRoutes = ({
  ...
  // none is reachable (no provider key configured).
  app.get("/api/openclaw/medic", (req, res) => {
    try {
      const enabled = readOpenclawMedicEnabled({
        fsModule: fs,
        openclawDir: OPENCLAW_DIR,
      });
      const availability = gatewayMedic
        ? gatewayMedic.getAvailability()
        : { enabled, ai: { available: false, reason: "not_wired" } };
      res.json({ ok: true, enabled, ai: availability.ai });
    } catch (err) {
      res
        .status(500)
        .json(
