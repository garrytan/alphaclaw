...
const registerOpenclawChannelRoutes = ({
  ...
  app.post("/api/openclaw/rollback", (req, res) => {
    try {
      ...
      const latestRun = (() => {
        try {
          return openclawChannelService.runLedger?.listRuns?.()[0] || null;
        } catch {
          return null;
        }
      })();
      const migrated =
        Boolean(latestRun?.dbPreflight?.migrationRequired) ||
        Boolean(info.gatewayHold);
      if (migrated && req.body?.confirmDataRisk !== true) {
        const backupFile = (() => {
          try {
            const backups =
              openclawChannelService.store?.readState?.()?.backups || [];
            return backups.find((b) => b?.verified)?.file || null;
          } catch {
            return null;
          }
        })();
  ...
  app.post("/api/openclaw/reconcile/retry", async (req, res) => {
    try {
      ...
      // with no hold recorded and the gateway already up, there is nothing
      // this route can safely do.
