...
const registerCodexRoutes = ({
  ...
  const deferCodexWrite = (credential, attempt) => {
    ...
    whenStateDbQuietReleased(async () => {
      ...
      try {
        ...
        const savedOutcome = deferredWriteOutcome;
        onAuthChanged();
        const runtime = await authProfiles.refreshGatewayAuth?.(undefined, restartRequiredState);
        if (deferredWriteOutcome === savedOutcome && runtime?.warning) setDeferredWriteOutcome("saved", runtime.warning);
  ...
  const persistCodexProfileOrDefer = async (credential) => {
    try {
      authProfiles.upsertCodexProfile(credential);
    } catch (err) {
    ...
    onAuthChanged();
    return { deferred: false, ...await authProfiles.refreshGatewayAuth?.(undefined, restartRequiredState) };
  };
  ...
  app.get("/auth/codex/callback", wrapAsync(async (req, res) => {
    ...
    try {
      ...
      const outcome = await persistCodexProfileOrDefer({ access, refresh, expires, accountId });
      if (outcome.deferred) {
        return res.send(`<!DOCTYPE html><html><body><script>
  ...
  app.post("/api/codex/exchange", wrapAsync(async (req, res) => {
    try {
      ...
      const { deferred, ...runtime } = outcome;
