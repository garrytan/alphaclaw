...
const registerGmailRoutes = ({
  ...
  app.post("/api/gmail/watch/start", wrapAsync(async (req, res) => {
    try {
      ...
      if (sendIfConfigUnreadable(res, err, { status: 409 })) return;
      return res.status(err.statusCode || 400).json({ ok: false, error: err.message, code: err.code || "gmail_start_failed", retryable: err.retryable === true });
    }
  }));

  app.post("/api/gmail/watch/stop", wrapAsync(async (req, res) => {
    try {
      const accountId = String(req.body?.accountId || "").trim();
      if (!accountId) return res.status(400).json({ ok: false, error: "accountId is required" });
      ...
      if (sendIfConfigUnreadable(res, err, { status: 409 })) return;
      return res.status(err.statusCode || 400).json({ ok: false, error: err.message, code: err.code || "gmail_stop_failed", retryable: err.retryable === true });
    }
  }));

  app.post("/api/gmail/watch/renew", wrapAsync(async (req, res) => {
    try {
      const accountId = String(req.body?.accountId || "").trim();
      ...
      if (sendIfConfigUnreadable(res, err, { status: 409 })) return;
      return res.status(err.statusCode || 400).json({ ok: false, error: err.message, code: err.code || "gmail_renew_failed", retryable: err.retryable === true });
    }
  }));
