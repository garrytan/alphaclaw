...
// glass path must never lock the operator out). Every event carries a
// suppressedSinceLast count so capped windows read as "N more suppressed",
// never as silence. HEAD: probe MISSES are unaudited (scanner noise), but a
// valid-token HEAD redemption IS audited — only a token holder can send one;
// the liveness kick stays GET-only and never runs concurrently with itself.
const kMissBody =
...
const registerRescueLinkRoutes = ({
  ...
  // GLOBAL bucket first, allocation-free: once the global window is
  // exhausted, a spoofed-X-Forwarded-For flood must not create per-client
  // Map entries at all (peak store cardinality ≈ the global cap per window,
  // not one entry per forged IP — the sweep bounds lifetime, this bounds
  // peak). Fail-open on any gate error: a broken cap store must cost bounded
  // noise, never forensics.
  const allowEvent = (gate, clientKey) => {
    if (!gate) return true;
    try {
      const now = nowFn();
      if (gate.evaluateGlobalThrottle?.(now)?.blocked) return false;
  ...
  // Capped windows must not read as silence: count suppressed events per
  // kind and stamp the count onto the NEXT event that gets through.
  const suppressedSinceLast = { rescue_link_redeemed: 0, rescue_link_probe_failed: 0 };

  const recordEvent = ({ kind, req, token }) => {
    if (typeof recordOperationEvent !== "function") return;
    try {
      const suppressed = suppressedSinceLast[kind] || 0;
      suppressedSinceLast[kind] = 0;
      // Promise.resolve: today's recordOperationEvent is synchronous, but a
      // future async sink's rejection must stay inside the best-effort
      // contract instead of becoming an unhandled rejection.
      Promise.resolve(
        recordOperationEvent({
          ...
          details: {
            ...
            // Control chars stripped: this attacker-writable string lands in
            // watchdog.db and operator UIs — no terminal-escape smuggling.
            userAgent: String(req.headers?.["user-agent"] || "")
              .replace(/[\u0000-\u001f\u007f]/g, " ")
              .slice(0, kUserAgentMaxChars),
  ...
  const countSuppressed = (kind) => {
    if (suppressedSinceLast[kind] != null) suppressedSinceLast[kind] += 1;
  };
  ...
  // redundantly on a box that is by definition already distressed. The
  // in-flight flag matters as much as the interval: a HUNG tmux must not
  // accumulate one extra pending check per interval.
  let lastLivenessKickAt = 0;
  let livenessKickInFlight = false;

  app.get("/rescue/:token", (req, res) => {
    res.set("Cache-Control", "no-store");
    res.set("Referrer-Policy", "no-referrer");
    // Trim ONCE and use everywhere — the resolver and the log/audit hash must
    // see the same canonical value, or %20-padded tokens fragment the audit
    // correlation id.
    ...
    if (!target) {
      ...
      if (!isHead) {
        if (allowEvent(throttle?.probe, req.ip)) {
          recordEvent({ kind: "rescue_link_probe_failed", req, token });
        } else {
          countSuppressed("rescue_link_probe_failed");
        }
      }
    ...
    if (allowEvent(throttle?.redeem, req.ip)) {
      ...
      countSuppressed("rescue_link_redeemed");
    }
    if (!isHead) {
      // Self-healing revocation: a click after an unexpected pane death
      // discovers it (the clicking request itself may still receive the
      // stale redirect — the underlying session is already dead; subsequent
      // clicks 404). Promise rejection MUST be handled here — a sync
      // try/catch does not catch a rejected promise.
      const now = nowFn();
      if (!livenessKickInFlight && now - lastLivenessKickAt >= kLivenessKickMinIntervalMs) {
        lastLivenessKickAt = now;
        livenessKickInFlight = true;
        try {
          Promise.resolve(claudeCodeLocalService?.checkSessionLiveness?.())
            .catch(() => {})
            .finally(() => {
              livenessKickInFlight = false;
            });
        } catch {
          livenessKickInFlight = false;
        }
