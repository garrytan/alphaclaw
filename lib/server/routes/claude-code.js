...
const registerClaudeCodeRoutes = ({
  ...
  // absolutize from the request so the QR code and phone taps get the origin
  // the operator's browser actually used (proxy-correct via trust proxy).
  const absolutizeRescueLink = (req, value) =>
    typeof value === "string" && value.startsWith("/") && typeof getBaseUrl === "function"
      ? `${getBaseUrl(req)}${value}`
      : value;
