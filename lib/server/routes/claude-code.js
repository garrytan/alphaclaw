...
const registerClaudeCodeRoutes = ({
  ...
  app.post(
    ...
    wrapAsync(async (req, res) => {
      if (!claudeCodeLocalService?.startSession) {
        res.status(503).json({ ok: false, error: "not_wired", message: kSessionRefusalMessages.not_wired });
        return;
      }
      // Same strict-boolean consent discipline as the routine fire.
      const confirmed = req.body?.confirmed === true;
      const result = await claudeCodeLocalService.startSession({ confirmed, source: "click" });
      if (!result.ok) {
        sendLocalRefusal(res, result);
        return;
      }
      if (result.status === "running") {
        res.json({ ok: true, status: "running", sessionId: result.sessionId, sessionUrl: result.sessionUrl });
        return;
      }
  ...
  app.post(
    ...
    requireAuth,
    wrapAsync(async (req, res) => {
      if (!claudeCodeLocalService?.stopSession) {
        res.status(503).json({ ok: false, error: "not_wired", message: kSessionRefusalMessages.not_wired });
        return;
      }
  ...
  app.post(
    ...
    requireAuth,
    wrapAsync(async (req, res) => {
      if (!claudeCodeLocalService?.startLogin) {
        res.status(503).json({ ok: false, error: "not_wired", message: kSessionRefusalMessages.not_wired });
        return;
      }
  ...
  app.post(
    ...
    requireAuth,
    wrapAsync(async (req, res) => {
      if (!claudeCodeLocalService?.submitLoginCode) {
        res.status(503).json({ ok: false, error: "not_wired", message: kSessionRefusalMessages.not_wired });
        return;
      }
  ...
  app.post(
    ...
    requireAuth,
    wrapAsync(async (req, res) => {
      if (!claudeCodeLocalService?.cancelLogin) {
        res.status(503).json({ ok: false, error: "not_wired", message: kSessionRefusalMessages.not_wired });
        return;
      }
  ...
  app.post(
    ...
    requireAuth,
    wrapAsync(async (req, res) => {
      if (!claudeCodeLocalService?.logout) {
        res.status(503).json({ ok: false, error: "not_wired", message: kSessionRefusalMessages.not_wired });
        return;
      }
  ...
  app.get(
    ...
    requireAuth,
    wrapAsync(async (req, res) => {
      if (!claudeCodeLocalService?.getTail) {
        res.status(503).json({ ok: false, error: "not_wired", message: kSessionRefusalMessages.not_wired });
        return;
      }
