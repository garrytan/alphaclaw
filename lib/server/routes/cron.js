...
const { kUnavailableCode } = require("../cron-run-errors");

const sendIfHistoryUnavailable = (res, error) => {
  if (sendIfStateDbQuietError(res, error)) return true;
  if (error?.code !== kUnavailableCode) return false;
  res.status(503).json({ ok: false, code: kUnavailableCode, error: error.message });
  return true;
};
...
const registerCronRoutes = ({
  ...
  app.get("/api/cron/jobs/:id/runs", requireAuth, (req, res) => {
    try {
      ...
      if (sendIfHistoryUnavailable(res, error)) return;
      res.status(400).json({ ok: false, error: error.message });
    }
  });

  app.post("/api/cron/jobs/:id/run", requireAuth, wrapAsync(async (req, res) => {
    try {
  ...
  app.post("/api/cron/jobs/:id/enable", requireAuth, wrapAsync(async (req, res) => {
    try {
  ...
  app.post("/api/cron/jobs/:id/disable", requireAuth, wrapAsync(async (req, res) => {
    try {
      const result = await cronService.setJobEnabled({
        jobId: req.params.id,
        enabled: false,
  ...
  app.put("/api/cron/jobs/:id/prompt", requireAuth, wrapAsync(async (req, res) => {
    try {
  ...
  app.put("/api/cron/jobs/:id/routing", requireAuth, wrapAsync(async (req, res) => {
    try {
