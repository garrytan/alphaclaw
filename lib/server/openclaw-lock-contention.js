...
// acquire <label> <scope>/<key>"). The <scope>/<key> token (one slash, no
// spaces) is what keeps "failed to acquire" from over-matching. The label is
// several words on the real CLIs ("legacy audit migration lease", verified
// live on 2026.8.2 and 2026.9.1-beta.1), so the label slot is a bounded
// same-line span, never a single token.
const kStateLeasePattern =
  /SQLite transaction lock wait failed|OPENCLAW_STATE_LEASE_(?:LOST|TIMEOUT|STORAGE_FAILED)|\b\S+\/\S+ was lost\b|timed out waiting for [^\n]{0,120}?\S+\/\S+|failed to acquire [^\n]{0,120}?\S+\/\S+/i;
