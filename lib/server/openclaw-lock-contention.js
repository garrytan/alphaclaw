...
const classifyOwnershipConflict = (text) => {
  ...
  return {
    kind: stateWriterLine != null ? "state_writer_conflict" : "gateway_conflict",
...
//       openclaw.mjs`, `<pkg>/openclaw/bin/openclaw.js`, or a bare
//       `openclaw.mjs` (launched from the package dir).
...
// The script operand of a JS-runtime argv, or null when the runtime was
// given none we can see (`node -e code`, `node --version`, only flags).
const findScriptOperand = (argv) => {
  for (let i = 1; i < argv.length; i += 1) {
    const token = normalizeArgvToken(argv[i]);
    if (token === "--") return i + 1 < argv.length ? normalizeArgvToken(argv[i + 1]) : null;
    if (token.startsWith("-")) {
      if (kValueTakingRuntimeFlags.has(token)) i += 1;
      continue;
    }
    return token;
  }
...
const isOpenclawArgv = (argv) => {
  ...
  if (kJsRuntimePattern.test(exe.slice(exe.lastIndexOf("/") + 1))) {
    const script = findScriptOperand(argv);
    if (script && kOpenclawEntryScriptPattern.test(script)) return true; // (2)
  }
