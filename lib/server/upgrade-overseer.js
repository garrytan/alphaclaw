...
const createUpgradeOverseer = ({
  ...
  const buildPrompt = ({ run, logTail, doctorOutput, channelInfo }) => {
    ...
    return [
      ...
      ...(run.result?.code === "backup_failed" ? [
        "This upgrade was never applied: the required backup failed before activation.",
        "Review the backup failure, not the health of a new build. Recommend fixing the backup and retrying the upgrade; never suggest Roll back or Mark as good.",
      ] : []),
      "",
  ...
  const persistOverseer = (operationId, overseer) => {
    try {
      return ledger.updateRun(operationId, (record) => {
        if (record.overseer && Object.prototype.hasOwnProperty.call(record.overseer, "notifyExpiresAt")) {
          const prior = overseerNotificationTiming(record.overseer, nowFn());
          const next = Object.prototype.hasOwnProperty.call(overseer, "notifyExpiresAt")
            ? overseerNotificationTiming(overseer, nowFn()) : prior;
          overseer.notifyCreatedAt = Math.min(prior.createdAt, next.createdAt);
          overseer.notifyExpiresAt = Math.min(prior.expiresAt, next.expiresAt);
        }
  ...
  const pickEligibleRun = () => {
    ...
    // A standalone backup run (v0.9.81, target.kind "backup") is not an
    // update: it is never reviewed as one, and it never hides the newest
    // update run behind it.
    const latest = runs.find((run) => run?.target?.kind !== "backup");
  ...
  const runOverseerFor = async ({ run }) => {
    ...
    const backupFailed = run.result?.code === "backup_failed";
    const parsed = parseVerdict(result.tail);
    if (parsed && backupFailed) {
      parsed.verdict = "suspect";
      parsed.summary = "The upgrade was never applied because the required backup failed. No new build was activated by this run.";
      parsed.recommendation = "Review the backup failure in the Upgrade tab, fix the backup, and retry the upgrade.";
    }
    ...
    const notifyTiming = !stale && parsed ? overseerNotificationTiming(run.overseer, nowFn()) : null;
    const doneRecord = {
    ...
    persistOverseer(operationId, doneRecord);

    if (stale || !parsed) return;
    const label = describeTarget(run.target);
    const message =
      backupFailed
        ? `🤖 Overseer: OpenClaw ${label} upgrade was never applied because the required backup failed. ${parsed.recommendation}`
