...
const kDisallowedTools =
  "Bash,Edit,Write,NotebookEdit,WebFetch,WebSearch,Task,Agent,TodoWrite,KillShell";
...
const createUpgradeOverseer = ({
  ...
  const collectDoctorOutput = async () => {
    try {
      ...
      // Fallback: spawn openclaw doctor with a gatewayEnv-free probe env.
      ...
      return String(doctor.tail || "").slice(0, 32_000);
    } catch (error) {
  ...
  const runOverseerFor = async ({ run }) => {
    ...
    const [logTail, doctorOutput, helpText] = await Promise.all([
    ...
    if (result.error || result.timedOut || !result.ok) {
      persistOverseer(operationId, {
        ...
        summary: result.timedOut
          ? "The overseer review timed out (5 minute deadline)."
