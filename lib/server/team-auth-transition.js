...
const enableTeamMode = async ({
  ...
  assertCanMutate = () => {},
  onAuthConfigApplied = () => {},
  onAuthConfigRestored = () => {},
  getGatewayUrl,
  request = performHttpProbe,
  probeOptions = {},
  logger = console,
  // Optional auto-fix notifier (fire-and-forget): the automatic auth restore
  // mutates the operator's gateway config and must announce itself.
  notify = null,
} = {}) => {
  ...
  // applyAuthConfig writes gateway.auth AND then team state in two steps; a
  // throw between them leaves trusted-proxy auth on disk (F8). Fold it into
  // the same try as the restart+probe so ANY post-snapshot failure falls
  // through to the restore below — the snapshot was already written above, so
  // the restore is possible and idempotent. Leaving trusted-proxy config on
  // disk with the team flag never flipped
  // is the split-brain that 401s every dashboard→gateway request.
  let probe;
  let configApplied = false;
  let gatewayApplied = false;
  const noteAuthApplied = () => {
    configApplied = true;
    onAuthConfigApplied();
  };
  try {
    assertCanMutate();
    await applyAuthConfig({ assertCanMutate, onAuthConfigApplied: noteAuthApplied });
    assertCanMutate();
    // Compatibility for injected legacy writers; the production writer
    // invokes this synchronously beside its config write.
    if (!configApplied) noteAuthApplied();
    ...
    probe = { ok: false, error: `gateway enable failed: ${error.message}`,
      ...(error instanceof GatewayMutationBlockedError ? { blocked: true, code: error.code, hint: error.hint } : {}) };
  }
  if (probe.ok) {
    return { ok: true };
  }
  ...
  try {
    ...
    const restoredProbe = await probeSharedSecret({ gatewayUrl: getGatewayUrl(),
      credential: credential.value, request, ...probeOptions });
    assertCanMutate();
    if (!restoredProbe.ok) throw new Error(restoredProbe.error);
  } catch (error) {
    // The WORST branch — enable failed AND the auth restore failed (the
    // operator-lockout case) — must be the loudest, not the silent one.
    fireAndForgetNotify(
      notify,
      `🔴 Team mode enable failed AND restoring the previous gateway auth also failed (${sanitizeNotificationText(error.message)}) — the gateway may be unreachable until auth is fixed by hand.`,
      { eventType: "health", id: `team-auth-restore-failed-${Date.now()}` },
    );
    return {
      ok: false,
      restored: false,
      ...
      ...(error instanceof GatewayMutationBlockedError
        ? { blocked: true, restartDeferred: true, code: error.code, hint: error.hint } : {}),
      error: `${probe.error}; restore also failed: ${error.message}`,
    };
  }
  // Timestamped id is correct here: each enable attempt is user-initiated
  // (no boot-loop risk), and each failed attempt is genuinely a new event.
  fireAndForgetNotify(
    notify,
    `⚠️ Team mode enable failed (${sanitizeNotificationText(probe.error)}) — the previous gateway auth was restored automatically.`,
    { eventType: "health", id: `team-auth-restored-${Date.now()}` },
  );
  return { ok: false, restored: true, configApplied, gatewayApplied,
    configRestored: true, gatewayRestored: true, error: probe.error };
};
...
const disableTeamMode = async ({
  ...
  try {
    ...
    // restartGateway THROWS when the gateway never becomes ready (this
    // branch's contract change). The token-auth config is already restored
    // on disk, so treat it as a failed probe — never strand
    // alphaclaw.json's team.enabled=true against a token-auth gateway.
    return {
      ok: false,
      configRestored: true,
      gatewayRestored: false,
      ...(error instanceof GatewayMutationBlockedError
        ? { blocked: true, restartDeferred: true, code: error.code, hint: error.hint } : {}),
      error: `Gateway restart failed after restoring token auth: ${error.message}`,
    };
  }
  ...
  try {
    ...
    return { ok: false, configRestored: true, gatewayRestored: false, error: probe.error };
  } catch (error) {
    return { ok: false, configRestored: true, gatewayRestored: false,
      ...(error instanceof GatewayMutationBlockedError
        ? { blocked: true, restartDeferred: true, code: error.code, hint: error.hint } : {}),
      error: `Could not verify restored gateway auth: ${error.message}` };
  }
};
