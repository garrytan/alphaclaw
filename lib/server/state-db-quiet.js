...
// One-shot callbacks armed mid-quiet by writers that already mutated
// something the barrier does not gate (config/env/an OAuth code) before the
// state-db half of their write was refused: they retain the pending write and
// finish it here, when the barrier lifts. Registered listeners' end() cannot
// serve that purpose — endListeners() only reaches the entries recorded when
// begin() ran, so a mid-quiet registration would never fire.
...
// Runs every armed release waiter exactly once, after the listeners' end()
// so the stores they write to are open again. A throwing waiter is reported,
// never allowed to starve the others.
const drainReleaseWaiters = (onEvent) => {
  const pending = [...releaseWaiters];
  releaseWaiters.clear();
  for (const waiter of pending) {
    try {
      waiter();
    } catch (error) {
      emit(onEvent, "release_waiter_error", { error: error?.message || String(error) });
    }
  }
};

// Arms `fn` to run once the CURRENT quiet period ends (released, expired, or
// a begin that rolled back). When no barrier is held, `fn` runs on the next
// macrotask so callers get one asynchronous contract. Returns a disarm.
const whenStateDbQuietReleased = (fn) => {
  if (typeof fn !== "function") {
    throw new TypeError("whenStateDbQuietReleased: fn must be a function");
  }
  if (!state.quiet && !state.beginning) {
    const timer = setImmediate(fn);
    timer.unref?.();
    return () => clearImmediate(timer);
  }
  releaseWaiters.add(fn);
  return () => {
    releaseWaiters.delete(fn);
  };
};
