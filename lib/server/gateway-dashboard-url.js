...
const { kControlUiBasePath } = require("./control-ui-mount");
...
const createDashboardUrlService = ({
  ...
  const buildDashboardUrl = (token, subPath = "") => {
    if (!subPath) {
      ...
      return token
        ? `${kControlUiBasePath}/#token=${encodeURIComponent(token)}`
        : kControlUiBasePath;
    }
    return `${kControlUiBasePath}${subPath}${token ? `#token=${encodeURIComponent(token)}` : ""}`;
  };
