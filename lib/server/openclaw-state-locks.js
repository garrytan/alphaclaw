...
// and the delete is untouched; only the quarantined inode is removed.
const reapEntry = ({ fullPath, fsModule }) => {
  ...
  for (const name of matches) {
    ...
    try {
      ...
      if (decision.action === "clear") {
        // Re-check right before the reap: the entry may have been replaced
        // since inspection (type change aborts; the quarantine-rename makes
        // the delete itself atomic against name reuse).
        const recheck = fsModule.lstatSync(fullPath);
        if (
          recheck.isFile() !== stats.isFile() ||
          recheck.isDirectory() !== stats.isDirectory() ||
