...
// with the JSON payload — scan line-wise for the protocol-marked object, then
// fall back to the widest brace span for pretty-printed output.
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (
        ...
        parsed.protocol === kRestartHandoffProtocol &&
        kRestartHandoffStatuses.has(parsed.status)
      ) {
        return parsed;
      }
    } catch {}
