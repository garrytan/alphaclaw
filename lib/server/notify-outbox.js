...
const createNotifyOutbox = ({
  ...
  const flush = async ({ deliver }) => {
    if (typeof deliver !== "function" || flushing) {
      return { delivered: 0, failed: 0, abandoned: 0, suppressed: 0, pending: 0 };
    }
    flushing = true;
    let delivered = 0;
    let failed = 0;
    ...
    try {
      ...
      for (const event of pendingEvents) {
        ...
        if (result?.ok) {
          ...
          // (nothing was lost; the operator opted out).
      ...
      return { delivered, failed, abandoned, suppressed, pending };
    } catch (error) {
      log(`flush failed: ${error.message}`);
      return { delivered, failed, abandoned, suppressed, pending: 0 };
    } finally {
      flushing = false;
    }
  };
