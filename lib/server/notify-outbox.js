...
const { notificationTiming, notificationExpired } = require("./notification-expiry");
...
// periodic flush and the boot re-drain — until the event's fixed deadline.
// Overseer notices expire after one hour as intentional suppression with a
// skipped audit event. Other notices retain their 48h lifetime and abandonment
// policy: one GIVING-UP log plus
...
const createNotifyOutbox = ({
  ...
  let flushing = false;
  let expiryAttemptsInFlush = null;
  const recordedExpiryIds = new Set();
  ...
  const normalizeEvent = (event) => {
    ...
    return {
      ...
      ...notificationTiming(event, { now: nowFn(), maxAgeMs }),
      expiryRecordedAt: event.expiryRecordedAt ?? null,
      ...
      // Which abandonment it was: true = every target failed deterministically
      // on delivery (one attempt), false = the 48h age-out. A terminal
      // tombstone revives on a fresh same-id enqueue (see enqueue); missing on
      // old files → false, i.e. the age-out semantics those files were
      // written under.
      abandonedTerminal: event.abandonedTerminal === true,
  ...
  const isTerminal = (entry) =>
    entry?.deliveredAt != null || entry?.abandonedAt != null || entry?.suppressedAt != null;

  // Dedup by id: re-enqueueing an already-known event (boot re-drain racing a
  // live enqueue) is a no-op, never a duplicate chat message. Two tombstone
  // kinds revive on a fresh enqueue instead, because a stale tombstone must
  // not swallow every future re-notify of a stable id forever:
  //   - terminally SUPPRESSED (adversarial review F2): the operator's settings
  //     allow the notice again (the enqueue gate already passed);
  //   - TERMINAL-abandoned (every target failed deterministically after ONE
  //     attempt — a blocked bot, a never-started chat): the operator fixing
  //     the channel is exactly the human action a fresh enqueue follows, and
  //     the daily `alphaclaw-update-<version>` re-notify must get a new
  //     attempt (attempts reset; if still blocked, one new
  //     `notification_abandoned` per abandonment — honest, never a storm:
  //     one attempt per enqueue).
  // Delivered and 48h-aged-out duplicates stay deduped (the age-out already
  // retried for two days; a same-id re-enqueue inside its audit tail is the
  // boot-re-drain race, not fresh news).
  const revivesOnEnqueue = (existing) =>
    Boolean(existing) &&
    existing.deliveredAt == null &&
    existing.suppressedReason !== "expired" &&
    ((Boolean(existing.suppressedAt) && !existing.abandonedAt) ||
      (Boolean(existing.abandonedAt) && existing.abandonedTerminal === true));
  const enqueue = (event) => {
    const normalized = normalizeEvent(event);
    if (!normalized) return null;
    const markExpired = (entry) => {
      if (entry.eventType === "overseer" && entry.deliveredAt == null && notificationExpired(entry, nowFn())) {
        entry.suppressedAt ??= nowFn();
        entry.suppressedReason = "expired";
        entry.nextAttemptAt = null;
      }
      return entry;
    };
    const acknowledgeEnqueue = (entry) => {
      if (entry.suppressedReason === "expired") noteExpired(entry);
      return entry;
    };
    try {
      ...
      if (existingIdx >= 0) {
        ...
        const timing = notificationTiming(existing, { now: nowFn(), legacy: true, maxAgeMs });
        normalized.createdAt = Math.min(normalized.createdAt, timing.createdAt);
        normalized.expiresAt = Math.min(normalized.expiresAt, timing.expiresAt);
        markExpired(normalized);
        if (revivesOnEnqueue(existing)) {
          events[existingIdx] = normalized;
          writeEvents(events);
          return acknowledgeEnqueue(normalized);
        }
        const retained = markExpired({ ...existing, createdAt: normalized.createdAt, expiresAt: normalized.expiresAt });
        if (existing.createdAt !== retained.createdAt || existing.expiresAt !== retained.expiresAt ||
            existing.suppressedReason !== retained.suppressedReason) {
          events[existingIdx] = retained;
          writeEvents(events);
        }
        // Return the actual retained record, including a terminal suppression.
        return acknowledgeEnqueue(retained);
      }
      markExpired(normalized);
  ...
  // delivery verdict both land here: abandonedAt stamp (this entry is never
  // retried; only a FRESH same-id enqueue of a terminal tombstone starts
  // over — see enqueue), ONE GIVING-UP log, ONE persisted
  // `notification_abandoned` watchdog event so the loss is visible in the
  // incidents UI even when no channel works.
  const abandonEvent = (entry, now, { terminal = false, failures = null } = {}) => {
    const createdAt = Number(entry.createdAt) || 0;
    entry.abandonedAt = now;
    entry.abandonedTerminal = terminal === true;
    entry.nextAttemptAt = null;
    const why = terminal
  ...
  const noteExpired = (entry, result = {}) => {
    if (entry.deliveredAt != null && result.expired !== true && entry.suppressedReason !== "expired") return;
    if (entry.expiryRecordedAt != null || recordedExpiryIds.has(entry.id)) return;
    const events = readEvents();
    const existing = events.find((candidate) => candidate?.id === entry.id);
    if (existing?.expiryRecordedAt != null) {
      entry.expiryRecordedAt = existing.expiryRecordedAt;
      return;
    }
    if (expiryAttemptsInFlush?.has(entry.id)) return;
    expiryAttemptsInFlush?.add(entry.id);
    const remember = () => {
      recordedExpiryIds.add(entry.id);
      if (recordedExpiryIds.size > keepCount) recordedExpiryIds.delete(recordedExpiryIds.values().next().value);
    };
    // A sink-free caller can dedupe locally, but cannot claim a durable audit.
    // A later process with a sink can still record the persisted tombstone.
    if (typeof insertEvent !== "function") { remember(); return; }
    result = result.expired === true ? result : entry.expiryDelivery || result;
    if ((Number(result.sent) || 0) > 0 &&
        ((Number(result.failed) || 0) > 0 || (Number(result.skippedTargets) || 0) > 0) &&
        entry.partialAt == null) {
      recordPartialDelivery(entry, nowFn(), result);
    }
    try {
      insertEvent({
        eventType: "notification_expired", source: kOutboxEventSource, status: "skipped",
        details: { id: entry.id, notifyEventType: entry.eventType,
          createdAt: entry.createdAt, expiresAt: entry.expiresAt,
          sent: Number(result.sent) || 0, failed: Number(result.failed) || 0,
          skipped: Number(result.skippedTargets) || 0 },
        correlationId: entry.operationId || "",
      });
    } catch (error) {
      log(`expiry event insert failed for ${entry.id}: ${error.message}`);
      return;
    }
    // The DB atomically dedupes this ID (0 means already present). Stamp only
    // after that confirmation; a failed insert stays eligible on later flushes.
    entry.expiryRecordedAt = nowFn();
    remember();
    if (existing) {
      existing.expiryRecordedAt = entry.expiryRecordedAt;
      try { writeEvents(events); } catch (error) { log(`expiry stamp write failed: ${error.message}`); }
    }
  };
  const expireEvent = (entry, result = {}) => {
    entry.suppressedAt = nowFn();
    entry.suppressedReason = "expired";
    entry.nextAttemptAt = null;
    // If the audit sink is down, its later retry must preserve a partial send.
    entry.expiryDelivery = { sent: Number(result.sent) || 0,
      failed: Number(result.failed) || 0, skippedTargets: Number(result.skippedTargets) || 0 };
    noteExpired(entry, result);
  };
    ...
    try {
      insertEvent?.({
        ...
        details: {
          ...
          ...(result.expired ? { skipped: Number(result.skippedTargets) || 0, reason: "expired" } : {}),
  ...
  const flush = async ({ deliver }) => {
    ...
    try {
      ...
      for (const entry of events) {
        if (!entry) continue;
        if (entry.suppressedReason === "expired" && entry.expiryRecordedAt == null) {
          noteExpired(entry);
          continue;
        }
        if (isTerminal(entry)) continue;
        Object.assign(entry, notificationTiming(entry, { now, legacy: true, maxAgeMs }));
        if (!notificationExpired(entry, now)) continue;
        if (entry.eventType === "overseer") { expireEvent(entry); suppressed += 1; }
        else { abandonEvent(entry, now); abandoned += 1; }
      ...
      for (const event of pendingEvents) {
        let result = null;
        // A previous queued delivery may have taken minutes. Recheck at the
        // actual dispatch, including records held behind retry backoff.
        if (notificationExpired(event, nowFn())) {
          if (event.eventType === "overseer") { expireEvent(event); suppressed += 1; }
          else { abandonEvent(event, nowFn()); abandoned += 1; }
        ...
        if (result?.expired) {
          if ((Number(result.sent) || 0) > 0) {
            ...
            delivered += 1;
            if (((Number(result.failed) || 0) > 0 || (Number(result.skippedTargets) || 0) > 0) && event.partialAt == null) {
              recordPartialDelivery(event, nowFn(), result);
              partial += 1;
            }
          }
          if ((Number(result.sent) || 0) > 0 || (Number(result.failed) || 0) > 0) event.attempts += 1;
          expireEvent(event, result);
          suppressed += 1;
          updates.set(event.id, event);
          continue;
        }
      ...
      if (updates.size > 0) {
        ...
        const merged = current.map((entry) => {
          ...
          const retainedTiming = notificationTiming(entry, { now: nowFn(), legacy: true, maxAgeMs });
          return { ...updated,
            createdAt: Math.min(updated.createdAt, retainedTiming.createdAt),
            expiresAt: Math.min(updated.expiresAt, retainedTiming.expiresAt),
            expiryRecordedAt: updated.expiryRecordedAt ?? entry.expiryRecordedAt ?? null,
          };
        });
      ...
      flushing = false;
      expiryAttemptsInFlush = null;
    }
  };

  const listEvents = () => readEvents();

  return { outboxPath, enqueue, flush, listEvents, noteExpired, now: nowFn };
};
