...
const createNotifyOutbox = ({
  ...
  const noteExpired = (entry, result = {}) => {
    if (entry.deliveredAt != null && result.expired !== true) return;
    ...
    // The outbox is bounded too; source records retain deadlines after pruning.
    ...
    } catch (error) { log(`expiry event insert failed for ${entry.id}: ${error.message}`); }
  };
