...
const readOpenclawConfig = ({
  ...
  try {
    ...
    // Backup inventory binds its source identity to these exact parsed bytes,
    // so a later copy cannot silently select a different configuration.
    if (typeof onReadSource === "function") onReadSource(raw);
