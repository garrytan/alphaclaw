...
//
// The verified restart-handoff contract (60s-TTL SQLite row + hidden
// `gateway restart-handoff consume` CLI, protocol v1) is documented in
// docs/designs/openclaw-context-contract.md; the watchdog consumes it on
// unmanaged clean exits via lib/server/gateway-restart-handoff.js.
let gatewayFeatureGates = null;
const setGatewayFeatureGates = (gates) => {
  gatewayFeatureGates = gates || null;
};

// Watchdog-side counterpart of supervisorModeEnv: the restart-handoff consume
// is gated on the SAME feature gates, so installs where supervisor mode never
// opened (stable 2026.7.x — no handoff rows exist) never spawn the hidden CLI.
const isSupervisorModeActive = () => {
  try {
    return gatewayFeatureGates?.supportsFeature?.("supervisorMode") === true;
  } catch {
    return false;
  }
};
...
const supervisorModeEnv = () =>
  isSupervisorModeActive() ? { OPENCLAW_SUPERVISOR_MODE: "external" } : {};
  ...
  new Promise((resolve) => {
    ...
    if (signal?.aborted) {
      resolve({
        ...
        stdout: "",
        stderr: "",
        error: new Error("Aborted before spawn (shutting down)"),
      });
      return;
    }
    // signal is passed straight to execFile: abort kills the CLI child with
    // SIGTERM natively, without accumulating listeners on a shared per-op
    // signal across many sequential CLI calls.
    execFile(
      "openclaw",
      args,
      { env: gatewayEnv(), timeout: timeoutMs, encoding: "utf8", signal },
      (error, stdout, stderr) => {
        if (error && secrets.length) {
          // message is what call sites log, but cmd/stack also embed the raw
          // argv on execFile errors — scrub every string field a caller
          // could plausibly forward to a logger.
        ...
        resolve({
          ok: !error,
          stdout: scrubSecrets(String(stdout || ""), secrets),
          stderr: scrubSecrets(String(stderr || ""), secrets),
          error: error || null,
        });
      },
    );
...
const waitForGatewayReady = async ({
  ...
  while (Date.now() - startedAt < timeoutMs) {
    // Shutdown cancels the in-flight lifecycle op via this signal — without
    // the check, cancel() would wait out the full 120s ready window and the
    // 10s shutdown deadline would hard-exit mid-drain.
...
const runGatewayRestartCmd = async (cmd, { signal } = {}) => {
  ...
  // Shutdown-cancel must reap this supervisor too — only execOpenclaw
  // children used to observe the abort signal. Escalate to SIGKILL if the
  // CLI traps/delays SIGTERM: a lingering supervisor can hold the gateway
  // port past the successor process's EADDRINUSE retry window.
  if (signal) {
    const onAbort = () => {
      try {
        child.kill("SIGTERM");
  ...
  const ready = await waitForGatewayReady({ signal });
  if (signal?.aborted) {
    try {
      child.kill("SIGTERM");
    } catch {}
...
const launchGatewayProcessUnlocked = async ({ signal } = {}) => {
  ...
  // Classification runs on "close", not "exit": close fires only after the
  // stdio pipes have drained, so the FINAL stderr chunk (which can carry the
  // exit-78 step-aside signature) is always in gatewayStderrTail before the
  // watchdog classifies. close still delivers (code, signal).
