...
// Reactive stale-lock-sweep gate: the launch-path sweep runs ONLY after a
// failed attempt (racing healthy incumbents / migrations / external CLI runs
// on every relaunch is exactly what the sweep's reviewers forbade). Set on
// unexpected child exits and failed restart attempts; cleared when a gateway
// is verified ready. Evidence regex mirrors the upstream lock messages.
const kLockEvidencePattern = /state-locks|state-lifecycle|owns state/i;
const gatewayLaunchGate = { lastAttemptFailed: false, lockEvidenceSeen: false };
const markGatewayAttemptFailed = (...tails) => {
  gatewayLaunchGate.lastAttemptFailed = true;
  const text = tails
    .filter(Array.isArray)
    .map((lines) => lines.join("\n"))
    .join("\n");
  if (kLockEvidencePattern.test(text)) {
    gatewayLaunchGate.lockEvidenceSeen = true;
  }
};
const clearGatewayLaunchGate = () => {
  gatewayLaunchGate.lastAttemptFailed = false;
  gatewayLaunchGate.lockEvidenceSeen = false;
};
const sweepStateLocksSafely = (options) => {
  try {
    const swept = stateLocks.sweepStaleOpenclawStateLocks(options);
    if (swept.cleared.length > 0) {
      console.log(
        `[alphaclaw] pre-start lock sweep cleared ${swept.cleared.length} stale openclaw state lock(s)`,
      );
    }
    return swept;
  } catch (error) {
    console.warn(
      `[alphaclaw] state-lock sweep failed: ${String(error?.message || error)}`,
    );
    return { cleared: [], kept: [], errors: [{ error: String(error) }] };
  }
};
...
const runGatewayRestartCmd = async (cmd, { onStep = null } = {}) => {
  ...
  markGatewayAttemptFailed(stderrTail.snapshot(), stdoutTail.snapshot());
  if (supervisorFailed()) {
...
const runGatewayColdStart = async ({ onStep = null } = {}) => {
  ...
  // Between the settled stop and the fresh spawn: a dead-owner lock here is
  // as unambiguous as it gets mid-life; portReleased gates the mtime
  // fallback (a wedged still-running gateway is NOT abandoned).
  sweepStateLocksSafely({
    site: "cold_start",
    portReleased,
    afterFailedReady: gatewayLaunchGate.lastAttemptFailed,
  });
...
const launchGatewayProcess = async () => {
  ...
  // Reactive stale-lock sweep: only after a failed attempt (covers watchdog
  // crash-relaunch loops and the light-restart fallback without racing
  // healthy incumbents on routine launches).
  if (gatewayLaunchGate.lastAttemptFailed || gatewayLaunchGate.lockEvidenceSeen) {
    sweepStateLocksSafely({
      site: "launch",
      afterFailedReady: gatewayLaunchGate.lastAttemptFailed,
    });
  ...
  child.stdout.on("data", (d) => {
    ...
    if (
      ...
      // A managed launch that comes up disarms the reactive sweep gate —
      // without this, one failure would arm the launch-path sweep FOREVER
      // (every healthy watchdog relaunch would sweep, exactly what the
      // gate's reviewers forbade).
      clearGatewayLaunchGate();
      try {
        gatewayLaunchHandler({
          pid: child.pid,
  ...
  const finalizeGatewayExit = (code, signal) => {
    ...
    if (!expectedExit) {
      // Arms the reactive stale-lock sweep for the next launch attempt.
      markGatewayAttemptFailed(stderrTail.snapshot());
    }
    console.log(
...
const notifyGatewayLaunch = async () => {
  ...
  // A verified-up gateway disarms the reactive sweep gate on every path
  // (managed launch, restart supervisor, already-running detection).
  clearGatewayLaunchGate();
...
module.exports = {
  ...
  gatewayLaunchEnv,
  // Test seams for the reactive stale-lock-sweep gate.
  __resetGatewayLaunchGateForTests: clearGatewayLaunchGate,
  __getGatewayLaunchGateForTests: () => ({ ...gatewayLaunchGate }),
  stopGatewayChild,
