...
const kGatewayPrelaunchHookPath = path.join(
  OPENCLAW_DIR,
  "hooks",
  "pre-gateway-launch",
);
...
// Optional operator hook under the managed state dir, run before every
// gateway spawn with the gateway's own (allowlisted) environment — restores
// runtime patches and sidecars before OpenClaw imports its bundle. Fails
// closed: an invalid or failing hook means no launch. Async so the event
// loop keeps answering /health while it runs.
const runGatewayPrelaunchHook = async ({
  hookPath = kGatewayPrelaunchHookPath,
  ...
  if (!stat.isFile() || (stat.mode & 0o111) === 0) {
    throw new Error(
      `Gateway prelaunch hook must be an executable regular file: ${hookPath}`,
    );
  }
  console.log(`[alphaclaw] Running gateway prelaunch hook: ${hookPath}`);
  const { stdout, stderr } = await execFileImpl(hookPath, [], {
    env: gatewayEnv(),
    timeout: kGatewayPrelaunchHookTimeoutMs,
    maxBuffer: kGatewayPrelaunchHookMaxBuffer,
  });
  if (stdout) process.stdout.write(`[prelaunch-hook] ${stdout}`);
  if (stderr) process.stderr.write(`[prelaunch-hook] ${stderr}`);
  return true;
};
...
const launchGatewayProcess = async () => {
  ...
  try {
    await runGatewayPrelaunchHook();
  } catch (error) {
    // Fail closed like an aborted preflight: no child, the watchdog reports
    // "launchGatewayProcess returned no child" and the operator sees why.
    console.error(
      `[alphaclaw] gateway prelaunch hook failed — gateway not launched: ${error.message}`,
    );
    return null;
  }
