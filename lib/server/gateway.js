...
let kLastPreflightSuccessHash = null;
...
const prepareOpenclawChannelPlugins = async () => {
  ...
  if (desiredHash && desiredHash === kLastPreflightSuccessHash) {
    return { skipped: true };
  }
  cleanupOpenclawPluginInstallStages();
  try {
    await runPluginRuntimeDepsPreflight();
    kLastPreflightSuccessHash = desiredHash;
...
let kOpenclawConfigMemo = { at: 0, readFn: null, existsFn: null, config: null };
const kOpenclawConfigMemoTtlMs = 1500;
const readOpenclawConfigCached = () => {
  const now = Date.now();
  if (
    kOpenclawConfigMemo.config !== null &&
    kOpenclawConfigMemo.readFn === fs.readFileSync &&
    kOpenclawConfigMemo.existsFn === fs.existsSync &&
    now - kOpenclawConfigMemo.at < kOpenclawConfigMemoTtlMs
  ) {
    return kOpenclawConfigMemo.config;
  }
  const configPath = `${OPENCLAW_DIR}/openclaw.json`;
  if (!fs.existsSync(configPath)) return null;
