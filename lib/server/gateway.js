...
const runGatewayRestartCmd = async (cmd, { onStep = null } = {}) => {
  ...
  // Evidence honesty: a fresh per-attempt tail means the evidence only ever
  // contains THIS attempt's stderr — never leftovers from a previous launch.
...
const launchGatewayProcess = async () => {
  ...
  // Captured by THIS child's handlers below: its 'close' — however late it
  // fires — snapshots this tail, never a successor launch's.
  const stderrTail = createStderrTail();
  ...
  // classifies. close still delivers (code, signal).
