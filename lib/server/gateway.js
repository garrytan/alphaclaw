...
// True while gatewayChild is an ADOPTED cold-restart supervisor (issue #56):
// the OpenClaw launcher forwards SIGTERM to the real gateway and exits with
// its code, but SIGKILL cannot be forwarded — killing the launcher would only
// orphan a still-draining gateway that keeps the port. Stop paths consult
// this flag to skip the SIGKILL escalation for that shape.
let gatewayChildIsSupervisor = false;
// The adopted supervisor's real gateway child (Linux /proc walk at adoption),
// carried into exit classification: the restart-handoff row is keyed by the
// GATEWAY's pid, not the launcher's.
let gatewayChildWorkerPid = null;
// Launch generation: a module counter stamped on every spawn AlphaClaw makes
// (the `gateway run` child in requestGatewayLaunch and the `gateway --force`
// supervisor in runGatewayRestartCmd). The watchdog fences late launch/exit
...
const runGatewayShortCmd = async (
  cmd,
  { timeoutMs = kGatewayShortCmdTimeoutMs, abortable = true, shouldAbort = null } = {},
) => {
  const abortIfFenced = () => {
    if (shouldAbort?.()) throw new GatewayRestartError("Gateway command aborted by caller before dispatch", {
      aborted: true, reason: kCallerAbortReason, phase: "stop_command",
    });
  };
  abortIfFenced();
...
const runGatewayRestartCmd = async (
  cmd,
  { onStep = null, stopEvidence = null, shouldAbort = null } = {},
) => {
  ...
  // Caller fence (a lifecycle-lock holder whose lease expired during the stop
  // phase): checked immediately before the spawn — a holder that no longer
  // owns the lock must not launch into its successor's operation.
  if (shouldAbort?.()) {
    throw new GatewayRestartError(`Gateway ${cmd} aborted by caller`, {
      aborted: true,
  ...
  // Every spawn AlphaClaw makes gets the next generation (see the counter).
  ...
  const onSupervisorExit = (code, signal) => {
  ...
  // The caller's fence composes with the dead-supervisor fast fail: a holder
  // that lost its lease stops WAITING (the successor holder owns the port
  // from here), without reaping a supervisor that may be bringing the
  // gateway up.
  const callerAborted = () => shouldAbort?.() === true;
  let ready = await waitForGatewayReady({
    timeoutMs: kGatewayRestartReadyTimeoutMs,
    shouldAbort: () => supervisorFailed() || callerAborted(),
  });
  if (!ready && callerAborted() && !supervisorFailed() && !isGatewayWaitsAborted()) {
    throw new GatewayRestartError(`Gateway ${cmd} aborted by caller`, {
      aborted: true,
      reason: kCallerAbortReason,
  ...
  if (ready) {
    ...
    if (stopEvidence) {
      ...
      if (!verdict.ok) {
        ...
        gatewayChild = null;
        gatewayChildIsSupervisor = false;
    ...
    // Issue #56: a supervisor that is still alive once the gateway is up is
    // the gateway's process-tree root for the rest of its life (on the
    // OpenClaw launcher it forwards signals and exits with the gateway's
    // code — it never respawns). Forgetting it (`gatewayChild = null`) made
    // every post-cold-restart gateway exit invisible to the watchdog: no
    // crash relaunch, no restart-handoff consume, and a memory monitor with
    // no pid to sample. Adopt it as the managed child instead. A supervisor
    // that already exited (older builds daemonize and return) has nothing
    // to adopt — the TCP watcher owns that gateway as before.
    const isSupervisorAlive = () =>
      !supervisorSpawnError &&
      supervisorExit === null &&
      child.exitCode === null &&
      child.signalCode === null;
    // Quiet period: a daemonizing CLI returns within a beat of the port
    // answering; only a supervisor still alive after it is adopted.
    if (isSupervisorAlive() && !isGatewayWaitsAborted()) {
      await sleep(kSupervisorAdoptQuietMs);
    }
    const supervisorAlive = isSupervisorAlive();
    if (supervisorAlive) {
      // The launch-time exit logger's job is done — the managed exit
      // classification below owns this child's exit from here (one log line,
      // one classification).
      if (typeof child.off === "function") child.off("exit", onSupervisorExit);
      gatewayChild = child;
      gatewayChildIsSupervisor = true;
      gatewayChildWorkerPid = resolveFirstChildPid(child.pid);
      gatewayChildGeneration = generation;
      attachManagedGatewayExitClassification(child, {
        stderrTail,
        launchedAt: startedAt,
        workerPid: gatewayChildWorkerPid,
        supervisor: true,
        generation,
      });
    } else {
    ...
    console.log(
      `[alphaclaw] Gateway ${cmd} ready (${Date.now() - startedAt}ms); ${
        supervisorAlive
          ? `supervisor pid ${child.pid} adopted as the managed gateway process${
              gatewayChildWorkerPid ? ` (gateway pid ${gatewayChildWorkerPid})` : ""
            }`
          : "supervisor already exited; gateway tracked by TCP probes only"
      }`,
    );
...
//        ▼
//   shouldAbort?() ─true─► GatewayRestartError {reason: aborted_by_caller} (no spawn)
//        │ false/unset
//        ▼
//   `openclaw gateway --force` (stamps generation = ++gatewayLaunchGeneration)
//        ▼
//   waitForGatewayReady({ shouldAbort: supervisor died ∨ shouldAbort() })
//        │ ready ─► assessRestartIncumbent(stopEvidence, live pids)
//        │            ├ ok ─────────► stamp autotune, { ok:true, durationMs, downtimeMs }
//        │            │               (+ adopt a live supervisor: payload carries generation)
//        │            └ incumbent ──► throw GatewayIncumbentRestartError (no stamp)
//        ├ caller aborted ─► GatewayRestartError {aborted, reason: aborted_by_caller}
...
// `shouldAbort` is the caller's lease fence (a lifecycle-lock holder passes
// `() => !hold.isValid()`): evaluated before the hook, before `gateway stop`
// (the plugin preflight can take minutes), before the spawn and on every
// ready poll — a holder that lost its lease must not stop the serving gateway
// for its successor.
const runGatewayColdStart = async ({ onStep = null, shouldAbort = null } = {}) => {
  ...
  const stop = await runGatewayShortCmd("stop", { shouldAbort });
  ...
  const result = await runGatewayRestartCmd("--force", {
    onStep,
    stopEvidence,
    shouldAbort,
  });
...
// Adoption quiet period (issue #56): older builds' `gateway --force` can
// daemonize and return 0 a beat AFTER the port answers. Adopting such a
// supervisor would book a spurious exit-0 seconds later (and blind the
// memory monitor mid-life). One second of confirmed liveness after ready
// separates the launcher-that-stays from the CLI-that-returns.
const kSupervisorAdoptQuietMs = 1000;

// Exit classification for a MANAGED gateway child. Two callers hand a child
// here: the `gateway run` launch (requestGatewayLaunch) and the adopted
// cold-restart supervisor (runGatewayRestartCmd, issue #56) — one owner so
// both classify identically and the watchdog sees every managed exit.
const attachManagedGatewayExitClassification = (
  child,
  { stderrTail, launchedAt, workerPid = null, supervisor = false, generation = null },
) => {
    ...
    if (gatewayExitHandler) {
      try {
        gatewayExitHandler({
          ...
          pid: child.pid ?? null,
          // For an adopted supervisor: the real gateway's pid (the handoff
          // row is keyed by it). null for a direct `gateway run` child.
          workerPid,
          // Adopted supervisor shape: the launcher exits 1 (not 143) when its
          // 2s hard-kill backstop reaped a still-draining gateway — an
          // EXPECTED stop with code 1 is our own stop, not a crash.
          supervisor,
          launchedAt,
...
const requestGatewayLaunch = async ({
    ...
    if (
      ...
      try {
        gatewayLaunchHandler({
          startedAt: Date.now(),
          pid: child.pid,
          // A direct `gateway run` child IS the serving process (no
          // launcher/worker split), so root and serving pid coincide.
  ...
  attachManagedGatewayExitClassification(child, { stderrTail, launchedAt, generation });
...
//             the resolved worker (adopted supervisor) or the child itself
...
const startGateway = async ({ shouldAbort = null } = {}) => {
  ...
  const child = await launchGatewayProcess({ shouldAbort });
...
const restartGateway = async (reloadEnv, { onStep = null, shouldAbort = null } = {}) => {
  reloadEnv();
  return runGatewayColdStart({ onStep, shouldAbort });
};
...
const stopGatewayForBackup = async ({
  ...
  const childExited = await stopGatewayChildAndWait({ shouldAbort });
  if (shouldAbort?.()) return false;
...
// escape hatches (server-lifecycle). Supervisor-aware for the same reason as
// stopGatewayChildAndWait: SIGKILLing an adopted launcher only orphans the
// gateway it is about to reap itself (its backstop SIGKILLs the gateway 2s
// after the forwarded SIGTERM and exits 1 at 3s). Returns false when there
// was nothing safe to kill.
const killManagedGatewayChildNow = () => {
  if (gatewayChildIsSupervisor) return false;
  return stopGatewayChild({ signal: "SIGKILL", force: true });
};
...
const stopGatewayChildAndWait = async ({ graceMs = 2000, shouldAbort = null } = {}) => {
  ...
  const isSupervisor = gatewayChildIsSupervisor;
  ...
  // An adopted supervisor (issue #56) is the OpenClaw launcher, which runs its
  // OWN backstop on a forwarded SIGTERM: re-SIGTERM at 1s, SIGKILL the gateway
  // at 2s, exit 1 at 3s. SIGKILLing the launcher here would race that backstop
  // and orphan the gateway on the port (a false "stopped"). Leave it to exit
  // with its child; the callers' `openclaw gateway stop` + port-release wait
  // own the rest.
...
module.exports = {
  ...
  // Issue #56 diagnostics/tests: is the managed child an adopted supervisor?
  isManagedGatewayChildSupervisor: () => gatewayChildIsSupervisor,
