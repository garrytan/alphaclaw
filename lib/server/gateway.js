...
const gatewayCliCmd = async (cmd, { timeoutMs = kGatewayShortCmdTimeoutMs } = {}) => {
...
const isGatewayStopForceSupported = async () => {
  try {
    return (await getGatewayCapabilities().get("gatewayStopForce")) === "supported";
  } catch (error) {
    console.warn(
...
// module-level abort has already fired.
const runGatewayShortCmd = async (
  ...
  if (
    args[1] === "stop" &&
    !args.includes("--force") &&
    (await isGatewayStopForceSupported())
  ) {
    args.push("--force");
    forced = true;
  }
...
const listGatewayPids = () => {
  try {
    return lockContention
      ...
      .filter((proc) => kGatewayProcessPattern.test(String(proc.cmdline || "")))
      .map((proc) => proc.pid);
  } catch {
    return [];
  }
...
//   (stopConfirmed ∨ ¬wasRunningBefore) ∧ (new gateway pid ∨ every pre-stop pid gone)
// Otherwise the port that answered belongs to the incumbent the CLI refused
// to stop (or that ignored the stop), and reporting success would flip the
// restart-required banner over a gateway still running the OLD env/config.
const assessRestartIncumbent = ({ stopEvidence, postReadyPids = [], supervisorPid = null }) => {
  ...
  const stopSatisfied = stopConfirmed || !wasRunningBefore;
...
//        │            └ incumbent ──► { ok:false, incumbent:true, evidence } (no stamp)
...
const startGateway = async () => {
  ...
  // (which runs after the module-level abort has fired and can no longer
  // spawn a probe) finds a cached answer. Fire-and-forget, never throws.
...
const stopGatewayForShutdown = async () => {
  ...
  // so --force rides along when the installed CLI supports it.
  await runGatewayShortCmd("stop", {
