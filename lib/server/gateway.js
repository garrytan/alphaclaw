...
const launchGatewayProcess = async () => {
  ...
  gatewayChild = child;
  // Autotune spawn stamp: record the env values this launch actually consumed
  // so ledger rows flip pending_restart → applied. Best-effort by contract —
  // never blocks or fails a launch. Any heap flag in the child env is
  // autotune's (the strip removes all others); UV is stamped only when it
  // matches the derivation (an operator-set UV_THREADPOOL_SIZE is not ours).
  try {
    const {
