...
// here: the `gateway run` launch (launchGatewayProcess) and the adopted
...
const attachManagedGatewayExitClassification = (
  child,
  { stderrTail, launchedAt, workerPid = null, supervisor = false },
) => {
  ...
  const finalizeGatewayExit = (code, signal) => {
    ...
    // "already running" (launchGatewayProcess' alive guard) and skip.
...
const launchGatewayProcess = async () => {
  ...
  attachManagedGatewayExitClassification(child, { stderrTail, launchedAt });
  return child;
};
...
const stopGatewayForBackup = async ({
  ...
  // child) need a CLI stop as well — bounded, best-effort.
  try {
    await runGatewayShortCmd("stop");
  } catch {}
  return waitForGatewayStopped({ timeoutMs });
};
