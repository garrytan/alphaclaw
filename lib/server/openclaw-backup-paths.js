...
const resolveBackupStateRoot = ({ stateDir, fsModule = fs }) => {
  const requestedStateDir = path.resolve(stateDir);
  const rootSymlink = fsModule.lstatSync(requestedStateDir).isSymbolicLink();
  return { requestedStateDir, stateDir: fsModule.realpathSync(requestedStateDir), rootSymlink };
};
const remapBackupSourcePath = (file, { stateDir, requestedStateDir }) => {
  const resolved = path.resolve(file);
  return resolved === requestedStateDir || resolved.startsWith(`${requestedStateDir}${path.sep}`)
    ? path.join(stateDir, path.relative(requestedStateDir, resolved)) : resolved;
};
const isBackupEnvPath = (file) => String(file).split(path.sep).includes(".env");

module.exports = { resolveBackupPath, resolveBackupStateRoot, remapBackupSourcePath, isBackupEnvPath };
