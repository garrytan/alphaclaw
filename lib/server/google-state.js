...
// The client "slot" name is interpolated into the gog credentials filename
// (credentials-<client>.json) — a slug, never a path (fix wave F094/F206).
const kGoogleClientNamePattern = /^[a-z0-9]+(?:[-_][a-z0-9]+)*$/i;
const isValidGoogleClientName = (value) => {
  const name = String(value ?? "").trim();
  return name.length > 0 && name.length <= 64 && kGoogleClientNamePattern.test(name);
};
...
const normalizeGmailWatch = (gmailWatch = {}) => {
  ...
  const operation = gmailWatch?.remoteOperation;
  const remoteOperation = operation &&
    ["start", "stop", "disconnect"].includes(operation.kind) &&
    ["pending", "succeeded", "failed"].includes(operation.status)
    ? {
        kind: operation.kind,
        status: operation.status,
        code: String(operation.code || "").replace(/[^a-z0-9_]/g, "").slice(0, 64),
        message: String(operation.message || "").replace(/[\x00-\x1f\x7f]+/g, " ").slice(0, 240),
        updatedAt: normalizeTimestamp(operation.updatedAt),
      }
    : null;
  // Disabled is the local delivery intent. A pending/failed stop can still
  // own a child and its port; retain that evidence until teardown confirms.
  const retainLocal = enabled || (remoteOperation && remoteOperation.status !== "succeeded" &&
    remoteOperation.kind !== "start");
  return {
    enabled,
    port: retainLocal ? normalizePositiveInt(gmailWatch?.port) : null,
    expiration: normalizeTimestamp(gmailWatch?.expiration),
    lastPushAt: normalizeTimestamp(gmailWatch?.lastPushAt),
    pid: retainLocal ? normalizePositiveInt(gmailWatch?.pid) : null,
...
class GoogleStateReadError extends Error {
  constructor(message, { filePath, cause } = {}) {
    super(message);
    this.name = "GoogleStateReadError";
    this.code = "GOOGLE_STATE_UNREADABLE";
    this.filePath = filePath;
    if (cause) this.cause = cause;
  }
}

// Parse the on-disk file for a WRITE decision (fix wave F214): a missing file
// is empty state, an existing file this code cannot parse is a refusal. The
// lenient readGoogleState below turned that case into empty state, and every
// save path — GET /api/gmail/config on dashboard mount, an inbound Gmail push,
// the locked updateGoogleState — then persisted the emptiness, wiping every
// Google account and the Gmail push token/topics.
const readGoogleStateRawForWrite = ({ fs, statePath }) => {
  let raw;
  try {
    raw = fs.readFileSync(statePath, "utf8");
  } catch (error) {
    if (error && error.code === "ENOENT") return null;
    if (typeof fs.existsSync === "function" && !fs.existsSync(statePath)) return null;
    throw new GoogleStateReadError(`Cannot read ${statePath}: ${error.message}`, {
      filePath: statePath,
      cause: error,
    });
  }
  try {
    ...
    throw new GoogleStateReadError(
      `Refusing to overwrite ${statePath}: existing Google state is not JSON alphaclaw can parse (${error.message})`,
      { filePath: statePath, cause: error },
    );
  }
};

const readGoogleStateForWrite = ({ fs, statePath }) => {
  const raw = readGoogleStateRawForWrite({ fs, statePath });
  if (raw === null) return createEmptyGoogleState();
  if (typeof raw.version === "number" && raw.version > kGoogleStateVersion) {
    return normalizeGoogleStateV2(raw);
  }
  if (raw.version === kGoogleStateVersion && Array.isArray(raw.accounts)) {
    return normalizeGoogleStateV2(raw);
  }
...
const writeGoogleState = ({ fs, statePath, state }) => {
  ...
  withFileLockSync(
    statePath,
    () => {
      // Fail closed (F214): whatever the caller read, never overwrite a file
      // that exists on disk but cannot be parsed — that is the wipe path.
      readGoogleStateRawForWrite({ fs, statePath });
...
const updateGoogleState = ({ fs, statePath, mutator }) => {
  const runLocked = () => {
    // Strict read (F214): an unparseable existing file throws instead of
    // handing the mutator empty state to "update" and persist.
    const current = readGoogleStateForWrite({ fs, statePath });
...
const readGoogleState = ({ fs, statePath }) => {
  ...
  try {
    ...
    // Display reads stay lenient (an empty account list beats a 500 on the
    // Google tab) but say so once in the log + incidents; every WRITE path
    // goes through readGoogleStateForWrite / writeGoogleState's guard and
    // refuses (F214).
    try {
      const { noteConfigUnreadable } = require("./utils/config-unreadable");
      noteConfigUnreadable({
        error: new GoogleStateReadError(
          `Google state at ${statePath} is not JSON alphaclaw can parse (${error.message})`,
          { filePath: statePath, cause: error },
        ),
        source: "google-state",
      });
