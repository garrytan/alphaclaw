...
const readConfigValueAtPath = (config, dottedKey) => {
  let node = config;
...
// D5: a default flip only matters when the installation RELIES on the old
// default — a key the operator set explicitly keeps its value across the
// upgrade, so its flip is noise for them. Unknown config (unreadable) keeps
// every flip (fail-open toward showing the warning).
const filterFlipsForInstallation = (flips, config) => {
  if (!config || typeof config !== "object") return flips;
  return flips.filter(
    (flip) => readConfigValueAtPath(config, flip?.key) === undefined,
  );
};
...
const resolveWhatsNew = ({
  ...
  // Optional installed openclaw.json (parsed) for D5 flip filtering.
  ...
  return {
    ...
    securityFlips: filterFlipsForInstallation(
      Array.isArray(entry.securityFlips) ? entry.securityFlips : [],
      installedConfig,
    ),
