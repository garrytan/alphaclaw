...
//     │ COPY openclaw.json → openclaw.json.pre-doctor-<ts>.bak (keep 3):
//     │   doctor --fix rewrites the file in place, and a tripwire miss must
//     │   still leave the operator a byte-exact pre-fix copy (issue #76 A5)
//     ▼
...
// tripwires, not a semantic differ). The one addition allowed since: the
// guard result carries key-path COUNTS of what doctor changed
// (`guard.configDiff`, from utils/config-key-diff.js — added/removed/changed
// paths, never values). Counts are reporting, not a signal: no tripwire is
// derived from them.
...
// Pre-fix copies of openclaw.json, one per guarded doctor run, newest 3 kept.
const kPreDoctorBackupPattern = /^openclaw\.json\.pre-doctor-\d+\.bak$/;
const kPreDoctorBackupKeep = 3;
...
// Key-path counts of a config change (identifiers only, never values).
const countKeyPathDiff = (before, after) => {
  const diff = diffConfigKeyPaths(before, after);
  return {
...
const createDoctorGuard = ({
  ...
  const recoverQuarantinedLastGood = () => {
    let entries = [];
    try {
      entries = readDirectoryNamesBounded(openclawDir, { fsModule });
    } catch {
      return { recovered: 0 };
    }
    let recovered = 0;
  ...
  const withDoctorRestoreGuard = async ({ operationId = null, run }) => {
    ...
    // Pre-doctor copy (issue #76 A5): the exact bytes doctor is about to
    // rewrite, written atomically beside the config and pruned to the newest
    // kPreDoctorBackupKeep. Best-effort — a copy that cannot be written must
    // not stop a repair the operator asked for (the tripwires still guard the
    // known restore mechanism); the result says whether one exists.
    let preDoctorBackup = null;
    if (preRaw != null) {
      const target = path.join(
        openclawDir,
        `openclaw.json.pre-doctor-${nowFn()}.bak`,
      );
      try {
        writeFileAtomic(target, preRaw, { fsModule });
        preDoctorBackup = path.basename(target);
        pruneFilesMatching({
          fsModule,
          dir: openclawDir,
          pattern: kPreDoctorBackupPattern,
          keep: kPreDoctorBackupKeep,
        });
      } catch (error) {
        log(`pre-doctor backup skipped (${error.message}) — tripwires still armed`);
      }
    }
    ...
    // What doctor changed, as key-path counts (reporting only — see header).
    const configDiff = countKeyPathDiff(preConfig, postConfig);
    const guardEvidence = {
      quarantined: Boolean(quarantined.path),
      preDoctorBackup,
      configDiff,
    };
    const signals = [];
    ...
    if (!signals.length) {
      return { ...result, guard: guardEvidence };
    }
    ...
    log(
      `restore detected (${signals.join(", ")}); ${reverted ? "reverted to the pre-doctor config" : "REVERT FAILED"}; dropped: ${droppedKeyPaths.length} key path(s); doctor's change: +${configDiff.added} −${configDiff.removed} ~${configDiff.changed} key path(s)`,
    );
    return {
      ok: false,
