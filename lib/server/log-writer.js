...
const readLogDelta = ({ gen, offset, maxBytes = 65536 } = {}) => {
  const safeMax = Math.max(1024, Number.parseInt(String(maxBytes || 65536), 10) || 65536);
  ...
  if (!validCursor) {
    ...
    return {
      gen: logGeneration,
      offset: size,
      data: readRange(startPos, size - startPos),
      reset: true,
    };
  }
  const len = Math.min(size - callerOffset, safeMax);
  const data = readRange(callerOffset, len);
  return {
    gen: logGeneration,
    offset: callerOffset + Buffer.byteLength(data),
    data,
    reset: false,
  };
};
