...
// `hitCap` tells the caller the file was larger than the requested tail, i.e.
// the returned text was cut at the front. Without it a reader cannot tell
// "the log begins here" from "there was more before this" — the overseer's
// evidence line must not promise a 30-minute window the bytes did not cover.
const readLogTailInfo = (tailBytes = 65536) => {
  if (!logPath || !fsImpl.existsSync(logPath)) return { text: "", hitCap: false };
  ...
  return { text: buffer.toString("utf8"), hitCap: stat.size > readBytes };
};

const readLogTail = (tailBytes = 65536) => readLogTailInfo(tailBytes).text;
