...
const createChatWsService = ({
  ...
  const deliverToSocket = (ws, sessionKey, serialized) => {
    if (!ws || ws.readyState !== kWsOpen) return;
    const state = getSocketState(ws);
    if (ws.bufferedAmount > kMaxBufferedAmountBytes) {
  ...
  const sendStream = (record, frame) => {
    ...
    if (record.resumable && Array.isArray(record.replay)) {
      record.replay.push({ seq: record.lastSeq, size: serialized.length, framed });
      record.replayBytes += serialized.length;
      while (
    ...
    for (const ws of record.sockets) {
      deliverToSocket(ws, record.sessionKey, serialized);
    }
  };
  ...
  wss.on("connection", (ws) => {
    ...
    sendJson(ws, {
      ...
      // cursor, so the server advertises what can be re-attached. Same member
      // auth surface as the session list — registry-tracked runs only.
      activeRuns: registry
        .startedRecords()
