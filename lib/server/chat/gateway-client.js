...
const createGatewayClient = ({
  ...
  const markGatewayDisconnected = (socket, reason = "Gateway disconnected") => {
    // A STALE socket (a timed-out attempt, or a replaced connection) closing
    // later must not tear down the current one: it would reject the
    // replacement's RPCs and falsely finalize every active run.
    if (gatewaySocket !== null && socket !== null && gatewaySocket !== socket) {
      return;
    }
      ...
      gatewayConnectPromise = withTimeout(
        new Promise((resolve, reject) => {
          ...
          socket.on("error", (err) => {
            ...
            markGatewayDisconnected(socket, "OpenClaw gateway websocket failed");
          });

          socket.on("close", (code) => {
            markGatewayDisconnected(socket, `Gateway disconnected (code ${code})`);
          });
        }),
        kConnectTimeoutMs,
        ...
        .catch((error) => {
          ...
          // A timed-out attempt's socket must not linger half-open — its
          // eventual events are stale-guarded, but the fd is ours to reap.
          if (attemptSocket && attemptSocket !== gatewaySocket) {
            try {
