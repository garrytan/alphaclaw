...
// Buffered events can carry whole rawEvent tool-result blobs — cap bytes per
// run too, or one chatty foreign run spikes memory inside the count caps.
const kMaxBufferedBytesPerRun = 256 * 1024;
...
const createRunRegistry = () => {
  ...
  // Records a socket ORIGINATED (sent the message for) — distinct from
  // records it merely resumed onto as a watcher: the per-browser cap must
  // bound a tab's own pending sends, not punish it for org-wide activity it
  // auto-attached to (32 live runs elsewhere must not block this tab's send).
  const browserOriginated = new WeakMap(); // ws -> Set<record>
  const pendingAgentEventsByRunId = new Map(); // runId -> {list, bytes}

  const countForBrowser = (ws) => browserOriginated.get(ws)?.size || 0;
  ...
  const createRecord = ({ clientMsgId, sessionKey, ws, messageId, now }) => {
    ...
    const originated = browserOriginated.get(ws);
    if (originated) originated.add(record);
    else browserOriginated.set(ws, new Set([record]));
    record.originWs = ws;
    return record;
  };
  ...
  const finalize = (record) => {
    ...
    if (record.originWs) {
      const originated = browserOriginated.get(record.originWs);
      if (originated) {
        originated.delete(record);
        if (originated.size === 0) browserOriginated.delete(record.originWs);
      }
    }
  ...
  const bufferEvent = (runId, eventPayload) => {
    ...
    const entry = pendingAgentEventsByRunId.get(normalizedRunId) || {
      list: [],
      bytes: 0,
    };
    let eventBytes = 0;
    try {
      eventBytes = Buffer.byteLength(JSON.stringify(eventPayload) || "", "utf8");
    } catch {
      eventBytes = 0;
    }
    if (
      entry.list.length < kMaxEventsPerRun &&
      entry.bytes + eventBytes <= kMaxBufferedBytesPerRun
    ) {
      entry.list.push(eventPayload);
      entry.bytes += eventBytes;
    }
    pendingAgentEventsByRunId.set(normalizedRunId, entry);
  ...
  const drainBufferedEvents = (runId) => {
    const entry = pendingAgentEventsByRunId.get(String(runId || ""));
    if (!entry || entry.list.length === 0) return [];
  ...
  return {
    ...
    bufferStats: () => ({
      ...
      events: Array.from(pendingAgentEventsByRunId.values()).reduce(
        (total, entry) => total + entry.list.length,
        0,
      ),
    }),
    stats: () => ({
      ...
      replayBytes: Array.from(byKey.values()).reduce((bytes, record) => bytes + record.replayBytes, 0),
    }),
  };
};
