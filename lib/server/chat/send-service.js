...
const createSendService = ({
  ...
  const handleMessage = async ({ ws, payload }) => {
    ...
    // Replay dedupe applies to outcomes where a turn actually ran (done/
    // stopped/interrupted) or is ambiguous (unknown). An `error` terminal is
    // a FAILED attempt — a re-send with the same clientMsgId is the retry
    // path working as designed: fall through to a fresh chat.send (recordSend
    // upserts the row back to pending).
    if (terminalRow && terminalRow.status !== "error") {
      sendJson(ws, { type: "ack", clientMsgId, sessionKey });
