...
const createSendService = ({
  ...
  const sendSendFailure = (
    ...
    sendJson(ws, {
      ...
      notSubmitted: classified.notSubmitted === true,
      message: classified.message,
    });
  };
  ...
  const finalizeAndEmit = (
    ...
    try {
      ...
      // The durable claim remains ambiguous. Never tell the browser that it
      // is safe to retry if the non-submission proof failed to reach disk.
      if (!record.runId) {
      ...
      notSubmitted = false;
      logTransition(record, "terminal:persistence-failed");
    }
  ...
  const replayTerminalFromRow = (ws, row) => {
    ...
    if (!row.runId && (status === "error" || status === "unknown")) {
      sendJson(ws, {
        type: "send-failed",
  ...
  const handleMessage = async ({ ws, payload }) => {
    ...
    // Dedupe (D4): a live record re-acks (and re-announces the run); a retained
    ...
    if (existing) {
      ...
      // live stream too. A socket that is NEW to this record has seen none of
      // the run's output — replay the buffer (resume semantics), or it renders
      // an empty bubble until the run ends and history refreshes. An
      // already-attached socket just gets a re-announce; its cursor must not
      // regress, so the synthetic started carries the run's current seq.
      const isNewSocket = !existing.sockets.has(ws);
      registry.attachSocket(existing, ws);
      sendJson(ws, { type: "ack", clientMsgId, sessionKey });
      if (existing.runId) {
        if (isNewSocket && existing.replay.length) {
          for (const entry of existing.replay) sendJson(ws, entry.framed);
          if (existing.replayGap) sendJson(ws, { type: "desync", sessionKey });
        } else {
          sendJson(ws, {
            type: "started",
            clientMsgId,
    ...
    const failAdmission = (code, notSubmitted = false) => sendSendFailure(
      ws, { legacy, clientMsgId, sessionKey, messageId }, {
        code, notSubmitted, retryable: notSubmitted,
        message: code === "unknown_outcome" ? kUnknownOutcomeMessage :
          code === "too_many_pending" ? "Too many chat runs are pending. Try again after a run finishes." :
          "Chat storage is unavailable. This message was not sent. Try again shortly.",
      },
    );
    const replayOrUnknown = (row) => {
      if (row && kTerminalStatuses.includes(row.status)) {
        sendJson(ws, { type: "ack", clientMsgId, sessionKey });
        replayTerminalFromRow(ws, row);
      } else failAdmission("unknown_outcome");
    };
    let previous;
    try {
      previous = store.findRun({ sessionKey, clientMsgId });
    } catch {
      failAdmission("unknown_outcome");
      return;
    }
    const safelyRejected = previous?.status === "error" && previous.submissionState === "not_submitted";
    if (previous && !safelyRejected) {
      replayOrUnknown(previous);
      return;
    }
    if (!previous && payload.retry === true) {
      failAdmission("unknown_outcome");
      return;
    }
    ...
    const activeRecord = registry.getActiveForSession(sessionKey);
    if (activeRecord) {
      // Attach the waiting sender to the active run: it sees the live stream
      // AND the run's terminal — so an interrupted ending reaches this tab
      // and holds its queued turn instead of auto-flushing into a possibly
      // still-live run.
      registry.attachSocket(activeRecord, ws);
      sendSendFailure(
        ws,
        { legacy, clientMsgId, sessionKey, messageId },
        {
    ...
    if (registry.countForBrowser(ws) >= kMaxLiveRecordsPerBrowser ||
        registry.stats().live >= kMaxLiveRecordsGlobal) {
      sendSendFailure(
        ws,
        { legacy, clientMsgId, sessionKey, messageId },
        {
    ...
    // Claim before registry creation or any await. This must durably replace
    // safe-error evidence before another chat.send can leave this process.
    try {
      const admission = store.claimSend({ sessionKey, clientMsgId, messageId, retry: payload.retry === true });
      if (admission?.claimed !== true) {
        if (admission?.reason === "too_many_pending") failAdmission("too_many_pending", true);
        else replayOrUnknown(admission?.row);
        return;
      }
    } catch {
      failAdmission(payload.retry === true ? "unknown_outcome" : "storage_unavailable", payload.retry !== true);
      return;
    }
    ...
    try {
      ...
      // The pending claim is already durable and forbids redispatch. Keep
      // routing the known live run; terminal persistence may recover.
      logTransition(record, "running:persistence-failed");
    }
  ...
  const handleStop = async ({ ws, payload }) => {
    ...
    if (requestedRunId && (!record || record.sessionKey !== sessionKey)) {
      // The targeted run is already gone (finalized before this stop landed).
      // It IS stopped — settle the client without touching the session's
      // CURRENT run: a late stop for run A must never abort run B.
      sendJson(ws, {
        type: "done",
        sessionKey,
    ...
    if (record.state === "stopping") {
      // A stop is already in flight for this record — re-acknowledge instead
      // of firing a second abort: overlapping aborts can interleave with the
      // first one's failure rollback (state flips back to "running") and a
      // racing lifecycle end would then record the user's stop as a natural
      // completion.
      sendControl(record, {
        type: "stopping",
        sessionKey: record.sessionKey,
    ...
    try { store.markStopRequested({
      sessionKey: record.sessionKey,
      clientMsgId: record.clientMsgId,
    }); } catch { logTransition(record, "stop:persistence-failed"); }
  ...
  const handleGatewayEvent = (eventPayload = {}) => {
    ...
    if (eventName === "agent") {
      ...
      if (stream === "lifecycle" && String(data?.phase || "") === "end") {
        // A session-key-routed lifecycle end can belong to a FOREIGN run
        // finishing during our send window — finalizing our still-pending
        // record would fail the send and then orphan-abort our own run when
        // the RPC resolves. Only STARTED records take lifecycle terminals.
  ...
  const handleResume = ({ ws, payload }) => {
    ...
    // — the client refetches history to fill the hole. A cursor ABOVE the
    // run's own lastSeq is stale too (seq is per-run; the client's
    // per-session cursor can carry over from an earlier, longer run): frames
    // may have been missed, so the client must reconcile, never trust an
    // empty replay as complete.
    const gap =
      afterSeq > record.lastSeq
        ? true
        : firstSeq !== null
          ? firstSeq > afterSeq + 1
          : record.lastSeq > afterSeq;
    sendJson(ws, {
      type: "resumed",
      sessionKey,
      runId,
