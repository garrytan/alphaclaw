...
// Event-loop lag monitor. Sampled in fixed windows (not on read) so the
// sustained-lag warning fires even when nobody is polling the resources
// endpoint. Values are from the most recently completed window.
...
const nsToMs = (value) =>
  Number.isFinite(value) ? Math.round((value / 1e6) * 10) / 10 : null;

const sampleLoopLagWindow = () => {
  const { histogram } = loopLagState;
  if (!histogram) return;
...
const startLoopLagMonitor = () => {
  if (loopLagState.histogram) return;
  try {
    const { monitorEventLoopDelay } = require("perf_hooks");
    loopLagState.histogram = monitorEventLoopDelay({ resolution: 20 });
    loopLagState.histogram.enable();
...
module.exports = { getSystemResources, startLoopLagMonitor };
