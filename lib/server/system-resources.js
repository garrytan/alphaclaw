...
// failure mode behind every slow-UI complaint. Sampled continuously, read and
// reset per poll so each reading covers the window since the last one.
...
const readEventLoopLag = ({ warn = console.warn } = {}) => {
  const meanMs = nsToMs(eventLoopHistogram.mean);
  const p99Ms = nsToMs(eventLoopHistogram.percentile(99));
  const maxMs = nsToMs(eventLoopHistogram.max);
...
const getSystemResources = ({ gatewayPid = null } = {}) => {
  ...
  return {
    ...
    eventLoop: readEventLoopLag(),
  };
};

module.exports = { getSystemResources, readEventLoopLag };
