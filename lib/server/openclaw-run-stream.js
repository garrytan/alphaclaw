  ...
  const runStreamed = ({
    ...
    new Promise((resolve) => {
      ...
      const handleChunk = (streamName) => (data) => {
        ...
        if (outputCountsAsProgress) lastActivityAt = Date.now();
        appendTail(chunk);
      ...
      if (Number.isFinite(inactivityTimeoutMs) && inactivityTimeoutMs > 0) {
        ...
        const readProbe = () => {
          ...
          if (value && typeof value === "object") {
            if (value.phase === "verify" && verificationStartedAt === null && verificationTimeoutMs > 0) {
              verificationStartedAt = Date.now();
            }
            value = Number.isFinite(value.bytes) ? value.bytes : null;
          }
          if (value !== null && (outputCountsAsProgress ? value !== lastProbeValue :
              Number.isFinite(value) && value > (lastProbeValue ?? 0))) {
        ...
        const check = () => {
          ...
          if (verificationStartedAt !== null) {
            const left = verificationTimeoutMs - (Date.now() - verificationStartedAt);
            if (left <= 0) {
              timedOut = true;
            ...
            inactivityTimer = setTimeout(check, Math.min(pollMs, left));
