...
// Every queued op can cost seconds of CLI spawns — an unbounded queue lets a
// stuck client retry loop back up every future gateway operation. Far above
// any legitimate concurrent-operator burst.
const kDefaultMaxQueueDepth = 50;

const createLifecycleLock = ({ maxQueueDepth = kDefaultMaxQueueDepth } = {}) => {
  const state = {
  ...
  const run = (name, fn) => {
    ...
    if (state.queue.length >= maxQueueDepth) {
      return Promise.reject(
        new Error(
          `Lifecycle queue full (${state.queue.length} operations pending) — retry shortly`,
        ),
      );
    }
    let resolve;
