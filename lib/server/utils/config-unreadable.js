...
const kConfigUnreadableCodes = Object.freeze(
  new Set([
    ...
    "MANAGED_UPDATE_ATTEMPT_UNREADABLE",
  ]),
);
...
const kDefaultFileByCode = Object.freeze({
  ...
  MANAGED_UPDATE_ATTEMPT_UNREADABLE: "managed-update-attempt.json",
});
...
const configUnreadableEnvelope = (error) => {
  ...
  if (error?.code === "AUTH_STORE_UNREADABLE") {
    return {
      ok: false,
      error: `The auth store (${file}) is unavailable or has an unsupported format. AlphaClaw refused to rewrite it.`,
      code: "config_unreadable",
      hint: "Retry after any backup or OpenClaw update finishes. If this persists, use Doctor to diagnose the auth store or restore a verified backup; do not edit SQLite as JSON.",
      file,
