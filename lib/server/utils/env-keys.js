...
// Acyclic: env.js and its dependencies do not require this module.
...
const kEnvVarsReservedForUserInput = new Set([
  ...
  // Managed alongside its four siblings above (issue #25): a user-set value
  // in .env would fight the state-dir resolution alphaclaw exports for every
  // process — and a wrong one silently builds a second state database.
  "OPENCLAW_STATE_DIR",
  "XDG_CONFIG_HOME",
]);

const kReservedUserEnvVarKeys = Array.from(
  new Set([...kSystemVars, ...kEnvVarsReservedForUserInput, ...kDeploymentOnlyEnvKeys]),
);
const kReservedUserEnvVarKeySet = new Set(kReservedUserEnvVarKeys);
...
const kAgentProtectedEnvKeys = new Set([
  ...
  // The local rescue-session keys reconfigure the same one-click launcher —
  // CLAUDE_CODE_LOCAL_CWD is the sharpest (the target dir's .claude/ hooks
  // load into sessions and spawn pre-seeds trust for it), and AUTOSTART /
  // SPAWN_ON_INCIDENT cause autonomous spawns with no operator click at all.
...
const isReservedUserEnvVar = (key) =>
  kReservedUserEnvVarKeySet.has(normalizeEnvKey(key));
