...
const tailBytes = (filePath, maxBytes, { throwOnError = false } = {}) => {
  ...
  try {
    ...
    if (throwOnError && error.code !== "ENOENT") throw error;
...
const tailLines = (filePath, maxBytes, options) => {
  const { text, truncated } = tailBytes(filePath, maxBytes, options);
  if (!text) return [];
