...
// Cheap capacity re-read for the watchdog's live-resize check. Compares only
// the fields a platform resize changes — never detectedAt or GPU enrichment
// (or every tick would look like a resize). Never throws.
const hasCapacityChanged = () => {
  try {
    const profile = getMachineProfile();
    const next = computeProfile();
    return (
      next.memory.limitBytes !== profile.memory.limitBytes ||
      next.cpu.cores !== profile.cpu.cores ||
      (next.disk.totalBytes ?? null) !== (profile.disk.totalBytes ?? null)
    );
  } catch {
    return false;
  }
