  ...
  state.gpuEnrichment = new Promise((resolve) => {
    ...
    try {
      execFileFn(
        ...
        {
          encoding: "utf8",
          timeout: kGpuProbeTimeoutMs,
          // SIGKILL, not the default SIGTERM: a wedged probe that traps
          // SIGTERM would outlive its timeout and accumulate across reapplies.
          killSignal: "SIGKILL",
          maxBuffer: 64 * 1024,
          // Minimal env: the probe needs PATH resolution only, never the
          // supervisor's secret-bearing environment.
...
const readCurrentCapacity = () => {
  ...
  return {
    ...
    // The cgroup memory read failed and a host value was substituted. On a
    // box whose memoized profile IS cgroup-sourced, that substitution is a
    // transient read failure wearing a resize costume — callers must not
    // confirm a resize from it.
    degraded: memory.limitBytes == null,
  };
};
