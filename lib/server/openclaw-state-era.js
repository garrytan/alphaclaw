...
// once-per-process warning — never silently. While the state-DB quiet period
// holds (a backup is snapshotting the state dir) the db is not opened at all
// and the same `fallback` is returned — every caller already handles it as
// "unavailable".
const withReadonlyStateDb = ({ fsModule = fs, openclawDir, logger = console }, key, fn, fallback) => {
  if (isStateDbQuiet()) return fallback;
  try {
    ...
    const db = openTrackedReadonlyDatabase(databasePath);
    try {
      return fn(db);
    } finally {
      db.close();
...
// transient lock, or the state-DB quiet period) is distinct from "legacy":
// readers may fall back to the agent db, writers must fail closed.
const readAuthSharedStoreLocation = ({ fsModule = fs, openclawDir, logger = console } = {}) => {
  if (isStateDbQuiet()) return "unreadable";
  try {
    const databasePath = resolveOpenclawStateDbPath({ openclawDir });
    if (typeof fsModule.existsSync !== "function" || !fsModule.existsSync(databasePath)) {
      return "legacy";
    ...
    try {
      ...
      const schema = tableSchemaState(db, "config_machine_state", ["state_key", "value_json"]);
      if (schema === "mismatch") return "unreadable";
      if (schema === "absent") {
        return Number(db.prepare("PRAGMA user_version").get().user_version) >= 12 ? "unreadable" : "legacy";
      }
      const row = db
      ...
      if (value?.location === "state-db") return "state-db";
      if (value?.location === "legacy" || value?.location === "legacy-main") return "legacy";
      return "unreadable";
    } finally {
      db.close();
    }
    ...
    warnOnce(
      ...
      "[alphaclaw] could not read the auth.sharedStore machine state — auth is unavailable until the state database can be read",
    );
    return "unreadable";
  }
};
...
// Pairing mutations are direct state-db writes: while the quiet barrier
// holds they THROW (not { ok:false }) so routes can map the named class to
// 409 backup_in_progress instead of a generic retry-later 503.
