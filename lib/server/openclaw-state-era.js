...
const deletePairingRequestByCode = ({
  ...
  try {
    ...
    if (!tableHasColumns(opened.db, "channel_pairing_requests", kPairingRequestColumns)) {
...
const deleteChannelPairingRows = ({
  ...
  try {
    ...
    if (tableHasColumns(opened.db, "channel_pairing_allow_entries", kAllowEntryColumns)) {
      const statement = normalizedAccountId
        ? opened.db.prepare(
