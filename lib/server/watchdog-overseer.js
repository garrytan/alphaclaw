...
const buildIncidentPrompt = ({
  ...
  return [
    ...
    JSON.stringify(statusNow || null, null, 2),
    "",
...
const createWatchdogOverseer = ({
  ...
  const runReviewFor = async (incidentInput, { manual = false } = {}) => {
    ...
    // Model OUTPUT through the redactor too — a verdict can echo a secret it
    // saw in the (already-redacted-but-defense-in-depth) evidence.
    const parsed = parseVerdict(redactor.scrub(String(result.tail || "")));
  ...
  const requestReview = async ({ incidentId = null } = {}) => {
    ...
    try {
      ...
      return { ok: !result.failed && !result.skipped, ...result };
    } catch (error) {
