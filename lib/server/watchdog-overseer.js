...
const sanitizeVerdictText = (value, maxLength = 500) => {
  ...
  // auto-linkify plain http(s) — a prompt-injected "visit https://evil"
  // must not land as a live clickable link. The notification's own deep
  // link is server-built, never model text.
  text = text.replace(/\bhttps?:\/\//gi, "hxxp://");
  ...
  const trustedRecord = {
    ...
    statusSnapshot: summary.statusSnapshot
      ? {
          ...
          // Every gateway-echoed string leaves the trusted tier: probe
          // reasons AND /readyz component names / channel names are parsed
          // verbatim from gateway responses.
          degradedReason: undefined,
          readyzFailing: undefined,
          suppressedChannels: undefined,
        }
      : null,
...
const createWatchdogOverseer = ({
  ...
  const spawnIsolation = () => {
    const home = ensureOverseerHome();
    return { env: buildOverseerEnv(), cwd: home };
  };
  ...
  const runReviewFor = async (incidentInput, { manual = false } = {}) => {
    ...
    const scrubbedTail = scrub(String(result.tail || ""));
