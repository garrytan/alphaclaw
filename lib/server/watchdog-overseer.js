...
const createWatchdogOverseer = ({
  ...
  const runReviewFor = async (incidentInput, { manual = false } = {}) => {
    ...
    if (!reviewedNow || reviewedNow.status !== incident.status) {
      ...
      // Y3: a new outage began mid-spawn. An informational verdict about the
      // previous incident is noise now; a critical-class incident or an
      // actionable verdict (action_needed, or any concrete action — the same
      // predicate the quiet-mode class uses below) is still handed to the
      // outbox, which owns delivery retries.
