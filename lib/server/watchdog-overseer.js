const { overseerNotificationTiming } = require("./notification-expiry");
const fs = require("fs");
const os = require("os");
...
const { kGatewayCrashCauses } = require("./gateway-crash-cause");
// Closed readiness enums for the trusted status projection (#87). watchdog.js
// never requires this module (only server.js does), so there is no cycle.
const { kReadinessStatuses, kReadinessProbeKinds } = require("./watchdog");
const {
  createSituationSlot,
  projectRecordForApi,
  kStalePendingMs,
...
// floor between reviews. Manual reviews run in any watchdog state and bypass
// the floor but never the mutex or the availability/redaction gates; a manual
// incident re-review resets the floor, a situation report never touches it.
const kGlobalFloorMs = 10 * 60 * 1000;
const kEventQuietMs = 60 * 1000;
// Automatic-review admission (#87): a settled incident older than this is a
// recap, not a review — it is marked `skipped` (reason `stale`) instead of
// spawning a model call and paging the operator hours after the fact.
// Critical-class incidents (isCriticalClassIncident) ignore the bound.
const kAutoReviewMaxAgeMs = 60 * 60 * 1000;
...
// notifyOutcome `suppressed:<reason>` echoes the notifier's reason enum; cap
// it so a future free-text reason can never bloat the persisted record.
const kNotifyOutcomeReasonMaxChars = 64;
...
// --- admission + notification class (#87, one predicate for both) -----------
//
// "Critical class" is decided ONCE here and used by admission
// (classifyAutoReviewEligibility) and by the quiet-mode split at notify time,
// so an incident admitted as critical can never be silenced as informational.
// Reads slim list rows (lib/server/db/watchdog/index.js mapIncidentRow) and
// pre-column fixtures that only carry `summary.severity` / `summary.cause`.
// Incident keys mirror kCriticalEventTypes in watchdog-incidents.js — every
// critical event type that OPENS an incident (has a kIncidentKeyByTrigger
// entry) is listed here, and the overseer test pins that. `auto_repair_paused`
// is deliberately absent: it never opens an incident of its own, it escalates
// the open incident's severity to critical, which the first clause of
// isCriticalClassIncident already catches.
const kCriticalClassIncidentKeys = new Set([
  "crash_loop",
  "config_error",
  "channel_rollback",
  "version_mismatch",
...
const summaryOf = (incident) =>
  incident?.summary && typeof incident.summary === "object" ? incident.summary : null;

const isCriticalClassIncident = (incident) => {
  if (!incident || typeof incident !== "object") return false;
  const summary = summaryOf(incident);
  if ((incident.severity ?? summary?.severity) === "critical") return true;
  if (kCriticalClassIncidentKeys.has(incident.incidentKey)) return true;
  return (incident.cause?.cause ?? summary?.cause?.cause) === "oom";
};

// "Actionable" verdict content (#87 C3/Y3, one predicate for the quiet-mode
// class and the mid-outage delivery gate): the model asked for action, or
// named a concrete one. The label alone ("resolved" vs "monitoring") never
// decides anything.
const isActionableVerdict = (parsed) =>
  !!parsed &&
  (parsed.verdict === "action_needed" ||
    (typeof parsed.action === "string" && parsed.action !== "none"));

// Deterministic admission gate evaluated BEFORE any model call. Pure over a
// settled incident row: { eligible, reason } with reason one of
//   recovered_no_action — resolved, outcome recovered, an EXPLICIT empty
//                         `actions` array (the watchdog did nothing; there
//                         is nothing to review)
//   invalid_resolved_at — the settle time cannot be parsed (age unknowable)
//   stale               — settled more than maxAgeMs ago (a recap, not a review)
// A null/absent summary cannot prove "no action" → eligible unless stale; nor
// can a summary whose `actions` is missing or not an array (#87 G7) — absent
// history is not proof of no action.
// Critical-class incidents are always eligible. `abandoned` rows follow the
// same rules (their outcome is never `recovered`).
// The skip-reason vocabulary is exported: the overseer card's
// kOverseerSkipReasonCopy must carry a line for every reason (drift-guarded
// in tests/frontend/watchdog-overseer-card.test.js).
// Bound of the failed-skip-write memory (markSkipped, #87 G8).
const kSkipMarkedMaxEntries = 500;
const kAutoReviewSkipReasons = Object.freeze([
  "recovered_no_action",
  "invalid_resolved_at",
  "stale",
]);
const [kSkipRecoveredNoAction, kSkipInvalidResolvedAt, kSkipStale] = kAutoReviewSkipReasons;
const classifyAutoReviewEligibility = (
  incident,
  nowMs,
  { maxAgeMs = kAutoReviewMaxAgeMs } = {},
) => {
  if (isCriticalClassIncident(incident)) return { eligible: true, reason: null };
  const summary = summaryOf(incident);
  if (
    ...
    return { eligible: false, reason: kSkipRecoveredNoAction };
  }
  const resolvedAtMs = Date.parse(incident?.resolvedAt || "");
  if (!Number.isFinite(resolvedAtMs)) return { eligible: false, reason: kSkipInvalidResolvedAt };
  if (nowMs - resolvedAtMs > maxAgeMs) return { eligible: false, reason: kSkipStale };
...
// The crash classifier's closed cause enum (#76 A3). The watchdog reads the
// cause OUT of stderr, but only the enum VALUE rides the trusted tier — the
// matched line and detail are gateway text and stay in the event rows.
const kTrustedCrashCauses = new Set(kGatewayCrashCauses);
const kTrustedReadinessStatuses = new Set(kReadinessStatuses);
const kTrustedReadinessProbeKinds = new Set(kReadinessProbeKinds);
...
const pickTrustedStatus = (s) =>
  s && typeof s === "object"
    ? {
        ...
        // Serving identity + readiness axis (v0.9.75): numbers/enums the
        // watchdog computes. readinessReason (gateway-echoed component names)
        // deliberately stays OUT of the trusted tier.
        servingPid: s.servingPid ?? null,
        supervisionMode: s.supervisionMode ?? null,
        readiness: s.readiness ?? null,
        // #87: what the last /readyz body said about its phase and how the
        // last read went — closed enums from watchdog.js; anything else
        // (a smuggled string) reads null.
        readinessStatus: pickClosedEnum(s.readinessStatus, kTrustedReadinessStatuses),
        readinessProbe: pickClosedEnum(s.readinessProbe, kTrustedReadinessProbeKinds),
        safeMode: s.safeMode,
        degradedSince: s.degradedSince,
        lastExit: s.lastExit
          ? {
              code: s.lastExit.code,
              signal: s.lastExit.signal,
              at: s.lastExit.at,
              // The classified cause (closed enum, #76 A3) and whether a fact
              // gathered independently of stderr corroborated it: true /
              // false once the async corroboration settled, null while it is
              // pending or when the cause has no corroborator.
              cause: pickClosedEnum(s.lastExit.cause, kTrustedCrashCauses),
              corroborated:
                typeof s.lastExit.corroborated === "boolean"
                  ? s.lastExit.corroborated
                  : null,
            }
          : null,
...
// "budget" = the operator's watchdog.memory.budgetMb was the tightest cap (issue #56).
const kTrustedCapSources = new Set(["heap", "container", "derived_group_budget", "budget", "none"]);
const kTrustedContainerStates = new Set(["unknown", "normal", "critical", "disabled"]);
const kTrustedMemorySampleStatuses = new Set(["fresh", "stale", "partial", "unavailable"]);
const kTrustedContainerEpisodeReasons = new Set(["recovered", "detection_disabled"]);
const kTrustedMemoryTriggers = new Set(["trend", "fast_pressure"]);
const { projectMemoryEvidence } = require("./gateway-memory/evidence");
...
const pickContainerEpisodeId = (value) => typeof value === "string" && /^container-\d{1,16}$/.test(value) ? value : null;
const pickNonnegativeNumber = (value) => Number.isFinite(value) && value >= 0 ? value : null;
const pickTrustedContainerMemory = (value) => value && typeof value === "object" ? {
  state: pickClosedEnum(value.state, kTrustedContainerStates),
  since: pickIsoTimestamp(value.since),
  usedBytes: pickNonnegativeNumber(value.usedBytes),
  limitBytes: Number.isFinite(value.limitBytes) && value.limitBytes > 0 ? value.limitBytes : null,
  pressureFraction: pickNonnegativeNumber(value.pressureFraction),
  sampledAt: pickIsoTimestamp(value.sampledAt),
  sampleStatus: pickClosedEnum(value.sampleStatus, kTrustedMemorySampleStatuses),
  episodeId: pickContainerEpisodeId(value.episodeId),
  lastEpisodeSummary: value.lastEpisodeSummary && typeof value.lastEpisodeSummary === "object" ? {
    episodeId: pickContainerEpisodeId(value.lastEpisodeSummary.episodeId),
    endedAt: pickIsoTimestamp(value.lastEpisodeSummary.endedAt),
    reason: pickClosedEnum(value.lastEpisodeSummary.reason, kTrustedContainerEpisodeReasons),
  } : null,
} : null;
const pickTrustedMemoryTrend = (trend) =>
  trend && typeof trend === "object"
    ? {
        ...
        projectedExhaustionAt: pickIsoTimestamp(trend.projectedBudgetCrossingAt) ?? pickIsoTimestamp(trend.projectedExhaustionAt),
        projectedBudgetCrossingAt: pickIsoTimestamp(trend.projectedBudgetCrossingAt) ?? pickIsoTimestamp(trend.projectedExhaustionAt),
        episodeId: pickEpisodeId(trend.episodeId),
        ...(trend.sampleStatus !== undefined ? {
          sampleStatus: pickClosedEnum(trend.sampleStatus, kTrustedMemorySampleStatuses),
          sampledAt: pickIsoTimestamp(trend.sampledAt),
          groupPressureFraction: pickNonnegativeNumber(trend.groupPressureFraction),
          containerPressureFraction: pickNonnegativeNumber(trend.containerPressureFraction),
        } : {}),
        ...(trend.container ? { container: pickTrustedContainerMemory(trend.container) } : {}),
        ...(trend.evidence ? { scope: "gateway_group", evidence: projectMemoryEvidence(trend.evidence) } : {}),
        ...(trend.pressureSource ? { pressureSource: pickClosedEnum(trend.pressureSource, kTrustedCapSources) } : {}),
        lastEpisodeSummary:
          trend.lastEpisodeSummary && typeof trend.lastEpisodeSummary === "object"
            ? {
                ...
                ...(trend.lastEpisodeSummary.evidence ? { evidence: projectMemoryEvidence(trend.lastEpisodeSummary.evidence) } : {}),
                ...(trend.lastEpisodeSummary.capSource ? {
                  effectiveCapMb: pickFiniteNumber(trend.lastEpisodeSummary.effectiveCapMb),
                  capSource: pickClosedEnum(trend.lastEpisodeSummary.capSource, kTrustedCapSources),
                  pressureSource: pickClosedEnum(trend.lastEpisodeSummary.pressureSource, kTrustedCapSources),
                  trigger: pickClosedEnum(trend.lastEpisodeSummary.trigger, kTrustedMemoryTriggers),
                  groupPressureFraction: pickNonnegativeNumber(trend.lastEpisodeSummary.groupPressureFraction),
                  containerPressureFraction: pickNonnegativeNumber(trend.lastEpisodeSummary.containerPressureFraction),
                  projectedBudgetCrossingAt: pickIsoTimestamp(trend.lastEpisodeSummary.projectedBudgetCrossingAt),
                } : {}),
              }
            : null,
      }
...
const createWatchdogOverseer = ({
  ...
  autoReviewMaxAgeMs = kAutoReviewMaxAgeMs,
  skipMarkedMaxEntries = kSkipMarkedMaxEntries,
} = {}) => {
  ...
  const persistOverseer = (incident, current, { supersede = false } = {}) => {
    try {
      ...
      // Stable incident notification ids keep their first delivery lifetime
      // through pending stamps and manual/history rotations.
      const priorTimings = [existing.current, ...existing.history].filter(
        (entry) => entry && Object.prototype.hasOwnProperty.call(entry, "notifyExpiresAt"),
      );
      if (priorTimings.length) {
        const previous = priorTimings.map((entry) => overseerNotificationTiming(entry, nowFn()))
          .reduce((earliest, timing) => ({ createdAt: Math.min(earliest.createdAt, timing.createdAt),
            expiresAt: Math.min(earliest.expiresAt, timing.expiresAt) }));
        const next = Object.prototype.hasOwnProperty.call(current, "notifyExpiresAt")
          ? overseerNotificationTiming(current, nowFn()) : previous;
        current.notifyCreatedAt = Math.min(previous.createdAt, next.createdAt);
        current.notifyExpiresAt = Math.min(previous.expiresAt, next.expiresAt);
      }
      const history =
  ...
  // Ineligible rows get ONE `skipped` marker write per process. A persisted
  // marker is terminal for automatic review through the `record.current`
  // check below — the durable record, not this set, remembers a SUCCESSFUL
  // skip (#87 G8). Only a FAILED write (read-only db mid-incident) is
  // remembered here, so the 60s tick neither retries the write nor repeats
  // the one log line persistOverseer wrote — the next process gets one more
  // attempt (#87 decision 2.2). Bounded (insertion order, oldest evicted) so
  // a long-lived process under a persistently failing db cannot grow it
  // without limit; an evicted id costs one more write attempt + log line.
  const skipMarkedBound =
    Number.isInteger(skipMarkedMaxEntries) && skipMarkedMaxEntries > 0
      ? skipMarkedMaxEntries
      : kSkipMarkedMaxEntries;
  const skipMarkedThisProcess = new Set();
  const markSkipped = (incident, reason, at) => {
    if (skipMarkedThisProcess.has(incident.id)) return;
    // No supersede: nothing was reviewed, so nothing rotates into history. A
    // later manual review supersedes the marker like any other current.
    // A stale `pending` current (pickEligibleIncident's retryablePending) is
    // replaced in place the same way — intentional: an attempt that never
    // settled is not a verdict worth keeping in the 3-slot history.
    const persisted = persistOverseer(incident, { state: "skipped", reason, manual: false, at });
    if (persisted) return;
    skipMarkedThisProcess.add(incident.id);
    while (skipMarkedThisProcess.size > skipMarkedBound) {
      skipMarkedThisProcess.delete(skipMarkedThisProcess.values().next().value);
    }
  };
  ...
  // Admission is deterministic and precedes the model: a settled row that
  // fails classifyAutoReviewEligibility is marked `skipped` and never spawns.
  const pickEligibleIncident = () => {
    ...
    for (const incident of settled) {
      ...
      const eligibility = classifyAutoReviewEligibility(incident, now, {
        maxAgeMs: autoReviewMaxAgeMs,
      });
      if (!eligibility.eligible) {
        markSkipped(incident, eligibility.reason, now);
        continue;
      }
      return incident;
    }
  ...
  const spawnAndParse = async ({ prompt, flags, scrub, verdicts, actions }) => {
    ...
    if (result.error || result.timedOut || !result.ok) {
      return {
        ...
        transcriptTail,
        // The spawn error is a runner-level message today, never process
        // output — scrub it anyway so that invariant is not load-bearing.
        summary: result.timedOut
          ? `The overseer review timed out (${Math.round(deadlineMs / 60000)} minute deadline).`
          : `The claude call failed${result.error ? `: ${scrub(String(result.error))}` : ""}.`,
      };
    }
  ...
  const runReviewFor = async (incidentInput, { manual = false } = {}) => {
    ...
    // Re-check before send (#87 C2): a spawn can run for minutes, and the
    // world the verdict describes may be gone. Re-read the incident; on a
    // read error fall back to the pre-spawn snapshot (decision 2.3 — a
    // transient db error must never drop a critical-class notice). The
    // decision is persisted on the record so the operator can see WHY a
    // verdict did or did not page them.
    let reviewedNow = incident;
    try {
      reviewedNow = incidentsDb.getIncidentById(incident.id);
    } catch (error) {
      reviewedNow = incident;
      log(
        `recheck read failed for incident #${incident.id} — using the pre-spawn snapshot: ${error.message}`,
      );
    }
    // notifyDecision enum: `manual | eligible | incident_changed |
    // ineligible_now | not_steady_state`. A manual review never notifies (the
    // operator is watching the card), so it records `manual` and skips the
    // eligibility recompute (#87 F9) — the other four describe why an
    // AUTOMATIC review did or did not page.
    let notifyDecision = manual ? "manual" : "eligible";
    if (!reviewedNow || reviewedNow.status !== incident.status) {
      if (!manual) notifyDecision = "incident_changed";
      reviewedNow = incident;
    } else if (!manual) {
      if (
        !classifyAutoReviewEligibility(reviewedNow, nowFn(), { maxAgeMs: autoReviewMaxAgeMs })
          .eligible
      ) {
        notifyDecision = "ineligible_now";
      } else if (
        !isHealthySteadyState() &&
        !isCriticalClassIncident(reviewedNow) &&
        !isActionableVerdict(parsed)
      ) {
        // Y3: a new outage began mid-spawn. An informational verdict about
        // the previous incident is noise now; a critical-class incident or
        // an actionable verdict (action_needed, or any concrete action — the
        // same predicate the quiet-mode class uses below) is still handed to
        // the outbox, which owns delivery retries.
    ...
    // Y5: the outcome is written from the notifier's ACTUAL return, after the
    // send — never assumed. Reviews that will not attempt a send say so now.
    const notifyTiming = willNotify
      ? overseerNotificationTiming(readOverseerRecord(incident).current, nowFn()) : null;
    const doneRecord = {
      state: stale ? "stale" : "done",
      ...(notifyTiming ? { notifyCreatedAt: notifyTiming.createdAt, notifyExpiresAt: notifyTiming.expiresAt } : {}),
      ...verdictRecord,
      manual,
      at: nowFn(),
      notifyDecision,
      ...(willNotify ? {} : { notifyOutcome: "not_attempted" }),
      transcriptTail,
    };
    ...
    // notifyOutcome enum (`sent | held | suppressed:<reason> | failed |
    // not_attempted`), mapped from the wired notifier's return shapes:
    //   { ok:false, skipped|suppressed:true, reason }  → suppressed:<reason>
    //     (policy declined it — quiet mode, disabled, dedupe; not a failure)
    //   { ok:true, held:true }                          → held (parked by the
    //     notifier, e.g. a direct send held behind a pending upgrade)
    //   { ok:true, queued:true, id } / any ok:true      → sent — "accepted by
    //     the notifier": queued to the durable outbox, which owns delivery
    //     and its retries; it is NOT a delivery receipt
    //   anything else, or a throw                       → failed
    let notifyOutcome = "failed";
    try {
      const delivery = await notify(message, {
        eventType: "overseer",
        id: `watchdog-overseer-${incident.id}`,
        createdAt: doneRecord.notifyCreatedAt,
        expiresAt: doneRecord.notifyExpiresAt,
        // Quiet-mode class is decided from the verdict's CONTENT and the
        // incident's class, never from the verdict label alone (#87 C3):
        // informational unless the model asks for action or the incident is
        // critical class — the same predicate that admitted it.
        verbose: !(isActionableVerdict(parsed) || isCriticalClassIncident(reviewedNow)),
      });
      if (delivery?.skipped || delivery?.suppressed) {
        notifyOutcome = `suppressed:${String(delivery.reason || "unknown").slice(
          0,
          kNotifyOutcomeReasonMaxChars,
        )}`;
      ...
      // notifyOutcome stays "failed"; the reason must still reach the log
      // (no silent catch — plan Error & Rescue Registry).
      log(`notify failed for incident #${incident.id}: ${error?.message || error}`);
    }
    // Same `at`, no supersede: this completes the record above — it is not a
    // new attempt, so the 3-slot history must not rotate.
    persistOverseer(incident, { ...doneRecord, notifyOutcome });
    return notifyOutcome === "sent" ? { ...outcome, notified: true } : outcome;
  };
...
module.exports = {
  ...
  kManualRateLimitMs,
  // #87: deterministic admission + the shared critical-class predicate.
  ...
  kSkipMarkedMaxEntries,
  // Exported for the trusted-projection tests: the status allowlist and the
  // memory-trend validator must provably drop smuggled strings/malformed enums.
  pickTrustedStatus,
