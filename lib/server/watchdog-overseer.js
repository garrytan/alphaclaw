...
// Incident keys mirror kCriticalEventTypes in watchdog-incidents.js (the
// pause event escalates severity, which the first clause already catches).
...
const classifyAutoReviewEligibility = (
  ...
  if (
    ...
    return { eligible: false, reason: "recovered_no_action" };
  }
  const resolvedAtMs = Date.parse(incident?.resolvedAt || "");
  if (!Number.isFinite(resolvedAtMs)) return { eligible: false, reason: "invalid_resolved_at" };
  if (nowMs - resolvedAtMs > maxAgeMs) return { eligible: false, reason: "stale" };
...
const createWatchdogOverseer = ({
  ...
  const runReviewFor = async (incidentInput, { manual = false } = {}) => {
    ...
    try {
      ...
      if (delivery?.ok === true) notifyOutcome = "sent";
      else if (delivery?.skipped) {
        notifyOutcome = `suppressed:${String(delivery.reason || "unknown").slice(0, 64)}`;
      }
    } catch (error) {
