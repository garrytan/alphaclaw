...
//   recovered_no_action — resolved, outcome recovered, zero actions (the
//                         watchdog did nothing; there is nothing to review)
...
// A null/absent summary cannot prove "no action" → eligible unless stale.
...
const createWatchdogOverseer = ({
  ...
  // check below; when the write fails (read-only db mid-incident) the id is
  // remembered here so the 60s tick neither retries the write nor repeats the
  // log line — the next process gets one more attempt (#87 decision 2.2).
  ...
  const markSkipped = (incident, reason, at) => {
    ...
    persistOverseer(incident, { state: "skipped", reason, manual: false, at });
  };
