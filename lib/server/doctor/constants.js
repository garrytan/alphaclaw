...
// Upstream doctor `checkId` values are scraped from noisy CLI stdout, so
// only STRICTLY structural identifiers may be interpolated into prompts or
// event rows (openclaw-doctor fixPrompt, watchdog readiness_advisory). Accepts
// every real 2026.9.3 id shape (`core/doctor/gateway-health`,
// `gateway.probe_auth_secretref_unavailable`, `config.secrets.*`); anything
// else falls back to a generic template / "unknown".
