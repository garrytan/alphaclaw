...
const createFingerprintClient = ({
  ...
  const ensureWorker = () => {
    ...
    worker.on("message", (message) => {
      ...
      clearTimeout(entry.timer);
      if (message.ok) {
        // A healthy round-trip proves the worker is sound. Without this
        // reset, 3 failures accumulated over the whole process lifetime
        // (e.g. sporadic scan timeouts weeks apart) would permanently brick
        // snapshot computation; the cap should only brake an actively
        // failing worker.
        state.respawnCount = 0;
        entry.resolve(message.snapshot);
      } else {
        entry.reject(new Error(message.error || "Fingerprint worker failed"));
      }
    });
