...
const kDoctorHardeningExtraPrefix = "hooks/bootstrap/";
...
const analyzeBootstrapContext = ({
  ...
  const extraFiles = extraFilePaths.map((relativePath) => {
    ...
    const fileState = readFileChars(workspaceRoot, normalizedPath);
    const injectable = allowedBasenames.has(path.posix.basename(normalizedPath));
    return {
      path: normalizedPath,
      kind: "extra",
      exists: fileState.exists,
      rawChars: fileState.chars,
      injectable,
      active: hooksEnabled && injectable,
      activeReason: !hooksEnabled
        ? "hook_disabled"
        : !injectable
          ? "invalid_basename"
          : "",
  ...
  const hardeningFiles = files.filter(
    (file) => file.kind === "extra" && file.path.startsWith(kDoctorHardeningExtraPrefix),
  );
...
const createBootstrapContextAnalyzer = ({
  ...
  const readConfigInputs = () => {
    ...
    const overrideNumber = (value, fallback) => {
    ...
    const value = {
      extraFilePaths,
      hooksEnabled,
      bootstrapMaxChars: overrideNumber(
        defaults.bootstrapMaxChars,
        kDoctorBootstrapMaxChars,
      ),
      bootstrapTotalMaxChars: overrideNumber(
