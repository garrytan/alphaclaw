  ...
  return {
    ...
    hardening: {
      ...
      files: hardeningFiles.map((file) => ({
        ...
        // Activation reason outranks budget reason: a file OpenClaw never
        // injects at all (hook disabled, rejected basename) must not surface
        // budget advice just because it is also over a cap.
        reason: file.activeReason || file.reason || "",
      })),
    },
  };
};
