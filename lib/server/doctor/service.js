...
const createDoctorService = ({
  ...
  const executeDoctorRun = async (runId) => {
    try {
      ...
      writeMetaValue("last_env_signature", buildEnvSignature());
  ...
  const buildFreshSourcedCards = () => {
    ...
    const bootstrapCards = attachBootstrapCardSources(
      buildBootstrapTruncationCards(bootstrapContext),
    );
  ...
  const autoRunTick = async () => {
    ...
    try {
      ...
      const envSignature = buildEnvSignature();
      ...
      // scan only when the latest run has not already reported it.
      const hardeningBad =
      ...
      if (hardeningBad) {
        ...
        hardeningNew = !latestCards.some(
          (card) =>
            card.sourceKey === "det:hardening" ||
            String(card.sourceKey || "").startsWith("boot:"),
        );
      }
      ...
      if (notify && result?.runId && outcome === "ran") {
        const findingsCount = getDoctorCardsByRunId(result.runId).length;
