...
// Evidence snippets come from AI `openclaw doctor` output or the /api/doctor/import
// rawOutput, i.e. untrusted paths. Cap the read so a huge in-workspace file
// can't exhaust memory / block the event loop (CX4).
const kMaxSnippetFileBytes = 2 * 1024 * 1024;
...
const readFileSnippet = (rootDir, relativePath, startLine, endLine) => {
  try {
    ...
    // Realpath-safe containment: the evidence path is untrusted, so a
    // `../../etc/passwd` or an in-workspace symlink must not escape the
    // workspace root (H6, reuses the shared H4 primitive).
    const contained = resolveContainedPath(
      path.resolve(root, String(relativePath || "")),
      root,
    );
    if (!contained.ok) return null;
    const fullPath = contained.absolutePath;
    const stats = fs.statSync(fullPath);
    // Regular files only: a FIFO would block forever, a huge file OOMs (CX4).
    if (!stats.isFile() || stats.size > kMaxSnippetFileBytes) return null;
