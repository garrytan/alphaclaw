...
// Snippets serve ≤ kMaxSnippetLines from the head region of a file — never
// buffer an unbounded file (or a runaway line) to produce one.
...
const readFileSnippet = (rootDir, relativePath, startLine, endLine, redact = (text) => text) => {
  try {
    ...
    const fullPath = path.resolve(resolvedRoot, requestedPath);
    if (!fullPath.startsWith(resolvedRoot + path.sep)) return null;
    // The lexical check above cannot see a symlink placed INSIDE the
    // agent-writable workspace that points outside it — resolve the real
    // path and require it to stay under the real root too.
    const realRoot = fs.realpathSync(resolvedRoot);
    const realPath = fs.realpathSync(fullPath);
    if (!realPath.startsWith(realRoot + path.sep)) return null;
    // Bounded read through an fd: refuse non-regular files (devices, FIFOs)
    // and read at most kSnippetReadMaxBytes instead of buffering the whole
    // file for a ≤ kMaxSnippetLines snippet.
    ...
    try {
      ...
      readCapped = stats.size > kSnippetReadMaxBytes;
      const readLength = Math.min(stats.size, kSnippetReadMaxBytes);
      const buffer = Buffer.allocUnsafe(readLength);
      let bytesRead = 0;
      while (bytesRead < readLength) {
        const chunk = fs.readSync(fd, buffer, bytesRead, readLength - bytesRead, bytesRead);
        if (chunk <= 0) break;
        bytesRead += chunk;
      }
    ...
    return {
      text: redact(lines.slice(start, cappedEnd).join("\n")),
      startLine: start + 1,
      endLine: start + (cappedEnd - start),
      // A byte-cap hit means lines beyond the read window are unavailable —
      // the snippet is truncated even when the requested range "fit".
      truncated: cappedEnd < end || readCapped,
