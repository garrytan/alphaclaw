...
const createDoctorService = ({
  ...
  // Last-known upstream doctor CLI availability ({status, reason, detail,
  // at, ...} or null) — surfaces "the doctor TOOL is broken" distinctly from
  // "zero findings" on the status API.
  ...
  const buildStatus = () => {
    ...
    const status = {
      ...
      stale,
      bootstrapContext,
      // Upstream doctor CLI availability (null until first classified run).
      openclawDoctorCli: (() => {
        try {
          return getDoctorAvailability?.() ?? null;
        } catch {
          return null;
        }
      })(),
  ...
  const executeDoctorRun = async (runId) => {
    try {
      ...
      if (bridgeResult.unavailable) {
        // A CLI that cannot start is DISTINCT from "zero findings" — the run
        // stays fail-soft, but the operator gets a greppable line and the
        // availability tracker (fed by the shared runner) carries the state.
        console.warn(
          `[doctor] openclaw doctor CLI unavailable (${bridgeResult.reason}${bridgeResult.detail ? `: ${bridgeResult.detail}` : ""}) — upstream findings skipped this run`,
        );
      }
  ...
  const buildFreshSourcedCards = async ({
    ...
    return filterDismissedSourcedCards(
      [...bootstrapCards, ...deterministicCards, ...dashboardTokenCards],
      dismissedKeys,
    );
  };
      ...
      const reuseEnrichment = (async () => {
        try {
          ...
          if (bridgePromise) {
            ...
            if (bridgeResult.unavailable) {
              console.warn(
                `[doctor] openclaw doctor CLI unavailable (${bridgeResult.reason}${bridgeResult.detail ? `: ${bridgeResult.detail}` : ""}) — upstream findings skipped this reuse run`,
              );
            }
            if (bridgeResult.droppedCount > 0) {
              console.warn(
