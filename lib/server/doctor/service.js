...
// Hard bound on the total bytes scanned to produce one snippet. A citation
// whose window cannot be completed within this bound yields no snippet (null)
// rather than an unbounded read on the import path; when the file itself is
// larger than the bound, totalFileLines is dropped instead of scanning to EOF
// just to count lines.
...
const readFileSnippet = (rootDir, relativePath, startLine, endLine, redact = (text) => text) => {
  try {
    ...
    // Bounded windowed read through an fd: refuse non-regular files (devices,
    // FIFOs), then stream kSnippetReadMaxBytes-sized chunks counting newlines
    // until the requested [start, cappedEnd) window is captured — a citation
    // deep in a multi-MB file still gets its real lines, but never more than
    // kSnippetScanMaxBytes are read for one snippet.
