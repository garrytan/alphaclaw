...
// (kStructuralCheckIdPattern lives in ./constants — shared with the watchdog
// advisory-findings helper.)
...
const { classifyFromRunStream } = require("./classify-doctor-cli");
...
const runOpenclawDoctorBridge = async ({ runLintJson, sanitize }) => {
  ...
  try {
    ...
    return {
      ...
      unavailable: true,
      reason: "spawn_failed",
      detail: String(error?.message || "spawn failed").slice(0, 300),
    };
  }
  // Classify instead of the old ad-hoc branches: a CLI that cannot START
  // (cli_error envelope, crash text, exit 2) is now DISTINCT from garbage
  // output — `unavailable: true` lets the service surface "upstream doctor
  // broken" instead of a silent zero-findings run. Shape contract: the
  // unavailable return keeps cards/droppedCount — BOTH consumers read them.
  const classification =
    result?.classification || classifyFromRunStream(result || {});
  ...
  if (classification.status === "unusable") {
    return { ok: false, cards: [], droppedCount: 0, reason: "unusable doctor output" };
  }
  const payload =
    classification.payload || extractDoctorPayload(result.stdout);
  if (!payload) {
    // usable/legacy: real output, but no structured findings to mint cards
    // from (the pinned stable's shape) — same fail-soft as unparseable.
