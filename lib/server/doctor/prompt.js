...
const renderContextInjectionBlock = ({
  ...
  const extraPaths = (bootstrapContext?.hardening?.files || []).map((file) => file.path);
  const activeExtraPaths = extraPaths.length
    ? extraPaths
    : (bootstrapContext?.files || [])
        .filter((file) => file.kind === "extra")
        .map((file) => file.path);
  const perFileMax = bootstrapContext?.bootstrapMaxChars || 20000;
  const totalMax = bootstrapContext?.bootstrapTotalMaxChars || 60000;
  const lines = [
    ...
    activeExtraPaths.length
      ? `- AlphaClaw additionally injects these extra bootstrap files (appended AFTER the root files): ${renderFileList(
          activeExtraPaths,
        )}.`
      : "- No extra bootstrap files are currently configured.",
    `- Budgets: ${perFileMax} chars per file and ${totalMax} chars total across all injected files. The total budget is spent in injection order — files late in the order (including AlphaClaw's extras) are truncated or skipped FIRST. A file with under 64 chars of remaining budget is skipped entirely.`,
