...
const { describeMemoryEvidence, describeMemoryBudget } = require("../gateway-memory/evidence");
const { buildContainerMemoryCard } = require("./container-memory-card");
...
// det:plugin-api-mismatch (issue #76 A4 doctor evidence): the plugin loader's
// refusal wording, verified against OpenClaw 2026.9.2 / 2026.7.1-2
// (gateway-crash-cause.js carries the same table). Matched over the latest
// PERSISTED doctor run (raw payload + error text) — never over live stderr,
// which the crash classifier owns. Version tokens are interpolated into the
// card only when they look like versions/ranges (the text is CLI-echoed and
// untrusted); anything else renders as "unknown".
const kPluginApiMismatchPattern =
  /plugin requires plugin API\s*>=\s*(\S+), but this host is (\S+)/i;
const kVersionTokenPattern = /^[A-Za-z0-9._+-]{1,40}$/;
const kPluginApiScanMaxChars = 256 * 1024;
// Default reader: the doctor db singleton (the accessor the doctor service
// itself uses). Lazy so this pure module never opens the db at load, and
// fail-open — an uninitialized db (tests, CLI) means "no evidence".
const defaultReadLatestDoctorRun = () => {
  try {
    return require("../db/doctor").getLatestDoctorRun();
  } catch {
    return null;
  }
};
const detectPluginApiMismatch = (readLatestDoctorRun) => {
  let run = null;
  try {
    run = typeof readLatestDoctorRun === "function" ? readLatestDoctorRun() : null;
  } catch {
    return null;
  }
  ...
  const rawText =
    typeof run.rawResult === "string"
      ? run.rawResult
      : run.rawResult && typeof run.rawResult === "object"
        ? (() => {
            try {
              return JSON.stringify(run.rawResult);
            } catch {
              return "";
            }
          })()
        : "";
  const haystack = [rawText, typeof run.error === "string" ? run.error : ""]
    .join("\n")
    .slice(0, kPluginApiScanMaxChars);
  const match = kPluginApiMismatchPattern.exec(haystack);
  if (!match) return null;
  const clean = (token) => String(token).replace(/[;,.)"'`]+$/, "");
  const requires = clean(match[1]);
  const host = clean(match[2]);
  return {
    requires: kVersionTokenPattern.test(requires) ? requires : null,
    host: kVersionTokenPattern.test(host) ? host : null,
    matchedLine: match[0],
...
// Files the fail-closed readers guard (fix wave PR 7). `consequence` and
// `recovery` are the operator copy; the probe itself never parses beyond
// JSON.parse for the error position.
const kConfigUnreadableProbes = Object.freeze([
  {
    root: "managed",
    relativePath: ".alphaclaw/managed-update-attempt.json",
    validate: require("../managed-update-attempts").isManagedAttemptDocument,
    consequence: "Managed deployment updates and attempt resolution are refused so an uncertain deployment cannot be submitted twice.",
    recovery: "Check the deployment provider before restoring this record. Do not delete it to bypass a pending deployment.",
  },
  {
    root: "managed",
    relativePath: "openclaw.json",
    consequence: "OpenClaw itself may still start (it accepts JSON5), but channel, model, team, and webhook edits are refused.",
    recovery: "If the file is intentionally JSON5, keep it — AlphaClaw only refuses to REWRITE it; OpenClaw reads it fine.",
  },
  {
    root: "managed",
    relativePath: "gogcli/state.json",
    consequence: "Google accounts and the Gmail push token cannot be saved (they are not lost; the file is left untouched).",
    recovery: "Reconnecting an account will succeed once the file parses again.",
  },
  {
    root: "managed",
    relativePath: "gogcli/config.json",
    consequence: "Gmail account→client mappings are not updated (gog's own keys stay untouched).",
    recovery: "Delete the file only if you are sure gog has no other settings in it; it is re-seeded on the next boot.",
  },
  {
    root: "managed",
    relativePath: "exec-approvals.json",
    consequence: "Exec allowlist edits from the dashboard are refused.",
    recovery: "The gateway keeps using whatever it last loaded.",
  },
  {
    root: "managed",
    relativePath: "cron/system-sync.json",
    consequence: "The hourly git-sync schedule cannot be changed from the dashboard.",
    recovery: "The installed cron entry keeps running as-is.",
  },
  {
    root: "workspace",
    relativePath: "topic-registry.json",
    consequence: "Telegram topic pins cannot be synced into openclaw.json (a sync would otherwise wipe them).",
    recovery: "A `topic-registry.json.bak` is written before migrations; prefer it over hand edits.",
  },
]);

const probeUnreadableJsonFile = (filePath, validate = null) => {
  let stat;
  try {
    stat = fs.statSync(filePath);
  } catch {
    return null; // missing is fine: every reader treats absence as empty state
  }
  ...
  try {
    ...
    return {
      ...
      reason: `unreadable (${error.code || error.message})`,
    };
  }
  try {
    const parsed = JSON.parse(raw);
    if (validate && !validate(parsed)) throw new Error("invalid managed update attempt schema");
...
const listBackupSiblings = (filePath) => {
  const base = path.basename(filePath);
  try {
    return fs
      .readdirSync(path.dirname(filePath))
      .filter((name) => name !== base && name.startsWith(base) && /\.bak(?:\.\d+)?$|\.bak-/.test(name))
      .sort()
      .slice(0, 5);
  } catch {
    return [];
...
const buildDeterministicCards = ({
  ...
  // Latest persisted doctor run (det:plugin-api-mismatch evidence). Defaults
  // to the doctor db accessor; tests inject a stub.
  readLatestDoctorRun = defaultReadLatestDoctorRun,
} = {}) => {
  const cards = [];
  ...
  // 6b. Config files AlphaClaw refuses to rewrite (fix wave PR 7). The
  //     fail-closed readers surface `config_unreadable` on writes; this card
  //     is the durable, operator-facing recovery path. It reads BYTES only —
  //     size, mtime, the JSON parse error position, and the list of backup
  //     siblings — and never tries to parse or repair the file itself.
  for (const probe of kConfigUnreadableProbes) {
    const root = probe.root === "workspace" ? workspaceRoot : managedRoot;
    if (!root) continue;
    const filePath = path.join(root, probe.relativePath);
    const report = probeUnreadableJsonFile(filePath, probe.validate);
    if (!report) continue;
    const backups = listBackupSiblings(filePath);
    addCard({
      sourceKey: `det:config-unreadable:${probe.relativePath}`,
      priority: "P1",
      category: "config",
      title: `${probe.relativePath} cannot be parsed — AlphaClaw refuses to rewrite it`,
      summary:
        `${probe.relativePath} exists (${report.sizeBytes} bytes, modified ${report.mtimeIso}) but is not JSON ` +
        `AlphaClaw can parse: ${report.reason}. ${probe.consequence} Every dashboard/agent action that would ` +
        `rewrite this file now answers \`config_unreadable\` instead of rebuilding it from defaults.`,
      recommendation:
        (backups.length
          ? `Restore a backup sibling (${backups.join(", ")}) or fix the JSON by hand`
          : "Fix the JSON by hand (compare against git history if the workspace is synced)") +
        `, then retry the action. ${probe.recovery}`,
      evidence: [
        {
          type: "text",
          text: sanitize(
            `${probe.relativePath}: ${report.sizeBytes} bytes, mtime ${report.mtimeIso}, ${report.reason}`,
            { maxChars: kDisplayTextMaxChars },
          ),
        },
        ...backups.map((name) => ({
          type: "text",
          text: sanitize(`backup candidate: ${name}`, { maxChars: kDisplayTextMaxChars }),
        })),
      ],
      targetPaths: probe.root === "workspace" ? [{ path: probe.relativePath }] : [],
      fixPrompt: probe.root === "workspace"
        ? `${probe.relativePath} is not valid JSON (${report.reason}). Repair the syntax in place without dropping any keys; do not recreate the file from scratch.`
        : undefined,
    });
  }
  ...
  const containerMemoryCard = buildContainerMemoryCard(memoryTrend?.container);
  if (containerMemoryCard) addCard(containerMemoryCard);
  ...
  if (memoryTrend && typeof memoryTrend === "object") {
    ...
    const statsLine = () => {
      const cap = memoryTrend.effectiveCapMb
        ? ` of its ${memoryTrend.effectiveCapMb}MB group RSS budget`
        : "";
      const slope =
        memoryTrend.slopeMbPerHour != null
          ? `, rising ~${memoryTrend.slopeMbPerHour} MB/h`
          : "";
      return `Gateway group RSS is ${memoryTrend.rssMb ?? "?"}MB${cap}${slope}. ${describeMemoryEvidence(memoryTrend.evidence)} ${describeMemoryBudget(memoryTrend)}`;
    };
    const buildLeakFixPrompt = () =>
      "Diagnose measured gateway process-group memory growth. " +
      "(1) Read `alphaclaw admin GET /api/watchdog/resources`: compare gateway worker RSS, " +
      "child-process RSS/count, gateway-reported main-thread heap/external memory, PSS and container usage. " +
      "Keep measurement scope, freshness and missing evidence explicit. RSS growth alone does not establish a V8 leak. " +
      "(2) Use the attribution and frozen episode evidence to choose the next investigation: " +
      "post-major-GC heap floors for possible retention, child count/RSS for process growth, " +
      "or co-sampled external memory. Session ownership/activity remains unknown unless independently available. " +
      "(3) Inspect relevant plugin, session and queue logs only after identifying the growing scope. " +
      "(4) Group RSS budgets are policy thresholds; do not raise the heap or kill child processes from an RSS sum. " +
      "(5) A Watchdog-tab restart can mitigate gateway group growth under pressure; preserve and continue the diagnosis.";
    if (memoryTrend.state === "critical" && episodeId) {
      addCard({
        sourceKey: `det:gateway-memory-leak-critical:${episodeId}`,
        priority: "P0",
        category: "workspace state",
        title: "Gateway group growth under critical memory pressure",
        summary:
          `${statsLine()} Observed group growth coincided with high group-budget or measured container pressure. Check sample availability before acting.`,
        recommendation:
          "Mitigate now (restart the gateway from the Watchdog tab, or enable auto-restart in Watchdog settings), " +
          "then diagnose the growing memory scope and preserve the evidence.",
        evidence: [
          {
            type: "text",
            text: `memory trend: ${statsLine()} projected group-budget crossing ${memoryTrend.projectedExhaustionAt || "unknown"} (episode ${episodeId})`,
          },
        ],
        targetPaths: [],
      ...
      addCard({
        ...
        priority: "P1",
        category: "workspace state",
        title: "Gateway process-group memory rising steadily",
        summary:
          `${statsLine()} Per-bucket RSS floors have risen for a sustained window — ` +
          "confirming group growth while attribution identifies the available explanation.",
        recommendation:
        ...
        evidence: [
          {
            type: "text",
            text: `memory trend: ${statsLine()}${memoryTrend.projectedExhaustionAt ? ` projected group-budget crossing ${memoryTrend.projectedExhaustionAt}` : ""} (episode ${episodeId})`,
          },
        ],
        targetPaths: [],
        fixPrompt: buildLeakFixPrompt(),
      ...
      addCard({
        ...
        priority: "P2",
        category: "workspace state",
        title: "A gateway memory-growth episode ended recently",
        summary:
          `A memory-growth episode ended at ${summary.endedAt} (peak ${summary.peakRssMb ?? "?"}MB` +
          ...
          `${describeMemoryEvidence(summary.evidence)} ${describeMemoryBudget(summary)}`,
        recommendation:
          "Watch the Resources card over the next hours; if the climb resumes, run the memory attribution runbook via " +
  ...
  // Plugin API mismatch (issue #76 A4): a plugin built for a newer plugin API
  // than the running OpenClaw build exposes is skipped by the loader — a
  // silently swapped or lagging tree (installedDiverged) is the usual reason.
  // Evidence is read from the latest persisted doctor run; the sourceKey
  // carries the host version so a dismissal of one mismatch never hides the
  // next build's.
  const pluginApi = detectPluginApiMismatch(readLatestDoctorRun);
  if (pluginApi) {
    const requires = pluginApi.requires || "unknown";
    const host = pluginApi.host || "unknown";
    addCard({
      sourceKey: `det:plugin-api-mismatch:${host}`,
      priority: "P1",
      category: "config",
      title: "A plugin requires a newer plugin API than this OpenClaw build exposes",
      summary:
        `The doctor output reports a plugin that requires plugin API >=${requires} while this host exposes ${host}; ` +
        "OpenClaw skips such a plugin at startup, so its channels or tools are silently missing. " +
        "This usually means the running OpenClaw build is not the build the release channel chose (a version mismatch).",
      recommendation:
        "Check the Upgrade page: if the installed version differs from the expected one, re-activate the expected build " +
        "(restart AlphaClaw); otherwise upgrade OpenClaw to a build whose plugin API satisfies the plugin, or remove the " +
        "plugin from openclaw.json (plugins.*).",
      evidence: [
        {
          type: "text",
          text: sanitize(pluginApi.matchedLine).slice(0, kDisplayTextMaxChars),
        },
        ...(pluginApi.runId
          ? [{ type: "text", text: `source: doctor run #${pluginApi.runId}` }]
          : []),
      ],
      targetPaths: [],
      fixPrompt:
        "Do not edit workspace files for this. Report to the operator: a plugin requires plugin API " +
        `>=${requires} but the running OpenClaw build exposes ${host}, so the plugin is skipped at startup. ` +
        "Suggest checking the Upgrade page for an installed-vs-expected version mismatch first (re-activate the " +
        "expected build), then upgrading OpenClaw or removing the plugin from openclaw.json plugins.*.",
    });
  }
...
module.exports = {
  ...
  detectPluginApiMismatch,
  kPluginApiMismatchPattern,
};
