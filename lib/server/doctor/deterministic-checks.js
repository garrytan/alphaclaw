...
const readSkillFrontmatter = (filePath) => {
  ...
  try {
    ...
    try {
      const buffer = Buffer.alloc(kSkillsScanMaxFileBytes);
  ...
  const walk = (dir, depth) => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (frontmatter) {
          found.push({
            location: path.relative(workspaceRoot, dir).split(path.sep).join("/"),
...
const buildDeterministicCards = ({
  ...
  if (hardeningState === "blocked" || hardeningState === "starved") {
    ...
    addCard({
      ...
      summary:
        hardeningState === "blocked"
          ? "AlphaClaw's hooks/bootstrap hardening file is configured but OpenClaw is not injecting it " +
            "(rejected basename or disabled hook) — the agent is running WITHOUT AlphaClaw's safety rules."
      ...
      recommendation:
        hardeningState === "blocked"
          ? "AlphaClaw re-syncs its hardening on every boot; if this persists after a restart, check that " +
            "hooks.internal is enabled in openclaw.json and that no external change renamed the extra file."
      ...
      // the fix is shrinking the unmanaged root files, never editing ours.
      fixPrompt:
