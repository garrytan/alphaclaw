...
const buildDeterministicCards = ({
  ...
  if (memoryTrend && typeof memoryTrend === "object") {
    const episodeId = /^\d+-\d+$/.test(String(memoryTrend.episodeId || ""))
    ...
    if (memoryTrend.state === "critical" && episodeId) {
      ...
      /^\d+-\d+$/.test(String(memoryTrend.lastEpisodeSummary.episodeId || "")) &&
      Number.isFinite(Date.parse(memoryTrend.lastEpisodeSummary.endedAt)) &&
      Date.now() - Date.parse(memoryTrend.lastEpisodeSummary.endedAt) <
        24 * 60 * 60 * 1000
    ) {
      ...
      addCard({
        ...
        summary:
          ...
          `${summary.reason === "process_exited" ? "ended by a gateway restart/exit" : "recovered on its own"}). ` +
