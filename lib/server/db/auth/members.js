...
// Unknown or disabled emails used to return before any scrypt work, so the
// login round-trip time revealed whether an address exists (fix wave F061).
// Verifying the submitted password against a fixed decoy row costs the same
// scrypt as a real member and never matches.
const kDecoyRow = (() => {
  const salt = crypto.randomBytes(16).toString("hex");
  return {
    password_salt: salt,
    password_hash: hashPassword(crypto.randomBytes(24).toString("hex"), salt),
    scrypt_params: JSON.stringify(kScryptParams),
  };
})();
const burnDecoyVerification = (password) => {
  try {
    verifyPassword(String(password || ""), kDecoyRow);
  } catch {}
};
...
const createMembersStore = ({ getDb, nowFn = Date.now } = {}) => {
  ...
  const verifyMemberPassword = ({ email, password }) => {
    ...
    if (!row || Number(row.disabled || 0) === 1) {
      burnDecoyVerification(password);
      return null;
    }
