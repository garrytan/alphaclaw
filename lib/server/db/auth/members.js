...
// and a strict-schema JSON key (gateway.auth identityScopes/allowUsers): no
// whitespace or control chars (blocks CRLF header injection), exactly one @,
// non-empty local + dotted domain, bounded length. The invite holder is the
// lowest-trust actor and controls this input at accept-invite (H2).
const kEmailPattern =
  /^[^\s@\x00-\x1f\x7f]+@[^\s@\x00-\x1f\x7f]+\.[^\s@\x00-\x1f\x7f]+$/;
const assertValidEmail = (email) => {
  const value = normalizeEmail(email);
  if (!value || value.length > 254 || !kEmailPattern.test(value)) {
