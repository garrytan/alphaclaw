...
const createSchema = (database) => {
  ...
  // Fix-dispatch observability: the delivery params attached to a card's
  // "Ask agent to fix" dispatch ({attached, replyChannel, replyTo,
  // replyAccountId, dispatchedAt, gatewayOk}) — written before the gateway
  // call so failed dispatches leave a record.
  ensureColumn(database, "doctor_cards", "fix_delivery_json", "TEXT");
  // Scan coverage forensics: the caps + stats the run's snapshot was built
  // under ({capsUsed, totalFiles, skippedLargeCount, ...}).
  ensureColumn(database, "doctor_runs", "scan_stats_json", "TEXT");
  database.exec(`
