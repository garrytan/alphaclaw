...
const attachRunCountsBulk = (runs) => {
  const withIds = runs.filter((run) => run?.id);
  if (!withIds.length) return runs.map((run) => (run ? { ...run, ...emptyRunCounts() } : run));
  const database = ensureDb();
  const placeholders = withIds.map((_, i) => `$id${i}`).join(", ");
  const params = Object.fromEntries(withIds.map((run, i) => [`$id${i}`, run.id]));
  const rows = database
    .prepare(`
      SELECT run_id, priority, status, COUNT(*) AS n
      FROM doctor_cards
  ...
  for (const row of rows) {
    ...
    if (!byRun.has(runId)) byRun.set(runId, emptyRunCounts());
    const counts = byRun.get(runId);
    const n = Number(row.n || 0);
    counts.cardCount += n;
    // Mirror toCardModel's normalization: null/empty fall back to P2/open;
    // unknown non-empty values count toward cardCount but no bucket.
    const priority = row.priority || kDoctorPriority.P2;
    const status = row.status || kDoctorCardStatus.open;
    if (counts.priorityCounts[priority] != null) counts.priorityCounts[priority] += n;
    if (counts.statusCounts[status] != null) counts.statusCounts[status] += n;
  }
  return runs.map((run) =>
    run ? { ...run, ...(byRun.get(run.id) || emptyRunCounts()) } : run,
  );
};
...
const listDoctorRunSummaries = ({ limit = kDoctorDefaultRunsLimit } = {}) => {
  ...
  const rows = database
    ...
    `)
    .all({ $limit: safeLimit });
  return attachRunCountsBulk(rows.map(toRunSummaryModel));
};
...
const getLatestCompletedRunSummary = () => {
  ...
  return summary ? attachRunCountsBulk([summary])[0] : null;
};

const getDoctorRunManifest = (id) => {
  const database = ensureDb();
  const row = database
    .prepare(`
    ...
    `)
    .get({ $id: Number(id || 0) });
  return row ? parseJsonText(row.workspace_manifest_json, null) : null;
};
