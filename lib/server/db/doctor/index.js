...
const toCardModel = (row) => {
  ...
  return {
    ...
    fixDelivery: parseJsonText(row.fix_delivery_json, null),
  };
};
...
const toRunModel = (row) => {
  ...
  return {
    ...
    scanStats: parseJsonText(row.scan_stats_json, null),
...
// hot path never JSON.parses the heavy per-run payloads. Must keep carrying
// contextProfile/openclawVersion: the service's fingerprint-reuse guard
// compares them on the latest completed SUMMARY.
...
const createDoctorRun = ({
  ...
  const result = database
    ...
    .run({
      ...
      $scan_stats_json: scanStats == null ? null : JSON.stringify(scanStats),
      $prompt_version: String(promptVersion || ""),
      $context_profile: String(contextProfile || ""),
      $openclaw_version: String(openclawVersion || ""),
      $reused_from_run_id: Number(reusedFromRunId || 0),
    });
  const runId = Number(result.lastInsertRowid || 0);
  // Same statement batch as the insert (WAL, single writer): retention can
  // never observe a half-created run. Prune errors are swallowed inside —
  // bounded-growth housekeeping must not fail run creation.
  pruneRunManifests(database);
  return runId;
};

// Retention-lite: multi-MB manifests only serve as the NEXT run's incremental
// baseline, so keep the newest 2 manifest-bearing rows plus — unconditionally
// — the latest completed run's manifest (baseline loading is status-filtered;
// a failed/running backoff sequence must never push the active baseline out
// of the window). Best-effort: a retention error never fails run creation.
const pruneRunManifests = (database) => {
  try {
    database
      .prepare(`
        UPDATE doctor_runs
        SET workspace_manifest_json = NULL
        WHERE workspace_manifest_json IS NOT NULL
          AND id NOT IN (
          ...
          AND id NOT IN (
            ...
            WHERE status = $completed_status
              AND workspace_manifest_json IS NOT NULL
            ORDER BY started_at DESC, id DESC
            LIMIT 1
          )
      `)
      .run({ $completed_status: kDoctorRunStatus.completed });
  } catch {
    // retention is bounded-growth housekeeping, not correctness
  }
};
  ...
  try {
    ...
    for (const card of cards) {
      stmt.run({
        ...
        $source: String(card?.source || "llm"),
        $source_key: String(card?.sourceKey || ""),
        // Reuse-run clones must not erase the dispatch record — the
        // failed-dispatch chip (and the trace of an in-flight fix) has to
        // survive a no-change scan cloning the card.
        $fix_delivery_json:
          card?.fixDelivery == null ? null : JSON.stringify(card.fixDelivery),
      });
    }
...
// Targeted read for sourced-card dismissal suppression: the cards table is
// never pruned and grows with every run (reuse runs clone cards), so callers
// must not load the whole table to extract a handful of keys.
const listDismissedDoctorSourceKeys = () => {
  const database = ensureDb();
  const rows = database
    .prepare(`
...
// Status hot path: workspace_manifest_json/raw_result_json can each be
// multi-MB per run — a 10-run listing through toRunModel costs ~100ms of
// synchronous JSON.parse on a 15k-file workspace. Summaries skip both JSON
// columns entirely and replace the per-run card query with one aggregate.
const kRunSummaryColumns = `
        id,
        started_at,
        completed_at,
        status,
...
// One aggregate query attaches card counts to a batch of summary models.
const attachRunCountsToSummaries = (runs) => {
...
const listDoctorRunSummaries = ({ limit = kDoctorDefaultRunsLimit } = {}) => {
  ...
  const rows = database
    .prepare(`
      SELECT ${kRunSummaryColumns}
      FROM doctor_runs
      ORDER BY started_at DESC
      LIMIT $limit
    `)
    .all({ $limit: safeLimit });
  return attachRunCountsToSummaries(rows.map(toRunSummaryModel));
};

// Lean baseline lookup for the doctor-run reuse check: one row, no JSON parse.
const getLatestCompletedRunSummary = () => {
  const database = ensureDb();
  const row = database
    .prepare(`
      SELECT ${kRunSummaryColumns}
      ...
      LIMIT 1
    `)
    .get({ $status: kDoctorRunStatus.completed });
  const summary = toRunSummaryModel(row);
  return summary ? attachRunCountsToSummaries([summary])[0] : null;
};
...
// v0.9.36 name for the same reader; server wiring imports this alias.
const getDoctorRunManifest = getDoctorRunWorkspaceManifest;
...
// When `runId` is provided, the write is conditional on the card still
// belonging to THAT dispatch (fix_run_id match): a stale in-flight dispatch
// settling after the operator reopened the card and started a new one must
// not overwrite the newer dispatch's audit record.
const setDoctorCardFixDelivery = ({ id, runId = null, delivery = null }) => {
  const database = ensureDb();
  const result = database
    .prepare(`
      UPDATE doctor_cards
      SET
        fix_delivery_json = $fix_delivery_json,
        updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now')
      WHERE id = $id
        AND ($run_id IS NULL OR fix_run_id = $run_id)
    `)
    .run({
      $id: Number(id || 0),
      $run_id: runId == null ? null : String(runId),
      $fix_delivery_json: delivery == null ? null : JSON.stringify(delivery),
    });
