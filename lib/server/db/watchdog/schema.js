...
//
// Additive columns (issue #76 A3/A4, pragma-guarded like incident_id):
//   cause_json  — the crash-cause record the tracker last stamped on the open
//                 incident ({ cause, fingerprint, corroborated, by, ... })
//   severity    — "warning" | "critical", written the moment the rollup
//                 escalates (crash_loop / version_mismatch / ≥ 3 identical
//                 crash fingerprints) so a restart mid-incident keeps it
...
const createSchema = (database) => {
  ...
  `);
  // Same pragma guard for the incident row's additive columns.
  const incidentColumns = database
    .prepare("SELECT name FROM pragma_table_info('watchdog_incidents')")
    .all()
    .map((row) => row.name);
  if (!incidentColumns.includes("cause_json")) {
    database.exec("ALTER TABLE watchdog_incidents ADD COLUMN cause_json TEXT;");
  }
  if (!incidentColumns.includes("severity")) {
    database.exec("ALTER TABLE watchdog_incidents ADD COLUMN severity TEXT;");
  }
  database.exec(`
