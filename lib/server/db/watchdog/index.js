...
const insertWatchdogEvent = ({
  ...
  // Outbox tombstones are bounded and may disappear before their source
  // review does. An already-expired source revived after pruning/restart
  // must not append another audit for the same notification. Do the check
  // inside this INSERT so two processes cannot race a read-then-write.
  const expiryId = eventType === "notification_expired" && source === "notify-outbox" &&
    typeof details?.id === "string" && details.id ? details.id : null;
  // The attempt file can replay a transition after process death between
  // this INSERT and its durable acknowledgement. Each state occurs once
  // per attempt; keep the replay idempotent while its audit is retained.
  const managedTransition = eventType === "managed_update" && source === "alphaclaw_update" &&
    typeof details?.attemptId === "string" && details.attemptId &&
  ...
  const stmt = prepareCached(`
    INSERT INTO watchdog_events (
      ...
      $correlation_id,
      $incident_id
    ${expiryId ? `WHERE NOT EXISTS (
      SELECT 1 FROM watchdog_events
      WHERE event_type = 'notification_expired' AND source = 'notify-outbox'
        AND json_valid(details) AND json_extract(details, '$.id') = $expiry_id
    )` : managedTransition ? `WHERE NOT EXISTS (
      SELECT 1 FROM watchdog_events
      WHERE event_type = 'managed_update' AND source = 'alphaclaw_update'
        AND json_valid(details)
        AND json_extract(details, '$.attemptId') = $attempt_id
        AND json_extract(details, '$.state') = $attempt_state
    )` : ""}
  `);
  const result = stmt.run({
    ...
    ...(expiryId ? { $expiry_id: expiryId } : {}),
    ...(managedTransition ? { $attempt_id: details.attemptId, $attempt_state: details.state } : {}),
  });
...
// Rollup severity vocabulary (watchdog-incidents.js is the writer).
const kIncidentSeverities = new Set(["warning", "critical"]);
...
const getOpenIncident = () => {
  ...
  const row = database
    .prepare(
      "SELECT id, incident_key, opened_at, severity, cause_json FROM watchdog_incidents WHERE status = 'open' LIMIT 1",
    )
    .get();
  return row
    ? {
        ...
        // Persisted escalation state (#76 A3/A4): an adopted orphan keeps the
        // severity a fingerprint escalation already wrote — event-type counts
        // alone cannot reconstruct it.
        severity: kIncidentSeverities.has(row.severity) ? row.severity : null,
        cause: safeParseJson(row.cause_json),
      }
    : null;
};
...
// Crash-cause record for an incident (issue #76 A3): the tracker writes the
// classifier's verdict ({ cause, fingerprint, corroborated, by, ... }) the
// moment a crash / crash_cause row carries one, so the incident list and the
// diagnose bundle can name WHY without replaying the event stream. null
// clears it.
const updateIncidentCause = (incidentId, causeJson) => {
  const database = ensureDb();
  const result = database
    .prepare("UPDATE watchdog_incidents SET cause_json = $cause WHERE id = $id")
    .run({
      $cause: causeJson == null ? null : JSON.stringify(causeJson),
      $id: Number(incidentId),
    });
  return Number(result.changes || 0) > 0;
};

// Persist the rollup's severity mid-incident (issue #76 A4): an escalation
// (crash_loop, version_mismatch, ≥ 3 identical crash fingerprints) must
// survive a server restart that abandons the row, and the close-time rollup
// reads it back. Rejects anything outside the vocabulary instead of storing
// free text.
const updateIncidentSeverity = (incidentId, severity) => {
  if (!kIncidentSeverities.has(severity)) {
    throw new TypeError(
      `updateIncidentSeverity: severity must be one of ${[...kIncidentSeverities].join(", ")}`,
    );
  }
  const database = ensureDb();
  const result = database
    .prepare("UPDATE watchdog_incidents SET severity = $severity WHERE id = $id")
    .run({ $severity: severity, $id: Number(incidentId) });
  return Number(result.changes || 0) > 0;
};

// The overseer's standalone situation report lives in watchdog_meta under one
// key. Reads are TAGGED (never a record-shaped sentinel) so callers can tell
// "nothing stored yet" from "stored blob is corrupt".
const kOverseerSituationKey = "overseer_situation";

const getOverseerSituation = () => {
  const database = ensureDb();
  const row = database
    .prepare("SELECT value_json FROM watchdog_meta WHERE key = $key")
    .get({ $key: kOverseerSituationKey });
  if (!row) return { ok: false, reason: "missing" };
  const parsed = safeParseJson(row.value_json);
  if (parsed == null) return { ok: false, reason: "missing" };
  if (typeof parsed !== "object" || parsed.unreadable) {
    return { ok: false, reason: "unreadable" };
  }
  return { ok: true, record: parsed };
};

const setOverseerSituation = (record) => {
  const database = ensureDb();
  const result = database
    .prepare(`
      INSERT INTO watchdog_meta (key, value_json, updated_at)
      VALUES ($key, $value, strftime('%Y-%m-%dT%H:%M:%fZ','now'))
      ON CONFLICT(key) DO UPDATE SET
        value_json = excluded.value_json,
        updated_at = excluded.updated_at
    `)
    .run({ $key: kOverseerSituationKey, $value: JSON.stringify(record) });
  return Number(result.changes || 0) > 0;
};
...
// duration and the overseer's log window stay truthful. Overseer audit events
// are excluded: a situation report taken during the outage is not part of
// the outage's own timeline of what the gateway did.
          ...
          resolved_at = COALESCE(
            (SELECT MAX(created_at) FROM watchdog_events
             WHERE watchdog_events.incident_id = watchdog_incidents.id
               AND watchdog_events.event_type != '${kOverseerReviewEventType}'),
            opened_at
          )
      WHERE status = 'open'
    `)
...
const mapIncidentRow = (row) => ({
  ...
  // #76 A3/A4 additive columns: null on rows written before them.
  cause: safeParseJson(row.cause_json),
  severity: kIncidentSeverities.has(row.severity) ? row.severity : null,
...
// Default: first N chronologically — the trigger story matters more than the
// tail for a settled incident, and the rollup carries the outcome. `order:
// "desc"` returns the LATEST N (newest first) for callers reading a live
// incident that may have accrued thousands of rows. totalCount lets the route
// emit an honest truncation marker either way.
const getIncidentEvents = (
  incidentId,
  { limit = kIncidentEventsMaxLimit, order = "asc" } = {},
) => {
  const database = ensureDb();
