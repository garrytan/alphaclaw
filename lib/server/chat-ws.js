...
const createChatWsService = ({
  ...
  const clearRunTargetsForBrowser = (ws) => {
    ...
    // No browser is waiting on any run, so buffered foreign-run events
    // (Telegram/Slack/cron) can never be delivered — drop them so they can't
    // accumulate for the process lifetime (H13).
  ...
  const handleGatewayEvent = (eventPayload = {}) => {
    ...
    const resolveTargetForPayload = () => {
      ...
      // delete the browser's own runTarget and truncate its real reply (H16).
      if (!runId && !sessionKey && runTargets.size === 1) {
  ...
  wss.on("connection", (ws) => {
    ...
    // server (C2). Clean up this browser's run targets and close quietly.
    ws.on("error", () => {
