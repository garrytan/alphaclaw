...
const runOnboardedBootSequence = async ({
  ...
  // Issue #79 (g) / Codex 18: the boot sweep of pre-update backup debris
  // (openclawChannelService.sweepBackupDebris({ mode: "boot" }) — every
  // `.tmp` regardless of age, superseded `.unverified` quarantines, stale
  // offline-copy staging dirs). Runs SYNCHRONOUSLY here, under the boot
  // lifecycle lock and before startGateway, right after the dangling-record
  // closers — not in the post-boot timer: applies are refused while booting
  // and a quiesced backup cannot take the lock, so nothing can be writing a
  // `.tmp` at this point. lib/server.js runs it only when the pidfile
  // decision found no live owner. Swallow-and-log like every step here.
  ...
  //                                  startGateway; an injected gate must
  ...
  let release = null;
  const bootOperation = typeof runBootNativeMaintenance === "function" || typeof reconcileBootConfig === "function"
    ? createRepairOperation({ signal, isCurrent: () => release?.isValid?.() !== false })
    : null;
  ...
  release =
    typeof acquireLifecycleLock === "function"
      ? await acquireLifecycleLock("boot", {
          ...
          ...(bootOperation ? { cleanup: bootOperation.cleanup } : {}),
        })
      : null;
  bootOperation?.start(kOpenclawReconcileLifecycleLeaseMs, release?.expiresAt ?? Infinity);
  let bootError = null;
  ...
  try {
    ...
    //   (1) dangling closers → (1b) backup-debris sweep (#79 (g), Codex 18) →
    //   (2) boot-report server phase →
    //   (3) reconcileInstalled → (4) compat gate → (5) reconcileBootConfig →
    //   [config ensure steps] → (6) finalizeBootReport → (7) startGateway
    ...
    // Right after the closers, still under the boot lock and before anything
    // can spawn a backup: the closed records' `.tmp` staging debris goes with
    // them (#79 (g)).
    await runBootStep("backup-debris sweep", sweepBackupDebrisAtBoot);
    ...
    if (typeof assessLaunchCompatibilityAtBoot === "function" && (compat?.hold || compat?.compatible !== true)) {
      gatewayHeld = true;
      console.warn(
    ...
    let reconcile = gatewayHeld ? {
      status: "held",
      reason: compat?.hold?.reason || (compat?.compatible === false ? "version_mismatch" : "state_db_unverified"),
      hold: compat?.hold ?? null,
    } : null;
    if (!gatewayHeld && typeof reconcileBootConfig === "function") {
      try {
        reconcile = (await bootOperation.runWriter(() => reconcileBootConfig({
          hold: release,
          operation: bootOperation,
          ...(typeof normalizeBootConfig === "function" ? { normalizeBootConfig: (options) => normalizeBootConfig({ ...options, hold: release }) } : {}),
        }))) ?? null;
        bootOperation.assertActive();
        if (reconcile?.status !== "ok") {
          gatewayHeld = true;
          reconcile = reconcile || { status: "held", reason: "boot_config_unverified" };
          console.warn(
    ...
    const checkBootOwnership = () => {
      if ((release?.isValid && !release.isValid()) || signal?.aborted || bootOperation?.signal.aborted) {
        gatewayHeld = true;
        reconcile = { status: "held", reason: "boot_lease_expired" };
      }
      return !gatewayHeld;
    };
    checkBootOwnership();
    if (!gatewayHeld && typeof runBootNativeMaintenance === "function") {
      if (compat?.compatible !== true || reconcile?.status !== "ok") {
        gatewayHeld = true;
        reconcile = { status: "held", reason: "boot_native_unadmitted" };
      } else {
        try {
          ...
          reconcile = { status: "held", reason: "boot_native_maintenance_failed" };
