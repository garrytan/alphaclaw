...
const resolveWhatsAppCredentialCandidatePaths = ({
  ...
  return [
    ...
    // Defense-in-depth (H5): accountId can also arrive config-derived (via
    // readPairedCountsByAccount), so drop any candidate that escapes the
    // credentials dir even if a caller skipped isValidChannelAccountId.
    const resolved = path.resolve(candidatePath);
    return (
      resolved === resolvedCredentialsDir ||
      resolved.startsWith(`${resolvedCredentialsDir}${path.sep}`)
    );
  });
};
...
// Fail-closed read (H10): these configs are read to be mutated and written
// back (bindings/agents/channels). readOpenclawConfig({fallback:{}}) swallowed
// a JSON5/$include/transient-EACCES config as {}, so the paired saveConfig then
// wiped every channel/binding/hook/auth. readOpenclawConfigForWrite returns {}
// only for a genuinely missing file (ENOENT) and throws OpenclawConfigReadError
// on an existing-but-unparseable one — the route handlers surface that as an
// explicit error instead of a silent data-destroying no-op.
...
// Reserved object-key names are never identifiers: both validators guard
// values that become openclaw.json keys and path segments (fix wave F084).
const kReservedIdentifierNames = new Set(["__proto__", "constructor", "prototype"]);

const isValidChannelAccountId = (value) => {
  const id = String(value || "").trim();
  return kChannelAccountIdPattern.test(id) && !kReservedIdentifierNames.has(id);
};
...
const readPairedCountsByAccount = ({
  ...
  const credentialsDir = resolveCredentialsDirPath({ OPENCLAW_DIR });
  // openclaw >= 2026.9.1-beta.1 imports the *-allowFrom.json files into the
  // state db and deletes them — UNION the sqlite entries with whatever files
  // remain (a per-source sum would double-count identical ids during the
  // brief import-overlap window). Entries are collected into per-account
  // sets across both sources, then counted once below.
  const pairedEntriesByAccount = new Map(
    Array.from(counts.keys()).map((accountId) => [accountId, new Set()]),
  );
  try {
    if (String(channelId || "").trim() !== "whatsapp") {
      ...
      for (const [accountId, entries] of sqliteEntries) {
        const target = pairedEntriesByAccount.get(accountId);
        if (!target) continue;
        for (const entry of entries) target.add(String(entry).trim());
      }
    }
  } catch {}
  try {
  ...
  for (const [accountId, entries] of pairedEntriesByAccount) {
    if (entries.size === 0) continue;
    counts.set(accountId, Number(counts.get(accountId) || 0) + entries.size);
  }
...
const isValidAgentId = (value) => {
  const id = String(value || "").trim();
  return kAgentIdPattern.test(id) && !kReservedIdentifierNames.has(id);
};
