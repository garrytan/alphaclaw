...
const { StateDbQuietError } = require("../state-db-quiet");
...
const createChannelsDomain = ({
  ...
  // account is gone. This is the FIRST mutation of every delete, strictly
  // before the CLI/config/env writes: the state-db write is the one that
  // fails closed while the quiet barrier holds (StateDbQuietError → 409
  // backup_in_progress), and a barrier that begins during the awaited CLI
  // call must find nothing left to refuse — otherwise the account would
  // vanish from openclaw.json with its allow entries still authorized, and
  // the operator's retry would 404. A failed clear for any other reason is
  // security-relevant and must be LOUD (the delete still proceeds).
  const clearChannelAccountPairingRows = ({ provider, accountId }) => {
    try {
      ...
      if (result?.ok === false) {
        console.error(
          `[alphaclaw] Could not clear ${provider}/${accountId || "default"} pairing rows from the state db: ${result.error} — the deleted account's allow entries may still be authorized; retry the delete`,
        );
      }
    } catch (error) {
      if (error instanceof StateDbQuietError) throw error;
      console.error(
        `[alphaclaw] Could not clear ${provider}/${accountId || "default"} pairing rows from the state db: ${error.message}`,
      );
    }
  };
  ...
  const deleteChannelAccount = async (input = {}) => {
    ...
    // First mutation, before anything the barrier does not gate (see the
    // helper): a held barrier aborts here with nothing changed.
    clearChannelAccountPairingRows({ provider, accountId });
    ...
    if (provider === "discord") {
      ...
      try {
        ...
        return { ok: true, gatewayRestartFailed: true };
      }
      return { ok: true };
    }
