...
const createChannelsDomain = ({
  ...
  const cleanupChannelAccountPairingFiles = ({ provider, accountId }) => {
    ...
    // legacy files below are deleted at gateway startup) — clear the deleted
    // account's rows too, or its allow entries stay authorized after the
    // account is gone. A failed clear is security-relevant and must be LOUD
    // (the file cleanup below still proceeds either way).
    try {
  ...
  const deleteChannelAccount = async (input = {}) => {
    ...
    // The delete ends by clearing the account's pairing rows out of the state
    // db; while the quiet barrier holds that write must fail closed BEFORE
    // the config/env mutations, or the account vanishes from openclaw.json
    // while its allow entries stay authorized in the db.
