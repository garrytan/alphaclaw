...
const createChannelsDomain = ({
  ...
  const clearChannelAccountPairingRows = ({ provider, accountId, attempt = 0 }) => {
    ...
    try {
      ...
      if (result?.ok === false) {
        console.error(
          `[alphaclaw] Could not clear ${label} pairing rows from the state db: ${result.error} — the deleted account's allow entries may still be authorized; retry the delete`,
        );
      }
      ...
      if (!(error instanceof StateDbQuietError)) {
        console.error(
          `[alphaclaw] Could not clear ${label} pairing rows from the state db: ${error.message}`,
        );
        return { deferred: false };
      }
  ...
  const deleteChannelAccount = async (input = {}) => {
    ...
    if (provider === "discord") {
      ...
      const deferredFields = pairingRows.deferred
