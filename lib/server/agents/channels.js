  ...
  const clearChannelAccountPairingRows = ({ provider, accountId, attempt = 0 }) => {
    ...
    try {
      ...
      if (result?.ok === false) {
        ...
        console.error(
          `[alphaclaw] SECURITY: could not clear ${label} pairing rows from the state db: ${reason} — the deleted account's allow entries are STILL authorized; clear them by hand or re-run the delete`,
        );
      ...
      if (attempt >= kPairingRowsDeferredRetryMax) {
        console.error(
          `[alphaclaw] SECURITY: gave up clearing ${label} pairing rows after ${attempt} deferred attempts (a backup barrier held every time) — the deleted account's allow entries are STILL authorized; clear them by hand or re-run the delete`,
        );
