...
const createBuzzSetup = ({
  ...
  // Returns { env, home } — home is a freshly-created isolated dir that the
  // caller MUST clean up. Throws if it cannot be created: running external
  // package code with a shared ($TMPDIR) or data-volume HOME would defeat the
  // isolation, so we refuse rather than silently fall back.
  ...
  const install = async () => {
    ...
    try {
      ...
      return {
        ok: false,
        code: "install_env_unavailable",
        message: "Could not prepare an isolated environment for the install.",
        hint: `Free space in the temp directory, then retry (${error?.message || "mkdtemp failed"}).`,
      };
    }
    let result;
    try {
      result = await runStream.runStreamed({
        command: "openclaw",
      ...
      // The isolated HOME is scratch space — never leave it on the volume.
      try {
        fs.rmSync(installEnv.home, { recursive: true, force: true });
      } catch {}
    }
    if (!result.ok) {
      return {
