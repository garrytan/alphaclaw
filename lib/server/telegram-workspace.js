  ...
  const { actionsChanged, totalTopics, maxConcurrent, subagentMaxConcurrent } =
    updateOpenclawConfig({
      ...
      mutate: (cfg) => {
        ...
        // Strict read (F085): a corrupt registry must abort the sync rather
        // than read as "no topics" and wipe the per-topic pins below.
        const registryGroup =
          typeof topicRegistry.getGroupStrict === "function"
            ? topicRegistry.getGroupStrict(groupId)
            : topicRegistry.getGroup(groupId);
        const registryTopics = registryGroup?.topics || {};
        const promptTopics = {};
        ...
        const activeTopics =
          typeof topicRegistry.getActiveTopicCountStrict === "function"
            ? topicRegistry.getActiveTopicCountStrict()
