...
const buildMigrationInventoryUnchecked = async ({
  ...
  const resolveSelector = (value, fallback) => remapBackupSourcePath(resolveBackupPath(value, { spawnEnv, fallback }), rootInfo);
  const protect = (file) => {
    let ancestor = file;
  ...
  const addFile = (file, kind = "file", options = {}) => {
    if (isBackupEnvPath(path.relative(root, file))) fail(file, "environment secrets cannot be archived", null, "BACKUP_SECRET_SOURCE");
    ...
    if (isBackupEnvPath(path.relative(root, identity))) fail(file, "environment secrets cannot be archived", null, "BACKUP_SECRET_SOURCE");
  ...
  const copyStore = async (directory) => {
    ...
    await visit(directory, async (file, entry) => {
      if (entry.name === ".env") {
        skipped.push({ kind: "secret", sourcePath: file, reason: "environment secrets are never archived" });
      } else if (entry.isDirectory()) await copyStore(file);
    ...
    try {
      ...
      if (table) {
        for (const row of database.prepare("SELECT agent_id, path FROM agent_databases").iterate()) {
          ...
          const target = remapBackupSourcePath(path.isAbsolute(row.path) ? path.resolve(row.path) : path.resolve(root, row.path), rootInfo);
