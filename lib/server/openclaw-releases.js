...
const createOpenclawReleasesService = ({
  ...
  const toReleaseRow = ({ entry, channel, distTags, npmVersions }) => ({
    ...
    ...(entry.flaggedPrerelease === true ? { flaggedPrerelease: true } : {}),
  ...
  // NOT a row (it cannot be installed), and neither is a dist-tag target that
  // is missing from `versions` (isKnownVersion would refuse its apply); a
  // version npm marks `deprecated` is skipped (D11) even when a dist-tag still
  // points at it. Channel = the version's own suffix ONLY (classifyPrerelease
  // — the rule isKnownVersion's channel scoping applies), so a row can never
  // land in a channel whose apply the server refuses; a GitHub `prerelease`
  // flag on a stable-shaped version is kept as `flaggedPrerelease` for the UI.
  const buildNpmEntries = (npmDoc, normalizedReleases) => {
    ...
    return Object.keys(versionsDoc)
      .filter((version) => !versionsDoc[version]?.deprecated)
      .map((version) => {
        ...
        return {
          ...
          flaggedPrerelease: release?.prerelease === true && !classifyPrerelease(version),
    ...
    try {
      ...
      const entries = npmHasVersions
        ...
        : (normalizedReleases || [])
            .map((entry) => ({
              ...entry,
              // Same channel rule as the npm rows: the version's own suffix.
              prerelease: classifyPrerelease(entry.version),
              flaggedPrerelease: entry.prerelease === true && !classifyPrerelease(entry.version),
            }))
            .sort(byVersionDesc);
  ...
  const getCatalog = async ({ forceRefresh = false } = {}) => {
    ...
    // `refreshed` means the ROW source (npm) actually answered this forced
    // read — a forced fetch that failed and fell back to the stale copy is
    // not a refresh, whatever the click intended (the UI says so).
    const report = (catalog) => ({
      ...catalog,
      refreshed:
        effectiveForce &&
        catalog?.ok !== false &&
        Boolean(catalog?.sources?.npm?.fetchedAt) &&
        catalog.sources.npm.stale !== true,
