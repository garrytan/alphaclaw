...
function* walkStateTreeSteps({
  ...
  const snapshot = (complete = false) => {
    ...
    return {
      complete, entries: diagnosing ? visited : entriesSeen, measuredEntries: measured, measurementComplete,
    ...
    try {
      for (const entry of directoryEntries(directory)) {
        ...
        if (entry.name === ".env") {
          ...
          else if (entry.isFile()) { total.bytes = fsModule.statSync(full).size; total.files = 1; noteBytes(total.bytes); }
        ...
        if (ruleIndex >= 0 && !entry.isSymbolicLink() && !required && !db && !sidecar) {
          ...
          if (isDirectory) yield* measure(full, total);
            ...
            catch (error) { total.complete = measurementComplete = false; if (diagnosing) throw error; }
            total.files = 1;
          }
