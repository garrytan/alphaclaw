...
// npm >= 11.16 gates lifecycle scripts behind an allowlist; the beta docs say
// to pass `--allow-scripts=openclaw` so npm itself runs the package's
// preinstall/postinstall (the manual fallback in verifyStagedLifecycle stays
// as the backstop). Older npm rejects the flag, so it is version-conditional.
let cachedAllowScriptsFlag; // undefined = not yet probed
const resolveNpmAllowScriptsFlag = ({ getNpmVersion } = {}) => {
  if (cachedAllowScriptsFlag !== undefined && !getNpmVersion) {
    return cachedAllowScriptsFlag;
  }
  let version = "";
  try {
    version = String(
      getNpmVersion
        ? getNpmVersion()
        : execSync("npm --version", { timeout: 15000 }),
    ).trim();
  } catch {
    version = "";
  }
  ...
  const flag =
    major > 11 || (major === 11 && minor >= 16)
      ...
      : "";
  if (!getNpmVersion) cachedAllowScriptsFlag = flag;
  return flag;
};
...
const installOpenclawVersionToTempDir = ({
  ...
  new Promise((resolve, reject) => {
    ...
    execImpl(
      `npm install --omit=dev --prefer-online --package-lock=false --install-strategy=nested${resolveNpmAllowScriptsFlag()}`,
