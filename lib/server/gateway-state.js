...
const getGatewayHoldCopy = (hold) => {
  if (!hold || typeof hold !== "object" || isMigrationClassHold(hold)) return kGatewayHoldCopy;
  return {
    ...kGatewayHoldCopy,
    disabledReason: "Gateway held because its recorded build or databases need recovery — see the Upgrade page.",
    restartRefusal: "The gateway is held for build or database recovery — inspect diagnostics and Re-activate recorded build on the Upgrade page.",
    repairRefusal: "Doctor repair is held until the recorded build and databases are compatible — inspect diagnostics on the Upgrade page.",
    hint: "Use Re-activate recorded build on the Upgrade page or run alphaclaw diagnose.",
  };
};
...
const kReconcileInstalledCopy = {
  ...
  applyInProgressRefusal:
    "A channel update or backup is in progress — wait for it to finish before re-activating the recorded build.",
  ...
  const blocked = operationActive
    ...
    : relaunchActive
      ...
      : gatewayHoldUnreadable
        ...
        : gatewayHeld
          ? { disabledReason: getGatewayHoldCopy(gatewayHeld).disabledReason }
          : {};
  switch (state) {
    case "not_onboarded":
...
const reasonForState = (state, { watchdog, bootPhase, tcp, now }) => {
  switch (state) {
    ...
    case "running": {
      ...
      // not degraded; say so instead of a bare green "Running".
