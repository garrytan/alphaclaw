...
const reduceGatewayState = ({
  ...
  // A watchdog-owned relaunch in flight: crash relaunch (lifecycle
  // "crashed"/"restarting"), medic, or the exit-78 config-change auto-retry.
  // They hold the lifecycle lock only until spawn (or not at all), so the
  // operation badge cannot be trusted to disable Restart for them.
