...
// (boot_failed carries Retry for the failure case). Two things can block the
// offered action, always WITH a reason: another lifecycle operation holding
// the lock (the badge already explains the wait), or a reconciler gateway
// hold after a failed settings migration (the Upgrade page owns the remedy;
// Repair is blocked too — doctor --fix would rewrite the held config). The
// route re-validates both once it holds the lock, so the card and the server
// never disagree for longer than one status tick.
...
const kLifecycleActionBlockReasons = {
  ...
  // Generic on purpose: a hold's remedy (retry migration, strip blamed keys
  // with consent, manual recovery) is chosen on the Upgrade page — the card
  // names the destination, not one remedy.
  gatewayHeld:
...
const actionsForState = (
  state,
  { operationActive, inStabilizationWindow, gatewayHeld = false },
) => {
  // Precedence is explicit: a live operation outranks a hold — the badge is
  // already explaining the wait, and the hold reason returns once it clears.
  ...
  switch (state) {
    ...
    case "starting":
      // Disabled (with reason) while a leased operation owns the launch;
      // enabled when the watchdog's own relaunch is what's in flight — the
      // route then serializes on the lifecycle lock rather than racing it.
      return [
        action("view_logs", "secondary"),
        action("restart", "secondary", blocked),
      ];
...
const reduceGatewayState = ({
  ...
  // is set, so the card must say so up front instead of 409ing on click.
  gatewayHeld = false,
  now = Date.now(),
}) => {
