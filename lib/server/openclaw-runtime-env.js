...
const resolveOpenclawRuntimeEnv = (env, { fsModule = fs } = {}) => {
  const resolved = { ...env };
  for (const key of ["OPENCLAW_STATE_DIR", "OPENCLAW_CONFIG_PATH", "XDG_CONFIG_HOME"]) {
    const value = resolved[key];
    if (typeof value !== "string" || !path.isAbsolute(value)) continue;
    try {
      resolved[key] = key === "OPENCLAW_CONFIG_PATH"
        ? path.join(fsModule.realpathSync(path.dirname(value)), path.basename(value))
        : fsModule.realpathSync(value);
    } catch {}
  }
  return Object.keys(resolved).some((key) => resolved[key] !== env[key]) ? resolved : env;
};
...
const withOpenclawStartupEnv = (env = process.env) => {
  ...
  // Child-only instrumentation. The data bootstrap catches missing/broken
  // package files; ensureOpenclawStartupEnv never mirrors NODE_OPTIONS back
  // into AlphaClaw itself. Keep the startup path fail-open.
  try {
    return require("./gateway-memory/telemetry-bootstrap").withGatewayTelemetryEnv(next);
  } catch {
    return next;
  }
};
