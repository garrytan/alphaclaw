...
const kServeRestartBaseDelayMs = 5000;
const kServeRestartMaxDelayMs = 5 * 60 * 1000;
const kServeHealthyUptimeMs = 60 * 1000;
...
const createGmailWatchService = ({
  ...
  const ensurePushToken = ({ state, forceRegenerate = false }) => {
    ...
    if (current.token && !forceRegenerate) {
      return { state, token: current.token, changed: false };
    }
    ...
    return { state: updated.state, token, changed: true };
  };
  ...
  let serviceRef = null;
  // Serve auto-restart bookkeeping (fix wave F205/F099): one pending timer per
  // account (cancelled on stop), exponential backoff for a child that keeps
  // dying, and a `stopping` latch so a drain that outlives a timer never
  // respawns `gog serve` into an orphan.
  const serveRestartTimers = new Map();
  const serveRestartAttempts = new Map();
  let stopping = false;
  const clearServeRestartTimers = () => {
    for (const timer of serveRestartTimers.values()) clearTimeout(timer);
    serveRestartTimers.clear();
  };
  const nextServeRestartDelayMs = (accountId, uptimeMs) => {
    // A child that lived past the healthy threshold resets the ladder.
    const attempts =
      uptimeMs >= kServeHealthyUptimeMs ? 0 : serveRestartAttempts.get(accountId) || 0;
    serveRestartAttempts.set(accountId, attempts + 1);
    return Math.min(kServeRestartBaseDelayMs * 2 ** attempts, kServeRestartMaxDelayMs);
  };
  const serveManager = createGmailServeManager({
    ...
    onServeExit: (payload) => {
      ...
      const delayMs = nextServeRestartDelayMs(accountId, uptimeMs);
      const detail = payload?.error
        ? `spawn error: ${payload.error}`
        : `code ${payload?.code ?? "null"}, signal ${payload?.signal ?? "null"}`;
      console.warn(
        `[alphaclaw] gog serve for ${payload?.email || accountId} exited after ${Math.round(uptimeMs / 1000)}s (${detail}); restarting in ${Math.round(delayMs / 1000)}s` +
          (payload?.stderrTail ? ` — stderr: ${String(payload.stderrTail).slice(-300)}` : ""),
      );
      const previous = serveRestartTimers.get(accountId);
      if (previous) clearTimeout(previous);
      const timer = setTimeout(async () => {
        serveRestartTimers.delete(accountId);
        if (stopping) return;
        try {
          ...
          // on a timer that can fire concurrently with a disconnect/callback.
          updateState((current) =>
            setAccountGmailWatch({
              state: current,
              accountId,
      ...
      timer.unref?.();
      serveRestartTimers.set(accountId, timer);
    },
  });
  ...
  const getConfig = ({ req }) => {
    ...
    // Persist only when a token was minted (fix wave F100): this runs on every
    // dashboard mount and agent call, and each save is a locked rewrite of
    // gogcli/state.json. The gog account→client mapping sync is cheap and
    // compares before writing, so it still runs on every read.
    if (ensuredPush.changed) saveState(state);
    else ensureAccountClientMappings({ state });
  ...
  const renewWatch = async ({ accountId = "", force = false }) => {
    ...
    for (const account of allTargets) {
      const watch = getAccountGmailWatch(account);
      // An explicit accountId used to bypass `enabled` (fix wave F101): with
      // the route's force=true default a renew re-ENABLED a stopped watch via
      // startWatch. Renewal never flips a watch on.
      if (!watch.enabled) {
        results.push({ accountId: account.id, skipped: true, reason: "disabled" });
        continue;
      }
      const shouldRenew =
        force ||
  ...
  const stop = async () => {
    // Latch first (F099): an exit fired by stopAll's SIGTERM must not schedule
    // a respawn, and a timer already pending must not fire into the drain.
    stopping = true;
    clearServeRestartTimers();
    if (renewalTimer) {
