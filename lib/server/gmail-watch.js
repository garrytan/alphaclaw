...
const { createGmailWatchLifecycle } = require("./gmail-watch-lifecycle");
...
const createGmailWatchService = ({
  ...
  const lifecycle = createGmailWatchLifecycle({
    constants, updateState, runGogForAccount,
    readState: () => readGoogleStateForWrite({ fs, statePath: constants.GOG_STATE_PATH }),
    prepareStart: (args) => prepareStart(args),
    parseExpiration: parseExpirationFromOutput,
  });
  const { serveManager } = lifecycle;
  ...
  const getConfig = ({ req }) => {
    ...
    const accounts = listGoogleAccounts(state).map((account) => {
      ...
      return {
        ...
        running: Boolean(serve?.running && !serve?.stopping),
        remoteOperation: watch.remoteOperation || null,
        disconnecting: lifecycle.isDisconnecting(account.id),
      };
    });
    return {
      ok: true,
  ...
  const prepareStart = ({ accountId, req, destination = null }) => {
    let state = readState();
    ...
    return { account, client, topicPath, webhookToken, pushToken: ensuredPush.token };
  };
  ...
  return {
    ...
    getConfig,
    saveClientConfig,
    startWatch: lifecycle.startWatch,
    stopWatch: lifecycle.stopWatch,
    renewWatch: lifecycle.renewWatch,
    disconnectAccount: lifecycle.disconnectAccount,
    getTargetByEmail,
    markPushReceived,
