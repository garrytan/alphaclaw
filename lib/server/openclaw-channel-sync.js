...
const createOpenclawChannelSync = ({
  ...
  const runBackup = async ({
    ...
    const finishSuccess = async (
      ...
      // still keeps its verified backup.
    ...
    const runOfflineCopy = async ({ reason, lastAttempt, stopEvidence, quietToken, stopConfirmed }) => {
      ...
      // AlphaClaw's own transient CLI shell-outs (`sessions list` polled by
      // the chat sidebar, a cron run, a `--help` capability probe), which the
      // quiet barrier does not gate. Re-sample briefly while the list is
      // non-empty so a child that is already exiting does not turn the
      // last-resort copy into a terminal 409; a holder that stays is refused
      // with its argv named. Bounded by poll COUNT, not the clock (nowFn is
      // frozen in tests), and by a quarter of the copy budget. The /proc/*/fd
      // holder scan inside createOfflineCopy stays the hard refusal.
      ...
      try {
        const liveProcesses = await settleLiveProcesses();
