...
const createOpenclawChannelSync = ({
  ...
  const stepRecorder = (operationId, sink = null, { mirrorLastUpdateRun = true } = {}) => {
    ...
    const persistSteps = () => {
      ...
      // pointer; a standalone backup (v0.9.81) records only into its own
      // ledger run and must never overwrite the last update's steps.
  ...
  const runBackup = async ({
    ...
    const finishFailure = async (attempt) => {
      ...
      if (hardGate) {
        ...
        const reuseEligible = gateReason !== "manual" && kReuseEligibleKinds.includes(classified.kind);
