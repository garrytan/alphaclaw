...
const createOpenclawChannelSync = ({
  ...
  const classifyBackupFailure = (
    ...
    if (result?.stalled) {
      ...
      return {
        kind: "stalled",
        message: `The pre-update backup CLI made no progress for ${windowText} (no output, nothing written) and was stopped.${quoteLastOutput()}`,
        hint: gateHint,
    ...
    if (result?.timedOut) {
      return {
        kind: "timeout",
        message: `The pre-update backup timed out after ${Math.round((result.timeoutMs || backupBudget.cliTimeoutMs) / 60000)} minutes.${quoteLastOutput()}`,
        hint: gateHint,
        stepError: "timed out",
      };
    }
  ...
  //     │               every upstream `backup create` (paused or live) runs
  //     │               under runStreamed's inactivity policy (v0.9.81, D15):
  //     │               no output AND no staging bytes for
  //     │               upstreamInactivityMs (3 min) → SIGTERM/SIGKILL →
  //     │               `stalled` (offline_copy in the quiesce, reuse- and
  //     │               consent-eligible, never retried), under the 10-min
  //     │               ceiling → `timeout`; the ticker line, both messages
  //     │               and run.backup.lastOutput carry the CLI's last 3
  //     │               redacted lines
  //     │                 "upstream" → 3.
  ...
  //     backup_required_for_migration. A verified failed run can offer a
  //     session-bound, one-use waiver after immutable target preparation.
  ...
  const runBackup = async ({
    ...
    const runBackupAttempt = async ({
      ...
      const stopProgress = startProgressTicker({
        rung: "upstream",
        quiesced,
        sample: () => ({ ...measureCliProgressBytes(outputFile), lastOutput: outputRing.last() }),
      });
      let raw;
      try {
        raw = await runner.runStreamed({
          ...
          progressProbe: () => measureCliProgressBytes(outputFile).doneBytes ?? null,
      ...
      // The record carries the LAST failed CLI attempt's output (a success
      // needs none); a later success clears it so the record never quotes a
      // failure the ladder recovered from.
      track.lastOutput = raw?.ok ? null : outputRing.lines();
    ...
    const runQuiescedBackup = async () => {
      ...
      try {
        ...
        if (typeof onQuiesceLeased === "function") await onQuiesceLeased(release);
        try {
  ...
  const backupRisk = createBackupRiskCoordinator({
    ...
    assertPolicy: ({ hold, intent, recoveryHold, strict }) => {
      ...
      if (selfUpdating) throw Object.assign(new Error("self update in progress"), { code: "self_update_in_progress" });
  ...
  const isBackupRiskEligible = (backup) => !backup.backupRiskBlocked && !backup.orphanedBackup && !backup.offlineCopy?.orphanedBackup &&
    backup.offlineCopy?.stage !== "exclusivity" &&
  ...
  const runStandaloneBackup = async ({ operationId = null } = {}) => {
      ...
      if (isSelfUpdateInProgress()) {
        return {
          ...
          body: channelError(
            ...
            "Wait for it to finish (AlphaClaw will restart), then run the backup.",
          ),
        };
      }
    } catch {}
    ...
    const { emit, updateDetail } = stepRecorder(operationId, sink);
    ...
    const finish = (status, body) => {
      ...
      if (!body.ok) {
        ...
        queueNotify(`✅ OpenClaw backup written: ${path.basename(body.archive.file)}.`, {
          eventType: "info",
          operationId,
    ...
    try {
      ...
      return finish(200, {
        ok: true,
        archive: { file, bytes, producer, verified, at, sha256, partial: partial === true },
      });
    } catch (error) {
  ...
  const applyUpdate = async ({
    ...
    try {
      ...
      // Intent belt (v0.9.81): judged against the EXECUTING build's version —
      // the v0.9.79 apply_source_changed guard keeps that true through commit.
      // A direct caller that sent no intent is recorded as such (the HTTP
      // route never lets that happen for stable/beta).
