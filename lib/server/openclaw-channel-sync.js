...
const { detectAgentsShape } = require("./openclaw-config-migrations");
const {
...
const createOpenclawChannelSync = ({
  ...
  // Returns { state, failed, error, errorTail, bakWritten, fromVersion }: the
  // caller (syncAtBoot) HARD-GATES the boot on `failed` (issue #21 bug 2)
  // instead of treating a failed migration as a warning while the new build
  // boots — and one-way migrates the state — anyway.
  const runBootConfigMigration = ({
    installDir,
    state,
    warnings,
    ...
    const installedVersion = installDir
      ? channelStore.readInstalledVersion({ installDir })
      : null;
    if (!installedVersion) return outcome;
    ...
    const recordAttempt = (ok, error) => {
      currentState = channelStore.updateState((s) => {
        const prev =
    ...
    // Crash-rollback restore (issue #21 bug 4): a rollback marker landed on
    // this version AND the config was migrated away from it since its pre-fix
    // backup was written (lastAttempt names a different version). Restore that
    // backup even though completedForVersion may already equal this version —
    // the early return below would otherwise skip the restore forever, which
    // is exactly the blind spot that left the #21 box unbootable.
    if (
    ...
    if (migration.completedForVersion === installedVersion) return outcome;
    ...
    // The backup must NEVER be named after the version being migrated TO — a
    // same-version .bak would read as a downgrade-restore candidate on the
    // next boot and silently disarm the failed-migration retry.
    const notSelf = (candidate) =>
      candidate && candidate !== installedVersion ? candidate : null;
    const fromVersion =
      migration.completedForVersion ||
      notSelf(previousInstalledVersion) ||
      notSelf(currentState.pinVersion) ||
      "unknown";
    outcome.fromVersion = fromVersion;
    try {
      fsModule.copyFileSync(
        configPath,
        path.join(openclawDir, `openclaw.json.pre-fix-${fromVersion}.bak`),
      );
      outcome.bakWritten = true;
      pruneConfigBackups();
    } catch (error) {
      // A silently-missing pre-fix backup removes the only cheap recovery
      // artifact (issue #21 bug 4) — surface the failure instead of `catch {}`.
      warnings.push(
        `could not save pre-fix config backup for ${fromVersion}: ${error.message}`,
      );
      queueNotify(
        `⚠️ Could not save the pre-migration OpenClaw settings backup (${error.message}). If this update fails, automatic settings recovery will be limited.`,
        {
          eventType: "health",
          id: `config-backup-write-failed-${fromVersion}`,
        },
      );
    }
    ...
    if (!bin) {
      ...
      recordAttempt(false, "binary-unresolved");
      outcome.failed = true;
      outcome.error = "binary-unresolved";
      return outcome;
    }
    // Timeout (issue #21 bug 1): env-tunable default, scaled by total state-DB
    // size (the #21 box had ~400MB and needed several minutes), capped below
    // the boot placeholder's 15-minute /health ceiling, and never past the
    // shared boot-ops budget.
    let sizeScaledMs = 0;
    try {
      const totalBytes = enumerateStateDbs().reduce((sum, dbPath) => {
        try {
          return sum + fsModule.statSync(dbPath).size;
        } catch {
          return sum;
        }
      }, 0);
      sizeScaledMs =
        Math.ceil(totalBytes / kOpenclawDoctorMigrationBytesPerSec) * 1000;
    } catch {}
    const timeout = Math.max(
      10_000,
      Math.min(
        kOpenclawDoctorMigrationTimeoutCapMs,
        Math.max(doctorMigrationTimeoutMs, sizeScaledMs),
        remainingBootOpsMs(),
      ),
    );
    try {
      // Never combine --fix with --json (beta rejects the combo).
      // SIGKILL on timeout: a SIGTERM-catching doctor must not linger while
      // the migration gate activates a different binary (SQLite WAL locks
      // self-release when the process dies).
      ...
      // Doctor output can echo config values — redact through the shared
      // utils/redact.js helper before it reaches state, warnings, or chat.
      let tail = stripAnsi(
        `${error?.stderr || ""}\n${error?.stdout || ""}`.trim(),
      ).slice(-400);
      try {
        tail = redactSecrets(tail, {
          secrets: collectSecretValues({ env: gatewayEnv() }),
        });
      } catch {}
      const errorText = tail ? `${detail} — ${tail}` : detail;
      warnings.push(
        `config migration (doctor --fix) failed for ${installedVersion}: ${errorText}`,
      );
      queueNotify(
        `⚠️ OpenClaw settings migration for ${installedVersion} did not complete (${detail}) — the gateway may need attention. See the Upgrade page.`,
        { eventType: "health", id: `config-migration-failed-${installedVersion}` },
      );
      recordAttempt(false, errorText);
      outcome.failed = true;
      outcome.error = detail;
      outcome.errorTail = tail || null;
    }
    return outcome;
  };
  ...
  const chooseBootRollbackTarget = ({ marker, state, installDir }) => {
    ...
    try {
      ...
      if (target.kind === "package" && target.version) {
        ...
        if (state.pinVersion && state.pinVersion !== target.version) {
          candidates.push({
            kind: "pin",
            version: state.pinVersion,
            bin: pinBin(),
      ...
      for (const candidate of candidates) {
        ...
        const verdict = prober.probeBin(candidate.bin);
        if (verdict === "block") {
          rejected.push({
            version: candidate.version,
  ...
  // stranding every older version. All-sync, and every internal failure
  // degrades to today's warn-and-continue: boot stays fail-open by contract.
  const abortFailedMigrationBoot = ({
    installDir,
    state,
    warnings,
    migration,
    ...
    try {
      if (
        ...
        warnings.push(
          "migration gate disabled (OPENCLAW_MIGRATION_GATE=off) — continuing on the new build",
        );
        return none;
      }
      const applied = state.applied;
      ...
      if (!bakPath || !fsModule.existsSync(bakPath)) {
        warnings.push(
          `migration gate skipped for ${installedVersion}: no restorable pre-fix settings backup — continuing on the new build`,
        );
        return none;
      }
      ...
      if (!revertVersion) {
        warnings.push(
          revertBlocked
            ? `migration gate: no revert target can read the current state — staying on ${installedVersion}`
            : `migration gate skipped for ${installedVersion}: no local revert target — continuing on the new build`,
        );
        return none;
      }
      ...
      warnings.push(
        `migration gate error (${error.message}) — continuing on the new build`,
      );
      return none;
    }
  };
  ...
  const syncAtBoot = () => {
    ...
    try {
      ...
      // Migrate the config to the just-activated version BEFORE the ledger
      // resolution (a gate-reverted boot must resolve its run as NOT
      // activated), before the mirror write, and before the gateway ever
      // starts on it — a beta strict-root rejection must never crash-loop the
      // first boot, and a FAILED migration must never let the new build run
      // and one-way migrate the state (issue #21 bug 2).
      const migrationOutcome = runBootConfigMigration({
        installDir,
        state,
        warnings,
        rollbackTargetVersion,
      ...
      if (migrationOutcome.failed) {
        const gate = abortFailedMigrationBoot({
          installDir,
          state,
          warnings,
        ...
        state = gate.state;
        if (gate.aborted) action = "migration_gate_reverted";
      }
      ...
      try {
        ...
        const bootFellBack =
          ...
          action === "overlay_missing" ||
          action === "dev_unavailable" ||
          action === "migration_gate_reverted";
  ...
  //   exit ≠0 ─ workspace-discovery error? ► retry ONCE with
  //   │         (CLI names the flag)         --no-include-workspace against a
  //   │                                      FRESH filename → record
  //   │                                      {…, partial: true} + loud notify
  //   └──────── classifyBackupFailure ────► honest message; own artifact only:
  ...
  const newestArchiveName = () => {
    try {
      ...
      return best ? path.join(backupsDir, best.name) : null;
    } catch {
      return null;
    }
  };
  ...
  const runBackup = async ({
    ...
    if (result.ok && hardGate && !artifactFile) {
      // Live-verified nuance: on a FRESH install (no state databases yet) the
      // real stable binary exits 0 without writing anything — there is
    ...
    cleanupFailedBackup(outputFile);
    // Workspace-discovery failure (issue #21 bug 6): a config-broken box
    // cannot enumerate custom workspaces, and the CLI itself names the escape
    // hatch. Retry ONCE with --no-include-workspace against a FRESH filename
    // (the CLI refuses to overwrite, and a partial first artifact would
    // collide). Config + state databases — the data a migration could lose —
    // are still included; only workspace files are skipped.
    if (
      /--no-include-workspace|cannot reliably discover .*workspaces/i.test(
        String(result.tail || ""),
      )
    ) {
      const partialOutputFile = buildBackupOutputFile(crypto.randomUUID());
      backupLog(
        `[openclaw-update] backup: workspace discovery failed — retrying once with --no-include-workspace → ${partialOutputFile}`,
      );
      const retry = await runner.runStreamed({
        command: "openclaw",
        args: [
          "backup",
          "create",
      ...
      const partialArtifact = retry.ok
        ? backupArtifactAt(partialOutputFile)
        : null;
      if (partialArtifact) {
        emit("backup", "warning", {
          detail:
            "workspace excluded — config too broken to discover workspaces",
        });
        await pruneBackups();
        const artifact = {
          at: backupStartedAt,
          dir: backupsDir,
          file: partialArtifact,
          verified: true,
          partial: true,
        };
        channelStore.updateState((s) => {
        ...
        return { ok: true, artifact, partial: true };
      }
      cleanupFailedBackup(partialOutputFile);
    }
  ...
  const runDatabasePreflight = async ({
    ...
    return { ok: true, unsupported: anyUnsupported };
  };
