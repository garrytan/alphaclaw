...
const createOpenclawChannelSync = ({
  ...
  const pruneBackups = async () => {
    ...
    // budget so the operator shrinks the keep count or adds disk.
    try {
      ...
      if (budgetBytes != null) {
        ...
        if (keptBytes > budgetBytes) {
          ...
          log(
            `backup retention warning: kept archives use ${gb(keptBytes)} of the ${gb(budgetBytes)} disk budget — reduce the keep count or add disk`,
          );
        }
      }
    } catch {}
