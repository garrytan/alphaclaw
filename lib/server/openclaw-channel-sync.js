...
const createOpenclawChannelSync = ({
  ...
  const reconcileOpenclawJsonMirror = (channel) => {
    try {
      ...
      // on, even before sign-in. Only beta/dev get a stripe; on beta/dev the active
      // OpenClaw supports gateway.controlUi.environment (implicit capability gate via
      // the channel). Stable removes any managed stripe.
      const desiredStripe = environmentStripeForChannel(channel);
      ...
      if (updateChanged || stripeChanged) {
        ...
        updateOpenclawConfig({
          ...
          mutate: (config) => {
            ...
            if (stripeChanged) applyEnvironmentStripe(config, desiredStripe);
          },
        });
        log(
  ...
  // be limited.
  const runBackup = async ({ emit, hardGate, gateReason = "downgrade" }) => {
    emit("backup", "running");
    ...
    // return success without writing anything. For a HARD-gated transition
    // (downgrade/dev/cross-channel, where an unrecoverable migration is the
    // risk) require a backup artifact that actually appeared during this run.
    const artifactWritten = () => {
      try {
        return fsModule
          ...
          .map((name) => {
            try {
              return fsModule.statSync(path.join(backupsDir, name)).mtimeMs;
            } catch {
              return 0;
            }
          })
          .some((mtimeMs) => mtimeMs >= backupStartedAt - 1000);
      } catch {
        return false;
      }
    };
    if (result.ok && hardGate && !artifactWritten()) {
      ...
      emit("backup", "failed", { error: "no backup artifact produced" });
      return channelError(
        "backup_failed",
        "The backup command reported success but produced no backup file.",
        "This OpenClaw build's backup is not trustworthy — fix it or choose a same-channel version before a migration-risky switch.",
      );
    }
    if (result.ok) {
      emit("backup", "completed");
      await pruneBackups();
      const artifact = { at: backupStartedAt, dir: backupsDir, verified: true };
      channelStore.updateState((s) => {
        s.backups = [
          artifact,
    ...
    if (hardGate) {
      ...
      return channelError(
        "backup_failed",
        unknownCommand
          ? `This OpenClaw version has no backup command, and ${gateNoun.toLowerCase()} require a verified backup.`
          : "The pre-update backup failed to verify.",
    ...
    queueNotify(
      "⚠️ Pre-update backup failed — continuing (upgrades are protected by auto-rollback, but rollback recovery may be limited). Check disk space.",
  ...
  const runUpdateRepair = async ({ operationId = null } = {}) => {
    ...
    try {
      ...
      // An unexpected throw (runStreamed rejecting, a sink write error) must
      // still terminate the ledger run and the SSE subscription — otherwise
      // the run hangs in "running" and the Upgrade page waits until next boot.
      const body = channelError(
        "repair_failed",
        "OpenClaw repair did not complete.",
        error?.message ? `Details: ${String(error.message).slice(-400)}` : null,
      );
      emit("repair", "failed", { error: error?.message || "repair crashed" });
      if (operationEvents && operationId) {
        operationEvents.fail(
          operationId,
