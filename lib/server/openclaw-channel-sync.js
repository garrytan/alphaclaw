...
const createOpenclawChannelSync = ({
  ...
  const kFreshTreeAllowedEntries = new Set([".alphaclaw", "logs", "backups", "tmp", ".env"]);
  ...
  const isFreshStateTree = () => {
    try {
      ...
      for (const entry of entries) {
        if (kFreshTreeAllowedEntries.has(entry.name)) continue;
  ...
  const runBackup = async ({
    ...
    const finishSuccess = async (attempt, { partial = false, offlineCopy = null } = {}) => {
      ...
      // tightened its own, so this is a no-op there. Best-effort: a
      // filesystem that refuses chmod still keeps its verified backup.
      if (attempt.artifactFile) {
        try {
          fsModule.chmodSync(attempt.artifactFile, 0o600);
        } catch (error) {
          backupLog(
            `[openclaw-update] backup: chmod 0600 on ${attempt.artifactFile} failed (${sanitizeForDisplay(error.message, 200)}) — it keeps the filesystem's default mode`,
          );
        }
      }
      await pruneBackups();
