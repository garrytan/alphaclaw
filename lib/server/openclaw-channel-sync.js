...
const createOpenclawChannelSync = ({
  ...
  const backupBudget = {
    ...
    stateDbQuietMaxMs: kOpenclawStateDbQuietMaxMs,
  ...
  // no state databases, no session transcripts, an absent or `{}` config,
  // and no applied/last-known-good history (the pin's own self-promotion to
  // LKG is not history). Any fs error → not fresh (fail closed).
  const isFreshStateTree = () => {
    try {
      if (enumerateStateDbs().length > 0) return false;
      const root = stateDir();
      const agentsDir = path.join(root, "agents");
      if (fsModule.existsSync(agentsDir)) {
        for (const agentId of fsModule.readdirSync(agentsDir)) {
          const sessionsDir = path.join(agentsDir, agentId, "sessions");
          if (!fsModule.existsSync(sessionsDir)) continue;
          if (fsModule.readdirSync(sessionsDir).some((name) => name.endsWith(".jsonl"))) {
            return false;
          }
        }
      }
      ...
      if (fsModule.existsSync(configPath)) {
        const raw = fsModule.readFileSync(configPath, "utf8").trim();
        if (raw !== "") {
  ...
  // Prior-run calibration: the newest run that recorded how long the backup
  // took against how many state bytes.
  const priorBackupThroughput = () => {
    try {
      for (const run of ledger.listRuns()) {
        ...
        if (
          backup &&
          Number.isFinite(backup.durationMs) &&
          backup.durationMs > 0 &&
    ...
    try {
      ...
      if (prior && stateBytes > 0) {
        ...
        diagnosis.predictedUpstreamMs = Math.round(
          prior.durationMs * (stateBytes / prior.stateBytes),
        );
      }
    } catch (error) {
  ...
  const runBackup = async ({
    ...
    const runBackupAttempt = async ({
      ...
      if (result.ok) {
        return { status: "success", artifactFile, outputFile, quiesced };
      }
      cleanupFailedBackup(outputFile);
    ...
    const finishSuccess = async (attempt, { partial = false, offlineCopy = null } = {}) => {
      ...
      if (attempt.artifactFile && !offlineCopy) {
        const usable = await runUsableCheck(attempt.artifactFile, {
          timeoutMs: Math.max(1, Math.min(backupBudget.reuseVerifyTimeoutMs, remainingMs())),
        });
        if (!usable.ok) {
          backupLog(
      ...
      if (partial) {
        ...
        emit("backup", offlineCopy.partial ? "warning" : "completed", {
          detail: `succeeded via AlphaClaw offline copy after ${track.attempts} upstream attempt${
            track.attempts === 1 ? "" : "s"
          } (gateway paused)${offlineCopy.partial ? " — workspace files excluded (over the inline size limit)" : ""}`,
        });
      } else {
      ...
      let bytes = null;
      try {
        bytes = attempt.artifactFile ? fsModule.statSync(attempt.artifactFile).size : null;
      } catch {}
      const artifact = {
        at: backupStartedAt,
        dir: backupsDir,
        ...
        stateBytes: diagnosis.stateBytes,
        durationMs: nowFn() - backupStartedAt,
        ...(partial || offlineCopy?.partial ? { partial: true } : {}),
        ...(offlineCopy
          ? {
    ...
    const reuseWindowStartMs = () => {
      ...
      try {
        const state = channelStore.readState();
        if (Number.isFinite(state.applied?.at)) since = Math.max(since, state.applied.at);
        const migration = state.configMigration?.lastAttempt;
        if (migration?.ok && Number.isFinite(migration.at)) since = Math.max(since, migration.at);
        const lastRun = state.lastUpdateRun;
        if (
          lastRun &&
          lastRun.operationId !== operationId &&
          lastRun.ok === true &&
          Number.isFinite(lastRun.startedAt)
        ) {
          since = Math.max(since, lastRun.startedAt);
        }
      } catch {}
      try {
        for (const run of ledger.listRuns()) {
          if (run.operationId === operationId) continue;
          const activated =
            run.ok === true || run.state === "activated" || run.state === "restart_expected";
          if (activated && Number.isFinite(run.startedAt)) {
            since = Math.max(since, run.startedAt);
          }
        }
      } catch {}
      return since;
    ...
    const verifyReuseCandidate = async (candidate) => {
      const budgetMs = Math.max(1, Math.min(backupBudget.reuseVerifyTimeoutMs, remainingMs()));
      ...
      try {
        ...
        const usable = await runUsableCheck(candidate.file, { timeoutMs: remaining() });
        ...
        return { ok: true, sha256, producer: usable.producer || candidate.producer };
      } catch (error) {
        return {
          ok: false,
      ...
      try {
        const copy = await createOfflineCopy({
          ...
          exclusivity: {
            ...
            stopEvidence,
            quietToken,
            liveProcesses: probes.listProcesses() || [],
        ...
        track.offlineCopy = { ok: false, reason, stage, error: detail };
        logEvent("backup_offline_copy", "failed", { operationId, reason, stage, error: detail });
        backupLog(`[openclaw-update] backup: offline copy failed at ${stage}: ${detail}`);
        cleanupFailedBackup(outputFile);
        if (stage === "exclusivity") {
          return {
            done: await finishFailure({
              classified: {
    ...
    const runQuiescedAttemptLoop = async ({ quiesceDeadline, stopEvidence, quietToken, stopConfirmed }) => {
      ...
      if (shortCircuit) {
        ...
        for (;;) {
          ...
          if (attempt.status === "success") return { done: await finishSuccess(attempt) };
          ...
          return { done: await finishFailure(attempt) };
        }
      }
      ...
      if (copy.done) return copy;
      return { fallback: true, lastAttempt: copy.lastAttempt };
    };

    // Quiesce transaction. Returns { done: <final result> } when the backup
    // step is decided either way, { fallback: true, relaunched } when the live
    // ladder should take over (stop unavailable, quiesced attempt timed out,
    // an exogenous writer raced even the paused gateway, or the offline copy
    // failed), or { workspaceRetry: true } when the caller must run the
    // one-shot --no-include-workspace retry LIVE (never in-quiesce).
    const runQuiescedBackup = async () => {
        ...
        Promise.resolve()
          .then(() =>
            gatewayQuiesce.acquireLock({
              ...
              // copy; the default 10-min lease would force-release mid-copy.
              leaseMs: backupBudget.quiesceTimeoutMs + backupBudget.offlineCopyBudgetMs,
            }),
          )
          .then((rel) => {
      ...
      try {
        ...
        // left of it, never a fresh budget.
        const quiesceDeadline =
          nowFn() + Math.min(backupBudget.quiesceTimeoutMs, Math.max(remainingMs(), 1));
        ...
        return { ...outcome, relaunched: gatewayDown && !outcome.done };
      } finally {
  ...
  // archives) can never evict a verified backup by being newer. The archive a
  // still-fenced migrating run recorded is exempt from eviction (WI-4.2).
