...
const createOpenclawChannelSync = ({
  ...
  const runDatabasePreflight = async ({ version, emit }) => {
    emit("db-preflight", "running");
    const overlayPkgDir = channelStore.overlayPackageDir(version);
    const bin = channelStore.resolvePackageBin(overlayPkgDir);
    if (!bin) {
