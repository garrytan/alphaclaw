...
const createOpenclawChannelSync = ({
  ...
  const environmentStripeForChannel = (channel) => {
    if (channel === "beta") {
      return { label: "BETA", color: "amber", _alphaclawManaged: true };
    }
    if (channel === "dev") {
      return { label: "DEV", color: "purple", _alphaclawManaged: true };
    }
  ...
  const applyEnvironmentStripe = (config, desiredStripe) => {
    if (!desiredStripe) {
      ...
      if (current && current._alphaclawManaged === true) {
  ...
  const reconcileOpenclawJsonMirror = (channel) => {
    try {
      ...
      const stripeIsManaged =
        !liveStripe ||
        (liveStripe && typeof liveStripe === "object" && liveStripe._alphaclawManaged === true);
      const stripeChanged =
        stripeIsManaged &&
        ...
        if (verdict === "unsupported") {
          ...
          return {
            ...
            error: channelError(
              ...
              "Activating it could corrupt saved state. A backup was created before this change — restore it if needed, or pick a different version.",
            ),
          };
        } else {
          anyChecked = true;
