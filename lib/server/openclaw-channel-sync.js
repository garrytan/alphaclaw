...
const { createDoctorGuard } = require("./doctor-guard");
const {
  detectAgentsShape,
  agentsArrayToKeyed,
...
const createOpenclawChannelSync = ({
  ...
  const getChannelInfo = () => {
    ...
    return {
      ...
      stabilizationEndsAt:
        ...
        acceptedAt &&
        applied?.acceptedSource !== "manual"
          ? acceptedAt + stabilizationWindowMs
          : null,
    };
  };
  ...
  //   SNAPSHOT openclaw.json.pre-fix-<fromVersion>.bak (keep 5)
  ...
  // 120s killed a 767MB migration ~30% through (issue #20 bug 1).
  const sizedMigrationBudgetMs = () => {
    const gb = totalStateDbBytes() / (1024 * 1024 * 1024);
    return Math.min(
      kOpenclawBootMigrationMaxTimeoutMs,
      Math.round(kOpenclawBootMigrationBaseTimeoutMs + gb * kOpenclawBootMigrationPerGbMs),
    );
  };
  ...
  const validateConfigWithBin = async (bin) => {
    ...
    if (!blamedKeys.length && kUnknownCommandPattern.test(text)) {
  ...
  const probeDbMigrationNeeded = async (bin) => {
    ...
    for (const dbPath of dbPaths) {
      ...
      const text = String(result.tail || "");
      if (kUnknownCommandPattern.test(text)) return null;
  ...
  const reconcileBootConfig = async ({
    force = false,
    ...
    try {
      doctorGuard.recoverQuarantinedLastGood();
    } catch {}
    ...
    // (deterministic beats hopeful). LOUD by design: this replaces the whole
    ...
    const fromVersion =
      migration.completedForVersion || state.pinVersion || "initial";
    try {
    ...
    if (renamedKeys.length || removedKeys.length) {
      ...
      queueNotify(
        `ℹ️ OpenClaw settings migrated for ${installedVersion}: ${[...renamedKeys, ...removedKeys.map((k) => `removed ${k}`)].join(", ")}. A backup was saved first (openclaw.json.pre-fix-${fromVersion}.bak).`,
    ...
    if (needsDoctor) {
      ...
      if (doctorOutcome.code === "doctor_restored_stale_config") {
        ...
        queueNotify(
          `⚠️ OpenClaw's doctor tried to replace your settings with a stale backup (${doctorOutcome.droppedKeyPaths.length} setting path(s) would have been lost). AlphaClaw blocked it — your settings are unchanged. The gateway is held; see the Upgrade page.`,
    ...
    if (configHealthy) {
      ...
      if (dbMigrationNeeded && doctorRan && doctorOutcome?.ok) {
        bootStep("db-migrate", "completed");
      }
    ...
    recordAttempt(false, doctorNote, {
      gateHash,
      tail: validation.tail ? validation.tail.slice(-2000) : null,
    });
    queueNotify(
  ...
  const syncAtBoot = () => {
    ...
    try {
      ...
      if (marker && marker.target) {
        ...
        if (target.kind === "package" && target.version) {
          ...
          if (channelStore.hasOverlay(target.version)) {
            // Honest-rollback preflight (C1): probe the rollback TARGET's own
            // binary against the current state DBs. Asymmetric by design — a
            // stable target usually lacks `database preflight`, and boot must
            // never strand a crashing install — so this warns loudly instead
            // of blocking; the pre-apply backup is the actual recovery path.
            const preflightWarning = runBootRollbackPreflight({
              version: target.version,
            });
            if (preflightWarning) {
              warnings.push(preflightWarning);
              queueNotify(`⚠️ ${preflightWarning}`, {
                eventType: "health",
                id: `boot-rollback-preflight-${target.version || "pin"}`,
              });
            }
            const result = channelStore.activateOverlay({
              installDir,
      ...
      // state DBs, doctor-guarded, and fail-CLOSED (issue #20).
  ...
  // kBackupArchivePattern — a bespoke suffix would escape keep-N retention
  ...
  // tail at call sites. The pre-fix catch-all claimed every CLI failure
  // "failed to verify" and hinted at the wrong subsystem — the exact
  // misdirection issues #7/#9 reported.
  const classifyBackupFailure = (result, { outputFile, gateNoun }) => {
    const tail = String(result.tail || "");
    const lastLine = sanitizeForDisplay(
      tail
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean)
  ...
  //     │        lock(≤90s, self-releasing on late acquire) → wasRunning?
  //     │        → suppress watchdog → stop(≤30s) → one attempt (≤7min)
  //     │        → finally: start (if wasRunning) → unsuppress → unlock
  //     │      lock busy        → fail honestly (gateway_busy)
  //     │      stop unavailable → fall through to the live ladder
  //     │      vanished anyway  → exogenous writer; fall through
  //     ▼
  //   LIVE LADDER (soft gates always; hard gates as fallback):
  //     attempt (fresh --output file each time, kBackupArchivePattern-
  //     conforming so keep-N retention still prunes) → on vanished_file only:
  //     short delay, retry (≤kOpenclawBackupLiveAttempts, all attempts inside
  //     one kOpenclawBackupPhaseEnvelopeMs envelope)
  //     ▼
  //   exit 0 ── artifact at exact path? ──► record + prune (+ attempts/quiesced)
  //   exit ≠0 ─ classifyBackupFailure ────► honest message incl. attempt count;
  //                                         own artifact only: empty → delete,
  //                                         non-empty → .unverified
  const runBackup = async ({
    ...
    const track = { attempts: 0, quiesced: false, vanishedPaths: [] };
    const describeAttempts = () =>
      track.attempts <= 1 && !track.quiesced
        ? ""
        : ` (after ${track.attempts} attempt${track.attempts === 1 ? "" : "s"}${
            track.quiesced ? ", including one with the gateway paused" : ""
          })`;
    ...
    const runBackupAttempt = async ({ timeoutMs, detail }) => {
      track.attempts += 1;
      ...
      if (result.ok && hardGate && !artifactFile) {
        // Live-verified nuance: on a FRESH install (no state databases yet)
        // the real stable binary exits 0 without writing anything — there is
        // nothing to back up, and nothing a migration could lose, so blocking
        // the switch would brick fresh installs' first channel change.
        if (enumerateStateDbs().length === 0) {
          cleanupFailedBackup(outputFile);
          return { status: "fresh_install", outputFile };
        }
        cleanupFailedBackup(outputFile);
        return { status: "no_artifact", outputFile };
      }
      if (result.ok) {
        return { status: "success", artifactFile, outputFile };
      }
      cleanupFailedBackup(outputFile);
      ...
      return { status: "failed", classified, result, outputFile };
    };

    const finishSuccess = async (attempt) => {
      const detail =
        track.attempts > 1 || track.quiesced
          ? `succeeded on attempt ${track.attempts}${track.quiesced ? " (gateway paused briefly)" : ""}`
          : undefined;
    ...
    const finishFailure = (attempt) => {
      ...
      if (hardGate) {
        ...
        return {
          ...channelError("backup_failed", message, classified.hint),
    ...
    // Quiesce transaction. Returns { done: <final result> } when the backup
    // step is decided either way, or { fallback: true } when the live ladder
    // should take over (stop unavailable, or an exogenous writer raced even
    // the paused gateway).
    const runQuiescedBackup = async () => {
      ...
      try {
        release = await Promise.race([
          Promise.resolve()
            .then(() => gatewayQuiesce.acquireLock())
            .then((rel) => {
        ...
        backupLog(
          `[openclaw-update] backup: quiesce lock unavailable (${error.message}) — falling back to live attempts`,
        );
      ...
      try {
        ...
        try {
          gatewayQuiesce.suppress(
            backupBudget.quiesceTimeoutMs + 2 * 60 * 1000,
          );
        } catch {}
        ...
        logEvent("backup_quiesce", "engaged", { operationId });
        const timeoutMs = Math.min(
          backupBudget.quiesceTimeoutMs,
          Math.max(remainingMs(), 1),
        );
        const attempt = await runBackupAttempt({ timeoutMs });
        if (attempt.status === "success") {
          return { done: await finishSuccess(attempt) };
        }
        if (attempt.status === "fresh_install") {
          return { done: finishFreshInstall() };
        }
        if (attempt.status === "no_artifact") {
          return { done: finishNoArtifact(attempt) };
        }
        if (attempt.classified.kind === "vanished_file") {
          // Even the paused gateway didn't stop the churn — an exogenous
          // writer owns those files. The ladder gets its chance.
    ...
    if (hardGate && gatewayQuiesce) {
      const quiesced = await runQuiescedBackup();
      if (quiesced.done) return quiesced.done;
    }

    // Live ladder: retries help only when the race window is transient, so
    // they fire on vanished_file alone — ENOSPC or a missing subcommand will
    // not heal by trying again.
    let liveAttempts = 0;
    let lastAttempt = null;
    while (liveAttempts < backupBudget.liveAttempts) {
      const budget = Math.min(backupBudget.cliTimeoutMs, remainingMs());
      if (budget < 1) break;
      liveAttempts += 1;
      const detail =
        track.attempts > 0
          ? `attempt ${track.attempts + 1} — retrying after a live-file race${
              track.vanishedPaths.length
                ? ` (${track.vanishedPaths[track.vanishedPaths.length - 1]})`
                : ""
            }`
          : undefined;
      ...
      lastAttempt = attempt;
      if (attempt.classified.kind !== "vanished_file") break;
      if (liveAttempts >= backupBudget.liveAttempts) break;
      if (remainingMs() < 1) break;
      await sleepMs(backupBudget.retryDelayMs);
    }
    if (!lastAttempt) {
      // Envelope exhausted before any live attempt could run (a quiesced
      // attempt consumed it). Report the truth rather than a fake attempt.
      const message = `The pre-update backup ran out of time${describeAttempts()} — the ${Math.round(backupBudget.phaseEnvelopeMs / 60000)}-minute backup window was exhausted.`;
      emit("backup", "failed", { error: "backup window exhausted" });
      return {
        ...channelError(
          "backup_failed",
          message,
          "Retry the update; if this repeats, the backup itself is too slow for the window — check archive size and disk speed.",
        ),
        attempts: track.attempts,
        quiesced: track.quiesced,
        vanishedPaths: track.vanishedPaths,
  ...
  const enumerateStateDbs = () => {
    const dbs = [];
    const globalDb = path.join(openclawDir, "state", "openclaw.sqlite");
  ...
  const classifyPreflight = (result) => {
    ...
    if (!result?.ok) {
      if (kUnknownCommandPattern.test(text)) return "unsupported";
  ...
  // Synchronous preflight for the boot rollback path (C1). Boot is offline and
  // synchronous, so this uses execFileSync like the boot config migration.
  // Returns a warning STRING (never blocks): a stable rollback target usually
  // lacks `database preflight`, and boot must never strand a crashing install.
  const runBootRollbackPreflight = ({ version }) => {
    try {
      const bin = channelStore.resolvePackageBin(
      ...
      for (const dbPath of dbPaths) {
        ...
        try {
          ...
          continue; // our snapshot failure is not the target's incompatibility
        }
        try {
          execFileSyncImpl(
            process.execPath,
            [bin, "database", "preflight", snapshot, "--json"],
            { env: probeEnv(), timeout: 120000, stdio: "pipe" },
          );
        } catch (error) {
          const text = `${error?.stdout || ""}\n${error?.stderr || ""}\n${error?.message || ""}`;
          if (kUnknownCommandPattern.test(text)) {
            return (
          ...
          try {
            fsModule.rmSync(snapshot, { force: true });
          } catch {}
        }
      }
      return null;
  ...
  const applyUpdate = async ({
    ...
    try {
      ...
      // gateway was paused, and which volatile files kept vanishing.
