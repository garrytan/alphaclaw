  ...
  const runBackup = async ({
    ...
    const runOfflineCopy = async ({ reason, lastAttempt, stopEvidence, quietToken, stopConfirmed }) => {
      ...
      try {
        ...
        // A budget/quiet abort whose sqlite backup() did not settle inside
        // the orphan bound is still stepping when the barrier lifts below —
        // that fact travels with the failure (record + event), never only
        // in the log.
        ...
        backupLog(
          `[openclaw-update] backup: offline copy failed at ${stage}: ${detail}${
            orphanFields.orphanedBackup
              ? " — an orphaned sqlite backup() is still stepping against the closed source (its temp destination was unlinked; it may hold a read lock on the state DB until it finishes on its own)"
              : ""
          }`,
        );
        cleanupFailedBackup(outputFile);
