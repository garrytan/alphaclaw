...
const createOpenclawChannelSync = ({
  ...
  const runBackup = async ({
    ...
    const finishSuccess = async (attempt, { partial = false, offlineCopy = null } = {}) => {
      ...
      // The digest a later consented reuse binds to (the reuse gate re-hashes
      // over its own fd anyway, so a missing digest only disables pre-consent
      // for this archive — it is never a safety hole). Bounded like the
      // usable check; a timeout or read error is logged, not fatal.
      let sha256 = null;
      if (attempt.artifactFile) {
        let fd = null;
        try {
    ...
    const runQuiescedAttemptLoop = async ({ quiesceDeadline, stopEvidence, quietToken, stopConfirmed }) => {
      ...
      if (copy.done || copy.failure) return copy;
      return { fallback: true, lastAttempt: copy.lastAttempt };
    };

    // Quiesce transaction. Returns { done: <final result> } when the backup
    // step succeeded in-quiesce, { failure: <attempt> } when it failed
    // terminally (the caller finalizes it — 409 + reuse gate — only after the
    // finally below has resumed the state DB and relaunched the gateway),
    ...
    const runQuiescedBackup = async () => {
        ...
        Promise.resolve()
          .then(() =>
            gatewayQuiesce.acquireLock({
              ...
              // begin, usable check, prune, relaunch ready budget); the
              // default 10-min lease would force-release mid-copy.
      ...
      try {
        ...
        // decided outcome (success or terminal failure) runs none.
        return { ...outcome, relaunched: gatewayDown && !outcome.done && !outcome.failure };
      } finally {
    ...
    if (willQuiesce) {
      ...
      // relaunched the gateway, and released the lock — so the reuse gate's
      // candidate re-verification never runs against a paused box.
  ...
  // `<backupsDir>/.offline-copy-<pid>-<rand>` and removes the dir in its
  // finally — a crash or a SIGTERM (gracefulExit hard-exits after 10 s) skips
  // that, leaving a full copy of the state tree on disk. A dir older than the
  // offline-copy budget plus slack cannot belong to a copy still in flight.
  // Fresh dirs are never touched: they may be this very run's copy.
  const kOfflineCopyTempDirPattern = /^\.offline-copy-/;
  const sweepStaleOfflineCopyDirs = async () => {
    ...
    for (const name of names) {
      if (!kOfflineCopyTempDirPattern.test(name)) continue;
      const full = path.join(backupsDir, name);
      let st;
      try {
        st = fsModule.lstatSync(full);
