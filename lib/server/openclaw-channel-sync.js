...
const createOpenclawChannelSync = ({
  ...
  const notifyDayBucket = () =>
    new Date(nowFn()).toISOString().slice(0, 10).replace(/-/g, "");
  ...
  const reconcileBootConfig = async (options = {}) => {
    try {
      ...
      queueNotify(
        `🔴 OpenClaw settings reconciliation crashed (${String(reason).slice(0, 160)}). The gateway is HELD to protect your data — open the Upgrade page to retry.`,
        {
          eventType: "health",
          ...
          if (
            ...
            if (
              ...
              if (!repair.ok) {
                ...
                // the overlay) — previously a warnings.push that
                // flushBootNotifications drops whenever any notification is
                // queued the same boot. Day-bucketed: boot loops dedupe.
                queueNotify(
    ...
    if (state.applied?.acceptedAt) {
      ...
      if (source === "acceptance") {
        queueNotify(
          ...
          { eventType: "recovery", verbose: true },
        );
      }
  ...
  //     │      timed out        → relaunch, then the ladder's 10-min ceiling
  //     │      workspace broken → relaunch + unlock FIRST, then the one-shot
  //     │                         --no-include-workspace retry runs LIVE
  //     ▼
  ...
  const runBackup = async ({
    ...
    const runBackupAttempt = async ({ timeoutMs, detail, noWorkspace = false }) => {
      track.attempts += 1;
    ...
    const finishSuccess = async (attempt, { partial = false } = {}) => {
      if (partial) {
    ...
    // step is decided either way, { fallback: true } when the live ladder
    // should take over (stop unavailable, quiesced attempt timed out, or an
    // exogenous writer raced even the paused gateway), or
    // { workspaceRetry: true } when the caller must run the one-shot
    // --no-include-workspace retry LIVE (never in-quiesce — see the branch).
    const runQuiescedBackup = async () => {
      ...
      try {
        ...
        if (attempt.classified.kind === "timeout") {
          // A quiesced timeout is bounded by the 7-min quiesce budget, but
          // ~18 min of the 25-min phase envelope remain and the live
          // ladder's 10-min CLI ceiling may still fit — a box whose backup
          // takes 7-10 minutes must not be terminally locked out of every
          // hard gate. The finally relaunches the gateway before the ladder
          // runs.
          return { fallback: true, lastAttempt: attempt };
        }
        if (attempt.classified.kind === "workspace_discovery") {
          // NEVER run the workspace retry in-quiesce: its budget is
          // min(cliTimeoutMs = 10 min, envelope), which blows the quiesce
          // invariant (lock + stop + backup + start must fit the 10-min
          // lifecycle lease — hence the 7-min budget) and outlives the
          // watchdog suppression, letting the force-released lease relaunch
          // the gateway MID-TAR while the archive records verified+partial.
          // Config brokenness does not heal by pausing the gateway, so the
          // caller runs the retry on the LIVE path after the finally below
          // has restarted the gateway and released the lock.
          return { workspaceRetry: true, lastAttempt: attempt };
        }
