...
const createOpenclawChannelSync = ({
  ...
  const runBackupDiagnosis = async ({ operationId, policy, spawnEnv, sourceStateDir }) => {
    ...
    try {
      ...
      try {
        ...
        const tree = await walkStateTreeAsync({ stateDir: root, fsModule, checkpoint, policy: effectivePolicy, inventory, nowFn });
        ...
        diagnosis.tarSetBytes = tree.diagnostics?.measurementComplete === false ? null : dbBytes + assetBytes + workspaceBytes + excludedBytes;
  ...
  const runBackup = async ({
    ...
    const sourceStateDir = stateDir(spawnEnv);
    ...
    let activeDeadline = phaseDeadline;
    ...
    const describeRefusedCopy = (base) =>
      track.offlineCopy?.ok === false && track.offlineCopy.stage === "exclusivity"
        ...
        : "";
    // Producers return outcomes; only the dispatcher below may finalize a
    // failure, after both pauses have unwound and the final fresh rung ran.
    ...
    const runOfflineCopy = async ({ stopEvidence, quietToken, stopConfirmed, quiesceRemaining, profile = "full", isLeaseValid = () => true }) => {
      ...
      const onProgress = ({ stage, doneBytes, totalBytes, rawWorkspaceBytes, diagnostics } = {}) => {
        ...
        if (!minimal && Number.isFinite(rawWorkspaceBytes)) diagnosis.rawWorkspaceBytes = rawWorkspaceBytes;
        if (!minimal && diagnostics) diagnosis.directories = diagnostics;
      ...
      try {
        ...
        if (minimal && ["inventory", "exclusivity", "quiet_lost", "lease_lost"].includes(stage)) track.backupRiskBlocked = true;
        if (!minimal && error?.diagnostics) diagnosis.directories = error.diagnostics;
    ...
    const runQuiescedBackup = async ({ profile = "full" } = {}) => {
      const minimal = profile === "migration-minimal";
      // A fresh pause earns coverage only after its own exclusivity proof.
      // Every early exit (including an unconfirmed stop) remains blocked.
      if (minimal) track.backupRiskBlocked = true;
      const holdMs = minimal ? minimalBudgets.leaseMs : quiesceHoldMs();
      ...
      const hasCopyAuthority = () => {
        ...
        catch { ownershipLost = true; track.backupRiskBlocked = true; return false; }
      };
      try {
        ...
        let quiesceDeadline =
          nowFn() +
          Math.min(
            (minimal ? 0 : backupBudget.quiesceTimeoutMs) + backupBudget.offlineCopyBudgetMs,
        ...
        try {
          quietToken = await dbQuiet({
            ...
            maxMs: minimal ? backupBudget.offlineCopyBudgetMs + backupBudget.stateDbQuietSlackMs : backupBudget.stateDbQuietMaxMs,
        ...
        if (minimal) quiesceDeadline = nowFn() + minimalBudgets.copyMs;
        track.quiesced = true;
        ...
        assertAdmission();
        // A clean second proof can clear a historical refusal; source corruption
        // and orphan evidence live in the separate cumulative safety verdict.
        if (minimal && nowFn() - acquiredAt > holdMs - minimalBudgets.copyMs - minimalBudgets.unwindMs) {
          return refused("Not enough lifecycle lease remains to copy and restart safely.");
        }
        ...
        const outcome = minimal
          ? await runOfflineCopy({ ...args, profile, quiesceRemaining: () => Math.max(0, quiesceDeadline - nowFn()) })
          : await runQuiescedAttemptLoop(args);
        assertAdmission();
        if (minimal && outcome.publish) track.backupRiskBlocked = false;
        if (minimal && outcome.copyFailed) return { failure: backupFailureAttempt(
          "offline_copy_failed", `Migration backup failed at ${outcome.copyFailed.stage}: ${outcome.copyFailed.error}`) };
        if (minimal && outcome.copyRefused) return { failure: outcome.copyRefused };
        ...
        if (error?.blocked) {
          ...
          track.backupRiskBlocked = true;
          if (minimal) track.migrationMinimal = { ...track.migrationMinimal, ok: false, stage: "admission", error: sanitizeForDisplay(error.message, 300) };
        ...
        if (cleanupOverrunMs > 0) {
          backupLog(`[openclaw-update] backup: ${profile} pause cleanup exceeded its lease scheduling reserve by ${cleanupOverrunMs} ms`);
          if (minimal) track.migrationMinimal = { ...track.migrationMinimal, cleanupOverrunMs };
        }
        ...
        if (ownershipLost && completedOutcome && (completedOutcome.publish || completedOutcome.done)) {
          ...
          if (minimal) track.migrationMinimal = { ...track.migrationMinimal, ok: false, stage: "lease_lost" };
        }
        inQuiesce = false;
      }
    };
    ...
    // One operation, one final dispatcher:
    // frozen policy -> bounded diagnosis -> full copy / upstream ladder
    //                                      -> complete unwind
    //                                      -> fresh lease + minimal copy
    //                                      -> verify -> resume -> publish
    //                                      -> final reuse / consent decision.
    // Full and minimal results stay separate. Cumulative source/orphan safety
    // cannot be cleared by a later success; fresh ownership must be proved.
    const broad = await runBroaderAttempts();
    if (!broad?.failure) return broad;
    if (track.safetyFailure || !willQuiesce) return finalizeFailure(broad.failure);
    const minimalPhaseStartedAt = nowFn();
    activeDeadline = minimalPhaseStartedAt + minimalBudgets.phaseMs;
    emit("backup", "running", { detail: "Broader backups failed — pausing for a fresh migration backup of databases and essential files" });
    const minimal = await runQuiescedBackup({ profile: "migration-minimal" });
    minimalPublicationDeadline = Math.min(activeDeadline, nowFn() + minimalBudgets.publicationMs);
    if (minimal.publish && !track.safetyFailure) {
      const published = await minimal.publish();
      if (!published?.failure) return published;
      minimal.failure = published.failure;
      track.migrationMinimal = { ...track.migrationMinimal, ok: false, stage: "publication",
        error: published.failure.classified.message };
    }
    if (minimal.done) return minimal.done;
    const failure = minimal.failure || minimal.lastAttempt || backupFailureAttempt(
      "offline_copy_refused", "The migration backup could not establish exclusive access to the state directory.");
    track.migrationMinimal ||= { ok: false, stage: "exclusivity", error: failure.classified.message };
    // Retain the broad availability failure for existing reuse policy, but
    // never use it to erase an unsafe-source or unresolved ownership refusal.
    return finalizeFailure(track.safetyFailure || track.backupRiskBlocked ? failure : broad.failure);
