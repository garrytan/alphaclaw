...
const { createBackupProgress } = require("./openclaw-backup-progress");
...
const createOpenclawChannelSync = ({
  ...
  const classifyBackupFailure = (
    ...
    if (result?.stalled) {
      ...
      return {
        kind: "stalled",
        message: `${subject} CLI made no progress for ${windowText} (no new staging bytes) and was stopped.${quoteLastOutput()}`,
        hint: gateHint,
    ...
    if (result?.timedOut) {
      return {
        kind: "timeout",
        message: result.verificationTimedOut
          ? `${subject} verification exceeded its ${Math.round(backupBudget.upstreamVerificationMs / 60000)}-minute allowance.${quoteLastOutput()}`
          : `${subject} timed out after ${Math.round((result.timeoutMs || backupBudget.cliTimeoutMs) / 60000)} minutes.${quoteLastOutput()}`,
        hint: gateHint,
        stepError: "timed out",
      };
    }
  ...
  const runBackupDiagnosis = async ({ operationId, policy, spawnEnv, sourceStateDir }) => {
    ...
    try { diagnosis.priorFailure = priorBackupFailure(ledger.listRuns().filter((run) => run.operationId !== operationId)); } catch {}
    try {
  ...
  const runBackup = async ({
    ...
    const spawnEnv = Object.freeze({ ...sourceEnv });
    ...
    // Live attempts share the remaining envelope, keeping verification and
    // process cleanup back. A nonpositive budget cannot start an attempt.
    const attemptBudgetMs = () =>
      liveBackupAttemptBudget(remainingMs(), backupBudget);
    ...
    const runBackupAttempt = async ({
      ...
      if (gatewayQuiesce?.signal?.aborted) {
        track.backupRiskBlocked = true;
        return { status: "failed", ...backupFailureAttempt("cancelled", "The backup was cancelled during shutdown.") };
      }
      ...
      let stagingRoot;
      try {
        stagingRoot = fsModule.mkdtempSync(path.join(backupsDir, `${kCliPublishStagingPrefix}alphaclaw-`));
      } catch (error) {
        track.safetyFailure ||= backupSafetyFailure(error);
        const result = { ok: false, error: redactBackupOutput(error.message), tail: redactBackupOutput(error.message) };
      ...
      const stagingOutput = path.join(stagingRoot, path.basename(outputFile));
      const progress = createBackupProgress({ root: stagingRoot, outputFile: stagingOutput, fsModule });
      const redactAttemptOutput = (text) => redactBackupOutput(String(text).split(`${stagingRoot}${path.sep}`).join(`${backupsDir}${path.sep}`));
      ...
      try {
        raw = await runner.runStreamed({
          ...
          env: { ...spawnEnv, TMPDIR: stagingRoot, TMP: stagingRoot, TEMP: stagingRoot },
          timeoutMs,
          deadlineAt: Date.now() + Math.min(timeoutMs, liveBackupAttemptBudget(remainingMs(), backupBudget)),
          // Output is diagnostic, never proof that backup work advanced.
          inactivityTimeoutMs: backupBudget.upstreamInactivityMs,
          progressProbe: progress.probe,
          outputCountsAsProgress: false,
          verificationTimeoutMs: backupBudget.upstreamVerificationMs,
          killGraceMs: backupBudget.upstreamCleanupReserveMs,
          signal: gatewayQuiesce?.signal,
      ...
      const progressReceipt = progress.sample();
      let publicationFailed = false;
      try {
        if (!progress.ownsRoot()) throw new Error("backup staging directory identity changed");
        const staged = fsModule.existsSync(stagingOutput) ? fsModule.lstatSync(stagingOutput) : null;
        if (staged?.isFile() && staged.size > 0) {
          const verified = raw?.ok && !backupSafetyFailure({ ...raw, message: raw.tail });
          fsModule.renameSync(stagingOutput, verified ? outputFile : `${outputFile}.unverified`);
        }
      } catch (error) {
        publicationFailed = true;
        raw = { ...raw, ok: false, error: `backup publication failed: ${error.message}` };
      }
      if (!publicationFailed && progress.ownsRoot()) await removeBackupDebris(stagingRoot);
      outputRing.flush();
      const result = {
        ...raw,
        ...(raw?.tail != null ? { tail: redactAttemptOutput(raw.tail) } : {}),
        ...(raw?.error != null ? { error: redactAttemptOutput(raw.error) } : {}),
        timeoutMs,
        inactivityTimeoutMs: backupBudget.upstreamInactivityMs,
        lastOutput: outputRing.lines(),
      };
      Object.assign(track.attemptsDetail.at(-1), { progress: progressReceipt, timeoutMs,
        verificationTimeoutMs: backupBudget.upstreamVerificationMs });
      ...
      if (result.cancelled) {
        track.backupRiskBlocked = true;
        closeRung({ ok: false, kind: "cancelled" });
        return { status: "failed", ...backupFailureAttempt("cancelled", "The backup was cancelled during shutdown."),
          result, outputFile, quiesced };
      }
      const safety = backupSafetyFailure({ ...result, message: result.tail });
      if (safety) {
    ...
    const finishSuccess = async (
      ...
      if (attempt.artifactFile && !offlineCopy) {
        // The attempt kept this time back; verification cannot extend the
        // phase or consume its own process-cleanup allowance.
        const usableBudgetMs = remainingMs() - backupBudget.upstreamCleanupReserveMs;
        const usable = usableBudgetMs < 1 ? { ok: false, timedOut: true, stage: "deadline" } : await runUsableCheck(attempt.artifactFile, {
          sourceStateDir,
          timeoutMs: Math.min(backupBudget.reuseVerifyTimeoutMs, usableBudgetMs),
        });
    ...
    const publishSuccess = async (attempt, { partial, offlineCopy }) => {
      ...
      const publicationDeadline = minimal ? minimalPublicationDeadline : activeDeadline;
      await pruneBackups({ keepPaths: [attempt.artifactFile].filter(Boolean), deadline: publicationDeadline });
      if (!attempt.artifactFile) {
        backupLog(
      ...
      // not fatal.
      let sha256 = null;
      if (attempt.artifactFile && nowFn() < publicationDeadline) {
        let fd = null;
        try {
          fd = (fsModule.openSync || fs.openSync).call(fsModule, attempt.artifactFile, "r");
          sha256 = await sha256OverFd(fd, {
            timeoutMs: Math.min(backupBudget.reuseVerifyTimeoutMs, publicationDeadline - nowFn()),
          });
        } catch (error) {
          backupLog(
    ...
    const runQuiescedBackup = async () => {
      ...
      const assertLease = () => {
        if (!isLeaseValid() || gatewayQuiesce.isCancelled?.()) {
          track.backupRiskBlocked = true;
      ...
      try {
        ...
        try {
          ...
          suppressionOwner = gatewayQuiesce.suppress(holdMs + kOpenclawBackupQuiesceSuppressSlackMs);
          suppressionEngaged = true;
        } catch {}
        ...
        if (relaunched && hasCopyAuthority() && !track.relaunchFailure) {
          updateDetail("backup", "Gateway relaunched — waiting for native readiness before continuing");
          const ready = await waitForBackupReadiness({ gateway: gatewayQuiesce,
            timeoutMs: Math.min(backupBudget.postQuiesceReadyTimeoutMs + backupBudget.postQuiesceSettleMs,
              remainingMs(), Math.max(0, acquiredAt + holdMs - nowFn())),
            pollMs: Math.max(1, backupBudget.postQuiescePollMs), settleMs: backupBudget.postQuiesceSettleMs,
            shouldAbort: () => !hasCopyAuthority() });
          if (!ready || !hasCopyAuthority()) {
            track.backupRiskBlocked = true;
            track.relaunchFailure = backupFailureAttempt("gateway_relaunch_failed",
              "The gateway did not become ready after the backup pause. The upgrade was not applied.");
          }
        }
        try {
          ...
          hasCopyAuthority();
          if (suppressionEngaged && (suppressionOwner || isLeaseValid())) gatewayQuiesce.unsuppress(suppressionOwner);
        } catch {}
        try {
          release?.();
        } catch {}
