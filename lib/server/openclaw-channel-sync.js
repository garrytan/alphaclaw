...
const buildDevUpdateEnv = (source) => {
  ...
  for (const [key, value] of Object.entries(source)) {
    if (kDevEnvAllowPrefixes.some((prefix) => key.startsWith(prefix))) {
      env[key] = value;
    }
  }
...
const createOpenclawChannelSync = ({
  ...
  const probeEnv = () => buildProbeEnv(process.env);
  ...
  const flushBootNotifications = async () => {
    ...
    // Boot-time warnings were queued in the pre-server process (bin) instance;
    // they persist in state.lastBoot so the server instance can surface them.
    try {
  ...
  const reconcileOpenclawJsonMirror = (channel) => {
    try {
      ...
      const update =
        config.update && typeof config.update === "object" ? config.update : {};
      const auto =
      ...
      if (
        JSON.stringify(config.update || null) !== JSON.stringify(nextUpdate)
      ) {
        writeOpenclawConfig({
          fsModule,
          openclawDir,
          config: { ...config, update: nextUpdate },
        });
  ...
  const pruneBackups = () => {
    try {
      ...
      for (const entry of entries.slice(kOpenclawBackupKeepCount)) {
        fsModule.rmSync(entry.full, { recursive: true, force: true });
      }
    } catch {}
  };
  ...
  const ensurePinSnapshot = async (installDir) => {
    ...
    if (installedVersion === state.pinVersion) {
      await channelStore.snapshotPinFromInstallAsync({
        installDir,
        pinVersion: state.pinVersion,
      });
    }
  ...
  const applyUpdate = async ({
    ...
    const finish = (status, body) => {
      // The flag resets FIRST and unconditionally: everything after is
      // best-effort, and a bookkeeping throw (ENOSPC on the state file) must
      // never leave the apply latch stuck blocking all future updates.
    ...
    try {
      ...
      const backup = await runBackup({ emit, isDowngrade });
  ...
  return {
    ...
    store: channelStore,
    // exported for tests
    _internals: { readDeclaredPin, stripGitShimEnv, enginesSatisfied },
  };
};
