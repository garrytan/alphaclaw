...
const { runBootNativeMaintenance } = require("../boot-native-maintenance");
const { normalizeBootConfig } = require("../boot-config-normalization");
const {
  createGatewayLifecycleLock,
} = require("../gateway-lifecycle-lock");
const { registerAuthRoutes } = require("../routes/auth");
const { registerOpenclawBackupPolicyRoutes } = require("../routes/openclaw-backup-policy");
...
const { registerDiagnoseRoutes } = require("../routes/diagnose");
...
const registerServerRoutes = ({
  ...
  gatewayMedic = null,
  // Issue #76: the boot sequence's listening-path steps (dangling-record
  // closers + boot-report server phase), composed in lib/server.js from
  // boot-report-steps.js. Optional: absent = today's boot (tests, older wiring).
  bootSteps = null,
  proxy,
  getGatewayUrl,
  SETUP_API_PREFIXES,
  ...
  registerOpenclawBackupPolicyRoutes({
    app, requireAdmin, fsModule: fs, OPENCLAW_DIR: constants.OPENCLAW_DIR,
    getSourceContext: () => openclawChannelService?.getBackupSourceContext?.(),
    getBackupPreflight: () => openclawChannelService?.getBackupPreflight?.(),
  });
  registerOpenclawChannelRoutes({
    app,
    fs,
    ...
    gatewayHoldActions: {
      ...
      startGateway: (options) => startGateway(options),
      ...
      // Issue #76 B1.2: the installed-tree reconcile route shares this bundle.
      // The fast `booting` refusal reads the active lock holder; the relaunch
      // after a successful swap goes through the watchdog's ONE relaunch
      // primitive when it is exposed, else the boot-style startGateway the
      // retry route uses (the route holds the lifecycle lock either way).
      getActiveOperation: () => gatewayLifecycleLock.getActiveOperation(),
      relaunch: async ({ hold = null } = {}) => {
        if (typeof watchdog?.runVerifiedRelaunch === "function") {
          const outcome = await watchdog.runVerifiedRelaunch({
            source: "repair/reconcile_installed",
            intent: "replace",
            hold,
          });
          return {
            ok: ["replacement_ready", "replacement_pending", "incumbent_adopted"].includes(
        ...
        await startGateway();
        return { ok: true, verdict: "started" };
      },
    },
  });
  ...
  registerGoogleRoutes({
    ...
    runGoogleAccountDisconnect: (args) => gmailWatchService.disconnectAccount(args),
  });
  const runOnboardedBoot = () =>
    runOnboardedBootSequence({
      ...
      // Issue #76 A7/A1: first steps inside the boot lock (startup.js owns
      // the order); each is optional and swallow-and-log there.
      closeDanglingRecordsAtBoot: bootSteps?.closeDanglingRecordsAtBoot
        ? () => bootSteps.closeDanglingRecordsAtBoot()
        : null,
      reconcileRestartOperationAtBoot: bootSteps?.reconcileRestartOperationAtBoot
        ? () => bootSteps.reconcileRestartOperationAtBoot()
        : null,
      // Issue #79 (g) / Codex 18: the boot `.tmp` sweep, right after the
      // closers (startup.js owns the order; lib/server.js gates it on the
      // pidfile decision).
      sweepBackupDebrisAtBoot: bootSteps?.sweepBackupDebrisAtBoot
        ? () => bootSteps.sweepBackupDebrisAtBoot()
        : null,
      recordBootReportServerPhase: bootSteps?.recordBootReportServerPhase
        ? () => bootSteps.recordBootReportServerPhase()
        : null,
      // Issue #76 C1 belt / C2 (boot-launch-steps.js): startup.js hands both
      // steps the boot lifecycle lease ({ hold }) — the lock is not
      // re-entrant, so the reconcile must run under the boot hold, never
      // acquire its own.
      reconcileInstalledAtBoot: bootSteps?.reconcileInstalledAtBoot
        ? (args) => bootSteps.reconcileInstalledAtBoot(args)
        : null,
      assessLaunchCompatibilityAtBoot: bootSteps?.assessLaunchCompatibilityAtBoot
        ? (args) => bootSteps.assessLaunchCompatibilityAtBoot(args)
        : null,
      // finalize runs on every reconcile outcome, before the launch decision:
      // a held boot settles the relaunch step a boot reconcile still owes
      // (onBootReportFinalize), then the report is finalized.
      finalizeBootReport:
        bootSteps?.finalizeBootReport || bootSteps?.onBootReportFinalize
          ? (outcome) => {
              bootSteps.onBootReportFinalize?.(outcome);
              return bootSteps.finalizeBootReport?.(outcome) ?? null;
            }
          : null,
      ...
      reconcileBootConfig: openclawChannelService?.reconcileBootConfig
        ? (options) => openclawChannelService.reconcileBootConfig(options)
        : null,
      normalizeBootConfig: (options) => normalizeBootConfig({ ...options, env: gatewayEnv() }),
      runBootNativeMaintenance: (options) => runBootNativeMaintenance({
        ...options,
        execFileCmd,
        gatewayEnv,
      }),
      ...
      ensureGatewayProxyConfig,
      resolveSetupUrl,
      // The boot's launch books the relaunch step on a reconcile run this
      // boot activated (Codex 7) — same startGateway, completed either way.
      startGateway: bootSteps?.wrapStartGateway
        ? bootSteps.wrapStartGateway(startGateway)
        : startGateway,
      watchdog,
      gmailWatchService,
      primeStatusCaches: () => {
  ...
  // GET /api/diagnose (#76 A9): the `alphaclaw diagnose` collector over THIS
  // process's live seams. The getters are late-bound (called per request, not
  // here) and the store hands the collector the managed dir every on-volume
  // artifact — boot reports, self-version stamp, schema table, pidfile — is
  // read from; readEnvFile is the collector's secret-redaction source.
  registerDiagnoseRoutes({
    app,
    requireAuth,
    fsModule: fs,
    rootDir: constants.kRootDir ?? null,
    openclawDir,
    readEnvFile,
    readLogTail,
    incidentsDb,
    channelStore: openclawChannelService?.store ?? null,
    getWatchdogStatus: () => watchdog?.getStatus?.() ?? null,
    getChannelInfo: () => openclawChannelService?.getChannelInfo?.() ?? null,
  });
