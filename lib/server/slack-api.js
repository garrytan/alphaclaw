...
const createSlackApi = (getToken) => {
  ...
  const postMessage = (channel, text, opts = {}) => {
    ...
    // Link previews: watchdog notifications carry rescue capability links —
    // Slack's unfurler must not follow (and thereby redeem) them.
    if (opts.unfurl_links === false) {
      payload.unfurl_links = false;
    }
    if (opts.unfurl_media === false) {
