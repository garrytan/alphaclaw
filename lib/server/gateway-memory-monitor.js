...
const createGatewayMemoryMonitor = ({ config = {} } = {}) => {
  ...
  const noteProcessExited = (atMs = null, pid = null) => {
    ...
    freezeEpisode("process_exited", Number.isFinite(atMs) ? atMs : lastSample?.atMs ?? 0);
    criticalEvalStreak = 0;
    watchStreak = 0;
    fastPathStreak = 0;
    nonPositiveSlopeStreak = 0;
    if (lastSnapshot) {
      lastSnapshot = {
        ...lastSnapshot,
        state: lastSnapshot.state === "disabled" ? "disabled" : "no_gateway",
        episodeId: null,
        episodeStartedAt: null,
        peakRssMb: null,
        projectedExhaustionAt: null,
  ...
  return { addSample, evaluate, getTrend, noteMitigation, noteProcessExited, reset };
};
