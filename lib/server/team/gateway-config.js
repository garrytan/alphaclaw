...
const createTeamGatewayConfig = ({
  ...
  const applyTeamGatewayConfig = async ({ assertCanMutate = () => {},
    onAuthConfigApplied = () => {}, shouldApply = () => true } = {}) => {
    const advertisedScopes = await Promise.resolve()
      .then(() => getAdvertisedScopes())
      .catch(() => null);
    assertCanMutate();
    // A roster refresh may have waited for disable (or outlived its lease
    // during discovery). It must never switch restored token auth back.
    if (shouldApply() !== true) return { skipped: true };
    ...
    // A transition's feature flag must describe the auth persisted here,
    // even if ownership is lost during the following restart/probe. This
    // callback is synchronous and runs before another task can take over.
    onAuthConfigApplied();
