...
// The proxy identity resolver reads this per gateway-bound request in team
// mode — identical re-reads come from an mtime/size-keyed parsed copy.
let kStateReadCache = null;
...
const readOperatorsState = ({ fsModule = fs, openclawDir = OPENCLAW_DIR } = {}) => {
  ...
  if (
    kStateReadCache &&
    kStateReadCache.filePath === filePath &&
    kStateReadCache.mtimeMs === stat.mtimeMs &&
    kStateReadCache.size === stat.size
  ) {
    return JSON.parse(kStateReadCache.serialized);
  }
  try {
    const raw = fsModule.readFileSync(filePath, "utf8");
    const state = normalizeOperatorsState(JSON.parse(raw));
    kStateReadCache = {
      filePath,
...
// world-readable in a shared state dir: 0600 and an atomic temp+rename write.
// Locking is the CALLER's responsibility (setOperators / setNotificationPrefs
// wrap this in withFileLockSync).
