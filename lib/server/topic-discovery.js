...
const createTopicDiscoveryService = ({
  ...
  const isAdmitted = () => {
    try { return canSweep() === true; } catch { return false; }
  };
  ...
  const sweep = async () => {
    ...
    if (!isAdmitted()) return { skipped: true, reason: "mutation_blocked" };
  ...
  const noteSessionSeen = (sessionKey) => {
    if (!isTopicDiscoveryEnabled(env) || !isAdmitted()) return;
    ...
    setImmediate(() => {
      ...
      if (now() - (state.recentlyNoted.get(noteKey) || 0) < kNoteDebounceMs) return;
      try {
        const cfg = readConfig();
