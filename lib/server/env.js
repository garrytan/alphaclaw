...
const { noteIgnoredDeploymentEnvKey } = require("./deployment-only-env");
const { stripLineBreaks, normalizeEnvKey } = require("./utils/env-values");
...
const writeEnvFile = (vars) => {
  ...
  // Secret-bearing: 0600 on a fresh inode (umask-default 0644 leaked API keys
  // to every same-host reader).
...
const reloadEnv = () => {
  const vars = readEnvFile().filter(({ key, value }) => {
    if (!kDeploymentOnlyEnvKeys.has(key)) return true;
  ...
  for (const key of allKnownKeys) {
    if (kDeploymentOnlyEnvKeys.has(key)) continue;
