...
const createGatewayLifecycleLock = ({
  ...
  const makeHold = (kind, holdLeaseMs = leaseMs, cleanup = null) => {
    ...
    hold.release = Object.assign(release, {
      ...
      kind,
      startedAt,
      expiresAt: startedAt + holdLeaseMs,
  ...
  // Shutdown also owns cleanup for repairs started outside the watchdog (the
  // Upgrade page). Never force-unlock: the same writer/restore barrier drains.
  const cancelActiveCleanup = (reason = "shutdown") => {
    const hold = active;
    if (!hold?.cleanup) return Promise.resolve();
    try { hold.cleanup.cancel(reason); } catch (error) {
      logger.warn?.(`[alphaclaw] lifecycle cancellation failed: ${error.message}`);
    }
    return hold.release();
  };
  return { acquire, tryAcquire, getActiveOperation, owns, cancelActiveCleanup };
};
