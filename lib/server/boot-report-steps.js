...
const createBootReportSteps = ({
  ...
  const closeDanglingRecordsAtBoot = () => {
    ...
    captured.danglingRecords = isPlainObject(result)
      ? {
          closedRuns: Array.isArray(result.closedRuns) ? result.closedRuns : [],
    ...
    if (captured.danglingRecords?.closedRuns.length || captured.danglingRecords?.closedLastUpdateRun) {
      log(
        `closed dangling records at boot: runs [${captured.danglingRecords.closedRuns.join(", ")}]${captured.danglingRecords.closedLastUpdateRun ? " + lastUpdateRun" : ""}`,
      );
    }
    return result;
  };
