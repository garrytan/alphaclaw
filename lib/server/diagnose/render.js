...
// (describeReportVersions — the same expected/running/diverged the verdict
// judged): "expected" is the bin phase's read, else the server phase's
// channel snapshot; "installed" is the build the gateway RUNS (the bin
// phase's post-sync resolvedForLaunch, else the server phase's own read).
// A report that only knows the pre-sync installedAtBoot says so with an
// "(at boot)" label rather than passing it off as the launch tree — an
// activation boot legitimately differs between the two — and when both are
// known and differ the pre-sync read is shown beside the launch tree so the
// sync's move is visible.
const installedText = (versions, openclaw) => {
  const running = versions.running;
  const atBoot = openclaw.installedAtBoot;
  const hasAtBoot = typeof atBoot === "string" && atBoot !== "";
  if (running === null) return hasAtBoot ? `${show(atBoot)} (at boot)` : "n/a";
  return hasAtBoot && atBoot !== running ? `${show(running)} (at boot ${show(atBoot)})` : show(running);
};
...
const renderBootReport = (report, { compact = false } = {}) => {
  ...
  if (compact) {
    return [
      `- ${head}; alphaclaw ${show(alphaclaw.version)}; openclaw expected ${show(versions.expected)} / installed ${installedText(versions, openclaw)}; sync ${show(bootSync.action)}${bootSync.reason ? ` (${bootSync.reason})` : ""}; pidfile ${show(pidfile.decision)}/${show(pidfile.reason)}`,
    ];
  }
  const lines = [
    `- ${head}`,
    ...
    bullet(
      "openclaw",
      `pin ${show(openclaw.declaredPin)}, applied ${show(openclaw.channelApplied)}, expected ${show(versions.expected)}, installed ${installedText(versions, openclaw)}, diverged ${show(versions.diverged)}`,
    ),
    bullet(
      "overlay",
...
const renderRun = (run) => {
  ...
  const recovery = isPlainObject(run.recovery)
    ? `recorded recovery ${show(run.recovery.kind)}: ${run.recovery.checkpoint?.verified === true ? "verified configuration checkpoint" : "configuration checkpoint unverified"}; ${run.recovery.kind === "database_set" && run.recovery.databases?.complete === true ? "complete database snapshot recorded (verify before restore)" : "database data not backed up"}`
    : null;
  return [
    `- ${code(run.operationId)} ${show(run.state)} → ${target}; started ${iso(run.startedAt)}, finished ${iso(run.finishedAt)}, ok ${show(run.ok)}${recovery || backup ? `; ${recovery || backup}` : ""}${run.result?.code ? `; code ${code(run.result.code)}` : ""}${run.result?.error ? `; error ${show(run.result.error)}` : ""}`,
    `  - steps: ${steps}`,
  ];
};
