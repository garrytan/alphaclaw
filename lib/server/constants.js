...
// Inactivity policy for the upstream `backup create` rung (v0.9.81, D15): a
// CLI that prints nothing AND grows no staging bytes for this long is stopped
// and classified `stalled` (reuse- and consent-eligible like `timeout`, never
// retried). Strictly below kOpenclawBackupTimeoutMs (cadence-pinned): the
// ceiling stays the bound for a SLOW backup, this bounds a HUNG one — the
// 2026.9.2 CLI idled the full 10 minutes against a relaunched gateway in
// production with nothing written and nothing said.
...
// Honest envelope (issue #79 (f), Codex 16): every live attempt may run to
// the CLI ceiling (kOpenclawBackupTimeoutMs, 10 min) and a success still
// needs the usable-check reserve, so the attempt cap must satisfy
//   liveAttempts × cliTimeoutMs + usableCheckReserveMs ≤ phaseEnvelopeMs
// (2 × 10 + 1 = 21 min ≤ 25). The former 3 never fit (31 min): the third
// attempt existed only on paper — the envelope refused it as
// window_exhausted — so the ladder promised coverage the clock could not
// deliver. backupBudgetPins (openclaw-backup-ladder.js) carries both
// envelope relations; constants-cadence.test.js pins them.
