...
// Past this age the npm doc — the catalog's row source — is AWAITED instead of
// served stale-while-revalidate (v0.9.81, D6): a box whose SWR revalidation
// kept failing served "as of 20 hours ago" with no follow-up. Bounded by the
// 20 s fetch timeout; GitHub (notes/dates only) stays SWR at every age.
const kOpenclawCatalogHardStaleMs = 60 * 60 * 1000;
...
// An upstream `backup create` with no trustworthy staging growth for this
// long is stopped as `stalled` (reuse- and consent-eligible like `timeout`,
// never retried). Output lines do not count as backup progress. Progressing
// live attempts use the remaining phase envelope; silent verification has
// its own finite allowance within that deadline.
const kOpenclawBackupUpstreamInactivityMs = 3 * 60 * 1000;
// Boot-time `doctor --fix` config migration (issue #21 bug 1): env-tunable
// BASE budget (default 600 s = the 10-min kOpenclawBootMigrationBaseTimeoutMs),
// scaled up by total state-DB size at kOpenclawBootMigrationPerGbMs and capped
// at max(kOpenclawBootMigrationMaxTimeoutMs = 30 min, base) — see
// sizedMigrationBudgetMs in openclaw-channel-sync.js. The cap sat under the
// boot placeholder's 15-minute /health flip until v0.9.45; that window now
// re-arms on every recorded step (the migration writes db-migrate /
// config-migrate steps) with a 60-min absolute ceiling (boot-placeholder.js),
// so the platform no longer restarts the container mid-migration.
const kOpenclawDoctorMigrationTimeoutMs =
  parsePositiveInt(process.env.OPENCLAW_DOCTOR_MIGRATION_TIMEOUT, 600) * 1000;


// Boot heavy-ops budget: the clock starts early in syncAtBoot and only the
// rollback-preflight prober draws from it (createBootPreflightProber in
// openclaw-channel-sync.js takes min(kOpenclawBootPreflightBudgetMs,
// remaining)). The doctor migration is sized separately by
// sizedMigrationBudgetMs (30-min ceiling since v0.9.45, env-raisable via
// OPENCLAW_DOCTOR_MIGRATION_TIMEOUT) and does NOT draw from this budget; the
// 12-min figure keeps the preflights under the boot placeholder's 15-minute
// no-progress /health flip.
...
// At most two live attempts share the remaining phase envelope. Each keeps
// usable-check and process-cleanup time back; retries never get a fresh
// ten-minute clock. backupBudgetPins (openclaw-backup-ladder.js) checks the
// quiesced path and the live envelope/reserve relation.
const kOpenclawBackupLiveAttempts = 2;
const kOpenclawBackupRetryDelayMs = 750;
// The upstream share of the single backup pause. Its dedicated lifecycle
// lease covers this share, the offline copy and the cleanup/relaunch reserve.
...
// Native readiness after a backup pause uses the normal gateway readiness
// budget, not the old twenty-second TCP gate. The caller additionally clamps
// readiness plus settling to the remaining phase envelope and owned lease.
const kOpenclawBackupPostQuiesceReadyTimeoutMs = kGatewayRestartReadyTimeoutMs;
...
// Pre-backup diagnosis (issue #79 (d)): the state walk that sizes the copy
// set (policy excludes applied) and predicts both rungs is bounded by this
// budget. It runs BEFORE the phase deadline is stamped, so it is a term of
// the quiesced-path envelope relation (backupBudgetPins). A walk that hits
// the budget predicts "unknown" and the ladder stays copy-first.
const kOpenclawBackupDiagnosisBudgetMs = 2 * 60 * 1000;
// After a failed offline copy the in-quiesce upstream `backup create` is
// worth attempting only when the set it would tar is bounded: upstream takes
// no excludes (only --no-include-workspace exists), so a multi-GB workspace
// set would spend the rest of the pause on a rung the copy already sized.
// chooseBackupRung's byte cap.
const kOpenclawBackupUpstreamMaxBytes = 2 * 1024 * 1024 * 1024;
// Prediction defaults (Codex 16: bytes / rate + files × per-file overhead),
// used until a prior run of the SAME rung calibrates the rate — the copy's
// series never predicts the upstream and vice versa. Conservative on
// purpose: a slow network volume is the case the prediction protects. The
// copy rate covers sqlite backup() + verbatim copy + `gzip -1`; the upstream
// rate the CLI's own tar + verify pass over the whole tree.
const kOpenclawBackupDefaultCopyBytesPerSec = 20 * 1000 * 1000;
const kOpenclawBackupDefaultUpstreamBytesPerSec = 15 * 1000 * 1000;
// The per-entry cost bytes/rate misses: an open/stat/header round trip per
// file dominates on trees of many small files (sessions, plugin catalogs).
// Deliberately conservative for a network volume (a local disk is far
// under it); calibration from prior runs replaces it.
const kOpenclawBackupPerFileOverheadMs = 2;
...
const kOpenclawBackupExclusivitySettleMs = 35 * 1000;
...
// The usable check (gzip -t + manifest) needs time of its own. Live attempt
// admission keeps this reserve plus process-cleanup time back; verification
// remains bounded by the actual remaining phase time, never a fresh clock.
...
// Progress ticker cadence for a running backup rung (issue #79 (h)): every
// interval the driver logs one line (rung, elapsed, bytes so far), publishes
// it on the apply's SSE output stream and rewrites the live step row's detail
// IN PLACE — a 10-minute backup therefore costs ~40 lines, never 40 step
// rows. Harness-tunable through backupTuning.progressIntervalMs.
const kOpenclawBackupProgressIntervalMs = 15 * 1000;
...
// node:sqlite's backup() is cancelled by a THROW from its `progress` hook (the
// job aborts at that step boundary — verified on Node 22.23); closing its
// source does NOT stop it (sqlite3_close_v2 zombifies the connection until the
// backup finishes). After the offline copy aborts (budget/quiet lost), the
// hook throws at the next step, so the job normally settles at once; this is
// the bound the copy waits for a step that never returns (the hook is never
// called again, so nothing can throw) before returning the failure, so the
// barrier is not released over a still-stepping backup silently — past the
// bound the failure carries `orphanedBackup: true`.
const kOpenclawBackupOrphanSettleMs = 2 * 1000;
// Rollback fence: the recorded archive's on-disk mtime must match the mtime
// captured at publish within this tolerance (an archive is never rewritten
// after publish, so a different size or a newer mtime is a swapped file).
// Coarse-timestamp filesystems (FAT-like 2 s granularity) stay inside it.
const kOpenclawBackupArchiveMtimeToleranceMs = 2 * 1000;
...
// Standalone backup runs (v0.9.81 "Back up now") have their own ring in the
// ledger: they never compete with update runs for the 10 slots above.
const kOpenclawBackupRunKeepCount = 5;
...
/** Consecutive failed liveness probes of an ESTABLISHED gateway (health was
 * not "unknown") before the watchdog escalates to `doctor --fix` + relaunch.
 * One transient timeout is not an incident: the degraded retry ladder
 * (5s -> 10s -> 20s) re-probes first and only a sustained failure repairs.
 * `1` restores the pre-0.9.75 "repair on the first steady-state failure".
 * Readiness-only degradation never counts. Read at module load; deployment
 * env only — never honored from the agent-writable .env (an agent must not be
 * able to raise the bar to the ceiling and keep its own broken gateway from
 * being repaired; see deployment-only-env.js). Startup probes keep the
 * separate kWatchdogStartupFailureThreshold. */
const kWatchdogDegradedRepairThreshold = readClampedEnvCount(
