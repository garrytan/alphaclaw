...
// node:sqlite's backup() has no cancel API and closing its source does NOT
// stop it (sqlite3_close_v2 zombifies the connection until the backup
// finishes — verified on Node 22.23: a backup whose source closed at step 3
// still ran to completion). After the offline copy's budget fires, the copy
// waits at most this long for the orphaned backup to settle before returning
// the budget error, so the barrier is not released over a still-stepping
// backup silently; past the bound the failure carries `orphanedBackup: true`.
