...
// `optionalFunctionNames` are repair steps upstream may RETIRE once the state
// they repaired can no longer exist. They are loaded when present and left
// undefined when absent — a caller must feature-detect before calling. Only
// steps whose omission still reaches the same end state belong here; a required
// name that vanishes must keep failing loudly (that is a real contract break).
