...
const { openReadonlyOpenclawStateDb, resolveOpenclawStateDbPath, hasTable } = require("./openclaw-state-db");
const { onStateDbQuiet } = require("./state-db-quiet");

const kCronStoreFile = "jobs.json";
const kCronStoreQuietListenerName = "cron-store";
...
// State-DB quiet period: the long-lived read handle is the one AlphaClaw
// connection that outlives a request, so the barrier closes it up front and
// refuses to reopen until release — listJobs serves the jobs.json fallback
// meanwhile (the same path a fresh install takes).
let refuseReopenWhileQuiet = false;
onStateDbQuiet({
  name: kCronStoreQuietListenerName,
  begin: () => {
    refuseReopenWhileQuiet = true;
    closeCronStoreDb();
  },
  end: () => {
    refuseReopenWhileQuiet = false;
  },
});

const getStateDbHandle = ({ openclawDir }) => {
  if (refuseReopenWhileQuiet) return null;
  ...
  if (cached && Date.now() - cached.openedAt >= kStateDbReopenTtlMs) {
    ...
    try {
      const stat = fs.statSync(cached.handle.databasePath);
      if (stat.dev !== cached.device || stat.ino !== cached.inode) {
        dropStateDbHandle(openclawDir);
        return getStateDbHandle({ openclawDir });
      }
  ...
  if (handle) {
    stateDbHandles.set(openclawDir, { handle, openedAt: Date.now(), device: stat?.dev, inode: stat?.ino });
  }
  return handle;
};
