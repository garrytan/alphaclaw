...
const ensureManagedExecDefaults = async ({
  ...
  if (backend.backend === "file" || backend.hint === "file-era") {
    ...
    if (mergeResult?.merged) {
      announce(
        `🩹 Merged ${mergeResult.recoveredEntries ?? 0} approval entr${(mergeResult.recoveredEntries ?? 0) === 1 ? "y" : "ies"} recovered from ${mergeResult.strays} stray legacy exec-approvals file(s).`,
        {
          eventType: "recovery",
