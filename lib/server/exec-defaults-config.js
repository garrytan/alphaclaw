...
const ensureManagedExecDefaults = async ({
  ...
  if (backend.backend === "file") {
    // Recover state a buggy earlier reaper renamed away before seeding, so
    // the seed never shadows recovered entries.
    mergeStrayLegacyExecApprovals({ fsModule, openclawDir, logger });
