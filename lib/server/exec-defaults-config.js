...
class ExecApprovalsReadError extends Error {
  constructor(message, { filePath, cause } = {}) {
    super(message);
    this.name = "ExecApprovalsReadError";
    this.code = "EXEC_APPROVALS_UNREADABLE";
    this.filePath = filePath;
    if (cause) this.cause = cause;
  }
}

// Mutator-side reader (fix wave F215): a MISSING file is the documented
// `{ version: 1 }` start, but an existing file this code cannot parse must
// never be rebuilt from that fallback — the allowlist an operator approved
// would be silently wiped by the next POST/DELETE.
const readExecApprovalsConfigForWrite = ({
  fsModule = fs,
  openclawDir,
  fallback = { version: 1 },
} = {}) => {
  ...
  try {
    ...
    throw new ExecApprovalsReadError(`Cannot read ${filePath}: ${error.message}`, {
      filePath,
      cause: error,
    });
  }
  try {
    ...
    throw new ExecApprovalsReadError(
      `Refusing to overwrite ${filePath}: existing file is not JSON alphaclaw can parse (${error.message})`,
...
const writeExecApprovalsConfig = ({
  ...
  const filePath = resolveExecApprovalsConfigPath({ openclawDir });
  // Atomic (F051): a torn exec-approvals.json is an empty allowlist on the
  // next fail-open read.
  writeFileAtomic(filePath, JSON.stringify(file, null, spacing) + "\n", { fsModule });
  return filePath;
};
...
const ensureManagedExecDefaults = async ({
  ...
  const announce = (message, opts) =>
    fireAndForgetNotify(notify, message, opts);
  ...
  if (backend.backend === "file" || backend.hint === "file-era") {
    ...
    if (mergeResult?.merged) {
      const recovered = mergeResult.recoveredEntries ?? 0;
      announce(
        recovered > 0
          ? `🩹 Merged ${recovered} approval entr${recovered === 1 ? "y" : "ies"} recovered from ${mergeResult.strays} stray legacy exec-approvals file(s).`
          : `🩹 Recovered settings from ${mergeResult.strays} stray legacy exec-approvals file(s).`,
        {
          eventType: "recovery",
          // Day-bucketed like the other recurrence-plausible repairs: a
          // genuinely new merge months later must not dedupe against a
          // stale count-keyed entry.
          id: `exec-approvals-merged-${mergeResult.strays}-${recovered}-${utcDayBucket()}`,
        },
      );
    }
  }
