...
const createUpgradeNotifier = ({
  ...
  const flushHeldDirectSends = async () => {
    ...
    for (const event of pending) {
      try {
        ...
        if (result?.expired) {
          outbox.noteExpired?.(event, result);
          log(`held direct send expired (${event.eventType})`);
        } else if (!result?.ok) {
          log(
  ...
  const holdDirectSend = (event) => {
    const existing = heldDirectSends.find((entry) => entry.id === event.id);
    if (existing) {
      existing.createdAt = Math.min(existing.createdAt, event.createdAt);
      existing.expiresAt = Math.min(existing.expiresAt, event.expiresAt);
  ...
  const notify = async (message, opts = {}) => {
    ...
    if (!event) {
      if (heldForStateDbQuiet) return holdDirectSend(envelope);
