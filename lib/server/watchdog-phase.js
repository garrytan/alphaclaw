...
const deriveWatchdogPhase = (snapshot = {}, now = Date.now()) => {
  ...
  // "restarting" alone is not enough: the lifecycle can sit there while the
  // window has expired and health has gone degraded/unhealthy — an unbounded
  // reassuring "planned restart" phase would mask a down gateway. Only treat
  // it as expected while windowed, or while health is still unknown.
  if (
    (expectedRestartInProgress && now < Number(expectedRestartUntilMs)) ||
    (lifecycle === "restarting" && health === "unknown")
  ) {
