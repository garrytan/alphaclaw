...
const createSituationSlot = ({
  ...
  const readRaw = () => {
    try {
      ...
      // A THROWING read (lock contention, I/O) is not a corrupt blob: the
      // stored record may be intact, so writers must not rebuild over it.
      return { ...emptyRecord(true), readFailed: true };
    }
  };
  ...
  const readRecord = () => {
    ...
    if (record.unreadable && !unreadableLogged) {
      ...
      log(
        record.readFailed
          ? "situation record is unreadable (read failed); the stored record is kept until the database can be read"
          : "situation record is unreadable (corrupt blob); a new review replaces it",
      );
    }
  ...
  const persist = (current, { supersede = false } = {}) => {
    const record = readRaw();
    // Never rebuild the blob over a record we could not read: a transient
    // read failure followed by a successful write would wipe lastVerdict and
    // history. The caller reports persisted:false and the report stays
    // ephemeral — the stored verdict survives for when the db is readable.
    if (record.readFailed) {
      return { ok: false, error: "slot read failed; refusing to overwrite the stored record blind" };
    }
    const history =
