...
const { renderTelegramHtml, renderHouseLinks } = require("./utils/telegram-html");
...
// The 403 descriptions that mean "this chat can never receive from this bot
// again without a human acting on the Telegram side": the user blocked the
// bot, the bot was kicked from the group, the account is deactivated. Every
// OTHER 403 ("bot can't initiate conversation with a user", "bot is not a
// member of the supergroup chat", …) is fixable by pairing/starting the bot
// and stays retryable. Deterministic regardless of where the target came from.
const kTelegramDeterministicForbiddenPattern =
  /blocked|kicked|deactivated|user is deactivated/i;
// Where a Telegram target was resolved from — the provenance that decides
// whether `400 chat not found` is final. Telegram answers "chat not found"
// BOTH for a dead chat id AND for a numeric id the bot has simply never
// exchanged a message with. A pairing-store target has a proven prior
// interaction (the user paired through the bot), so for it the shape means
// the chat is gone. An allowFrom-fallback id (openclaw.json, copied in by
// hand) or an explicit admin target may just be un-started — the SAME
// human-fixable state the 403 "can't initiate conversation" shape keeps
// retryable — so those stay on the 48 h retry ladder.
const kTelegramTargetSourcePairing = "pairing";
const kTelegramTargetSourceAllowFrom = "allowFrom";
const kTelegramTargetSourceAdmin = "admin";
// Slack mrkdwn: exactly these three characters are escaped in text
// (https://api.slack.com/reference/surfaces/formatting#escaping) and a link
// url is delimited by `<`, `|`, `>` — those may not appear raw inside it.
const kSlackTextEscapes = Object.freeze({
  "&": "&amp;",
  "<": "&lt;",
  ">": "&gt;",
});
const kSlackLinkUrlReservedPattern = /[|<>]/g;
...
// Deterministic Telegram failures: the bot is blocked/kicked/deactivated
// (403 with a matching description — a bare 403 is NOT enough), the chat id
// is dead (400 "chat not found" — ONLY for a pairing-store target, see the
// source constants above; an unknown/omitted source is not proven and stays
// retryable), or even the plain-text resend was rejected as unparseable
// (400 parse error AFTER the format fallback).
const classifyTelegramFailure = (
  err,
  { afterFormatFallback = false, source = null } = {},
) => {
  const errorCode = resolveErrorCode(err);
  const message = String(err?.message || "");
  const deterministic =
    (errorCode === kTelegramForbiddenCode &&
      kTelegramDeterministicForbiddenPattern.test(message)) ||
...
//                          └ other error ─▶ fail (403 blocked/kicked/deactivated
//                                            deterministic; 400 chat-not-found
//                                            deterministic iff source=pairing)
const sendTelegramRendered = async ({
  api,
  chatId,
  text,
  // Target provenance (kTelegramTargetSource*): decides the chat-not-found
  // verdict. Omitted = unproven = retryable.
  source = null,
  log = console,
  ...
  if (!shouldDeliver()) return expiredDelivery({ skipped: 1 });
  ...
  if (html === null) {
    ...
    try {
      ...
      return classifyTelegramFailure(err, { source });
    }
  }
  try {
    ...
    if (!isTelegramParseError(err)) return classifyTelegramFailure(err, { source });
    log.warn?.(
    ...
    try {
      ...
      return classifyTelegramFailure(fallbackErr, {
        afterFormatFallback: true,
        source,
      });
    }
  }
...
const escapeSlackText = (value) =>
  String(value ?? "").replace(/[&<>]/g, (ch) => kSlackTextEscapes[ch]);

// `|` would end the url and start the label, `<`/`>` would end the link:
// percent-encode them so the url survives intact and clickable.
const encodeSlackLinkUrl = (url) =>
  String(url ?? "").replace(kSlackLinkUrlReservedPattern, (ch) => encodeURIComponent(ch));
...
// Same link grammar as the Telegram renderer (renderHouseLinks) — including
// balanced parentheses inside the url — so the two transports never truncate
// differently. Literal runs are passed through unchanged (compose sites own
// their text; only the link we MINT gets Slack's escaping).
const formatSlackMessage = (message) =>
  renderHouseLinks(String(message || ""), {
    text: (run) => run,
    link: ({ label, url, http, raw }) =>
      http ? `<${encodeSlackLinkUrl(url)}|${escapeSlackText(label)}>` : raw,
  });
...
const createWatchdogNotifier = ({
  ...
  const sendWhatsApp = async (target, message, { shouldDeliver = () => true } = {}) => {
    if (!shouldDeliver()) return expiredDelivery({ skipped: 1 });
    try {
      if (nativeDelivery !== null) {
        if (typeof nativeDelivery !== "function") return failTarget("native_delivery_unavailable");
        const result = await nativeDelivery({ target, message: String(message || ""), shouldDeliver });
        if (result?.expired) return result;
        return result?.ok ? { ok: true } : failTarget(result?.reason || "native_delivery_unavailable");
      }
      if (!clawCmd) return failTarget("whatsapp_unconfigured");
      const result = await clawCmd(
        `message send --channel whatsapp --target ${quoteShellArg(target)} --message ${quoteShellArg(String(message || ""))}`,
        { quiet: true, timeoutMs: 30000 },
      );
      return result?.ok ? { ok: true } : failTarget(result?.stderr || result?.stdout || "send failed");
  ...
  const notify = async (message, opts = {}) => {
    ...
    const failures = [];
    let expiredCount = 0;
    const skipExpired = (channel) => {
      if (opts.shouldDeliver?.() !== false) return false;
      summary[channel].skipped = true;
      expiredCount += 1;
      return true;
    };
    ...
    // Provenance travels with the target list into the send helper: a
    // pairing-store id has provably talked to the bot, an allowFrom id has
    // not — and that is what decides whether "chat not found" is final.
    let telegramTargetSource = kTelegramTargetSourcePairing;
    ...
    if (telegramTargets.length === 0) {
      ...
      telegramTargetSource = kTelegramTargetSourceAllowFrom;
      if (telegramTargets.length > 0) {
    ...
    if (!telegramApi?.sendMessage || !getTelegramToken() || telegramTargets.length === 0) {
      ...
      for (const chatId of telegramTargets) {
        if (skipExpired("telegram")) continue;
        const result = await sendTelegramRendered({
          api: telegramApi,
          chatId,
          text: String(message || ""),
          source: telegramTargetSource,
          shouldDeliver: opts.shouldDeliver,
        });
        if (result.expired) { expiredCount += 1; summary.telegram.skipped = true; continue; }
        if (result.ok) {
    ...
    if (!discordApi?.sendDirectMessage || !getDiscordToken() || discordTargets.length === 0) {
      ...
      for (const userId of discordTargets) {
        if (skipExpired("discord")) continue;
        try {
    ...
    if ((!clawCmd && nativeDelivery === null) || whatsappTargets.length === 0) {
      summary.whatsapp.skipped = true;
    } else {
      for (const target of whatsappTargets) {
        if (skipExpired("whatsapp")) continue;
        try {
          const result = await sendWhatsApp(String(target || "").trim(), message, opts);
          if (result.expired) {
            summary.whatsapp.skipped = true;
            expiredCount += 1;
            continue;
          ...
          if (!result?.ok) {
            recordFailure("whatsapp", target, result);
            console.error(`[watchdog] whatsapp notification failed for ${target}: ${result.reason}`);
            continue;
          }
    ...
    if (expiredCount > 0) return { ...expiredDelivery({ sent, failures, skipped: expiredCount }), channels: summary };
    return {
      ok: sent > 0,
      sent,
      failed,
  ...
  // { ok:false, reason, errorCode, deterministic } and never throws. An
  // admin target is operator-typed, never proven to have talked to the bot,
  // so its provenance defaults to "admin" (chat-not-found stays retryable);
  // a caller that resolved the target from the pairing store may say so.
  const sendToTarget = async (
    { channel, target, accountId, source = kTelegramTargetSourceAdmin } = {},
    message,
    { shouldDeliver = () => true } = {},
  ) => {
    if (!shouldDeliver()) return expiredDelivery({ skipped: 1 });
    ...
    try {
      ...
      if (safeChannel === "whatsapp") {
        const result = await sendWhatsApp(safeTarget, message, { shouldDeliver });
        if (!result?.ok) return result;
        lastDeliveredAt = new Date().toISOString();
        return { ok: true };
      }
