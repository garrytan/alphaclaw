      ...
      bootstrapWork = (async () => {
        ...
        updateState((state) => {
          ...
          for (const account of state.accounts || []) {
            ...
            if (!watch.enabled && watch.pid && !serveManager.isPidRunning(watch.pid)) {
              next = setAccountGmailWatch({ state: next, accountId: account.id, watch: { port: null, pid: null } }).state;
            }
            if (watch.remoteOperation?.status !== "pending") continue;
        ...
        // Canonical ownership of existing ports first; duplicate/missing
        // ports then take the first free slot in account-id order.
        const state = readState();
        const enabled = listWatchEnabledAccounts(state).sort((a, b) => a.id.localeCompare(b.id));
        const used = new Set(reservations.values());
        // Disabled accounts may still own a process whose exit is unconfirmed.
        // Their persisted ports cannot be handed to an enabled boot restore.
        for (const account of state.accounts || []) {
          const watch = getAccountGmailWatch(account);
        ...
        for (const account of enabled) {
          ...
          if (port && !used.has(port) && !reservations.has(account.id)) { reservations.set(account.id, port); used.add(port); }
        }
        for (const account of enabled) {
          if (stopping || generation !== gen) return;
