...
const buildGogSkillContent = ({ fs, accounts, openclawDir = "" }) => {
  ...
  lines.push("");
  lines.push(
    // Root-aware (#121): never name the Docker-only /data path — on npx and
    // VPS installs the managed root lives under the operator's home.
    `- In AlphaClaw-managed deployments, gog state lives under \`$OPENCLAW_STATE_DIR\`${
      openclawDir ? ` (this install: \`${openclawDir}\`)` : ""
    }.`,
  );
  lines.push(
...
const installGogCliSkill = ({ fs, openclawDir }) => {
  try {
    ...
    const content = buildGogSkillContent({ fs, accounts, openclawDir });
