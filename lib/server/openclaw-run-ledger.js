...
const kRunStates = [
  ...
  "activation_failed",
  "interrupted",
  // v0.9.81: a standalone backup run (target.kind === "backup") that finished
  // — no activation restart follows it.
  "completed",
];
...
const createRunLedger = ({
  ...
  keepBackupRuns = constants.kOpenclawBackupRunKeepCount,
} = {}) => {
  ...
  // Creation sequence for startedAt tie-breaks (see listRuns).
  let createSeq = 0;
  ...
  const normalizeRecord = (raw) => {
    ...
    return {
      ...
      // Per-instance creation sequence (v0.9.84): breaks startedAt ties in
      // listRuns. Present only when the record was stamped — legacy records
      // (pre-v0.9.84) keep their exact persisted shape (the state-file-compat
      // fixtures pin it) and sort behind a stamped record at the same
      // millisecond.
      ...(Number.isFinite(raw.seq) ? { seq: raw.seq } : {}),
      ...
      ...(raw.recovery && typeof raw.recovery === "object" && !Array.isArray(raw.recovery)
        ? { recovery: raw.recovery } : {}),
      ...(raw.recoveryIntent && typeof raw.recoveryIntent === "object" && !Array.isArray(raw.recoveryIntent)
        ? { recoveryIntent: raw.recoveryIntent } : {}),
      ...(raw.recoveryReview && typeof raw.recoveryReview === "object" && !Array.isArray(raw.recoveryReview)
        ? { recoveryReview: raw.recoveryReview } : {}),
      ...
      // v0.9.81 (D21): what the declared-intent belt verified or skipped for
      // this apply ({ direction, latest, … }); only present once recorded.
      ...(raw.intentCheck && typeof raw.intentCheck === "object"
        ? { intentCheck: raw.intentCheck }
        : {}),
      ...(["update", "downgrade", "switch"].includes(raw.intent) ? { intent: raw.intent } : {}),
      ...(typeof raw.expectLatest === "boolean" ? { expectLatest: raw.expectLatest } : {}),
      ...(["config_only", "database_set"].includes(raw.recoveryMode) ? { recoveryMode: raw.recoveryMode } : {}),
  ...
  const listRuns = () => {
    ...
    // Newest first. startedAt is a millisecond clock, so two runs created
    // inside the same millisecond tie — and every "latest run" reader
    // (the upgrade overseer picker, the Upgrade page, both prune rings,
    // diagnose) takes [0]. Before v0.9.84 a tie fell through to directory
    // listing order (the CI Node 26 lane listed the older run first). The
    // per-instance creation sequence breaks the tie, then the operationId
    // for legacy records without one, so the order never depends on readdir.
    runs.sort(
      (a, b) =>
        (b.startedAt || 0) - (a.startedAt || 0) ||
        (b.seq || 0) - (a.seq || 0) ||
        String(b.operationId).localeCompare(String(a.operationId)),
    );
    return runs;
  };
  ...
  const closeInterruptedRuns = () => {
    ...
    for (const run of listRuns()) {
      ...
      const backupRun = run.target?.kind === "backup";
      const next = completeRun(run.operationId, {
        ...
        result: {
          ...
          code: "interrupted",
          message: backupRun
            ? "AlphaClaw restarted before the backup finished."
            : "AlphaClaw restarted before the update finished.",
          hint: backupRun
            ? "No archive was published. Run Back up now again from the Upgrade page."
            : "Nothing was activated. Start the update again from the Upgrade page.",
          docsUrl: null,
        },
      });
  ...
  // Backup runs (v0.9.81 "Back up now") keep their own, smaller ring so a few
  // manual backups never evict the failed update's record and log the
  // operator is diagnosing — and vice versa.
  const pruneRuns = ({ keep = keepRuns, keepBackups = keepBackupRuns } = {}) => {
    const runs = listRuns();
    const { pinnedOperationIds } = selectMigrationBackupProtection(runs, { nowMs: nowFn() });
    const pinned = new Set(pinnedOperationIds);
    for (const run of runs) if (run.recoveryReview?.active === true) pinned.add(run.operationId);
    for (const kind of ["config_only", "database_set"]) {
      const retained = runs.filter((run) => run.recovery &&
        (run.recovery.kind === "database_set" ? "database_set" : "config_only") === kind).slice(0, 3);
      for (const run of retained) {
        try { if (fsModule.lstatSync(run.recovery.checkpoint.file).isDirectory()) pinned.add(run.operationId); } catch {}
      }
    }
    const isBackupRun = (run) => run?.target?.kind === "backup";
    const stale = [
      ...runs.filter((run) => !isBackupRun(run)).slice(Math.max(1, keep)),
      ...runs.filter(isBackupRun).slice(Math.max(1, keepBackups)),
    ].filter((run) => !pinned.has(run.operationId));
