...
const telemetryDirectory = (stateDir) =>
  typeof stateDir === "string" && path.isAbsolute(stateDir)
    // Upstream backup create excludes the state root's tmp tree. Publication
    // and exit cleanup must never race the upstream archive's source walk.
    ? path.join(stateDir, "tmp", "alphaclaw", "gateway-memory") : null;
