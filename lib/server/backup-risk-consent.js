...
const databaseFileIdentity = (stat, suffix) => suffix === "-wal"
  ? [stat.dev, stat.ino, stat.size, stat.mtimeMs, stat.mode, stat.nlink]
  : statIdentity(stat);
...
const fingerprintConsentConfigs = ({ openclawDir, openclawConfigPath = null, fsModule = fs }) => {
  const entries = [
    { file: openclawConfigPath || resolveOpenclawConfigPath({ openclawDir }), code: "OPENCLAW_CONFIG_UNREADABLE" },
...
const createBackupRiskConsentStore = ({ now = Date.now, ttlMs = kConsentTtlMs, maxEntries = kMaxConsentEntries } = {}) => {
  ...
  const revoke = (operationId) => {
    offers.delete(operationId);
    for (const [key, entry] of tokens) if (entry.operationId === operationId) tokens.delete(key);
  };
  const protectedTargets = () => { sweep(); return [...offers.values()].map((entry) => structuredClone(entry.facts.target)); };
  return { offer, getOffer, issue, peek, consume, revoke, protectedTargets, size: () => { sweep(); return { offers: offers.size, tokens: tokens.size }; } };
};
...
const fingerprintBuild = async (build, { fsModule = fs, contentOnly = false } = {}) => {
  ...
  for (const file of [path.join(build.packageDir, "package.json"), build.bin]) {
    ...
    if (!contentOnly) hash.update(file).update(JSON.stringify(signature(after)));
  ...
  if (!contentOnly) hash.update(JSON.stringify(signature(rootAfter)));
...
const fingerprintDatabase = async (filePath, { fsModule = fs } = {}) => {
  ...
  for (const [suffix, headerBytes] of [["", 100], ["-wal", 32], ["-journal", 28]]) {
    ...
    try {
      ...
      if (JSON.stringify(databaseFileIdentity(before, suffix)) !== JSON.stringify(databaseFileIdentity(after, suffix)) ||
          JSON.stringify(databaseFileIdentity(after, suffix)) !== JSON.stringify(databaseFileIdentity(named, suffix))) throw new Error("state_db_changed");
      facts.push({ suffix, identity: databaseFileIdentity(after, suffix), header: digest(header.subarray(0, bytesRead)) });
    } catch (error) {
...
const assertDatabaseFilesUnchanged = (databases, fsModule = fs) => {
  for (const entry of databases) {
    for (const file of entry.files || []) {
      ...
      if (file.missing ? Boolean(current) : JSON.stringify(databaseFileIdentity(current, file.suffix)) !== JSON.stringify(file.identity)) throw refusal("state_db_changed");
    }
  }
};
...
const createBackupRiskCoordinator = ({ now, fsModule, openclawDir, describeSource, describeTarget, readVersions,
  ...
  const collectFacts = async (target, { hold = null, intent, recoveryHold = null, strict = true, ownedQuiet = false } = {}) => {
    assertPolicy({ hold, intent, recoveryHold, strict });
    if (isQuiet() && !(ownedQuiet && hold)) fail("state_db_quiet");
    ...
    const versions = await readVersions(candidate);
    const configOptions = { openclawDir, openclawConfigPath: versions.inventory?.configPath, fsModule };
    const configurations = strict ? fingerprintConsentConfigs(configOptions) : null;
    ...
    const databases = versions.entries.map((entry) => ({ path: entry.path, kind: entry.kind,
      userVersion: entry.userVersion, contentVersion: entry.contentVersion ?? entry.userVersion, status: entry.status })).sort((a, b) => a.path.localeCompare(b.path));
    for (const entry of databases) {
    ...
    // A human waiver binds exact database contents. Ordinary apply admission
    // leaves the gateway running during preparation: normal WAL writes must
    // not invalidate an otherwise compatible build.
    // Both paths still compare database paths, schemas and probe status.
    if (strict) {
      for (const entry of databases) {
    ...
    if (strict) assertDatabaseFilesUnchanged(databases, fsModule);
    if (strict && JSON.stringify(configurations) !== JSON.stringify(fingerprintConsentConfigs(configOptions))) fail("backup_consent_stale");
    return {
      source: { ...source, fingerprint: sourceFingerprint },
      target: { ...target, ...candidate, fingerprint: targetFingerprint },
      state: { pinVersion: info.pinVersion, expectedVersion: info.expectedVersion, appliedId: info.appliedId, gatewayHold: info.gatewayHold ?? null },
      databases,
      ...(versions.inventory ? { inventory: versions.inventory } : {}),
  ...
  return { collectFacts, request, offer: store.offer, peek: store.peek, consume: store.consume, revoke: store.revoke, protectedTargets: store.protectedTargets };
};
