...
// AlphaClaw's own prefs (feature flags, release channel, team mode) are
// git-synced so they survive restore-from-git (D4). The generated agent-admin
// skill is excluded — it renders masked admin identities and is rebuilt at boot.
const kOpenclawGitignoreAgentAdminEntries = [
  "!alphaclaw.json",
  "skills/alphaclaw-admin/",
];

const kOpenclawGitignoreAppendEntries = [
  ...kOpenclawGitignoreHookEntries,
  ...kOpenclawGitignoreCronRuntimeEntries,
  ...kOpenclawGitignoreAgentAdminEntries,
];
  ...
  try {
    ...
    if (fs.existsSync(gitignorePath)) {
      ...
      if (missing.length) {
        ...
        writeFileAtomic(gitignorePath, next, { fsModule: fs });
      }
    }
    migrateOne({
