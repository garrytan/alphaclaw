...
// `stalled` (v0.9.81, D15) is the inactivity policy's verdict — the CLI made
// no progress for kOpenclawBackupUpstreamInactivityMs — and follows `timeout`
// everywhere: a hung CLI is not a race, so the copy (or the live ladder) is
// the next rung, never a replay of the same hang.
...
const kDefaultBackupBudget = Object.freeze({
  ...
  // The upstream inactivity window measures trustworthy staging growth,
  // independently of output. Verification has a separate finite allowance.
  upstreamInactivityMs: kOpenclawBackupUpstreamInactivityMs,
  upstreamVerificationMs: kOpenclawBackupReuseVerifyTimeoutMs,
  upstreamCleanupReserveMs: 10 * 1000,
...
//   live ladder     attempts share the remaining envelope minus verification
//                   and process-cleanup reserves; retries get no fresh clock
//
...
const kBackupEnvelopePinTerms = Object.freeze({
  ...
  live_ladder_fits_envelope: Object.freeze(["phaseEnvelopeMs", "usableCheckReserveMs", "upstreamCleanupReserveMs"]),
});
const finiteOrNull = (value) => (Number.isFinite(value) ? value : null);
const liveBackupAttemptBudget = (remainingMs, budget = kDefaultBackupBudget) =>
  Math.max(0, remainingMs - budget.usableCheckReserveMs - budget.upstreamCleanupReserveMs);
const priorBackupFailure = (runs) => {
  for (const run of runs) {
    const attempt = run?.backup?.attemptsDetail?.findLast((entry) => entry.rung === "upstream" && entry.ok === false);
    if (!attempt) continue;
    return { operationId: run.operationId, kind: attempt.kind, elapsedMs: attempt.elapsedMs,
      progress: attempt.progress ?? null, timeoutMs: attempt.timeoutMs ?? null };
  }
  return null;
};
const backupBudgetPins = (budget = kDefaultBackupBudget) => {
  ...
  const liveTotalMs =
    liveMissing.length > 0
      ? null
      : liveBackupAttemptBudget(envelopeMs, table) + liveTerms.usableCheckReserveMs + liveTerms.upstreamCleanupReserveMs;
  return Object.freeze([
    ...
    Object.freeze({
      ...
      relation: "remaining live attempt + usableCheckReserveMs + upstreamCleanupReserveMs ≤ phaseEnvelopeMs",
