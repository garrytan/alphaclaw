...
// Backup reuse consent (WI-4.5) is HUMANS-ONLY: an agent that can wave an old
// archive through the hard gate can downgrade onto state that backup does not
// hold. Any body carrying allowBackupReuse — valid or not — resolves to
// denied; the route 403s the agent actor as well (belt and braces).
const applyTierResolver = (req) => {
  const body = req?.body;
  if (body && typeof body === "object" && !Array.isArray(body) && "allowBackupReuse" in body) {
    return "denied";
  }
  return "dangerous";
};
