...
const kHumansOnlyApplyFields = ["allowBackupReuse", "confirmNoBackup"];
...
module.exports = {
  ...
  ops: [
    ...
    },
    {
      ...
      params: {
        fields: [
          ...
          },
          {
            ...
            required: false,
            description:
              "HUMANS ONLY (agent denied when present). Strict boolean (a non-boolean is a 400). Resend `true` after a 409 backup_required_for_migration — a soft-gated update whose database preflight says the target migrates the databases while no backup could be taken; the update then continues WITHOUT a rollback path and the consent is recorded on the run (backup.noBackupConfirmed) and named in the outcome notification. It never relaxes a 409 backup_failed (hard gate).",
          },
          {
            name: "channel",
            location: "body",
      ...
      notes:
        "Fast failures return synchronously; otherwise 202 with operationId + SSE events URL. Two distinct backup refusals: 409 backup_required_for_migration (soft gate — the target migrates the databases and no backup exists; overridable by confirmNoBackup: true) and 409 backup_failed (hard gate — downgrade, dev, prerelease or channel-boundary crossing; NOT overridable: fix the backup or pick a same-channel version, or accept the reusableBackup offer it may carry via allowBackupReuse).",
    },
    {
      id: "updates.rollback",
