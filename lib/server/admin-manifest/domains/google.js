        ...
        fields: [
          {
            ...
            required: false,
            description:
              "Account to revoke and remove. Required when multiple accounts are configured (the request is refused otherwise); with exactly one account it may be omitted. Always pass it.",
          },
        ],
        example: '{"accountId":"ga-1a2b3c"}',
      },
      notes:
        "Revokes the Google refresh token and deletes local auth — reconnecting needs a human OAuth consent in the Setup UI. A response of ok:false with retryable:true means upstream revocation could not be confirmed (timeout/5xx) and the account was KEPT; it echoes the resolved accountId — retry by passing THAT accountId explicitly (never rely on the first-account fallback across retries). Only a confirmed-dead token (or nothing to revoke) proceeds to removal. A response of ok:true with a warning means the account was removed but local keyring cleanup failed — a credential entry may remain in the gog keyring.",
    },
  ],
};
