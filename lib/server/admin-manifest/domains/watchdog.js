...
module.exports = {
  ...
  ops: [
    ...
    },
    {
      ...
      notes:
        "409 codes: gateway_held (reconciler hold — doctor --fix would rewrite the held config; recover through updates.reconcile-retry), gateway_hold_unreadable (release-channel state unreadable; fail-closed). Other skips return 200 with result.skipped and a reason. 200 ok:false WITHOUT result.skipped = doctor ran but the relaunch failed (verdict names why: launch_failed | launch_aborted | replacement_failed | incumbent_unhealthy; message is operator copy). 200 ok:true pending:true = a replacement was requested but not yet verified — poll watchdog.status until replacementPending is null (lastRepairVerdict replacement_ready). A healthy gateway is never cold-restarted by a repair (verdict child_retained | incumbent_adopted).",
    },
    {
