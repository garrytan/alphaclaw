...
module.exports = {
  ...
  ops: [
    ...
    {
      id: "watchdog.diagnose",
      title:
        "Read-only diagnostic bundle (boot reports + verdicts, AlphaClaw version stamp, channel state, pidfile decision, state DB schema vs supported, recent incidents with cause, update runs, backups, log tail)",
      method: "GET",
      path: "/api/diagnose",
      tier: "safe",
      params: {
        fields: [
          {
            ...
            required: false,
            description:
              "json (default): { ok, bundle } where bundle is exactly what `alphaclaw diagnose --json` prints. text: the markdown `alphaclaw diagnose` prints (Content-Type text/markdown, not a JSON envelope).",
          },
        ],
        example: "GET /api/diagnose",
      },
      notes:
        "Same collector as the `alphaclaw diagnose` CLI plus this process's live watchdog status, channel info and incident rows. Every section is stamped live | disk | unavailable (with a reason); a failing section never fails the request (200 with that section unavailable). The bundle is secret-redacted before it is returned. bundle.summary.bootVerdict is the current boot report's verdict ([] = consistent).",
    },
    {
    ...
    },
    {
      ...
      title: "Read memory-growth monitor settings",
      method: "GET",
    ...
    },
    {
      ...
      title:
        "Update memory-growth monitor settings (enabled, autoRestart, budgetMb, maxRestartsPerDay)",
      method: "PUT",
      path: "/api/watchdog/memory",
      tier: "write",
      ...
      params: {
        fields: [
          ...
          },
          {
            ...
            required: false,
            description:
              "Operator RSS budget for the gateway process group in whole MB (256–1048576), or null to use the active configured heap plus 192 MiB derived group budget. The tighter valid group budget applies. Container usage is monitored against its own limit; container pressure alone cannot trigger a gateway restart. A budget at or below current group RSS is rejected. Always requires an operator confirm (restart lever).",
          },
          {
        ...
        fields: [
          {
            ...
            required: false,
            description:
              "One-shot resume of a latched auto-repair pause (watchdog.status autoRepairPaused): clears the pause for THIS attempt only; the same cause re-pauses if the repair fails again. Omit for a normal repair.",
          },
        ],
        example: '{"force":true}',
      },
      notes:
        "409 codes: gateway_held (reconciler hold — doctor --fix would rewrite the held config; recover through updates.reconcile-retry), gateway_hold_unreadable (release-channel state unreadable; fail-closed), auto_repair_paused (a proven structural cause paused automatic repair — the body carries `pause` {cause, fingerprint, installedVersion, reason}; resend with force:true to resume once, or apply a compatible build / restore a backup). Other skips return 200 with result.skipped and a reason (repair_attempts_exhausted = kWatchdogMaxRepairAttempts reached for automatic sources; a manual repair still runs). 200 ok:false WITHOUT result.skipped: verdict null = doctor --fix itself failed (result.result is Doctor's output, nothing relaunched; error doctor_failed); a verdict = doctor ran but the relaunch failed (launch_failed | launch_aborted | replacement_failed | incumbent_unhealthy). message is operator copy in both cases. 200 ok:true pending:true = a replacement was requested but not yet verified — poll watchdog.status until replacementPending is null (lastRepairVerdict replacement_ready). A healthy gateway is never cold-restarted by a repair (verdict child_retained | incumbent_adopted).",
    },
    {
