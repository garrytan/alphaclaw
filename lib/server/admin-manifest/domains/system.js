...
const redactDashboardUrl = (body) => {
  ...
  return { ...body, url: "/openclaw", dashboardToken: kMask };
};
...
module.exports = {
  ...
  ops: [
      ...
      params: {
        fields: [
          ...
          },
          {
            ...
            required: false,
            description:
              "Five space-separated NUMERIC cron fields within real ranges (minute 0-59, hour 0-23, dom 1-31, month 1-12, dow 0-7; */n steps and a-b ranges allowed). Named days/months and @aliases are rejected (400). Omitted keeps the current schedule.",
          },
        ],
    ...
    {
      id: "system.update.resolve",
      title: "Resolve a managed deployment attempt after checking the provider",
      method: "POST",
      path: "/api/alphaclaw/update/:attemptId/resolve",
      tier: "denied",
      hint: "Only a human admin may attest that the deployment provider has finished or cancelled the attempt.",
    },
    {
      ...
      notes:
        "The agent gets the tokenless URL; operators launch tokened links through the /gateway/launch redirect.",
    },
    {
    ...
    },
    {
      ...
      notes:
        "409 codes: apply_in_progress (a channel update is running), gateway_held (reconciler hold — recover through updates.reconcile-retry), gateway_hold_unreadable (release-channel state unreadable; fail-closed), booting (AlphaClaw is still starting the gateway). With ?async=1 the response is 202 + operationId; a refusal that lands after the restart queued behind another operation arrives as the operation's terminal error event with the same code — the ledger books it as skipped, not as a failed restart.",
    },
