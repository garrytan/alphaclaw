...
const { sanitizeLabel, toTableCell } = require("./utils/sanitize-label");
...
// sanitizeLabel/toTableCell live in utils/sanitize-label.js (shared with the
// agent-admin skill builder) — same chokepoint contract as before.
...
// Strict variants for WRITE-side consumers (fix wave F085): syncConfigForTelegram
// derives openclaw.json topic routing and maxConcurrent from the registry, so a
// corrupt file read as "empty" wiped every per-topic agentId/systemPrompt pin
// and floored concurrency on the next /configure, /workspace repair, or
// /topics/bulk. These throw TopicRegistryReadError (TOPIC_REGISTRY_UNREADABLE)
// instead; the telegram routes already map that code to a fail-closed 503.
const getGroupStrict = (groupId) => {
...
const countActiveTopics = (registry) => {
  let count = 0;
  for (const group of Object.values(registry.groups)) {
...
const getActiveTopicCountStrict = () => countActiveTopics(readRegistryRaw().registry);

const getActiveTopicCount = () => countActiveTopics(readRegistry());
...
// Render the topic registry as a markdown section for the merged bootstrap file
const renderTopicRegistryMarkdown = ({
  ...
  lines.push(
    ...
    "- Never edit `hooks/bootstrap/AGENTS.md` directly for topic changes",
    "",
  );
