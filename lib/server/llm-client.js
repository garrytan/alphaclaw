...
const createFrontierLlmClient = ({
  ...
  const completeWithCandidate = async (
    candidate,
    { system, prompt, maxTokens, timeoutMs, signal },
  ) => {
    ...
    let json = null;
    try {
      response = await waitForSignal(() => fetchImpl(request.url, {
        method: "POST",
      ...
      try {
        json = await waitForSignal(() => response.json(), controller.signal);
      } catch (error) {
      ...
      // undici wraps a body-read abort in `TypeError: terminated` (name is
      // NOT "AbortError"), so also trust the signal itself.
  ...
  const complete = async ({
    ...
    for (const candidate of ranking) {
      if (signal?.aborted) return { ok: false, cancelled: true, error: "operation cancelled", attempts };
      if (badKeyProviders.has(candidate.provider)) continue;
      if (!resolveKey(candidate)) continue;
      const remaining = Math.min(deadlineMs - (nowFn() - startedAt), deadlineAt - nowFn());
      if (remaining <= 0) {
