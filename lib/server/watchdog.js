...
const createWatchdog = ({
  ...
  // Two-tick debounce: a transient cgroup read failure (EMFILE under load)
  // reads as a capacity change; retuning on it would revert/re-adopt config
  // and send a spurious resize notification. Require the change to persist
  // across two consecutive ticks before acting.
  let resizeChangeSeen = false;
  const checkContainerResize = async () => {
    if (resizeCheckInFlight) return;
    try {
      const { hasCapacityChanged } = require("./machine-profile");
      if (!hasCapacityChanged()) {
        resizeChangeSeen = false;
        return;
      }
      if (!resizeChangeSeen) {
        resizeChangeSeen = true;
        return;
      }
      resizeChangeSeen = false;
    } catch {
  ...
  const onGatewayExit = ({
    ...
    // Scheduled AFTER this handler's synchronous crash/config_error logEvent
    // so the classifier's event lands INSIDE the incident those events open
    // (the incident tracker stamps events to the active incident).
    setImmediate(() => {
