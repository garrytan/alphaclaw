    ...
    try {
      ...
      if (kHeapOomPattern.test(tailText)) {
        ...
        try {
          ...
          if (active != null && memMb > 0) {
            const ceiling = Math.round(0.85 * memMb);
      ...
      if (code === 137 || signal === "SIGKILL") {
        ...
        await notifyOncePerIncident(
          ...
          "Gateway was killed by the container (out of memory at the BOX level, not the V8 heap). Reduce concurrent load or upgrade the container's memory plan — raising the gateway heap will not help.",
          correlationId,
          "autotune",
        );
      }
