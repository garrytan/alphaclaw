...
const createWatchdog = ({
  ...
  const maybeRunMemoryMitigation = async (
    ...
    try {
      ...
      try {
        ...
        const abortedByLease =
          err?.aborted === true && err?.reason === "aborted_by_caller";
  ...
  const applyHealthyProbeResult = async ({
    ...
    // 5. Deferred replacement verification: identity observed + ready. Only
    ...
    if (state.crashTimestamps.length >= kWatchdogCrashLoopThreshold) {
      ...
      if (state.autoRepair) {
        void runRepair({
          ...
          // transient; keep retrying on a short cadence (bounded) instead of
          // silently dropping the repair the notification promised.
