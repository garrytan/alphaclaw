...
// Liveness of a pid without signalling it: ESRCH = gone; EPERM = alive but
// not ours (still alive). Injected (createWatchdog pidAlive) for tests.
...
const createWatchdog = ({
  ...
  // ── Relaunch / identity seams (v0.9.74; all optional, defaults keep the
  ...
  // process.kill(pid, 0) liveness for the probe-detected death fast path.
  ...
  const state = {
    ...
    // Serving identity (v0.9.74): WHICH process answers the port, kept apart
    ...
    // sustained-failure gate in front of doctor --fix. Startup failures
    // count too, so the 3-strike startup behaviour is unchanged.
  ...
  //     │ installs pendingReplacement {generationWatermark} FIRST (a launch handler
    ...
    typeof requestGatewayLaunch === "function"
      ...
      : async () => {
          ...
          try {
            ...
            return child
              ? shape("launch_requested", { child, pid: child.pid ?? null })
              : shape("launch_aborted");
          } catch (error) {
            return shape("launch_failed", {
              error,
  ...
  const failPendingReplacement = (pending, reason, extra = {}) => {
    logEvent(
      "restart",
      pending.source,
      "failed",
    ...
    clearPendingReplacement(pending);
    flushPendingProbeRun();
    noteRepairVerdict(pending.source, kRestartVerdicts.REPLACEMENT_FAILED);
  };
  ...
  // A MANAGED launch payload matches by generation > watermark (a handler
  // that fired during the launch call), equal generation, or launcher pid.
  ...
  const runVerifiedRelaunch = async ({
    ...
    const applyLaunchOutcome = async (launch, { allowIncumbent }) => {
      const outcome = String(launch?.outcome || "launch_failed");
      ...
      if (outcome === "child_retained" || outcome === "incumbent_present") {
        if (intent === "replace") return replaceIncumbent(launch);
        if (outcome === "child_retained") {
          clearPendingReplacement(pending);
          logEvent(
            "restart",
            source,
    ...
    const resolveIncumbentPresent = async (launch, { allowIncumbent }) => {
      ...
      if (!healthy) {
        ...
        state.degradedReason = "incumbent_unhealthy";
    ...
    const replaceIncumbent = async (launch) => {
      ...
      try {
        ...
        onExpectedRestartSettled();
        const aborted = err?.aborted === true && err?.reason === "aborted_by_caller";
        const reason =
          err?.incumbent === true
        ...
        state.health = "unhealthy";
        return finish(kRestartVerdicts.REPLACEMENT_FAILED, { reason });
      }
    };
  ...
  const runRepair = async ({ source, correlationId, force = false }) => {
    ...
    try {
      ...
      if (ok) {
        ...
        if (verdict !== kRestartVerdicts.REPLACEMENT_PENDING) {
        ...
        // notification and settle probe (restarting → running | stopped).
        if (!relaunch.coldRestart) {
          state.health = "unknown";
          state.lifecycle = "running";
        }
  ...
  const applyHealthyProbeResult = async ({
    ...
    // 2'. Identity gate: while a replacement is pending and unobserved, this
    ...
    if (pending && !pending.identityObservedAt) {
      ...
      evaluatePendingReplacementDeadline({
        onExpired: () => {
          state.health = "unhealthy";
          state.degradedReason = "replacement_not_ready";
        },
      });
      return probePassed({
        identityClear: false,
    ...
    if (state.readiness === "not_ready") {
      ...
      state.degradedReason = "readiness_failing";
      ...
      await notifyOncePerIncident(
        ...
        [
          "🐺 *AlphaClaw Watchdog*",
          withViewLogsSuffix(`🟡 Gateway is up but not ready — ${reason}`),
        ].join("\n"),
        correlationId,
        "health_check",
        { verbose: true },
    ...
    // 3. Healthy and identity-clear: recovery.
    flushPendingProbeRun();
    ...
    // 5. Deferred replacement verification: identity observed + ready.
    let verdict = null;
    if (pending) {
      if (state.readiness === "ready") {
        ...
        verdict = evaluatePendingReplacementDeadline({
          onExpired: () => {
            state.health = "unhealthy";
            state.degradedReason = "replacement_not_ready";
          },
    ...
    if (state.crashTimestamps.length >= kWatchdogCrashLoopThreshold) {
      ...
      void notifyOncePerIncident(
        ...
        [
          ...
          `Relaunch attempts: ${state.crashTimestamps.length} in the last ${Math.floor(kWatchdogCrashLoopWindowMs / 1000)}s`,
  ...
  const applyUnhealthyIncumbentConflict = ({
    ...
    void notifyOncePerIncident(
      ...
      [
        ...
        withViewLogsSuffix(
          stateWriter
            ? `🟡 Another OpenClaw process (${conflict.holderRole || "state writer"}, ${holder}) holds the state directory — the gateway will be relaunched once it releases`
  ...
  const onGatewayExit = ({
    ...
    if (
      state.pendingReplacement &&
      pendingMatchesExit(state.pendingReplacement, { pid, workerPid, generation })
    ) {
      failPendingReplacement(state.pendingReplacement, "replacement_exited", {
        code: code ?? null,
        signal: signal ?? null,
        expectedExit,
      });
  ...
  const onGatewayLaunch = ({
    ...
    if (
      ...
      // Idempotent redelivery; the worker may resolve later than the root.
  ...
  const stop = () => {
    ...
    state.awaitingAutoRepairRecovery = false;
    state.pendingRecoveryNoticeSource = "";
    // A stopped watchdog verifies nothing: the obligation dies with it.
  ...
  const getStatus = () => {
    ...
    return {
      ...
      // Serving identity + readiness axis (v0.9.74). Stable scalars/ISO only
