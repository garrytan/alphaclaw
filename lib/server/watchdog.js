...
const createWatchdog = ({
  ...
  const maybeRetryAfterConfigChange = () => {
    ...
    // fresh config-error latch); bounded once per distinct mtime above.
    void notify(
      [
        "🐺 *AlphaClaw Watchdog*",
  ...
  const notify = async (
    ...
    const result = await notifier.notify(message, { eventType, verbose });
    logEvent(
      "notification",
      "watchdog",
  ...
  const updateSettings = ({
    ...
    if (!hasAutoRepair && !hasNotificationsEnabled && !hasNotificationsVerbose) {
      throw new Error(
      ...
      void notifyOncePerIncident(
        ...
        [
          ...
          ...(state.autoRepair
            ? []
            : [
                // E5 (TODOS:149): name the remediation with the Watchdog
