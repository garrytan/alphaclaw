...
const createWatchdog = ({
  ...
  const state = {
    ...
    // (generation-local like the episode key: a liveness flap keeps it).
  ...
  // telemetry and the transitional clock. Three things are NOT part of the
  // axis and survive a liveness flap (resetReadinessAxis alone): the
  // degradation-episode key (readinessDegradedKey — what THIS generation's
  // last consumed /readyz said; the X1 recovery hold is keyed on it), the
  // hold clock (readinessProbeErrorSinceMs — the hold's bound) and the
  // telemetry floors. A generation change clears all of them too
  // (resetReadinessGeneration — TODOS T-e, done: a relaunched gateway that
  // fails on the same components starts a NEW episode with its own opening
  // row). readinessEpisodeSeq is monotonic and never reset.
  ...
  // AXIS only. The episode key, the hold clock and the floors stay: a
  // flapping /health must not turn the next /readyz transport error into an
  // assumed recovery (#87 RT2), re-arm a row or re-spawn the collector.
  ...
  // hold clock AND the floors (incl. the per-key advisory Doctor floor) start
  // clean. A new generation is a new readiness episode (TODOS T-e): the same
  // failing components on the relaunched gateway write their own
  // readiness_degraded/failed row and bump readinessEpisodeSeq; nothing said
  // by the previous process can hold or dedupe the new one.
  ...
  // One collector call → { stdout, spawnStartedAtMs }. The injected collector
  // is collectWithMeta (server.js): it resolves { stdout, spawnStartedAtMs }
  // with stdout = usable Doctor output or null. Anything that is not an
  ...
  const applyReadinessVerdict = ({ probe, source, correlationId, outcome }) => {
    ...
    // not early-return into the closed one.
    if (!activeIncidentKey && state.readinessDegradedKey) state.readinessDegradedKey = "";
  ...
  const applyHealthyProbeResult = async ({
    ...
    if (readinessError) {
      ...
      // #87 RT2) or a transitional not_ready still inside its budget (the
      // key is never set for those); `readiness === "not_ready"` is the
      ...
      // axis AND the key, so a fresh gateway never inherits a hold (E1, R1).
