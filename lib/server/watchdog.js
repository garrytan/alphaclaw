...
// opt-in via watchdog.memory.autoRestart, braked to 2 restarts per rolling
// 24h at least 6h apart. The brake is PERSISTED (fast-pressure path can
...
const kMemoryMitigationMaxPerWindow = 2;
const kMemoryMitigationMinIntervalMs = 6 * 60 * 60 * 1000;
// A FAILED restart never consumes the 2/24h success budget (that would let
...
const createWatchdog = ({
    ...
    try {
      ...
      await applyResourceAutotune({
        ...
        deps: {
          ...
          notify: (message) => void notify(message, "", "autotune"),
  ...
  //      ├─ persisted rate brake (2 per 24h, ≥6h apart) ─▶ skip + one notice
  ...
  const loadMemoryMitigationTimestamps = () => {
    ...
    try {
      ...
      memoryMitigationTimestamps = Array.isArray(parsed?.restarts)
        ? parsed.restarts.filter((t) => Number.isFinite(t))
        : [];
    } catch {
  ...
  const maybeRunMemoryMitigation = async (
    ...
    const braked =
      stamps.length >= kMemoryMitigationMaxPerWindow ||
      (stamps.length > 0 &&
        now - stamps[stamps.length - 1] < kMemoryMitigationMinIntervalMs);
    if (braked) {
      logEvent(
        "memory",
        "memory-monitor",
    ...
    // TOCTOU re-check: the notify above can take seconds. Re-read the gates
    // (fresh settings read too — a disarm PUT counts) and refund the brake
    // stamp on veto: no restart happened, the budget wasn't spent.
    const vetoReason = memoryMitigationVetoReason(readMemorySettingsSafe());
    if (vetoReason !== null) {
    ...
    recordOperationEvent({
      ...
      status: "started",
      details: { trigger: "memory_mitigation", episodeId: snapshot.episodeId },
    });
    onExpectedRestart({ expiresAt: Date.now() + kGatewayLifecycleLeaseMs });
    try {
      ...
      // Refund the budget stamp — a failed restart mitigated nothing, and two
      // transient failures must not disable protection for 24h. The shorter
      // failure cooldown (above) owns anti-thrash instead.
      memoryMitigationTimestamps = memoryMitigationTimestamps.filter(
        (t) => t !== now,
      );
      persistMemoryMitigationTimestamps();
      ...
      // Settle ALWAYS (mitigation success = restart resolved AND the settle
      // probe confirms healthy; a FAILED restart must never stay hidden
      // inside an expected-restart window).
  ...
  const checkMemoryTrend = async () => {
    ...
    try {
      ...
      await maybeRunMemoryMitigation(
        ...
        // A null RSS read is a MISS: the held critical verdict keeps
        // detecting, but enforcement demands this tick's own evidence.
        { freshEvidence: sample.rssBytes != null },
      );
    } catch (err) {
  ...
  const onGatewayExit = ({
    ...
    const exitedGatewayPid = state.gatewayPid;
    const monitoredExit =
    ...
    if (monitoredExit) {
      ...
      // Stop sampling the dead pid: the OS can reuse it before relaunch, and
      // a foreign process's RSS must never blend into the gateway's trend.
      // (exitedGatewayPid keeps it for the classification paths below.)
