...
const createWatchdog = ({
  ...
  // it:
  //   fence 1  after /health          (runHealthCheck)
  ...
  const probeGatewayReadiness = async () => {
    ...
    try {
      ...
      // Oversize bodies are never parsed: the classifier reads them as
      // `malformed` from the size alone.
      const oversize = rawBody.length > kReadyzBodyMaxChars;
      let parsedBody = null;
      if (!oversize) {
        try {
          parsedBody = rawBody ? JSON.parse(rawBody) : null;
  ...
  const runAdvisoryDoctorHint = async ({ probe, source, correlationId, degradedKey, episodeSeq }) => {
    ...
    try {
      if (typeof collectAdvisoryDoctorJson === "function") {
        // Production injects collectWithMeta ({ stdout, spawnStartedAtMs });
        // test seams may still hand back the bare stdout string (or null).
        ...
        // Fallback (test-only in practice): the verified cross-version
        // invocation, classified through the shared CLI classifier — usable
        // output only, never stderr (bare `doctor --json` is forbidden by the
        // context contract).
        ...
        stdout =
          classification.status === "usable" ? String(result?.stdout || "") : null;
      }
    } catch (error) {
      logAdvisoryFailure(probe)(error);
      return;
    }
    const dropReason = advisorySettleDropReason({
      probe,
      degradedKey,
      episodeSeq,
      spawnStartedAtMs,
  ...
  //             lifecycle left running AND ≠ lifecycleAtStart (launch/exit/stop landed) → identity only]
  ...
  const applyHealthyProbeResult = async ({
    ...
    // under the probe (exit, stop, expected restart — every event lifecycle
    // is non-running) owns state too: record nothing. An unchanged
