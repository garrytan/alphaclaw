...
const createWatchdog = ({
  ...
  const state = {
    ...
    // is gone or the port closed, when there is nothing left to wait for.
  ...
  const runRepair = async ({ source, correlationId, force = false }) => {
    ...
    if (!force && (await incumbentGraceActive())) {
      ...
      if (ok) {
        ...
        if (verdict !== kRestartVerdicts.REPLACEMENT_PENDING && !retained) {
          ...
          if (source !== "manual") {
            ...
            // not replace is gone or the port closed (nothingLeftToReplace).
  ...
  const runHealthCheck = async ({
    ...
    // can never come once the incumbent it could not replace is gone or the
    // port closed (the operator killed the wedged gateway the notice named).
    // Nothing is left to replace, so a relaunch is safe: lift the latch.
    if (state.awaitingAutoRepairRecovery && state.failedReplacement) {
      if (await nothingLeftToReplace(state.failedReplacement.pid)) {
        logEvent(
          "repair",
          source,
          "ok",
          {
            ...
            pid: state.failedReplacement.pid ?? null,
            failedVerdict: state.failedReplacement.verdict ?? null,
          },
          correlationId,
        );
        state.awaitingAutoRepairRecovery = false;
  ...
  // "Nothing left to replace/protect": the pid we could not stop is gone, or
  // the port no longer answers TCP. Dep-tolerant (a missing probe means
  // "unknown", never "gone").
  const pidGone = (pid) => {
  ...
  const portClosed = async () => {
    if (typeof probeGatewayTcp !== "function") return false;
    try {
      return (await probeGatewayTcp()) === false;
    } catch {
      return false;
    }
  };
  const nothingLeftToReplace = async (pid) => pidGone(pid) || (await portClosed());
  // The grace stands only while its holder is alive and the port answers.
  const incumbentGraceActive = async () => {
    if (!state.incumbentGraceUntilMs || Date.now() >= state.incumbentGraceUntilMs) return false;
    if (await nothingLeftToReplace(state.incumbentGracePid)) {
      state.incumbentGraceUntilMs = null;
      state.incumbentGracePid = null;
      return false;
    }
  ...
  const onExpectedRestart = ({ expiresAt = 0 } = {}) => {
    ...
    if (state.pendingReplacement) {
      failPendingReplacement(state.pendingReplacement, "replacement_superseded", {
