...
const createWatchdog = ({
  ...
  const runVerifiedRelaunch = async ({
    ...
    const resolveIncumbentPresent = async (launch, { allowIncumbent }) => {
      ...
      if (!healthy) {
        ...
        // a cold-boot budget before repair may stop it.
    ...
    const replaceIncumbent = async (launch) => {
      ...
      // cold-restarted by a repair.
      const healthyNow = await probeIncumbentHealthy();
      if (!holdStillValid(hold)) {
        clearPendingReplacement(pending);
  ...
  const runRepair = async ({ source, correlationId, force = false }) => {
    ...
    // latch): settle it here before the gate below reads it.
    ...
    if (state.readiness === "not_ready") {
      ...
      await notifyOncePerIncident(
        ...
        [
          "🐺 *AlphaClaw Watchdog*",
          withViewLogsSuffix(
            `🟡 Gateway is up but not ready — ${sanitizeNotificationText(reason)}`,
          ),
        ].join("\n"),
        correlationId,
        "health_check",
  ...
  const runHealthCheck = async ({
    ...
    state.readiness = "unknown";
    state.readinessReason = null;
    // A latched ownership conflict names the cause better than probe prose;
    // the enum survives until the latch clears (green probe / relaunch).
    ...
    // green yet keeps its cold-boot budget: repair (`replace`) would
    // `gateway stop` a gateway that is still coming up. One row per budget.
    if (state.incumbentGraceUntilMs && Date.now() < state.incumbentGraceUntilMs) {
  ...
  const getStatus = () => {
    ...
    return {
      ...
      incumbentConflict: state.incumbentConflict
        ...
        : null,
      incumbentGraceUntil: state.incumbentGraceUntilMs
