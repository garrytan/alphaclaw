...
// what came back. Precedence: unsupported statuses → other non-2xx/503 →
...
// → ready absent with failing[] (compat) → 503 without any of those (explicit)
// → ready. eventLoop.degraded is captured and never affects `kind`.
const classifyReadinessResponse = ({ httpStatus, body } = {}) => {
...
const createWatchdog = ({
  ...
  // Event-loop pressure telemetry episode + readiness_probe_error floors
  // (E4: reset with the readiness axis, so a relaunch starts clean).
  ...
  // correlationId, source, startedAtMs } — and every state write downstream
  // is fenced on it:
  ...
  //            also `lifecycle !== lifecycleAtStart` → identity-only result, R5)
    ...
    try {
      ...
      const httpStatus = Number.isFinite(response.status)
        ...
        : response.ok
          ? 200
          : 0;
      return classifyReadinessResponse({ httpStatus, body: parsedBody });
    } catch (error) {
  ...
  const evaluateReadiness = async (probe, source, correlationId) => {
    ...
    // The safe-mode axis is deliberately not seq-fenced (its own dedupe key
    // makes it idempotent); its notify await is the LAST await of the probe,
    // so every fenced write in applyHealthyProbeResult follows fence 3 with
    // nothing in between (#87 R3).
    if (observation.ok) {
  ...
  const applyReadinessObservation = (observation, source, correlationId) => {
    if (!observation.ok) {
      recordReadinessProbeError(observation, source, correlationId);
      return {
        probed: false,
    ...
    state.readyzFailing = observation.failing;
    state.readinessStatus = observation.status;
    ...
    let degradedKey = "";
    if (
      observation.kind === "not_ready" &&
      kReadinessTransitionalReasons.has(observation.unreadyReason)
    ) {
      ...
      if (state.readinessTransitionalExpired) {
        degraded = true;
        reason = `${observation.status} did not complete within ${Math.round(
          kGatewayRestartReadyTimeoutMs / 1000,
        )}s`;
        degradedKey = `${observation.status}:expired`;
      } else {
        transitional = true;
      ...
      if (degraded) {
        reason =
          observation.unreadyReason === "explicit"
            ? "ready:false"
            : observation.failing.join(", ").slice(0, 200) || "ready:false";
        degradedKey =
          observation.unreadyReason === "explicit"
            ? "ready:false"
            : observation.failing.slice().sort().join(",");
      }
    }
    ...
    return {
      ...
      unreadyReason: observation.unreadyReason,
      reason,
      degradedKey,
    };
  };
  ...
  //             lifecycle ≠ lifecycleAtStart (launch/exit/stop landed) → identity only]
