...
const createWatchdog = ({
  ...
  const maybeRunMemoryMitigation = async (
    ...
    if (vetoReason !== null) {
      ...
      persistMemoryMitigationTimestamps();
      // Refund the notification dedupe too: the message announced a restart
      // that never happened — the retry that actually restarts must notify.
      memoryNotifiedKeys.delete(`${snapshot.episodeId || "none"}:mitigation`);
      logEvent(
        "memory",
        "memory-monitor",
        "warning",
