...
const createWatchdog = ({
  ...
  // ownedHoldReasons (Stage 3 Eng 1C): the ONE caller that may repair under a
  // gatewayHold is a structural-repair caller that owns the hold's reason
  // (`version_mismatch`, once it has re-activated a binary that can read the
  // DBs — the doctor then runs from the RIGHT binary, C6). The Stage 3 ladder
  // itself never runs Doctor (Codex 11: relaunch first; Doctor only through
  // the crash-loop discipline if the corrected binary still crashes), so
  // today no caller passes it; the seam stays for one that does. Every other
  // source — manual included — refuses `gateway_held`; an unreadable hold
  // state refuses regardless.
  const runRepair = async ({
    ...
    if (typeof releaseChannelHooks?.getInfo === "function") {
      ...
      const ownsHold =
        Array.isArray(ownedHoldReasons) &&
        typeof holdInfo?.gatewayHold?.reason === "string" &&
        ownedHoldReasons.includes(holdInfo.gatewayHold.reason);
      const holdReason =
        unreadable != null
          ? "gateway_hold_unreadable"
          : holdInfo?.gatewayHold && !ownsHold
            ? "gateway_held"
            : null;
      if (holdReason) {
  ...
  return {
    ...
    setBootVerdict,
    latchVersionMismatch,
    // Stage 3 (Eng 1C / I3): the structural repair drives the doctor rung
    // through runRepair with ownedHoldReasons; routes keep using
    // triggerRepair (manual, forced, owns nothing).
    runRepair,
    start,
    stop,
  };
