...
const createWatchdog = ({
  ...
  const formatMemoryEta = (iso) => {
    ...
    return mins >= 90 ? `in ~${Math.round(mins / 6) / 10}h` : `in ~${mins}m`;
  };
  ...
  const maybeRunMemoryMitigation = async (snapshot, settings, correlationId) => {
    if (!settings.effectiveAutoRestart) return;
    if (
    ...
    if (memoizedRollbackEligible()) return;
    if (
      state.managedOperationActive ||
