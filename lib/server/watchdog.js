...
const { createCrashRecovery, waitForRecovery } = require("./watchdog-crash-recovery");
const fs = require("fs");
...
const createWatchdog = ({
  ...
  const runVerifiedRelaunch = async ({
    ...
    const applyLaunchOutcome = async (launch, { allowIncumbent }) => {
      ...
      if (outcome === kGatewayLaunchOutcomes.LAUNCH_REQUESTED) {
        if (source === "state_writer_conflict") state.conflictRelaunchTimestamps.push(Date.now());
  ...
  crashRecovery = createCrashRecovery({
    ...
    attempt: async (record, isCurrent) => {
      ...
      try {
        ...
        return result.verdict === kRestartVerdicts.LEASE_EXPIRED
          ? { retry: "lease_expired" } : { reason: result.verdict };
      } finally {
        releaseLifecycleLock?.();
        state.operationInProgress = false;
  ...
  const restartAfterCrash = (correlationId,
    ...
    trimCrashWindow();
    const recentCrashes = source === "state_writer_conflict"
