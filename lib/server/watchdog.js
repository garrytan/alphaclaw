...
const createWatchdog = ({
  ...
  // with the boot's own installed/expected pair.
  const setBootVerdict = (report) => {
    ...
    if (verdict.includes("installed_not_expected")) {
      latchVersionMismatch({
        expected: report?.openclaw?.expected ?? null,
        running:
          report?.openclaw?.installedAtBoot ??
          report?.serverPhase?.installedVersion ??
          null,
        source: "boot",
  ...
  const corroborateCrashCause = async ({ classified, code, signal, correlationId, exitAt }) => {
    if (!classified || !causeIsCorroboratable(classified.cause)) return;
    try {
      let facts = null;
      let factsError = null;
  ...
  return {
    ...
    // (boot-report-steps.js finalizeBootReport).
    setBootVerdict,
    start,
    stop,
  };
