...
const createWatchdog = ({
  ...
  const maybeRetryAfterConfigChange = () => {
    ...
    void (async () => {
      ...
      if (child) return;
      // No child: the prelaunch hook aborted the relaunch (fail-closed, not a
      // config error — see noteLaunchAbortedByPrelaunchHook) or the launch
      // was cancelled; only the latter re-latches EX_CONFIG.
      ...
      if (ok) {
        ...
        try {
          ...
          if (launchedGateway) {
            ...
            logEvent("restart", "repair", "failed", noChildDetails(), correlationId);
          }
        } catch (err) {
          logEvent(
            "restart",
  ...
  const restartAfterCrash = async (correlationId) => {
    ...
    try {
      ...
      if (child) {
        ...
        logEvent("restart", "exit_event", "failed", noChildDetails(), correlationId);
      }
    } catch (err) {
      logEvent(
        "restart",
  ...
  const runConfigMedic = async ({
    ...
    try {
      ...
      // A null child without a throw is the prelaunch hook's fail-closed
      // abort (or a shutdown cancel) — not a second EX_CONFIG. The hook path
      // stands down without the latch or its notification (the hook handler
      // already notified); a thrown launch keeps latching.
      const hookOutcome =
        !child && !launchThrew ? prelaunchHookAbortedLaunch() : null;
      if (child) {
...
//                    `prelaunch-hook-<code>-<site>`. `notify` is the
//                    upgradeNotifier.notify shape (message, opts) and is
//                    fire-and-forget here: gateway.js calls the handler
//                    synchronously inside a launch and only logs a throw.
const createGatewayPrelaunchHookHandler = ({ watchdog, notify = null } = {}) => {
      ...
      try {
        Promise.resolve(
          notify(message, {
            ...
            id: `prelaunch-hook-${code || status}-${site.replace(/\s+/g, "-")}`,
          }),
        ).catch((err) => {
          console.warn(
...
module.exports = { createWatchdog, createGatewayPrelaunchHookHandler };
