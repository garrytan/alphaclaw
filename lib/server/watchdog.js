...
const createWatchdog = ({
  ...
  const applyReadinessVerdict = ({ probe, source, correlationId, outcome }) => {
    ...
    // the same components logs `floor` instead of spawning again (P2) — and
    // at most one spawn per kAdvisoryDoctorGlobalFloorMs regardless of key
    // (#87 F4, logged `floor` … `(global)`): the key is built from
    // gateway-controlled component names, so a rotating failing[] list must
    // not buy a Doctor per probe. Both floors are generation-local
    // (resetReadinessTelemetryFloors).
    const now = Date.now();
    ...
    if (
      ...
      logAdvisoryDrop("floor", probe, " (global)");
      return;
    }
