...
const { createMedicAdmission, rollbackEligibleFrom, medicBlockedOutcome } = require("./medic-admission");
...
const createWatchdog = ({
  ...
  // runVerifiedRelaunch right before requestGatewayLaunch; absent legacy seams skip the step.
  ...
  const medicAdmission = createMedicAdmission({
    getChannelInfo: releaseChannelHooks?.getInfo ? () => releaseChannelHooks.getInfo() : null,
  });
  ...
  const notifyAutoRepairPaused = async ({ pause, corroborated, correlationId }) => {
    ...
    const stateDb = await readRecoveryDiscovery(readStateDbForLaunch).catch(() => null);
    ...
    const recoveryHint = pause.reason === "recovery_choice_required"
      ? "Next: open the Upgrade page and choose database-set protection or explicitly approve forward-only recovery before starting this build."
      : ["state_db_unreadable", "state_db_unverified"].includes(pause.reason)
        ? "Next: inspect the database recovery diagnosis and restore or repair unreadable state, then retry compatibility verification. No build was launched."
        : null;
    await notifyOncePerIncident(
      ...
      [
        ...
        `Last plan: ${lastPlan}`,
        recoveryHint || `Next: ${labels.join(" · ")} from the Watchdog tab — a Repair sent with force resumes automatic repair once.`,
  ...
  const runVerifiedRelaunch = async ({
    ...
    try {
      ...
      compat = await readRuntimeCompatibility(readOptions);
    } catch (error) {
      clearPendingReplacement(pending);
      if (!relaunchStillValid()) return leaseExpiredSkip({ phase: "launch_discovery" });
      if (typeof assessLaunchCompatibility !== "function") {
      ...
      compat = { compatible: null, migrationRequired: null,
        reasons: ["launch_discovery_timeout"], holdReason: "state_db_unverified" };
    }
    ...
    const refusal = await refuseUnsafeCompatibility({ compat, source, correlationId, intent,
      eventType: "restart", pending, hold, signal, isCurrent: relaunchStillValid });
    if (refusal) return finish(refusal.verdict, refusal);
    const launch = await requestLaunch({ reconcileIncumbent: true, shouldAbort });
    return applyLaunchOutcome(launch, { allowIncumbent: true });
  };
  const refuseUnsafeCompatibility = async ({ compat, source, correlationId, intent, eventType, pending = null,
    hold = null, signal = null, isCurrent = () => true, deadlineAt = hold?.expiresAt ?? Infinity }) => {
    if (!compat || (compat.compatible === true && compat.migrationRequired === false)) return null;
    const stillCurrent = () => !signal?.aborted && !state.stopRequested && holdStillValid(hold) && isCurrent();
    if (!stillCurrent()) return { verdict: kRestartVerdicts.LEASE_EXPIRED, reason: "lease_expired" };
    if (pending) clearPendingReplacement(pending);
    if (compat.compatible !== false || ["state_db_unreadable", "state_db_unverified"].includes(compat.holdReason)) {
      const migrationRequired = compat.compatible === true && compat.migrationRequired === true;
      const reason = migrationRequired ? "recovery_choice_required"
        : compat.holdReason === "state_db_unreadable" ? "state_db_unreadable" : "state_db_unverified";
      const reasons = Array.isArray(compat.reasons) ? compat.reasons : [];
      const hint = migrationRequired
        ? "Choose database-set protection or explicitly approve forward-only recovery from the Upgrade page."
        : "Restore or repair unreadable database state, then retry compatibility verification; unknown compatibility cannot be waived.";
      if (migrationRequired && typeof releaseChannelHooks?.holdRecoveryChoice === "function") {
        try {
          await readRecoveryDiscovery(() => {
            if (!stillCurrent()) return null;
            return releaseChannelHooks.holdRecoveryChoice({ verdict: compat, hold });
          }, { signal, deadlineAt });
        } catch (error) {
          if (stillCurrent()) console.warn(`[watchdog] recovery choice hold could not be recorded: ${error?.code || "recovery_hold_failed"}`);
        }
        if (!stillCurrent()) return { verdict: kRestartVerdicts.LEASE_EXPIRED, reason: "lease_expired" };
      }
      logEvent(eventType, source, "skipped", {
        reason, holdReason: reason, reasons, intent, hint,
        compatible: compat.compatible ?? null,
        migrationRequired: compat.migrationRequired ?? null,
      }, correlationId);
      state.health = "unhealthy";
      state.degradedReason = migrationRequired
        ? "Database migration requires an operator recovery choice."
        : "Database launch compatibility could not be verified.";
      await latchAutoRepairPause({
        cause: reason,
        fingerprint: fingerprintGatewayCrash({ cause: reason, matchedLine: reason }),
        reason, source: "launch_compat", correlationId,
        corroborated: migrationRequired || reason === "state_db_unreadable",
        plan: [{ step: "launch_compatibility", outcome: reason }],
      });
      return { verdict: kRestartVerdicts.RECOVERY_REQUIRED, reason, reasons, hint };
    }
    const info = channelInfo();
    ...
    const reasons = Array.isArray(compat.reasons) ? compat.reasons : [];
    logEvent(eventType, source, "skipped", {
      reason: "version_mismatch", expected, running, intent, reasons,
      ...(compat.holdReason ? { holdReason: compat.holdReason } : {}),
    }, correlationId);
    latchVersionMismatch({
      expected, running, source: eventType === "repair" ? "repair" : "relaunch", correlationId,
    ...
    return { verdict: kRestartVerdicts.VERSION_MISMATCH, reason: "version_mismatch", reasons };
  };
  const assessRelaunchCompatibility = async () => {
    if (assessLaunchCompatibility == null) return null;
    try {
      if (typeof assessLaunchCompatibility === "function") {
      ...
      console.warn(
        `[watchdog] launch compatibility check refused relaunch: ${err?.message || err}`,
      );
    }
    return { compatible: null, migrationRequired: null,
      reasons: ["launch_compatibility_unavailable"], holdReason: "state_db_unverified" };
  };
  const readRuntimeCompatibility = async (options) => {
    try { return await readRecoveryDiscovery(assessRelaunchCompatibility, options); }
    catch (error) {
  ...
  let expectedRestartOwner = null;
  const clearExpectedRestartWindow = () => {
    expectedRestartOwner = null;
  ...
  const readRepairAdmission = ({ source, correlationId, force = false, resumePaused = false }) => {
    ...
    if (autoRepairPauseActive()) {
      if (resumePaused && !["recovery_choice_required", "state_db_unreadable", "state_db_unverified"].includes(state.autoRepairPaused.reason)) {
  ...
  const runRepair = async ({
    ...
    let attempt = null;
    try {
      if (assessLaunchCompatibility != null) {
        const compat = await operation.read(() => readRuntimeCompatibility({ signal: operation.signal,
          deadlineAt: Math.min(operation.deadlineAt, releaseLifecycleLock?.expiresAt ?? Infinity) }));
        operation.assertActive();
        const blocker = repairPolicy.read({ hold: releaseLifecycleLock });
        if (blocker) {
          logEvent("repair", source, "skipped", { reason: blocker.code }, correlationId);
          return { ok: false, skipped: true, reason: blocker.code };
        }
        if (autoRepairPauseActive()) return { ok: false, skipped: true, reason: "auto_repair_paused", pause: state.autoRepairPaused };
        const refusal = await refuseUnsafeCompatibility({ compat, source, correlationId,
          intent: "doctor_repair", eventType: "repair", hold: releaseLifecycleLock,
          signal: operation.signal, deadlineAt: operation.deadlineAt });
        if (refusal) {
          noteRepairVerdict("repair", refusal.verdict);
          return { ok: false, skipped: true, launchedGateway: false, ...refusal };
        }
        operation.assertActive();
      }
      attempt = { id: ++state.repairAttemptSeq, correlationId, source };
      ...
      if (ok) {
        ...
        if (verdict === kRestartVerdicts.VERSION_MISMATCH || verdict === kRestartVerdicts.RECOVERY_REQUIRED) {
          ...
          return {
            ok: false,
            skipped: true,
            reason: relaunch.reason || "version_mismatch",
            verdict,
            launchedGateway: false,
            pending: false,
            result,
      ...
      logEvent("repair", source, "failed", { attemptId: attempt?.id ?? null,
  ...
  const applyLivenessLatches = ({ wasUnhealthy }) => {
    if (!expectedRestartOwner) clearExpectedRestartWindow();
    state.lifecycle = "running";
    state.lastExit = null;
  ...
  const runHealthCheck = async ({
    ...
    if (parsed.ok) {
      ...
      if (restartWindowActive && (state.lifecycle === "restarting" || expectedRestartOwner)) {
  ...
  const runConfigMedic = async ({
    ...
    const medicBlocker = medicAdmission.read();
    if (medicBlocker) {
      state.medicAttempts = Math.max(0, state.medicAttempts - 1);
      logEvent("medic", "exit_event", "skipped", {
        reason: medicBlocker.code,
        error: medicBlocker.error,
        attempt,
      }, correlationId);
      releaseLifecycleLock?.();
      try {
    ...
    try {
      ...
      try {
        outcome = await operation.read(() => configMedic.run({
            exitCode,
            stderrTail,
            allowDoctorFix: allowDoctorFix && !medicAdmission.read({ doctor: true }),
            attempt,
            budgetMs: medicRunBudgetMs,
            operation,
            signal: operation.signal,
            deadlineAt: operation.deadlineAt,
            assertMutationAllowed: ({ doctor = false } = {}) => medicAdmission.assert({ doctor, allowDoctorFix }),
          }));
        operation.finishWork();
        await operation.cleanup.wait();
      } catch (error) {
        outcome = medicBlockedOutcome(error) || {
          fixed: false, tier: operation.signal.aborted ? "timeout" : "error", error: error.message,
        };
      }
      ...
      logEvent(
        "medic",
        "exit_event",
        outcome.fixed ? "ok" : outcome.skipped ? "skipped" : "failed",
        {
          attempt,
      ...
      if (!outcome.fixed) {
        ...
        if (outcome.tier !== "blocked" && !operation.signal.aborted && tryForwardRecovery({ exitCode, correlationId })) return;
  ...
  const handleConfigErrorExit = ({
    ...
    if (releaseChannelHooks?.getInfo) {
      ...
      configChannelInfoUnreadable = configChannelSnapshot === null || !!configChannelSnapshot.stateCorrupted;
    }
    const configChannelInfo = configChannelInfoUnreadable ? null : rollbackEligibleFrom(configChannelSnapshot);
  ...
  const onGatewayExit = ({
    ...
    if (!expectedExit && expectedRestartOwner) clearExpectedRestartWindow();
  ...
  const onExpectedRestartSettled = (owner = null) => {
    if (owner !== expectedRestartOwner) return;
    clearExpectedRestartWindow();
    const generationAtSettle = pendingExitProbeToken;
    void runHealthCheck({
      source: "operation_end",
      allowDuringOperation: true,
      allowAutoRepair: false,
    }).then((healthy) => {
      if (generationAtSettle !== pendingExitProbeToken) return;
  ...
  const onExpectedRestart = ({ expiresAt = 0, retainUntilReady = false } = {}) => {
    expectedRestartOwner = retainUntilReady ? Object.freeze({}) : null;
    advancePendingExitProbeToken();
    clearDegradedHealthCheckTimer();
    clearSafeModeState();
    flushPendingProbeRun();
  ...
  const start = ({ child = null } = {}) => {
    ...
    if (child?.pid && child.exitCode == null && child.signalCode == null && !child.killed) {
      const pending = installPendingReplacement({
        source: "boot",
        correlationId: createCorrelationId(),
        intent: "relaunch_if_absent",
      });
      armPendingReplacement(pending, { pid: child.pid, generation: readLaunchGeneration() });
    }
    startBootstrapHealthChecks();
    startTcpWatcher();
    startMemoryMonitor();
