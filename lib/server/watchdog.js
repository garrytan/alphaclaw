...
const { classifyFromClawCmd } = require("./doctor/classify-doctor-cli");
...
// Detached advisory Doctor floor: one `doctor --lint --json` collect per
...
// non-object body → status ∈ {starting, draining} (transitional, always
// not_ready) → ready:false (components if failing[] non-empty, else explicit)
// → ready absent with failing[] (compat) → 503 → components if failing[]
// else explicit → ready. eventLoop.degraded is captured and never affects
// `kind`.
...
const createWatchdog = ({
  ...
  const state = {
    ...
    // not_ready (X2). readinessProbeErrorSinceMs bounds the X1 recovery hold.
  ...
  // readiness_probe_error floor and the per-key advisory Doctor floor. They
  // survive a liveness flap (a failed /health resets only the readiness AXIS
  // — resetReadinessAxis) and reset on a gateway generation change
  // (resetReadinessTelemetryFloors: launch, exit, adoption, expected
  // restart, relaunch request, start, stop), so a flapping /health cannot
  // re-arm a row or a collector spawn inside its floor.
  ...
  // /readyz said, the transport kind it said it with, the event-loop
  // telemetry and the transitional / probe-error clocks. readinessDegradedKey
  // deliberately survives (TODO T-e: the same failing components across a
  // relaunch are one incident-row run) and readinessEpisodeSeq is monotonic.
  // The telemetry FLOORS are not part of the axis: a liveness loss keeps
  // them (resetReadinessAxis alone); a generation change clears them too
  ...
  const scheduleTransitionalRecheck = () => {
    ...
    transitionalRecheckTimer = setTimeout(() => {
      transitionalRecheckTimer = null;
      void runHealthCheck({ source: "readiness_recheck" });
  ...
  // A gateway generation change (launch, exit, adoption, expected restart,
  // relaunch request, start, stop): the axis AND the floors start clean.
  ...
  //                                    telemetry is consumed post-claim, R3)
  ...
  //   fence 4  after the recovery notice    (applyHealthyProbeResult)
  ...
  const runVerifiedRelaunch = async ({
    ...
    stateDb = await readStateDbForLaunch();
    ...
    const compat = await assessRelaunchCompatibility();
    if (!holdStillValid(hold)) {
      clearPendingReplacement(pending);
    ...
    if (braked) {
      ...
      await notifyMemoryOncePerEpisode(
        ...
        [
          ...
          "Trigger: `memory_leak`",
          "Repeated restarts have reached the mitigation budget — the growing memory scope needs a diagnosis. Run a Drift Doctor scan and use \"Ask agent to fix\".",
        ].join("\n"),
        correlationId,
      );
      return;
  ...
  const startBootstrapHealthChecks = () => {
    ...
    const runBootstrapCheck = async () => {
      ...
      // what closes a crash incident within 5s of `started`. Terminates by
      // the transitional budget (X2) at the latest.
      if (!isReadinessTransitional() && (healthy || state.health !== "unknown")) {
  ...
  const probeGatewayReadiness = async () => {
    ...
    try {
      ...
      // Test fetch stubs may omit `status`; a stub that says ok is a 200.
      const httpStatus = Number.isFinite(response.status)
        ? response.status
        : response.ok
          ? 200
          : 0;
  ...
  // await):
  //   evaluateReadiness(probe)        awaits /readyz, applies fence 2, runs the
  //                                   safe-mode axis (not seq-fenced by design;
  //                                   its notify await stays BEFORE fence 3) and
  //                                   returns { superseded, observation } — NO
  //                                   readiness write of any kind.
  ...
  //                                   readiness_probe_error rows), the transitional
  ...
  // Safe-mode notices ride the probe's readiness path (evaluateReadiness
  // awaits them before fence 3): a rejecting notifier must never turn a
  // consumed /readyz body into a readiness_probe_error + fail-open (#87
  // RT3). The safe_mode row is already written by then; the delivery
  // failure is one console line.
  ...
  // consumed observation, independent of the readiness verdict.
  const applySafeModeObservation = async (observation, source, correlationId) => {
  ...
  //   degradation_cleared  the key changed or readiness is no longer not_ready
  ...
  //                   than kAdvisoryDoctorFloorMs ago (decided before the
  //                   collector is called — no spawn at all)
  ...
  const advisorySettleDropReason = ({ probe, degradedKey, episodeSeq, spawnStartedAtMs }) => {
    ...
    if (state.readinessDegradedKey !== degradedKey || state.readiness !== "not_ready") {
      return "degradation_cleared";
    }
  ...
  // One collector call → { stdout, spawnStartedAtMs }. Production injects
  // collectWithMeta ({ stdout, spawnStartedAtMs }); test seams may still hand
  // back the bare stdout string (or null → unusable). Without an injected
  // collector (test-only in practice) the verified cross-version invocation
  // runs, classified through the shared CLI classifier — usable output only,
  // never stderr (bare `doctor --json` is forbidden by the context contract).
  const collectAdvisoryDoctor = async () => {
    if (typeof collectAdvisoryDoctorJson === "function") {
      ...
      if (typeof collected === "string") {
        return { stdout: collected, spawnStartedAtMs: null };
      }
      if (collected && typeof collected === "object") {
        return {
          stdout: typeof collected.stdout === "string" ? collected.stdout : null,
          spawnStartedAtMs: Number.isFinite(collected.spawnStartedAtMs)
            ? collected.spawnStartedAtMs
            : null,
        };
      }
      return { stdout: null, spawnStartedAtMs: null };
    }
    const spawnStartedAtMs = Date.now();
    const result = await clawCmd("doctor --lint --json", {
      quiet: true,
      timeoutMs: 20000,
    });
    const classification = classifyFromClawCmd(result || {});
    return {
      stdout: classification.status === "usable" ? String(result?.stdout || "") : null,
      spawnStartedAtMs,
    };
  };
  ...
  // again is dropped for good — no third spawn.
  ...
  const runAdvisoryDoctorHint = async ({ probe, source, correlationId, degradedKey, episodeSeq }) => {
    ...
    for (let attempt = 0; ; attempt += 1) {
      try {
        ({ stdout, spawnStartedAtMs } = await collectAdvisoryDoctor());
      } catch (error) {
      ...
      if (dropReason !== "stale_doctor_job" || attempt >= kAdvisoryStaleRetries) break;
    ...
    if (!stdout) {
      logAdvisoryDrop("unusable", probe);
      return;
    }
  ...
  const evaluateReadiness = async (probe, source, correlationId) => {
    ...
    // applySafeModeObservation runs behind fence 2 but its post-await writes
    // are not re-fenced (its own dedupe key makes them idempotent); its
    // notify await is the LAST await of the probe, so every fenced write in
    // applyHealthyProbeResult follows fence 3 with nothing in between (#87 R3).
    if (observation.ok) {
      await applySafeModeObservation(observation, source, correlationId);
    }
  ...
  const applyReadinessVerdict = ({ probe, source, correlationId, outcome }) => {
    ...
    if (state.readinessDegradedKey === outcome.degradedKey) return;
    ...
    // immediately (decision 2.1). One collector spawn per failing-component
    // key per kAdvisoryDoctorFloorMs: a /readyz flapping ready↔not_ready on
    // the same components logs `floor` instead of spawning again (P2). The
    // floor map is generation-local (resetReadinessTelemetryFloors).
    const now = Date.now();
    const lastStartedAt = advisoryDoctorStartedAt[outcome.degradedKey];
    if (lastStartedAt != null && now - lastStartedAt < kAdvisoryDoctorFloorMs) {
      logAdvisoryDrop("floor", probe);
      return;
    }
    ...
    advisoryDoctorStartedAt[outcome.degradedKey] = now;
    void runAdvisoryDoctorHint({
      probe,
      source,
      correlationId,
      degradedKey: outcome.degradedKey,
      episodeSeq: state.readinessEpisodeSeq,
    }).catch(logAdvisoryFailure(probe));
  };
  ...
  const runRepair = async ({
    ...
    if (
      ...
      try {
        compatible = await releaseChannelHooks.compatibleBinForCurrentDb();
      } catch (err) {
    ...
    try {
      ...
      const result = repairRunner
        ? await repairRunner({ correlationId, bin: doctorBin?.bin ?? null })
        : doctorBin && typeof clawCmdWithBin === "function"
          ? await clawCmdWithBin(doctorBin.bin, "doctor --fix --yes", {
              quiet: true,
  ...
  // Transitional readiness (#87 A3): /readyz consumed a starting|draining
  // body inside the ready budget. The port answers, the gateway has not
  // finished coming up — NOT an incident, NOT a degradation, no hooks, no
  // notice: one deduped pending row keeps an open incident open (the tracker
  // appends on readinessPending) and the 5s cadence continues — the
  // bootstrap loop's own tick, or outside the loop one transitional recheck
  // shot per observation (#87 R7). A pending replacement is certified only
  // by a `ready` observation; its deadline still runs.
  const recordTransitionalReadiness = ({ source, correlationId, parsed, pending }) => {
  ...
  //                                        ├─ safe-mode axis (not seq-fenced; its notify await is
  //                                        │   the LAST await before fence 3 — R3)
  //                                        │ threw → readiness fails open below
  //                                        ▼
  //   [fence 3: superseded → return false, nothing written;
  //             lifecycle left running AND ≠ lifecycleAtStart during the /readyz + notify
  //             awaits (launch/exit/stop landed) → identity only]
  ...
  //   · applyReadinessObservation() (R3: telemetry, probe-error rows, transitional clock /
  //     X2 expiry, classification — all post-claim)
  //                    │
  //     ┌──────────────┼─────────────────────────┬──────────────────────────────┐
  //     ▼              ▼                         ▼                              ▼
  //  ready / unknown   not_ready (real)     not_ready (transitional)    probe error while not_ready
  //  (a fail-open      applyReadinessVerdict starting|draining           unavailable|timeout|malformed
  //   ends an open                          inside the ready budget     X1 recovery hold, DECIDED
  //   episode with                                                      HERE (R6): health restored to
  //   ok {assumed} +                                                    degraded + retry re-armed
  //   clears the key)                                                   (Y2) unless transitional →
  //     │              │                         │                      one recheck shot (R7)
  //     ▼              ▼                         ▼                              ▼
  ...
  //   → ok{verified} transition; Doctor      (X2 → real not_ready)
  //   + counter      DETACHED; ok
  ...
  //     │
  //   [fence 4 after the recovery notice: superseded → return false; pending moved → no reset]
  //
  ...
  const applyHealthyProbeResult = async ({
    ...
    try {
      readinessResult = await evaluateReadiness(probe, source, correlationId);
    } catch (err) {
      readinessError = err;
    }
    ...
    // during the /readyz + notify awaits (exit, stop, expected restart —
    ...
    if (readinessError) {
      ...
      // X1 recovery hold: the last accepted /readyz in THIS gateway
      ...
      // budget. `unsupported` (404 — endpoint gone) and `unconfigured` fail
      // open at once. Every launch/exit/stop resets readiness to "unknown",
      // so a fresh gateway never inherits a hold (E1, R1).
      const kind = readinessOutcome.kind ?? null;
      if (kReadinessTransportErrorKinds.has(kind) && state.readiness === "not_ready") {
    ...
    // 3b. X1 recovery hold: readiness is still the last accepted not_ready
    // and this probe's /readyz could not be read. No recovery, no "unknown":
    // one deduped pending row (the incident stays open — tracker appends on
    // readinessPending). Health and the retry loop were restored when the
    // hold was decided above (R6).
    if (recoveryHeld) {
    ...
    if (state.readiness === "not_ready") {
      ...
      if (isReadinessTransitional()) {
        return recordTransitionalReadiness({ source, correlationId, parsed, pending });
      }
  ...
  const runHealthCheck = async ({
    ...
    // AXIS only; the telemetry floors survive a liveness flap (P1).
    resetReadinessAxis();
  ...
  const restartAfterCrash = async (
    ...
    // A paused box relaunches nothing: the pause names a build that provably
    // cannot use what is on disk (B1.3); one row per crash says so.
    if (autoRepairPauseActive()) {
      logEvent(
        "restart",
        source,
