...
// Table first; the dist scan (resolveSupported) only for a version the table
// does not know at all (Eng 4A — a seeded/declared entry is already the
// dist's answer). Either oracle failing reads as unknown, never a throw.
const resolveChooserSupported = async (version, { table, resolveSupported }) => {
  ...
  if (fromTable?.source != null || typeof resolveSupported !== "function") return known;
  let declared = null;
  try {
  ...
  return { state: toSchemaInt(declared?.state), agent: toSchemaInt(declared?.agent) };
};
...
//                             only for versions the table does not know
