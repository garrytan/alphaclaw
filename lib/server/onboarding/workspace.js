...
const { resolveConfiguredOrigin } = require("../public-origin");
...
// Substituted path values are VALIDATED, never mutated (stripping would
// corrupt legal paths): a value with control chars, backticks, or template
// braces is not rendered — the token falls back to the env-var reference,
// which stays true because the gateway always exports OPENCLAW_STATE_DIR /
// OPENCLAW_HOME into the agent's shell.
const isSafePromptTemplateValue = (value) =>
  typeof value === "string" &&
  value.length > 0 &&
...
// {{TOKEN}} rendering for BOTH core-prompt templates (#121). Unknown tokens
// are left intact on purpose: a typo'd token stays visible in the rendered
// file instead of silently blanking an agent safety rule (and the shipped
// templates are pinned by a no-`{{`-survives test).
const renderPromptTemplate = (template, vars) =>
  String(template).replace(/\{\{([A-Z0-9_]+)\}\}/g, (match, tokenName) =>
    Object.prototype.hasOwnProperty.call(vars, tokenName) && vars[tokenName]
      ? vars[tokenName]
      : match,
  );

const buildPromptTemplateVars = ({
  setupUiUrl,
  openclawDir,
  rootDir,
  envFilePath,
}) => ({
  SETUP_UI_URL: setupUiUrl,
  STATE_DIR: isSafePromptTemplateValue(openclawDir)
    ? openclawDir
    : "$OPENCLAW_STATE_DIR",
  ROOT_DIR: isSafePromptTemplateValue(rootDir) ? rootDir : "$OPENCLAW_HOME",
  ENV_FILE: isSafePromptTemplateValue(envFilePath)
    ? envFilePath
    : "$OPENCLAW_HOME/.env",
});

// Configured-origin chain shared with lib/server/public-origin.js (PR 8a):
// an explicit base URL wins, then the platform's domain variables, then the
// localhost default. Never request-derived — this feeds persisted values.
const resolveSetupUiUrl = (baseUrl) => {
  const normalizedBaseUrl = String(baseUrl || "").trim().replace(/\/+$/, "");
  if (normalizedBaseUrl) return normalizedBaseUrl;
  return resolveConfiguredOrigin() || "http://localhost:3000";
};
...
// ≤500 chars, injected into every agent session: capacity only (tier, RAM,
// vCPU, GPU) plus the applied concurrency cap — live usage stays behind the
// watchdog API. Regenerated on boot via doSyncPromptFiles, so resizes are
// reflected automatically. Fail-open: any error renders nothing.
const renderMachineProfileMarkdown = (fs) => {
  try {
    ...
    const memoryGb = (limitBytes / (1024 * 1024 * 1024)).toFixed(1);
    const cores = profile.cpu?.cores ?? "?";
    // GPU name is external nvidia-smi output → sanitize before it reaches a prompt.
    const gpuName = profile.gpu?.present
      ? sanitizeLabel(profile.gpu.devices?.[0]?.name || profile.gpu.vendor || "GPU")
      : null;
    let section = "\n\n## Machine Resources\n\n";
    section += `- Capacity: ${profile.tier} tier — ${memoryGb} GB RAM, ${cores} vCPU`;
    if (gpuName) section += `, GPU: ${gpuName}`;
    section += "\n";
    // null when autotune is off or suppressed — say nothing rather than a stale cap.
    const cap = getAgentConcurrencyCap({ fsModule: fs });
    if (cap != null) {
      section += `- Agent concurrency cap (autotune): ${cap}\n`;
    }
    section +=
      "- Live usage: ask an admin, or run `alphaclaw admin GET /api/watchdog/resources` if agent administration is enabled.\n";
    return section;
  } catch {
    return "";
  }
  ...
  try {
    ...
    const agents =
      cfg.agents && typeof cfg.agents === "object" && !Array.isArray(cfg.agents)
        ? cfg.agents
        : {};
    // Mirror upstream roster precedence (dist readAgentRosterProperty): a
    // present `agents.entries` keyed map (key = agent id, value = entry) wins
    // over the `agents.list` array — property presence short-circuits, even
    // when both exist.
    let rosterEntries;
    if (agents.entries !== undefined) {
      const entriesMap =
        ...
        !Array.isArray(agents.entries)
          ? agents.entries
          : {};
      rosterEntries = Object.entries(entriesMap).map(([agentId, entry]) => ({
        id: agentId,
...
const syncBootstrapPromptFiles = ({
  ...
  try {
    ...
    const machineProfileSection = renderMachineProfileMarkdown(fs);
    // Rendered through ONE var map applied to BOTH templates (#121) — the
    // agents template used to be concatenated raw, so a token added to
    // core-prompts/AGENTS.md would have shipped to the agent literally.
    const promptVars = buildPromptTemplateVars({
      setupUiUrl,
      openclawDir,
      rootDir,
      envFilePath,
    });
    const renderedAgentsTemplate = renderPromptTemplate(
      agentsTemplate,
      promptVars,
    );
    const buildMergedContent = ({ agentId = "" } = {}) => {
      let toolsContent = renderPromptTemplate(toolsTemplate, promptVars);
      ...
      if (machineProfileSection) {
        toolsContent += machineProfileSection;
      }
      // Agent-admin pointer stanza — only when the skill is actually installed
      // (never advertise before live). Same pattern as the topic section.
      try {
        const { renderToolsStanza } = require("../agent-admin/skill");
        toolsContent += renderToolsStanza({ fs, openclawDir: OPENCLAW_DIR });
      } catch {}
      const merged = `${renderedAgentsTemplate.trimEnd()}\n\n${toolsContent}`;
...
const reconcileBootstrapExtraConfig = ({ fs, reportFailure }) => {
  try {
    ...
    // injected — the boot resync is the layer that repairs it. The entry's
    // `patterns`/`files` aliases fold into `paths` (the hook's non-empty
    // `paths` short-circuits the aliases completely); the alias keys stay
    // untouched.
    ...
    const currentPaths = Array.isArray(entry?.paths)
      ...
      : [];
    const reconciledPaths = reconcileBootstrapExtraEntryPaths(entry);
    const unchanged =
    ...
    updateOpenclawConfig({
      ...
      mutate: (liveConfig) => {
        ...
        liveConfig.hooks.internal.entries["bootstrap-extra-files"] = {
          ...liveEntry,
          enabled: true,
          paths: reconcileBootstrapExtraEntryPaths(liveEntry),
        };
      },
    });
...
const ensureOpenclawRuntimeArtifacts = ({
  ...
  try {
    ...
    // A full-root import can promote the repo's committed .env OVER the
    // managed symlink (fix wave F106): a regular file here is a stale
    // raw-secret copy that OpenClaw's dotenv, the hourly git sync and the git
    // shim would keep reading. Move it aside (its approved values were merged
    // into the live .env by the import) and restore the link.
    if (
      ...
      let linkStat = null;
      try {
        linkStat = fs.lstatSync(openclawEnvLink);
      } catch {}
      if (linkStat && !linkStat.isSymbolicLink() && linkStat.isFile()) {
        const asidePath = `${openclawEnvLink}.imported-${Date.now().toString(36)}`;
        fs.renameSync(openclawEnvLink, asidePath);
        console.warn(
          `[alphaclaw] ${openclawEnvLink} was a regular file (imported repo .env) — moved to ${asidePath}; the managed .env link is restored`,
        );
      }
    }
