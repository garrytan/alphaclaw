...
const syncBootstrapPromptFiles = ({ fs, workspaceDir, baseUrl }) => {
  try {
    ...
    const writeToWorkspace = (targetDir, toolsContent) => {
      ...
      // Atomic writes (E4.8): openclaw reads these at session bootstrap; a
      // torn TOOLS.md would feed the agent a truncated prompt.
      writeFileAtomic(
        path.join(bootstrapDir, "AGENTS.md"),
        fs.readFileSync(agentsSourcePath),
        { fsModule: fs },
      );
      writeFileAtomic(path.join(bootstrapDir, "TOOLS.md"), toolsContent, {
        fsModule: fs,
      });
    };
