...
const { updateOpenclawConfig, readOpenclawConfig } = require("../openclaw-config");
const {
...
// 20k per-file injection cap (0.85 mirrors upstream's near-limit ratio).
const kMergedBootstrapWarnChars = 17000;
...
// (3) delete the legacy AlphaClaw-owned hooks/bootstrap/TOOLS.md.
// A crash between steps leaves at worst a harmless stale TOOLS.md (duplicate
// tools guidance consuming budget) that self-heals on the next sync (every
// boot). Failures at any stage surface through onFailure (the server wires a
// watchdog event) — never console-only.
const syncBootstrapPromptFiles = ({ fs, workspaceDir, baseUrl, onFailure = null }) => {
  ...
  try {
      ...
      if (merged.length >= kMergedBootstrapWarnChars) {
        console.warn(
          `[onboard] Merged hooks/bootstrap/AGENTS.md is ${merged.length} chars — ` +
            `approaching OpenClaw's 20,000-char per-file injection cap; trim the topic ` +
            `registry or Google sections before content is truncated.`,
        );
      }
      return merged;
    };
    ...
    const removeLegacyToolsFile = (targetDir) => {
      const legacyPath = path.join(targetDir, "hooks", "bootstrap", "TOOLS.md");
      try {
    ...
    reconcileBootstrapExtraConfig({ fs, reportFailure });
...
const reconcileBootstrapExtraConfig = ({ fs, reportFailure }) => {
  try {
    ...
    // Not onboarded yet (or config unreadable): entry creation belongs to
    // onboarding's ensureManagedConfigShell.
    if (!entry) return;
