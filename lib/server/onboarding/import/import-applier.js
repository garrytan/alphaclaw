...
const resolveExtractionTargetPath = (baseDir, file, fsModule = null) =>
  resolveImportPathWithinBase(baseDir, file, fsModule);
...
const alignHookTransforms = ({ fs, baseDir, configFiles = [] }) => {
  ...
  for (const configFile of configFiles) {
    ...
    mappings.forEach((mapping, index) => {
      ...
      // Containment (H3): match.path / transform.module come from the imported
      // config (untrusted). Without this, a `../` module path moves a host file
      // into _backup (arb read/exfil) and writes a shim outside baseDir (arb
      // write) — both of which git then syncs. Reject any escaping path.
      const actualAbsolutePath = resolveImportPathWithinBase(
        baseDir,
        actualRelativePath,
        fs,
      );
      const expectedAbsolutePath = resolveImportPathWithinBase(
        baseDir,
        expectedRelativePath,
        fs,
      );
      if (!actualAbsolutePath || !expectedAbsolutePath) return;
      ...
      const sourceRootRelativePath =
        actualParts.length > 1
          ...
          : actualRelativePath;
      const sourceRootAbsolutePath = resolveImportPathWithinBase(
        baseDir,
      ...
      const backupRootAbsolutePath = resolveImportPathWithinBase(
        baseDir,
        backupRootRelativePath,
        fs,
      );
      if (!sourceRootAbsolutePath || !backupRootAbsolutePath) return;
...
const applySecretExtraction = ({ fs, baseDir, approvedSecrets }) => {
  ...
  for (const secret of approvedSecrets) {
    ...
    if (secret.file && !secret.file.startsWith(".env")) {
      const fullPath = resolveExtractionTargetPath(baseDir, secret.file, fs);
...
const canonicalizeConfigEnvRefs = ({ fs, baseDir, configFiles = [], envVars = [] }) => {
  ...
  for (const configFile of configFiles) {
    const fullPath = resolveExtractionTargetPath(baseDir, configFile, fs);
    if (!fullPath) continue;
    try {
