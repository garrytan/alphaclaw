...
const detectSecrets = ({ fs, baseDir, configFiles = [], envFiles = [] }) => {
  ...
  for (const cfgFile of configFiles) {
    try {
      // Containment (H2): cfgFile can originate from a crafted `$include`, so a
      // `../../../etc/real-config.json` would otherwise be read and its raw
      // secret values returned in the scan response.
      const fullPath = resolveImportPathWithinBase(baseDir, cfgFile, fs);
  ...
  for (const envFile of envFiles) {
    try {
      const fullPath = resolveImportPathWithinBase(baseDir, envFile, fs);
      if (!fullPath) continue;
