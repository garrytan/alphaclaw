...
const scanWorkspace = ({ fs, baseDir }) => {
  ...
  for (const cfgFile of gatewayConfig.files) {
    ...
    for (const inc of includes) {
      // Containment (H2): a crafted `$include` must not add a path that escapes
      // the import base to the scanned file set (downstream reads its secrets).
      const includeAbsolutePath = resolveImportPathWithinBase(baseDir, inc, fs);
      if (
        includeAbsolutePath &&
