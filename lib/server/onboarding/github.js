...
const kRepoSlugPattern = /^[A-Za-z0-9](?:[A-Za-z0-9_.-]*[A-Za-z0-9_])?\/[A-Za-z0-9](?:[A-Za-z0-9_.-]*[A-Za-z0-9_])?$/;

const buildLegacyCloneCommand = (cloneUrl, tempDir) =>
  [
    "git clone --depth=1 --",
    quoteShellArg(cloneUrl, { strategy: "single" }),
    quoteShellArg(tempDir, { strategy: "single" }),
  ].join(" ");

const cloneRepoToTemp = async ({ repoUrl, githubToken, shellCmd, execFileCmd }) => {
  // Defense in depth behind resolveGithubRepoUrl: the slug becomes a URL
  // operand, so anything that is not owner/repo is refused here too.
  const slug = String(repoUrl || "").trim();
  if (!kRepoSlugPattern.test(slug)) {
    return { ok: false, error: 'Failed to clone repo: repo must be in "owner/repo" format' };
  }
  const tempId = crypto.randomUUID().slice(0, 8);
  const tempDir = path.join(os.tmpdir(), `${kImportTempPrefix}${tempId}`);
  // Shared hardened askpass (H9 host-parse — only answers github.com) in a
  // private mkdtemp dir (H14).
  const { scriptPath: askPassPath, dir: askPassDir } = writeGitAskpassScript();
  ...
  try {
    if (typeof execFileCmd === "function") {
      // argv form, `--` before the operands: the URL can never become a git
      // option or a shell token (fix wave F102, the PR #28 C1/H1 class).
      await execFileCmd("git", ["clone", "--depth=1", "--", cloneUrl, tempDir], {
        timeout: 60000,
        env,
      });
    } else {
      // Legacy callers without execFileCmd: still never a raw interpolation.
      await shellCmd(buildLegacyCloneCommand(cloneUrl, tempDir), {
        timeout: 60000,
        env,
      });
    }
