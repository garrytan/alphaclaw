...
const clearImportedCredentialPairings = ({ fs, openclawDir }) => {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // An imported authorization we could not clear stays LIVE — say so
      // (fix wave F186) instead of swallowing it.
      console.warn(
        `[onboard] could not clear imported pairing allowFrom in ${filePath}: ${error?.message || error}`,
      );
    }
  }
};
  ...
  const completeOnboarding = async ({
    ...
    try {
      const commitMsg = importMode
        ...
        : "initial setup";
      // argv form — the message is a constant today, but the shell-string
      // shape is the class the guard test polices (fix wave, guard b).
      await execFileCmd("alphaclaw", ["git-sync", "-m", commitMsg], {
        timeout: 30000,
        env: {
          ...process.env,
