...
const createWatchdogIncidentTracker = ({
  ...
  const buildCloseSummary = (outcome) => {
    ...
    // pid death, moments before the crash opens the incident). The LIVE trend
    // stays regardless; it describes close-time reality.
    try {
      ...
      if (episode) {
        ...
        const inWindow =
          ...
          endedMs >= windowStart &&
          endedMs <= nowFn();
        if (!inWindow) trend.lastEpisodeSummary = null;
      }
    } catch {}
    return summary;
  };
