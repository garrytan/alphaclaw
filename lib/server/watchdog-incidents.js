...
//     channel_rollback (EX_CONFIG-in-stabilization rollbacks arrive with no
//     prior crash/config_error event — see kIncidentKeyByTrigger)
...
//     crash_loop · safe_mode_resume · anything else
//
...
const createWatchdogIncidentTracker = ({
  ...
  const wrapInsertEvent = (originalInsert) => (event = {}) => {
    ...
    if (activeIncidentId) {
      ...
      if (String(event.eventType || "") === "crash_loop") {
        // Severity escalation on an already-open incident — the one
        // append-only event that warms the rescue session.
