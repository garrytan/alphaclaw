...
//     and the mid-restart answer of the process about to be stopped —
//     {skipped, midRestart} — must never close an incident) · health_check/ok with
...
//     readiness_advisory (#87: the detached Doctor's structured finding for
//     an open readiness incident — evidence, never a trigger) ·
//     event_loop_pressure (#87: telemetry; OpenClaw's eventLoop diagnostic
//     never changes readiness, so it never opens or closes anything) ·
...
const createWatchdogIncidentTracker = ({
  ...
  const buildCloseSummary = (outcome) => {
    ...
    try {
      // The tracker owns its projection: correlating an episode must not
      // mutate a cached Resources object held by the watchdog or another UI.
      summary.resourceSample = structuredClone(getResourceSample() || null);
    } catch {
    ...
    try {
      ...
      if (episode) {
        ...
        const workerPid = Number(episode.evidence?.process?.worker?.pid);
        ...
        const pidMismatch =
          ...
          !(Number.isSafeInteger(workerPid) && workerPid > 0 && crashedPids.includes(workerPid));
