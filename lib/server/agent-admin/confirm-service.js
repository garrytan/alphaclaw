  ...
  const gate = ({ req, op, confirmCode }) => {
    ...
    if (typeof deliver === "function") {
      try {
        ...
        // truth; the 428 payload names it as the fallback.
      }
    }
    ...
    return {
      ...
      body: {
        ...
        summary: buildSummary(op),
        expiresAt: expiresAtIso,
        retryWith: `alphaclaw admin ${op.method} ${op.path} --confirm <code>`,
        delivery: hasAdminTargets()
          ? "A code was sent to your admin channel; it also appears in the dashboard (General tab)."
          : "See the dashboard (General tab) for the code.",
      },
    };
  };
