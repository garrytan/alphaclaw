...
const renderAdminsTable = (adminTargets) => {
  ...
  if (!rows.length) {
    return [
      "## Admins",
      "",
      "_No admin notification targets are configured._ Dangerous operations are unavailable until an operator sets one (Setup UI → Watchdog → Settings → Update notifications). Ask the operator to configure one before attempting them.",
      "",
    ].join("\n");
  }
  const lines = [
