...
const { applyOpRedaction, sanitizeAgentErrorBody, summarizeParams } = require("./redact");
const { attachEnforcementGrant } = require("./grant");
const {
  kConfirmHeader,
...
const createAgentAdminEnforcement = ({
  ...
  // Agent-visible error hygiene: route handlers pass raw err.message through
  // (`error: err.message`) for the dashboard's benefit; for the agent actor a
  // 5xx message may carry command lines or absolute paths from execSync, and
  // any error string may carry a token-shaped substring. sanitizeAgentErrorBody
  // keeps `code`/`hint` intact (the agent's decision inputs) and generalizes
  // the free-text `error` on 5xx.
  const attachRedaction = ({ res, op }) => {
    if (op.streaming) return; // SSE writes via res.write; never wrap
    const originalJson = res.json.bind(res);
    res.json = (body) =>
      originalJson(sanitizeAgentErrorBody(applyOpRedaction(op, body), res.statusCode));
  };

  // The grant is what lets `requireAdmin` admit the agent actor: it says the
  // manifest tier (and the confirm gate, for dangerous ops) already passed for
  // exactly this method/path/query/body. Attached ONLY on the paths below that
  // reach next(); every deny/428 path leaves the request grant-less (F067).
  const grant = ({ req, op, tier, confirmId = null }) =>
    attachEnforcementGrant({ req, op, tier, confirmId });
  ...
  return (req, res, next) => {
    ...
    if (tier === "dangerous") {
      ...
      grant({ req, op, tier, confirmId: outcome.confirmId });
