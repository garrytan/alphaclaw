...
const verifyArchiveManifest = async ({
  ...
  let extracted;
  try {
    // manifest.json is the first archive entry; --occurrence=1 makes GNU tar
    // stop there instead of decompressing a multi-GB payload it will discard.
    extracted = await runCommand({
      command: "tar",
      args: ["-xzOf", file, "--wildcards", "--occurrence=1", "*/manifest.json"],
      timeoutMs: remaining(),
    });
  } catch (error) {
