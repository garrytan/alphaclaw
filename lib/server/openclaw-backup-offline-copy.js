...
const createOfflineCopy = async ({
  ...
  try { inventory = await buildMigrationInventory({ stateDir, spawnEnv, fsModule, checkpoint }); }
    ...
    if (profile === "migration-minimal" || error.sourceCorrupt || ["BACKUP_EXTERNAL_SOURCE", "BACKUP_SQLITE_ALIAS"].includes(error.code) || ["budget", "quiet_lost", "lease_lost"].includes(error.stage)) throw error;
