...
// exclusivityEvidence/diagnosis/excludes/coverage (format 2). It never claims
// upstream tooling compatibility beyond the shared core fields — restore is
// the manual runbook.
//
//   createOfflineCopy(...)
...
//     │                    executables/entry scripts (isOpenclawArgv matches
//     │                    the program, never a path argument) · 0 in-process
//     │                    handles · /proc/*/fd scan
...
//                         inside workspaces the policy excludes
//                         (kOfflineCopyPolicyExcludes, or the operator's list;
//                         a pattern that could name a core asset is refused,
//                         reported and never applied) drop build/debris trees
//                         → skipped[{kind:"policy_exclude"}], measured, and the
//                         inline limit kOpenclawBackupWorkspaceInlineBytes is
//                         judged on the POST-exclude bytes · onProgress
//                         ({stage, doneBytes, totalBytes}) from here on
...
//                         excludes[{pattern,files,bytes}] and
//                         coverage{core: complete|partial, workspace:
//                         complete|policy_excluded|omitted} — `partial`
//                         stays reserved for a missing CORE asset (and the
//                         pre-existing over-limit workspace omission); a
//                         policy exclude never sets it
...
// Format 2 (issue #79) added `excludes[]` + `coverage{}`; a reader accepts
...
const kOfflineCopyFormatVersion = 2;
const kOfflineCopyReadableFormatVersions = Object.freeze([1, 2]);
...
// workspace the policy excludes below apply instead (issue #79): the upstream
// backup includes workspace dirs wholesale, but a workspace's `node_modules`
// or a heap snapshot is reproducible debris, not state — the 2026-09 incident
// copied 8 GB of it inside the quiesce and blew the budget. Every exclusion
// is measured and listed (`skipped[{kind:"policy_exclude"}]`, manifest
// `excludes[]`, `coverage.workspace: "policy_excluded"`) so a restore knows
// exactly what is absent; it never marks the copy `partial`, which stays the
// signal for a missing CORE asset. The inline size limit (judged on the
// post-exclude bytes), the entry cap, and the budget checkpoint inside the
// walk bound the cost.
const kSkipDirNames = new Set([".alphaclaw", "logs", "tmp", "node_modules", "backups"]);
// Default policy excludes, matched INSIDE workspaces only. Gitignore-style:
// a pattern without `/` matches any entry's basename at any depth; one with
// `/` matches the workspace-relative path anchored at the workspace root
// (`**` spans segments); a trailing `/` restricts the match to directories.
// A matched directory is excluded whole. Only unambiguous debris is default
// (Eng review Codex 17) — `tmp`, `.cache`, `caches` are operator opt-ins via
// the `excludes` option, which REPLACES this list when given.
...
const globSegmentToRegExpSource = (segment) => {
  let out = "";
...
// `logs/**/*.gz` → ^logs/(?:[^/]+/)*[^/]*\.gz$ — `**` matches zero or more
// whole segments, `*`/`?` never cross a `/`. Case-insensitive: the debris
// this exists for (`.TMP`, `Node_Modules` on a case-preserving volume) is
// the same debris either way.
const compileGlob = (body) => {
  ...
  segments.forEach((segment, index) => {
    ...
    source += globSegmentToRegExpSource(segment) + (last ? "" : "/");
  });
...
const compileExcludePattern = (raw) => {
  if (typeof raw !== "string") {
    return { refused: { pattern: String(raw), reason: "not a string" } };
  }
  const pattern = raw.trim();
  const refuse = (reason) => ({ refused: { pattern, reason } });
  ...
  const byBasename = segments.length === 1;
  const regExp = compileGlob(body);
...
const resolveExcludes = (excludes) => {
  if (excludes === undefined || excludes === null) {
    return resolveExcludes([...kOfflineCopyPolicyExcludes]);
  }
  if (!Array.isArray(excludes)) {
    return {
      applied: [],
      refused: [{ pattern: String(excludes), reason: "excludes must be an array of patterns" }],
    };
  }
  const applied = [];
  const refused = [];
  const seen = new Set();
  excludes.forEach((raw, index) => {
    if (index >= kOfflineCopyExcludeMaxPatterns) {
      refused.push({
        pattern: String(raw),
        reason: `more than ${kOfflineCopyExcludeMaxPatterns} patterns — the rest are ignored`,
      });
      return;
    }
    const outcome = compileExcludePattern(raw);
    if (outcome.refused) {
      refused.push(outcome.refused);
      return;
    }
    if (seen.has(outcome.compiled.pattern)) return;
...
function* walkStateTreeSteps({
  stateDir,
  fsModule,
  checkpointEvery = kWalkCheckpointEvery,
  excludes = undefined,
}) {
  const dbs = [];
  const files = [];
  ...
  const policy = resolveExcludes(excludes);
  const tallies = policy.applied.map((rule) => ({ pattern: rule.pattern, files: 0, bytes: 0 }));
  // Two counters: the cap bounds the COPY SET (an excluded tree is never
  // copied, so its entries do not count against it); the checkpoint cadence
  // counts every entry visited, measured ones included, so a huge excluded
  // node_modules still yields to the event loop and the budget.
  let entriesSeen = 0;
  let visited = 0;
  function* tick() {
    visited += 1;
  ...
  // Measure an excluded directory: bytes + regular-file count, never
  // following symlinks. Tolerant on purpose — an unreadable corner of a tree
  // we are NOT copying must not fail the backup; what could be measured is
  // reported as the exclusion's size.
  function* measure(dir, total) {
    let entries;
    try {
    ...
    for (const entry of entries) {
      ...
      try {
        total.bytes += fsModule.statSync(full).size;
        total.files += 1;
      } catch {}
    }
  }
  const matchExclude = (wsRel, isDirectory) =>
    policy.applied.findIndex((rule) => rule.test(wsRel, { isDirectory }));
  function* visit(dir, rel, workspaceRoot) {
    ...
    for (const entry of entries) {
      ...
      // Policy excludes: workspace-relative path, directories and files
      // alike, before the entry is counted against the copy-set cap.
      if (workspaceRoot && !entry.isSymbolicLink()) {
        const isDirectory = entry.isDirectory();
        const wsRel = toPosix(path.relative(workspaceRoot, full));
        const ruleIndex = isDirectory || entry.isFile() ? matchExclude(wsRel, isDirectory) : -1;
        if (ruleIndex !== -1) {
          yield* tick();
          ...
          const rule = policy.applied[ruleIndex];
          tallies[ruleIndex].files += total.files;
          tallies[ruleIndex].bytes += total.bytes;
          const ws = workspaces.get(workspaceRoot);
          ws.excludedBytes += total.bytes;
          ws.excludedFiles += total.files;
          skipped.push({
            kind: "policy_exclude",
            sourcePath: full,
            reason: `excluded by backup policy (${rule.pattern})`,
      ...
      if (entry.isDirectory()) {
        ...
        if (nextWorkspace && !workspaces.has(nextWorkspace)) {
          workspaces.set(nextWorkspace, { bytes: 0, files: [], excludedBytes: 0, excludedFiles: 0 });
        }
  ...
  return { dbs, files, skipped, workspaces, excludes: tallies, refusedExcludes: policy.refused };
}
...
const verifyArchiveManifest = async ({
  ...
  // An AlphaClaw offline copy must be a format this reader knows (1 or 2 —
...
const createOfflineCopy = async ({
  ...
  const tree = await walkStateTreeAsync({ stateDir, fsModule, checkpoint, excludes });
  ...
  try {
    ...
    const coverage = {
      core: coreMissing > 0 ? "partial" : "complete",
      workspace:
