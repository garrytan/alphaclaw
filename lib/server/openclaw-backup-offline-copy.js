...
// After a timeout the orphaned backup keeps stepping into its temp file
// until it finishes or fails on its own: the caller's cleanup removes the
// temp dir (writes to the unlinked inode go nowhere) and the orphan's
// eventual settlement is swallowed. Closing the source underneath it is the
// documented SQLite contract (sqlite3_close_v2 with an unfinished backup
// zombifies the connection until the backup releases it).
