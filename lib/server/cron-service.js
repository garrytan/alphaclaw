...
// ~256KB tail per job run log: comfortably hundreds of recent runs while
// keeping bulk endpoints O(jobs), not O(total history).
...
const readJobRuns = ({
  ...
  const lines = fs.existsSync(runLogPath)
    ? tailLines(runLogPath, kRunLogTailBytes).map((line) => line.trim()).filter(Boolean)
    : [];
  const entries = lines
