...
const { createCronRunStore } = require("./cron-run-store");
const {
  toFiniteNumber,
  sanitizeCronJobId,
  parseRunLogLine,
  readJobRuns,
  readJobDurationStats,
  readRunLines,
} = require("./cron-run-files");
const {
  resolveTimeZone,
...
const getJobTrends = ({
  ...
  // Client-zone day bucketing (opt-in via the x-client-timezone header): when
  // a resolvable zone is provided, day-aligned ranges bucket on that zone's
  // calendar days; otherwise the legacy server-local behavior is unchanged.
  const zone = timeZone ? resolveTimeZone(timeZone) : null;
  const dayStartOf = zone
    ? (valueMs) => getTimeZoneDayStartMs(valueMs, zone)
    : startOfLocalDayMs;
  const backTimeZoneDayStarts = (dayStartMs, dayCount) => {
    // Step back one calendar day at a time: 1ms before a day start lies
    // inside the previous day, so its day start is the previous day's.
    let cursor = dayStartMs;
    for (let step = 0; step < dayCount; step += 1) {
      cursor = getTimeZoneDayStartMs(cursor - 1, zone);
    }
    return cursor;
  };
  const windowStartMs = safeSinceMs > 0
    ? (rangeConfig.alignToLocalDay ? dayStartOf(safeSinceMs) : safeSinceMs)
    : rangeConfig.alignToLocalDay
      ? (zone
        ? backTimeZoneDayStarts(dayStartOf(safeNowMs), rangeConfig.bucketCount - 1)
  ...
  // Precomputed day-start boundaries for day-aligned ranges (bucket edges and
  // keys must agree, including across DST transitions).
  const alignedDayStartsList = rangeConfig.alignToLocalDay
    ? (() => {
      const list = [windowStartMs];
      for (let index = 1; index < rangeConfig.bucketCount; index += 1) {
        list.push(zone
          ? getNextTimeZoneDayStartMs(list[index - 1], zone)
  ...
  const read = readRuns((reader) => {
    reader.visit({ jobId: safeJobId, sinceMs: windowStartMs, untilMs: windowEndMs }, addEntry);
    return true;
  });
  if (!read) {
    const runLogPath = path.join(runsDir, `${safeJobId}.jsonl`);
    const lines = readRunLines(runLogPath);
  ...
  return {
    ...
    // Effective zone the day buckets were computed in (null = server-local
    // legacy buckets) so browser/server tzdb divergence is observable.
    timeZone: zone,
    points,
  };
};
...
const createCronService = ({
  ...
  const readRuns = createCronRunStore({ openclawDir: OPENCLAW_DIR, getInstalledVersion });
  ...
  // Every runCommand caller is a cron MUTATOR (run/enable/disable/edit) —
  // a gateway RPC through the CLI. During the state-DB quiet period the
  // gateway is stopped (confirmed) and the store is the barrier's to protect,
  // so the honest answer is StateDbQuietError (→ 409 backup_in_progress +
  // Retry-After at the route) rather than the CLI's connection error, or —
  // for the prompt edit — a false "unknown cron job id" from the jobs.json
  // fallback. Checked before anything spawns: never mutate-then-refuse.
  ...
  const updateJobPrompt = async ({ jobId, message }) => {
    const safeJobId = sanitizeCronJobId(jobId);
    // Before the flag lookup: while quiet, readCronStore serves the jobs.json
    // fallback, where a sqlite-era job is absent — the lookup would answer
    // "unknown cron job id", which is false.
    throwIfStateDbQuiet();
  ...
  const getJobRuns = ({
    ...
    sanitizeCronJobId(jobId);
    const runs = readRuns((reader) => reader.readPage(options)) || readJobRuns(options);
    return {
      ...runs,
  ...
  const getJobUsage = ({ jobId, sinceMs = 0 }) => {
    ...
    const durationStats = readRuns((reader) => reader.durationStats(durationOptions)) || readJobDurationStats(durationOptions);
    const totals =
  ...
  const getBulkJobUsage = ({ sinceMs = 0 } = {}) => {
    ...
    const byJobId = {};
    jobs.forEach((job) => {
      const usage = getSessionUsageByKeyPattern({ keyPattern: `%:cron:${sanitizeCronJobId(job.id)}%`, sinceMs: safeSinceMs }) || {};
  ...
  const getBulkJobRuns = ({
    ...
    const byJobId = {};
    let jobs;
    const sqliteRuns = readRuns((reader) => {
      jobs = listJobs({ sortBy: "name", sortDir: "asc" }).jobs;
      return reader.readPages(jobs.map((job) => job.id), {
        limit: safeLimitPerJob, status, deliveryStatus, sortDir, query,
      });
    });
    jobs ||= listJobs({ sortBy: "name", sortDir: "asc" }).jobs;
    jobs.forEach((job) => {
      const runs = sqliteRuns ? { entries: sqliteRuns[job.id].map(enrichRunEntryEstimatedCost) } : getJobRuns({
        jobId: job.id,
