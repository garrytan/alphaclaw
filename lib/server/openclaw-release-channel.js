...
const createOpenclawReleaseChannelStore = ({
  ...
  const readProcessStartTicks = (pid) => {
    try {
      ...
      // the LAST ")" so a command like "node (x)" cannot shift the fields.
  ...
  // null. A dead/absent/self pid — or a claim whose recorded identity does
  // not match the process now behind that pid — means the boot sync may
  // proceed.
  const readLiveServerPid = () => {
    try {
      ...
      return looksLikeAlphaclawServer(pid) ? pid : null;
    } catch {
      return null;
    }
  };
