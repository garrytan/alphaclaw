...
const { readProcStartTicks } = require("./utils/safe-file");
...
const createOpenclawReleaseChannelStore = ({
  ...
  nowFn = Date.now,
  logger = console,
  // Identity seam for the server pidfile guard (tests simulate a claim from
  // another container by giving it a different hostname).
  ...
  // is its id) and the process start time from /proc — and liveness requires
  // the identity to match, not just the pid to exist.
  ...
  // Shares the parser with the file-lock records (lib/server/utils/safe-file.js).
  ...
  // Legacy claims (pre-v0.9.73: {pid, at} only) carry no identity. On Linux
  // the process behind the pid must at least LOOK like an alphaclaw server
  // (argv names the bin entry); a reused pid running the placeholder child,
  // the gateway launcher or anything else is stale. Without /proc, fall back
  // to trusting liveness (the legacy behaviour).
  const looksLikeAlphaclawServer = (pid) => {
    try {
      const cmdline = fsModule
        .readFileSync(`/proc/${pid}/cmdline`, "utf8")
        .split("\0")
        .join(" ");
      if (!cmdline.trim()) return true; // kernel thread / unreadable → trust liveness
      return /alphaclaw\.js|\/alphaclaw(\s|$)/.test(cmdline) && !/boot-placeholder-child/.test(cmdline);
    } catch {
      return true;
    }
  };
  ...
  // plus the `corroborated` flag the launcher acts on (fix wave F004):
  //   - null when the pidfile is absent/garbage, names this process, was
  //     written by another host/container, names a dead pid, or /proc proves
  //     the pid was RECYCLED (start time differs from the record) — the boot
  //     sync may proceed.
  //   - { pid, corroborated: true } when the pid is alive AND its /proc start
  //     time matches the record — a live AlphaClaw server really owns the dir
  //     (bin/alphaclaw.js refuses to start a second instance).
  //   - { pid, corroborated: false } when the pid is alive but cannot be
  //     verified (a legacy {pid, at} claim that passes the argv sanity check,
  //     or no /proc) — callers stay fail-open: skip the destructive sync, keep
  //     booting; EADDRINUSE still stops a true duplicate.
  const readLiveServerPidEvidence = () => {
    try {
      ...
      // ours, so the pid cannot name a process we could be racing.
      if (typeof parsed.host === "string" && parsed.host && parsed.host !== hostnameFn()) {
        return null;
      }
      process.kill(pid, 0);
      const recordedTicks = Number(parsed.startTicks);
      if (Number.isFinite(recordedTicks) && recordedTicks > 0) {
        const liveTicks = readProcessStartTicks(pid);
        // A different start time = a different process reusing the pid.
        if (liveTicks != null && liveTicks !== recordedTicks) return null;
        return { pid, corroborated: liveTicks != null };
      }
      // Legacy claim without identity: apply the argv sanity check.
      return looksLikeAlphaclawServer(pid) ? { pid, corroborated: false } : null;
    } catch {
      return null;
    }
  };
