...
const createOpenclawReleaseChannelStore = ({
  ...
  // The record carries the owner's kernel start time (/proc/<pid>/stat field
  // 22, null where /proc is unavailable) so a recycled pid is never mistaken
  // for the live server. The case that bit: a container hard-killed
  // (`docker rm -f`) leaves its pidfile on the volume, and the replacement
  // container's early processes land on the same low pid numbers — kill(pid, 0)
  // alone then reports "alive" and the boot refuses to start.
  const writeServerPid = () => {
    try {
      ...
      if (owner) return;
      writeJsonFile(serverPidPath, {
        pid: process.pid,
        at: nowFn(),
        startTicks: readProcStartTicks(process.pid, fsModule),
      });
    } catch {}
  };
  ...
  // Evidence about the recorded owner: { pid, corroborated }.
  //   - null when the pidfile is absent/garbage, names this process, the pid
  //     is dead, or /proc proves the pid was RECYCLED (start time differs from
  //     the recorded one) — the boot sync may proceed.
  //   - corroborated: true when the pid is alive AND its /proc start time
  //     matches the record — a live AlphaClaw server really owns the dir.
  //   - corroborated: false when the pid is alive but cannot be verified
  //     (legacy record without startTicks, or no /proc) — callers must stay
  //     fail-open (skip the destructive sync, keep booting) rather than refuse.
  const readLiveServerPidEvidence = () => {
    try {
      ...
      process.kill(pid, 0);
      const recorded = Number.isFinite(parsed?.startTicks) ? parsed.startTicks : null;
      const current = readProcStartTicks(pid, fsModule);
      if (recorded !== null && current !== null && recorded !== current) return null;
      return { pid, corroborated: recorded !== null && current !== null };
    } catch {
      return null;
    }
  };
  ...
  // null. A dead/absent/self/recycled pid means the boot sync may proceed.
