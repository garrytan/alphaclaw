...
const createOpenclawReleaseChannelStore = ({
    ...
    try {
      const pkg = JSON.parse(
        fsModule.readFileSync(
          path.join(liveOpenclawDir(installDir), "package.json"),
          "utf8",
        ),
      );
  ...
  const validateBinShim = () => {
    // shimDir is prepended to PATH for the whole process tree, and it lives on
    // the agent-writable data volume. Anything in it that is not the expected
    // shim (planted "git"/"npm"/"node" impostors, stale temp files) is removed
    // at every boot so a writable directory never becomes a PATH hijack.
    try {
    ...
    if (!target || !fsModule.existsSync(target)) {
