...
const { normalizeDevCheckoutPath, resolveDevCheckoutForBin } = require("./openclaw-dev-candidates");
...
// Runtime activation stages the overlay copy BESIDE the live tree (issue #76,
// Codex 3): `node_modules/.openclaw-staging-<bootId>`. Same filesystem as
// node_modules/openclaw, so the final rename is atomic and the rm→rename
// window is milliseconds instead of a multi-hundred-MB copy.
const kOpenclawStagingDirPrefix = ".openclaw-staging-";
// A staging dir this old is debris from a process that died mid-swap (a live
// copy finishes in seconds; the sentinel rule re-activates the tree at the
// next boot). Swept by sweepStaleStagingDirs, never by the activation itself.
const kOpenclawStagingMaxAgeMs = 10 * 60 * 1000;
...
const normalizeApplied = (applied) => {
  ...
  return {
    ...
    ...(applied.checkoutDir != null ? { checkoutDir: normalizeDevCheckoutPath(applied.checkoutDir) } : {}),
    at: applied.at ?? null,
...
const normalizeLastKnownGood = (lastKnownGood) => {
  ...
  return {
    ...
    ...(base.devCheckoutDir != null ? { devCheckoutDir: normalizeDevCheckoutPath(base.devCheckoutDir) } : {}),
  };
};
      ...
      ? {
          version:
            typeof value.lastAttempt.version === "string"
              ...
              : null,
          buildId: typeof value.lastAttempt.buildId === "string" ? value.lastAttempt.buildId : null,
  ...
  return {
    completedForVersion:
      typeof value.completedForVersion === "string"
        ...
        : null,
    completedForBuild: typeof value.completedForBuild === "string" ? value.completedForBuild : null,
    lastAttempt,
...
//
// ONE hold model (issue #76, Codex 6): `state.gatewayHold` is the only hold
// key, and `reason` sorts it into two classes.
//   - STRUCTURAL holds are set by the version gates and carry a class token
//     from kStructuralHoldReasons: `version_mismatch` (installed tree ≠
//     recorded build), `state_db_unreadable`, `activation_failed`. They
//     carry `detail` (operator prose), `installed`/`expected` (the two
//     versions), `bootId` (the boot that set it) and, for activation_failed,
//     `error` (the fs error text).
//   - MIGRATION-CLASS holds belong to the boot reconciler's settings
//     migration. Today it writes them as free-text prose ("settings migration
//     for X failed: …", "config snapshot failed: …", "agent database … is at
//     agent schema …", "… a gateway process is running …", "reconcile error:
//     …"); kMigrationHoldReasons is the token vocabulary for the same class
//     (`config_migration_failed`, `migration_gate_error`, `doctor_failed`).
//     Any non-structural reason is migration-class, so the prose keeps its
//     meaning and only a token from the structural set can bypass the doctor.
// isMigrationClassHold is the ONE predicate the reconciler's migration
// branches consult; a structural hold returns `held` before any snapshot or
// doctor, and only the owner of a reason clears it.
const kStructuralHoldReasons = Object.freeze([
  "version_mismatch",
  "state_db_unreadable",
  "state_db_unverified",
  "activation_failed",
]);
const kMigrationHoldReasons = Object.freeze([
  "config_migration_failed",
  "migration_gate_error",
  "doctor_failed",
]);
const kStructuralHoldReasonSet = new Set(kStructuralHoldReasons);
const isMigrationClassHold = (hold) =>
  Boolean(
    hold &&
      ...
      hold.reason &&
      !kStructuralHoldReasonSet.has(hold.reason),
  );
...
const normalizeGatewayHold = (value) => {
  ...
  return {
    ...
    bootId: str("bootId"),
    // Sparse on purpose: `error` exists only on an activation_failed hold
    // (CEO 2.2), so a hold written without it round-trips byte-for-byte —
    // the reconciler compares the hold it set with the one it reads back.
    ...(error ? { error } : {}),
  };
};
...
const normalizeState = (raw) => {
  ...
  return {
    ...
    ...(base.previousDev != null ? { previousDev: {
      sha: typeof base.previousDev.sha === "string" && /^[a-f0-9]{40}(?:[a-f0-9]{24})?$/.test(base.previousDev.sha) ? base.previousDev.sha : null,
      checkoutDir: normalizeDevCheckoutPath(base.previousDev.checkoutDir),
    } } : {}),
...
const createOpenclawReleaseChannelStore = ({
  ...
  // Every version with a COMPLETE overlay, newest first (issue #76 B1.4): the
  // bootable-candidate chooser shortlists from this list. Half-saved entries
  // (no completion file) and stray files are excluded by hasOverlay; a
  // missing store is an empty list, never a throw.
  const listOverlays = () => {
    let entries = [];
    try {
      entries = fsModule.readdirSync(overlayStoreDir);
  ...
  const writeSentinel = ({ installDir, version } = {}) => {
    installedVersionCache.delete(path.join(liveOpenclawDir(installDir), "package.json"));
    try {
  ...
  // --- runtime activation (issue #76 B1.2 / Codex 3) --------------------------
  //
  // The async twin of activateOverlay for the LIVE server (reconcileInstalled,
  // the structural repair ladder): cpSync of a multi-hundred-MB tree would
  // block the event loop for tens of seconds, and the sync path's rm→cp window
  // is the whole copy. Here the overlay is copied to a SIBLING staging dir
  // first, verified (package.json version + a resolvable bin), and only then
  // swapped in: sentinel unlink → rm old tree → rename staging → sentinel
  // LAST. A failure before the swap leaves the live tree untouched; a crash
  // inside the swap leaves no sentinel, so the next boot re-activates from the
  // overlay (the sentinel rule), and sweepStaleStagingDirs removes the debris.
  // The sync bin-phase activateOverlay is unchanged on purpose.

  // The staging dir is named by the boot that owns it (getProcessBootId's
  // `<pid>:<startMs>`), so a sweep can tell this process's in-flight copy from
  // a dead one's. Path separators and shell-ish characters are replaced: a
  // caller-supplied id must not be able to point the rm/rename anywhere else.
  const stagingDirName = (bootId) => {
    const token = String(bootId ?? "").replace(/[^0-9A-Za-z._:-]/g, "-");
    if (!token) {
      throw new Error(`unsafe staging boot id: ${JSON.stringify(bootId)}`);
    }
    return `${kOpenclawStagingDirPrefix}${token}`;
  };

  const overlayStagingDir = ({ installDir, bootId } = {}) =>
    path.join(installDir, "node_modules", stagingDirName(bootId));

  const discardStaging = async (stagingDir) => {
    try {
      await fsp.rm(stagingDir, { recursive: true, force: true });
    } catch (error) {
      logger.warn?.(
        `[release-channel] failed to remove staging dir ${stagingDir}: ${error.message}`,
      );
    }
  };

  // Pure read of a staged copy: the version the overlay entry claims must be
  // the one package.json carries, and its bin must resolve to a real file —
  // a copy that cannot even name its entry point is never swapped in.
  const verifyStagedPackage = (stagingDir, version) => {
    let pkg = null;
    try {
      pkg = JSON.parse(
        fsModule.readFileSync(path.join(stagingDir, "package.json"), "utf8"),
      );
    } catch (error) {
      return { ok: false, error: `staged package.json unreadable: ${error.message}` };
    }
    if (pkg?.version !== version) {
      return {
        ok: false,
        error: `staged package.json is openclaw@${pkg?.version ?? "?"}, expected ${version}`,
      };
    }
    const bin = resolvePackageBin(stagingDir);
    if (!bin || !fsModule.existsSync(bin)) {
      return { ok: false, error: `staged openclaw@${version} has no resolvable bin` };
    }
    return { ok: true, bin };
  };

  const activateOverlayAsync = async ({
    installDir,
    version,
    ...
    // Same belt as activateOverlay: never put a guard-carrying tree live.
    const overlayGuardPath = path.join(
      overlayPackageDir(version),
      "dist",
      "openclaw-install-guard",
    ...
    let stagingDir = null;
    try {
      stagingDir = overlayStagingDir({ installDir, bootId });
    } catch (error) {
      return { ok: false, stage: "staging", error: error.message };
    }
    const liveDir = liveOpenclawDir(installDir);
    // 1. Stage: copy beside the live tree. Nothing live is touched yet.
    try {
      await fsp.rm(stagingDir, { recursive: true, force: true });
      await fsp.mkdir(path.dirname(stagingDir), { recursive: true });
      await fsp.cp(overlayPackageDir(version), stagingDir, { recursive: true });
    } catch (error) {
      await discardStaging(stagingDir);
      return { ok: false, stage: "copy", error: error.message };
    }
    // 2. Verify BEFORE anything destructive.
    const verified = verifyStagedPackage(stagingDir, version);
    if (!verified.ok) {
      await discardStaging(stagingDir);
      return { ok: false, stage: "verify", error: verified.error };
    }
    // 3. Swap. The sentinel goes first (a crash between rm and rename must
    // not leave sentinel + plausible package.json over a gutted tree), the
    // rename is atomic on the shared filesystem.
    try {
      try {
        await fsp.unlink(sentinelPath({ installDir }));
      } catch {}
      await fsp.rm(liveDir, { recursive: true, force: true });
      await fsp.rename(stagingDir, liveDir);
    } catch (error) {
      await discardStaging(stagingDir);
      return { ok: false, stage: "swap", error: error.message };
    }
    // The cache is mtime-keyed and would self-invalidate; dropping the entry
    // makes the next readInstalledVersion a guaranteed fresh read.
    installedVersionCache.delete(path.join(liveDir, "package.json"));
    // 4. Sentinel LAST so a crash anywhere above leaves no sentinel behind.
    const sentinel = writeSentinel({ installDir, version });
    return sentinel.ok
      ? { ok: true }
      : { ok: false, stage: "sentinel", error: sentinel.error };
  };

  // Boot-time debris sweep: `node_modules/.openclaw-staging-*` dirs older
  // than kOpenclawStagingMaxAgeMs (by mtime) are copies a process died holding
  // (rename never ran, or ran and the process died before the sentinel). The
  // dir named by THIS boot is never swept — it is an in-flight activation.
  // Sync on purpose: its ONE production caller is channel-sync's syncAtBoot
  // (bin phase, right after the shim check, before any activation branch
  // stages a new copy). Ages are wall clock against file mtimes (not nowFn,
  // which tests run on a logical clock). Never throws.
  const sweepStaleStagingDirs = ({
    installDir,
    bootId = getProcessBootId(),
    maxAgeMs = kOpenclawStagingMaxAgeMs,
    nowMs = Date.now(),
  } = {}) => {
    const removed = [];
    const kept = [];
    ...
    let entries = [];
    try {
      entries = fsModule.readdirSync(nodeModulesDir);
    } catch {
      return { removed, kept };
    }
    let own = null;
    try {
      own = stagingDirName(bootId);
    } catch {
      own = null;
    }
    for (const name of entries) {
      if (!name.startsWith(kOpenclawStagingDirPrefix)) continue;
      if (name === own) {
        kept.push(name);
        continue;
      }
      ...
      try {
        fsModule.rmSync(fullPath, { recursive: true, force: true });
        removed.push(name);
        logger.warn?.(
          `[release-channel] removed stale activation staging dir ${name}`,
        );
      } catch (error) {
        logger.warn?.(
          `[release-channel] failed to remove stale staging dir ${name}: ${error.message}`,
        );
      }
    }
    return { removed, kept };
  ...
  const validateBinShim = () => {
    ...
    const targetAllowed = (candidate) => {
      ...
      const checkoutRoot = process.env.OPENCLAW_GIT_DIR || path.resolve(rootDir, "openclaw");
      return (
        resolved.startsWith(path.resolve(overlayStoreDir) + path.sep) ||
        Boolean(resolveDevCheckoutForBin({ checkoutDir: checkoutRoot, targetBin: resolved, fsModule }))
      );
    };
