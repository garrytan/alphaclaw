...
const readWatchdogMemorySettings = (options = {}) => {
  try {
    ...
    return { enabled: false, autoRestart: false, effectiveAutoRestart: false };
  }
...
const updateWatchdogMemorySettings = ({
  ...
  return {
    config,
    changed:
      current.watchdog.memory.enabled !== config.watchdog.memory.enabled ||
      current.watchdog.memory.autoRestart !==
        config.watchdog.memory.autoRestart,
    settings: {
      enabled: config.watchdog.memory.enabled,
      autoRestart: config.watchdog.memory.autoRestart,
      effectiveAutoRestart:
        config.watchdog.memory.enabled && config.watchdog.memory.autoRestart,
    },
  };
};
