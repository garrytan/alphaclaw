...
const normalizeDoctorConfig = (doctor = {}) => {
  ...
  const autoRun = base.autoRun && typeof base.autoRun === "object" ? base.autoRun : {};
  return {
    ...base,
    autoRun: {
      ...autoRun,
      // Anything but literal true stays off — same doctrine as the overseer.
...
const writeAlphaclawConfig = ({
  ...
  // alphaclaw.json — readAlphaclawConfig silently falls back to defaults on a
  // parse failure, so a partial file would turn every opt-in feature off
  // without a trace. writeFileAtomic mkdirs the parent itself and degrades to
  // a plain write for injected fsModule mocks without renameSync. (Cross-
  // writer LOCK is still a TODO.)
...
const updateDoctorAutoRunEnabled = ({
  ...
  const next = normalizeAlphaclawConfig({
    ...
    doctor: {
      ...current.doctor,
      autoRun: {
        ...current.doctor?.autoRun,
        enabled: enabled === true,
      },
    },
  });
  writeAlphaclawConfig({ fsModule, openclawDir, config: next });
  return next.doctor.autoRun.enabled;
};
