...
// Mutation readers must not turn a damaged settings file into fresh defaults.
// Read under the caller's file lock and bypass the display-read cache.
const readAlphaclawConfigForWrite = ({ fsModule = fs, openclawDir } = {}) => {
  const configPath = resolveAlphaclawConfigPath({ openclawDir });
  let raw;
  try {
    ...
    if (cause?.code === "ENOENT") {
      try {
        fsModule.lstatSync(configPath);
      } catch (missing) {
        if (missing?.code === "ENOENT") return normalizeAlphaclawConfig(kDefaultAlphaclawConfig);
      }
    }
    throw Object.assign(new Error(`Cannot read ${configPath}`, { cause }), {
      code: "ALPHACLAW_CONFIG_UNREADABLE", configPath,
    });
  }
  try {
    ...
    return normalizeAlphaclawConfig(parsed);
  } catch (cause) {
    throw Object.assign(new Error(`Cannot parse ${configPath}`, { cause }), {
      code: "ALPHACLAW_CONFIG_UNREADABLE", configPath,
    });
  }
};
...
const updateAlphaclawConfig = ({ fsModule = fs, openclawDir, mutate, strictRead = false } = {}) => {
  const configPath = resolveAlphaclawConfigPath({ openclawDir });
  return withFileLockSync(
    configPath,
    () => {
      const current = (strictRead ? readAlphaclawConfigForWrite : readAlphaclawConfig)({ fsModule, openclawDir });
...
const readOpenclawBackupSettings = (options = {}) => {
  const backup = readAlphaclawConfigForWrite(options).updates.openclaw.backup;
  return backup === undefined ? {} : backup;
};

const updateOpenclawBackupSettings = ({ fsModule = fs, openclawDir, policy } = {}) => {
  const { config } = updateAlphaclawConfig({
    fsModule, openclawDir, strictRead: true,
    mutate: (current) => {
      const previous = current.updates.openclaw.backup;
      current.updates.openclaw.backup = {
        ...(previous && typeof previous === "object" && !Array.isArray(previous) ? previous : {}),
        excludes: [...policy.excludes], rootExcludes: [...policy.rootExcludes],
      };
    },
  });
  return config.updates.openclaw.backup;
};
...
module.exports = {
  ...
  readAlphaclawConfig,
  readAlphaclawConfigForWrite,
  readOpenclawBackupSettings,
  readOpenclawBackupPolicy: readOpenclawBackupSettings,
