...
9. **Do not edit managed files directly.** Never hand-edit `skills/alphaclaw-admin/`, `hooks/bootstrap/`, `openclaw.json`, or the managed `.env` file (the Envars tab owns it) — always go through `alphaclaw admin` so validation, restart marking, and audit happen.
