...
const reconcileCodexPlugin = ({ exec = execFileSync, logger = console } = {}) => {
  ...
  if (!activeVersion) {
    // A failed `openclaw --version` probe says nothing about which version is
    // ACTIVE — falling back to the npm pin here would re-enable the forced
    // downgrade on a box whose release channel has a beta active but whose
    // CLI is transiently broken. Skip; the next boot retries.
    logger.log(
      "[alphaclaw] Codex plugin reconcile skipped: could not determine the active openclaw version",
    );
    return { changed: false, reason: "active-version-unknown" };
  }
