...
// pid collision:
//
//   container A boots once ─▶ read a REAL thread id of its server process
//        │                       (/proc/<pid>/task, never the leader itself)
//        ▼
//   remove A; seed the volume with the legacy claim { pid: <tid>, at: <old> }
//        + a `running` ledger run + a `running` restart operation from a
//        foreign bootId + a `.tmp` archive in the backups dir
//        ▼
//   container B (same image, same entrypoint ⇒ deterministic pid allocation)
//   boots and boot-report.json must say:
...
// pinned OpenClaw). ~10-20 min end to end. The true AlphaClaw SELF-upgrade leg
// (boot 0.9.76 on the volume, then the branch build) stays deferred to TODOS:
// the harness builds one image per run.
// -----------------------------------------------------------------------------
...
describeContainer("container E2E: boot durability — legacy pidfile TID collision, dangling records, .tmp debris", () => {
  ...
  step("reads a REAL thread id of A's server process from /proc/<pid>/task", 2 * kMin, async () => {
    ...
    // The LOWEST non-leader tid: V8's platform workers are spawned at process
    // start, before anything the boot sequence does, so the same tid exists
    // again at the same point of B's boot (deterministic allocation in a fresh
    // pid namespace with the same image and entrypoint).
    ctx.threadIdA = tids.find((tid) => tid !== ctx.serverPidA);
    expect(ctx.threadIdA).toBeGreaterThan(ctx.serverPidA);
    console.log(`[container-boot-e2e] A: server pid ${ctx.serverPidA}, threads ${tids.join(",")} → seeding tid ${ctx.threadIdA}`);
  });
  ...
  step("container B boots on the seeded volume: UI + pin + gateway healthz", 12 * kMin, async () => {
    await runContainer({ name: kContainerB, image: kImageTag, volume: kVolume, env: containerEnv() });
  ...
  step("volume after boot: B's own format-2 pidfile, the restart operation interrupted, the .tmp archive swept", 2 * kMin, async () => {
    ...
    expect(pidfile.format).toBe(kServerPidFormat);
    expect(pidfile.pid).not.toBe(ctx.threadIdA);
