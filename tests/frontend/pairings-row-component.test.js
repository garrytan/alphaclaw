...
import { PairingRow } from "../../lib/public/js/components/pairings.js";
...
const kPairing = { id: "pair-1", channel: "telegram", accountId: "default", code: "ABC123" };

const renderRow = (props) => {
  harness.beginRender();
  return expandTree(PairingRow(props));
};

describe("frontend/channel pairing row", () => {
  beforeEach(() => {
    harness.reset();
    vi.clearAllMocks();
  });
  ...
  it("clears busy and renders a persistent inline error when approve fails", async () => {
    const error = new Error("channel unreachable");
    const onApprove = vi.fn(() => Promise.reject(error));
    const onReject = vi.fn();
    let tree = renderRow({ p: kPairing, onApprove, onReject });

    findAction(tree, "Approve").props.onClick();
    await flushAsync();
    tree = renderRow({ p: kPairing, onApprove, onReject });

    // Not stuck on the optimistic terminal state — the actions come back with
    // a persistent error adjacent to them.
    ...
    expect(chips[0].props.headline).toBe("Couldn't approve this pairing.");
    expect(chips[0].props.error).toBe(error);
    expect(collectText(tree).join(" ")).toContain("channel unreachable");
  });
  ...
  it("clears busy and renders a persistent inline error when reject fails", async () => {
    ...
    const chips = findAllByType(tree, InlineErrorChip);
    expect(chips.length).toBe(1);
    expect(chips[0].props.headline).toBe("Couldn't reject this pairing.");
  });
