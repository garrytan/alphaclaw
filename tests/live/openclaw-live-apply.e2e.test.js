...
const assertActivatedThinkingApi = (installDir) => {
  ...
  const source = `
    ...
    import(pathToFileURL(process.argv[2]).href).then((mod) => {
      ...
      // upstream re-letters `export { … as i, … as s }` per build, and on
      // 2026.9.5 `i` is listThinkingLevelLabels (plain strings) — the guess
      // that failed this tier from 2026-09-20 (lib fix: v0.9.88).
