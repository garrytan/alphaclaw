...
describe("agent-admin skill builder", () => {
  ...
  // 10 admins, every channel) is ~56.1k with the doctor.settings ops and the
  // overseer situation-report op (v0.9.69). 57k leaves headroom without
  ...
  it("stays within the on-demand size budget on a maximal fixture", () => {
    ...
    expect(content.length).toBeLessThan(57000);
  });
