...
| `ALPHACLAW_STATE_LOCK_SWEEP_DISABLED` | Optional | Kill-switch: set `1` to disable the pre-start sweep of stale upstream OpenClaw state-lifecycle locks (`$TMPDIR/openclaw-state-locks-*`). Use if the sweep ever misbehaves against a new OpenClaw lock layout; deployment env only — never honored from `.env`. |
