...
| `WATCHDOG_MAX_REPAIR_ATTEMPTS`    | Optional | Failed auto-repair attempts after which the watchdog raises the "Auto-repair failed repeatedly" alert (default `2`; a non-positive or non-numeric value falls back to the default). Read once at process start (restart AlphaClaw to change) |
