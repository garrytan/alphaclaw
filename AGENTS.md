...
- Rollback triggers (crash loop, exit 78, degraded >10 min) fire only on non-pin builds inside their 24h stabilization window — a build that never passes the 120s acceptance hold isn't rolled back directly, it just stays unaccepted until one of those triggers fires. Dev rollback targets the pin snapshot (falling back to a usable last-known-good stable overlay when the pin isn't locally recoverable) — never an in-crash rebuild. Unattended `doctor --fix` is suppressed inside that window (the 2026.7.1 plugins.allow bug is why). Exit 78 is disambiguated before it latches: 2026.8.1 overloads EX_CONFIG with a benign healthy-incumbent step-aside (a boot-race loser deliberately exits 78 after probing the incumbent), so the watchdog verifies the stderr signature with its own health probe of the incumbent and treats a confirmed step-aside as benign — no restart latch, no rollback trigger (`lib/server/watchdog.js`).
...
- **Notification outbox + admin routing.** Upgrade/watchdog notifications are enqueued durably first (deduped by id), then delivered: to explicit admin targets when configured (preferred channel first, fallback targets only on delivery error, prefixed "(fallback)"), else today's fan-out to every paired ID. Undelivered events are re-drained after the activation restart; a notifier `{ok:false}` is retried, never silently acknowledged. Prefs API: `GET/PUT /api/openclaw/notifications` (PII — state dir store, never git-synced alphaclaw.json).
...
- **Beta feature gates.** `lib/server/openclaw-feature-gates.js` (fail-closed per feature; dev shas fail closed): supervisorMode (`OPENCLAW_SUPERVISOR_MODE=external` in gatewayEnv; OpenClaw-requested restarts are consumed via the tarball-verified handoff protocol in `lib/server/gateway-restart-handoff.js` — per-PID exactly-once consume, 60s TTL cache — and relaunched by the watchdog without crash accounting; the unwired legacy stub `openclaw-restart-handoff.js` is slated for deletion, see TODOS), sqliteBackup (`POST /api/openclaw/backup-sqlite`, 503 when gated; creates with the required `--repository` into `<root>/backups/openclaw-sqlite/`, then verifies the exact snapshot the create reported — for the shared state DB AND each configured agent's DB; any unverified or skipped database fails the run loudly), sessionDashboards (gated sidebar link), secretEgressBinding (info callout). `GET /api/openclaw/features`.
