...
- **Beta feature gates.** `lib/server/openclaw-feature-gates.js` (fail-closed per feature; dev shas fail closed): supervisorMode (`OPENCLAW_SUPERVISOR_MODE=external` in gatewayEnv, handoff TODO in gateway.js), sqliteBackup (`POST /api/openclaw/backup-sqlite`, 503 when gated), sessionDashboards (gated sidebar link), secretEgressBinding (info callout). `GET /api/openclaw/features`.
...
- Server-side timezone logic (canonicalizing the `x-client-timezone` header, DST-safe local-midnight day bucketing) lives in `lib/server/utils/time-zone.js` (`resolveTimeZone`, `normalizeTimeZone`, `readClientTimeZone`, `getTimeZoneDayStartMs`) — reuse it instead of hand-rolling zone math in routes.
