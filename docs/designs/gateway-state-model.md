...
`supervision` in the reducer output is three-valued from v0.9.74: `managed` (live child handle), `adopted` (`supervisionMode === "adopted"` — identity discovered, health and memory monitored, exit events unavailable) and `detached` (no handle, no identity). The "estimated" detail fires for adopted AND detached.
...
### Repair contract (v0.9.74)

One relaunch primitive, `runVerifiedRelaunch({ source, correlationId, hold, intent })` in `watchdog.js`, serves every automatic relaunch site (`runRepair`, `restartAfterCrash`, `runConfigMedic`, `maybeRetryAfterConfigChange`, the probe-death fast path). It never throws; it maps `gateway.requestGatewayLaunch()` outcomes to verdicts and ledger rows.
