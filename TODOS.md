...
## P2 — Channel/team restart wrappers should share the restart route's blocker policy (2026-09-04, from the v0.9.72 ship review)
...
- **Why:** A channel connect, channel delete, WhatsApp pairing or team-mode toggle issued via the API or the agent actor while a hold is set relaunches the gateway on the exact held config, and `onGatewayLaunch` dissolves the watchdog latch while the hold stays set — the same class of gap v0.9.72 closed for the restart route and manual repair. Raised by the v0.9.72 red-team pass; deliberately kept out of that wave (different callers, own tests).
