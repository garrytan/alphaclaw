...
- **What:** The channel registry marks clickclack `pairing: true` and its accounts are created with `dmPolicy: "pairing"`, but the pairing channel list is hard-coded in four places (`lib/public/js/components/pairings.js` `ALL_CHANNELS`, the shared `kPairingChannels` in `lib/server/routes/pairings.js` — one constant for approve/reject since fix wave PR 1 — and `refreshPendingPairings`) as telegram|discord|slack|whatsapp. A configured-but-unpaired ClickClack account gets no pairing card, its requests are never listed, and approve returns 400. Decide whether ClickClack joins (does the pinned `openclaw pairing list --channel clickclack` exist on every supported line?) and derive every list from the registry's `pairing` flag so there is one owner.
- **Why:** README advertises ClickClack bot pairing; today it cannot complete from AlphaClaw.
- **Context:** `lib/server/channels/registry.js` (or the channel registry module), `routes/pairings.js`, `components/pairings.js`; `kChannelExtraEnvKeys` is duplicated client/server (not drifting — fold into the same registry pass).
...
- **Effort:** S (human ~1h / CC ~10min). **Priority:** P2. **Depends on:** v0.9.68's helper (landed).
