...
## P2 — Wire `updates.openclaw.backup.excludes` through both backup walks (2026-09-07, from the #79 wave, Stage 4 review)
- **What:** `createOfflineCopy({ excludes })` is module-level only: `runOfflineCopy` (lib/server/openclaw-channel-sync.js) passes no `excludes`, so production always runs `kOfflineCopyPolicyExcludes`, and `runBackupDiagnosis`'s sizing walk uses the same defaults. Add the `alphaclaw.json` key `updates.openclaw.backup.excludes` (normalizer in lib/server/alphaclaw-config.js; the module already validates ≤ 64 patterns / ≤ 256 chars / no absolute paths, `.`/`..` or backslashes and REFUSES core-asset patterns), thread ONE resolved list into BOTH `createOfflineCopy` and the diagnosis walk so the two walks cannot disagree, keep it in the manifest's `excludes[]`, expose it through the updates route + agent manifest (`dangerous` tier — it changes what a backup contains) and show it on the Upgrade tab's Backups card.
- **Why:** Stage 4 shipped the default list only (Codex 17 narrowed it to unambiguous debris — `node_modules`, `*.heapsnapshot`, `*.tmp`, `logs/**/*.gz`); an operator whose workspace carries a large `tmp` / `.cache` / `caches` tree has no opt-in. docs/designs/backup-offline-copy.md §2.1 documents the gap.
- **Context:** lib/server/openclaw-backup-offline-copy.js (`resolveExcludes`, `createOfflineCopy`, `walkStateTreeAsync`), lib/server/openclaw-channel-sync.js (`runOfflineCopy`, `runBackupDiagnosis`), lib/server/alphaclaw-config.js, lib/server/admin-manifest/domains/updates.js, tests/server/openclaw-backup-offline-copy.test.js (the only exerciser of the module option today).
...
## P2 — Live tier: `kOpenclawLines` → the 2026.9.2 pin, real `user_version` capture, pins for `database --help` and the agent-DB oracle (2026-09-07, from the #78 wave)
...
- **Effort:** S. **Priority:** P2. **Depends on:** network + the pinned CLI (live tier).
