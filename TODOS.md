...
- **Effort:** S (human ~1h / CC ~10min). **Priority:** P2. **Depends on:** v0.9.68's helper (landed) and v0.9.75's `readClampedEnvCount` (landed). Note `kWatchdogMaxRepairAttempts` is also only advisory today — see "Enforce `kWatchdogMaxRepairAttempts`" below; clamp and enforce together.
