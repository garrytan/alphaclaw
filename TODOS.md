...
- **Effort:** S (human ~1h / CC ~10min). **Priority:** P2. **Depends on:** v0.9.68's helper (landed) and v0.9.74's `readClampedEnvCount` (landed). Note `kWatchdogMaxRepairAttempts` is also only advisory today — see "Enforce `kWatchdogMaxRepairAttempts`" below; clamp and enforce together.
...
## P3 — Probe-result ordering across health-check sources (2026-09-01, from the degraded-backoff wave)
...
- **Context:** bin/alphaclaw.js (§5 env loading, `kDeploymentOnlyEnvKeys` import ~:420), lib/server/deployment-only-env.js, lib/server/env.js (`reloadEnv`), tests/server/constants-cadence.test.js, tests/server/env.test.js.
...
- **What:** The Envars editor's `PUT /api/env` (lib/server/routes/system.js, validator `isReservedUserEnvVar` in lib/server/utils/env-keys.js) accepts every key in `kDeploymentOnlyEnvKeys` (`ALPHACLAW_GATEWAY_ENV_UNRESTRICTED`, `ALPHACLAW_GATEWAY_ENV_PASSTHROUGH`, `GATEWAY_RESTART_READY_TIMEOUT`, and v0.9.68's three `WATCHDOG_*` cadence knobs), writes them to `.env`, and returns 200 — while both loaders (the `bin/alphaclaw.js` boot loop and `reloadEnv`) skip them by design. The operator sees success and no effect; the README says "never honored from `.env`" but the only editor most operators use does not say so.
...
- **Effort:** S. **Priority:** P3. **Depends on:** v0.9.74 (landed).

## P3 — Readiness-component allowlist knob (2026-09-05, from the honest-restart wave)
- **What:** Since v0.9.74 any failing `/readyz` component keeps the incident open and withholds the acceptance hook. A deployment with a permanently failing OPTIONAL component (an unused channel that fails auth, a plugin whose provider is gone) would sit degraded forever. Add `watchdog.readiness.ignoreComponents: string[]` in alphaclaw.json (dashboard toggle later) that `evaluateChannelSuppression` subtracts before deciding `not_ready`, logged in `readinessReason` as ignored.
...
- **Context:** lib/server/watchdog.js `evaluateChannelSuppression` / `applyHealthyProbeResult`, the readiness components list from `/readyz`.
...
- **What:** `kWatchdogMaxRepairAttempts` (2) is advisory: `repairAttempts` is counted and reset when the verifier records `replacement_ready` (v0.9.74 moved the reset there), but nothing refuses a third `doctor --fix` in one incident — the crash-loop pause and the degraded ladder bound the rate, not the count. Make the cap real: past the cap, `runRepair` books `repair/<source>/skipped {reason: "repair_attempts_exhausted"}`, the reducer's row 5 (`down`, "repair attempts exhausted") becomes reachable as the design doc §4 already claims, and the notice tells the operator that automatic repair has stopped.
- **Why:** Surfaced by the 2026-09 audit calibration: a wedged gateway that survives Doctor + replace twice will be recycled every ~15 s of sustained failure indefinitely. Not in the v0.9.74 scope (behaviour change in the pause semantics; wants its own notice copy).
...
- **Why:** Kept layered in v0.9.74 (plan decision D10 / second Codex pass point 16): a controller module would have rewritten #58/#59 inside the 7-day window and collided with #64. Once the dust settles, the duplication (two "is the incumbent gone" proofs, two expected-restart brackets) is the thing to remove.
...
- **Effort:** L. **Priority:** P3. **Depends on:** #58/#59/#64/v0.9.74 all >7 days old; carries a "Supersedes recent work" section.

## P3 — Ask upstream for an instance id on `/health` (2026-09-05, from the honest-restart wave)
- **What:** OpenClaw's `/health` answers only `{ ok: true, status: "live" }` (verified in the 2026.9.1-beta.1 dist, `isGatewayHealthzResponse`) and `/readyz` gates its details behind auth, so a probe cannot say WHICH gateway answered. v0.9.74 proves a replacement's identity by snapshot exclusivity (`listGatewayPids` contains only our launcher/worker) plus #59's port-released verdict under `replace`. Ask upstream (adjacent to openclaw/openclaw#121069, the lock owner-identity design) to include a per-process instance id (pid + start ticks, or a launch nonce) in `/health`; then `identityObservedAt` can be set from the probe itself and the /proc snapshot heuristics retire.
...
- **Context:** lib/server/watchdog.js `applyHealthyProbeResult` (the not-ready branch), `evaluateChannelSuppression`, the release-channel hooks' `onUnhealthy`; pairs with the allowlist knob item above.
- **Effort:** M. **Priority:** P3. **Depends on:** field evidence; the `alive_not_ready` phase item.

## ~~P3 — Watchdog status gatewayPid stays null~~ (ADDRESSED in v0.9.74 — root cause was the boot-around-incumbent path)
...
## P2 — `autotune-container` OOM-stderr assertion fails on every GitHub `live-e2e` run (2026-09-03, from the #54 ship pass)
