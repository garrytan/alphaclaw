...
## P2 — Google disconnect red-team deferrals (2026-08-31, grouped)
- **What:** (1) google-state read-modify-write serialization — `writeGoogleState`'s file lock covers only the write; the disconnect route now re-reads fresh state at removal time (v0.9.55), but the OAuth-callback save path and any future multi-step writer still derive from pre-await snapshots; add an in-process mutex (or lock-wrapped read-modify-write helper in google-state.js) used by ALL state writers — and extend it to a per-account mutex spanning disconnect and the OAuth-callback token import, closing the seconds-wide disconnect-vs-reauth window where a re-auth's freshly imported token is wiped by `auth remove` and its account row deleted, leaving a live never-revoked grant at Google. (2) Discriminate transient gog export failure from "account has no token" — `gogCmd` (lib/server/commands.js) returns only {ok,stdout,stderr} with no timedOut/exit-code field (clawCmd has one), so a hung/locked `gog auth tokens export` is indistinguishable from a token-less account and disconnect proceeds best-effort without revocation; add timedOut/code to gogCmd results and route transient export failures into the retryable path. Verify gog's actual no-such-account stderr contract against the pinned gog build first (contract-test it — do NOT guess the string). (3) Stop active Gmail watches on disconnect — the account row (watcher pid/port) is deleted without stopping the local watcher or calling Gmail users.stop, and revocation runs first so orderly cleanup loses its credentials; stop the watcher + best-effort users.stop BEFORE revocation. (4) Surface Google's grant-wide revocation semantics — revoking one refresh token can invalidate the user's whole grant for the client, so sibling account rows (same user, other records) may show authenticated with dead tokens; after disconnect, re-check sibling rows' auth status (the accounts list already probes `gog auth list --check`).
- **Why:** Both flagged by the /ship red-team pass on the v0.9.55 disconnect fix and verified real; deferred because (1) touches every state writer and (2) changes a shared exec primitive + needs the real gog contract — both beyond a hotfix's blast radius. The disconnect-local mitigations (fresh-read removal, retryable+accountId echo) landed with v0.9.55.
- **Context:** lib/server/routes/google.js (disconnect + OAuth callback saves), lib/server/google-state.js (withFileLockSync), lib/server/commands.js (gogCmd vs clawCmd timedOut).
- **Effort:** S (1), M (2 — includes the gog contract test). **Depends on:** nothing; (2) pairs naturally with any future live gog contract tier.
