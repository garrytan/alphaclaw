...
- **Hermetic tests still execute the real `gog` binary when the host has one.** Under `npm test`, a burst of ~20 `gog` invocations per run was traced (via the orphaned `dbus-daemon` env: `_=/usr/local/bin/gog`, `VITEST_WORKER_ID` set) to tests that reach `gogCmd`/`exec("gog …")` without a child_process mock; the run env now blocks the session-bus autolaunch so nothing leaks, but the tests remain host-dependent (slower and different on a box with `gog` installed). Fix: a `tests/setup-*` PATH stub directory with a fail-fast `gog` shim, or mocks at the call sites (`routes/google.js`, `routes/gmail.js`, `gmail-watch.js`, `server.js` boot paths).
